...
import { replaceInFile, traverseDirectory, updateNamespaceInPrettierConfig, updateWorkspacePackages } from './utils'
...
async function main() {
    ...
    function updateVersion(packageJson: PackageJson): PackageJson {
        ...
        if (!newVersion || (packageJson.name && excludePackages.includes(packageJson.name))) {
            return packageJson
        }
     ...
     * @returns updated package.json
     */
    function updatePackageName(packageJson: PackageJson, fullPath: string): PackageJson {
        if (!newNamespace) {
            return packageJson
        }
     ...
     * @returns updated package.json
     */
    function updatePackageJsonDetails(packageJson: PackageJson, fullPath: string): PackageJson {
     ...
     */
    function renameDependencies(dependencies: Record<string, string> | undefined): Record<string, string> {
        if (!dependencies) {
            return {}
        }
    ...
    await updateWorkspacePackages(process.cwd(), updatePackageJsonDetails, includeRoot)
    ...
    if (newNamespace) {
        ...
        try {
            ...
            await execAsync('bun format && bun turbo clean && bun install')
            console.log(
                '🎉 Done! Workspace namespaces have successfully been updated. You may wish to reload your IDE, to remove any errors.'
            )
        } catch (error) {
            console.error('Error updating workspace or running final commands:', error instanceof Error ? error.message : String(error))
            process.exit(1)
        }
    }
}
