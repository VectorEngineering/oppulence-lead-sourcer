...
import { boolean, datetime, int, mysqlEnum, mysqlTable, unique, varchar } from 'drizzle-orm/mysql-core'
...
export const ratelimitOverrides = mysqlTable(
    ...
    (table) => {
        return {
            uniqueIdentifierPerNamespace: unique('unique_identifier_per_namespace_idx').on(table.identifier, table.namespaceId)
        }
    }
)
