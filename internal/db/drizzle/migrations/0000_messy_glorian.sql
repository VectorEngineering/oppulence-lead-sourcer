...
CREATE TABLE `keys` (
	...
	`deleted_at_m` bigint,
	`deleted_at` datetime(3),
	`refill_interval` enum('daily','monthly'),
	`refill_day` tinyint,
	`refill_amount` int,
...
CREATE TABLE `events` (
	...
	`workspace_id` varchar(256) NOT NULL,
	`webhook_id` varchar(256) NOT NULL,
	`event` enum('verifications.usage.record','transaction.created','transaction.updated','transaction.failed','transaction.cancelled','transaction.disputed','transaction.refunded','transaction.settled','transaction.flagged','account.created','account.updated','account.suspended','account.closed','account.reactivated','account.frozen','account.unfrozen','account.limits.updated','kyc.verification.started','kyc.verification.completed','kyc.verification.failed','kyc.document.submitted','kyc.document.verified','kyc.document.rejected','kyc.risk.level.changed','kyc.screening.completed','fraud.alert.created','fraud.case.opened','fraud.case.updated','fraud.case.closed','fraud.transaction.blocked','fraud.pattern.detected','report.generated','report.scheduled','report.failed','analytics.threshold.exceeded','balance.updated','balance.reconciled','balance.threshold.reached','interest.credited','fee.charged','compliance.report.filed','compliance.alert.triggered','compliance.review.required','regulatory.filing.submitted','regulatory.requirement.updated') NOT NULL,
	`time` bigint NOT NULL,
