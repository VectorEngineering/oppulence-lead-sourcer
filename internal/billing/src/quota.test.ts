...
describe("Quota System", () => {
  describe("Tier Validation", () => {
    ...
    test("pro tier should have higher limits than free tier", () => {
      ...
      expect(QUOTA.pro.invoicing.maxMonthlyInvoices).toBeGreaterThan(
        QUOTA.free.invoicing.maxMonthlyInvoices
      );
    });
  ...
  describe("checkQuotaLimit", () => {
    ...
    test("should throw error when specified", () => {
      expect(() => 
        checkQuotaLimit(15, 10, { throwError: true })
      ).toThrow("Quota exceeded: usage 15 exceeds limit 10");
    });

    test("should not throw error for valid usage even when throwError is true", () => {
      expect(() => 
        checkQuotaLimit(5, 10, { throwError: true })
      ).not.toThrow();
    });
  });
  ...
  describe("Quota Progression", () => {
    test("limits should increase from free to pro tier", () => {
      ...
      compareNumericLimits(
        QUOTA.free.invoicing.maxMonthlyInvoices,
        QUOTA.pro.invoicing.maxMonthlyInvoices
      );
      compareNumericLimits(
        QUOTA.free.storage.maxFileSize,
        QUOTA.pro.storage.maxFileSize
      );
      compareNumericLimits(
        QUOTA.free.ai.maxMonthlyQueries,
        QUOTA.pro.ai.maxMonthlyQueries
      );
    });
  ...
  describe("Feature Limits", () => {
    describe("Invoicing Limits", () => {
      ...
      test("should enforce monthly invoice limits", () => {
        expect(QUOTA.free.invoicing.maxMonthlyInvoices).toBeLessThan(
          QUOTA.pro.invoicing.maxMonthlyInvoices
        );
      });
    });
    ...
    describe("Time Tracking Limits", () => {
      test("should have appropriate project limits", () => {
        expect(QUOTA.free.timeTracking.maxProjects).toBeLessThan(
          QUOTA.pro.timeTracking.maxProjects
        );
      });

      test("should scale tasks per project appropriately", () => {
        expect(QUOTA.free.timeTracking.maxTasksPerProject).toBeLessThan(
          QUOTA.pro.timeTracking.maxTasksPerProject
        );
      });
    });
    ...
    describe("AI Feature Limits", () => {
      test("should have reasonable query limits", () => {
        expect(QUOTA.free.ai.maxMonthlyQueries).toBeLessThan(
          QUOTA.pro.ai.maxMonthlyQueries
        );
      });

      test("should scale document processing limits", () => {
        expect(QUOTA.free.ai.maxDocumentProcessing).toBeLessThan(
          QUOTA.pro.ai.maxDocumentProcessing
        );
      });
    });
  });
