import { Err, Ok, type Result, SchemaError } from '@playbookmedia/error'
import { z } from 'zod'
 ...
 *
 */
export function calculateTieredPrices(rawTiers: BillingTier[], units: number): Result<TieredPrice, SchemaError> {
    /**
     * Validation logic:
     */
    ...
    for (let i = 0; i < tiers.length; i++) {
        if (i > 0) {
            ...
            if (previousLastUnit === null) {
                return Err(new SchemaError({ message: 'Every tier except the last one must have a lastUnit' }))
            }
  ...
  */
    ...
    for (const tier of tiers) {
        ...
        const quantity = tier.lastUnit === null ? remaining : Math.min(tier.lastUnit - tier.firstUnit + 1, remaining)
        remaining -= quantity
