...
function createLinksMapFromFile(filePath: string) {
  ...
  for (const line of lines) {
    try {
      ...
      const uniqueId = `${data.domain}/${data.key ? data.key.toLowerCase() : '_root'}` // Customize this based on your data structure
...
async function main() {
  ...
  Papa.parse<Record<string, string>>(csvStream, {
    header: true,
    skipEmptyLines: true,
    step: ({ data }) => {
      const typedData = data as Record<string, string>
      const link = links.get(
        `${typedData.domain}/${typedData.key ? typedData.key.toLowerCase() : '_root'}`
      )
      ...
      const { domain, key, id, projectId, url, alias, timestamp, ...rest } = typedData
