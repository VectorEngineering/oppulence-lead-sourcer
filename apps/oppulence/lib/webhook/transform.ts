...
import { Webhook } from '@dub/prisma-oppulence/client'
...
interface TransformWebhookProps
  extends Pick<Webhook, 'id' | 'name' | 'url' | 'secret' | 'triggers' | 'disabledAt'> {
  links: { linkId: string }[]
}
...
export const transformLeadEventData = (data: any) => {
  ...
  return leadWebhookEventSchema.parse({
    ...
    customer: {
      ...
      avatar:
        lead.customerAvatar || `https://api.dicebear.com/9.x/micah/svg?seed=${lead.customerId}`,
...
export const transformSaleEventData = (data: any) => {
  ...
  return saleWebhookEventSchema.parse({
    ...
    customer: {
      ...
      avatar:
        sale.customerAvatar || `https://api.dicebear.com/9.x/micah/svg?seed=${sale.customerId}`,
