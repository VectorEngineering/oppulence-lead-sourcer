...
> {
  ...
  if (domain === 'dub.sh' || domain === 'dub.link') {
    ...
    if (key !== '_root' && allowedHostnames && !allowedHostnames.includes(urlDomain)) {
      return {
        link: payload,
        error: `Invalid destination URL. You can only create ${domain} short links for URLs with the ${pluralize(
          'domain',
          allowedHostnames.length
        )} ${allowedHostnames.map(d => `"${d}"`).join(', ')}.`,
        code: 'unprocessable_entity',
      }
    }
    ...
    if (webhookIds && webhookIds.length > 0) {
      ...
      if (webhooks.length !== webhookIds.length) {
        const invalidWebhookIds = webhookIds.filter(
          webhookId => webhooks.find(({ id }) => webhookId === id) === undefined
        )
