...
export default function AnalyticsProvider({
  ...
  const showConversions =
    adminPage || demoPage || partnerPage || conversionEnabled || dashboardProps?.showConversions
      ? true
      : false
  ...
  const [persistedSaleUnit, setPersistedSaleUnit] = useLocalStorage<AnalyticsSaleUnit>(
    `analytics-sale-unit`,
    'saleAmount'
  )
      ...
      onError: error => {
        try {
          ...
          if (error.status === 403 && errorMessage.toLowerCase().includes('upgrade')) {
            toast.custom(() => (
              <UpgradeRequiredToast title="Upgrade for more analytics" message={errorMessage} />
            ))
