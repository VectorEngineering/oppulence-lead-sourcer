  ...
  const filters: ComponentProps<typeof Filter.Select>['filters'] = useMemo(
    () => [
      ...
      ...(dashboardProps
        ...
        : [
            ...
            },
            {
              ...
              getOptionIcon: (value, props) => {
                const tagColor =
                  props.option?.data?.color ?? tags?.find(({ id }) => id === value)?.color
      ...
      },
      {
        ...
        label: 'Country',
        getOptionIcon: value => (
          <img alt={value} src={`https://flag.vercel.app/m/${value}.svg`} className="h-2.5 w-4" />
        ),
      ...
      },
      {
        ...
        label: 'Device',
        getOptionIcon: value => (
          <DeviceIcon display={capitalize(value) ?? value} tab="devices" className="h-4 w-4" />
        ),
        options:
      ...
      },
      {
        ...
        getOptionIcon: (_, props) => (
          <LinkLogo apexDomain={getApexDomain(props.option?.value)} className="size-4 sm:size-4" />
        ),
        options:
          urls?.map(({ url, count }) => ({
            value: url,
...
function UpgradeTooltip({ rangeLabel, plan }: { rangeLabel: string; plan?: string }) {
  ...
  return (
    <TooltipContent
      title={`${rangeLabel} can only be viewed on a ${
        isAllTime ? 'Business' : getNextPlan(plan).name
      } plan or higher. Upgrade now to view more stats.`}
...
function LinkIcon({
  ...
  return url ? (
    <LinkLogo apexDomain={getApexDomain(url)} className="h-4 w-4 sm:h-4 sm:w-4" />
  ) : (
