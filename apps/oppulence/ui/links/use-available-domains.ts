...
export function useAvailableDomains(
  ...
  const { allActiveDomains, primaryDomain, allDomains, allWorkspaceDomains, loading } = useDomains({
    ignoreParams: true,
  ...
  const domains = useMemo(() => {
    ...
    } else if (currentDomain && !allActiveDomains.find(({ slug }) => slug === currentDomain)) {
      const domain = allDomains.find(({ slug }) => slug === currentDomain)

      return domain
        ? [...allActiveDomains, domain].filter(Boolean).sort((a, b) => a.slug.localeCompare(b.slug))
