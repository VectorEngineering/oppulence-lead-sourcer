...
export const ShortLinkInput = forwardRef<HTMLInputElement, ShortLinkInputProps>(
  (
    ...
    const generateAIKey = useCallback(async () => {
      setKeyError(null)
      complete(
        `For the following URL, suggest a relevant short link slug that is at most ${Math.max(
          25 - (domain?.length || 0),
          12
        )} characters long. 
                  ...
                  <div className="flex max-w-xs items-start space-x-2 bg-white p-4">
                    ...
                    <div>
                      <p className="text-sm text-gray-700">
                        Short links longer than 25 characters will show up differently on some
                        platforms.
                      </p>
...
function DomainCombobox({
  ...
  useEffect(() => {
    if (allWorkspaceDomains && !useAsync && allWorkspaceDomains.length >= DOMAINS_MAX_PAGE_SIZE)
      setUseAsync(true)
