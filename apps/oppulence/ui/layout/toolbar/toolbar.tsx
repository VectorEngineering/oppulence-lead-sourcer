...
async function ToolbarRSC({ show = ['onboarding', 'help'] }: ToolbarProps) {
  const { popularHelpArticles, allHelpArticles } = await fetch('https://dub.co/api/content', {
    next: {
  ...
  return (
    <div className="flex items-center gap-3">
      ...
      {show.includes('help') && (
        <div className="shrink-0">
          <HelpButton popularHelpArticles={popularHelpArticles} allHelpArticles={allHelpArticles} />
        </div>
      )}
    </div>
  )
