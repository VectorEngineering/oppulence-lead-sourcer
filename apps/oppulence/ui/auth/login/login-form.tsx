...
export default function LoginForm({
  ...
  const [lastUsedAuthMethodLive, setLastUsedAuthMethod] = useLocalStorage<AuthMethod | undefined>(
        ...
        <AnimatedSizeContainer height>
          <div className="flex flex-col gap-3 p-1">
                ...
                {!showEmailPasswordOnly && authMethod === lastUsedAuthMethod && (
                  <div className="text-center text-xs">
                    <span className="text-gray-500">
                      You signed in with{' '}
                      {lastUsedAuthMethod.charAt(0).toUpperCase() + lastUsedAuthMethod.slice(1)}{' '}
                      last time
                    </span>
                  </div>
                )}
            ...
            {showEmailPasswordOnly ? (
              ...
              authProviders
                .filter(
                  provider => provider.method !== authMethod && methods.includes(provider.method)
                )
                .map(provider => (
