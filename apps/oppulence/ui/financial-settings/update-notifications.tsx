...
export default function UpdateNotifications({
  ...
  const [thresholds, setThresholds] = useState({
    largeTransactionThreshold:
      THRESHOLD_OPTIONS.find(opt => opt.value === String(initialLargeTransactionThreshold)) || null,
    lowBalanceThreshold:
      THRESHOLD_OPTIONS.find(opt => opt.value === String(initialLowBalanceThreshold)) || null,
  })
          ...
          {NOTIFICATION_FEATURES.map(feature => (
            <div key={feature.id} className="space-y-3">
              <div className="flex items-center justify-between rounded-lg border border-gray-200 p-4 transition-all hover:bg-gray-50">
                ...
                <Switch
                  fn={() =>
                    handleUpdateFeature(feature.id, !features[feature.id], feature.thresholdField)
                  }
