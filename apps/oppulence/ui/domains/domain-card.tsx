...
export default function DomainCard({ props }: { props: DomainProps }) {
  ...
  const { data, isValidating, mutate } = useSWRImmutable<{
    ...
    workspaceId && isVisible && `/api/domains/${domain}/verify?workspaceId=${workspaceId}`,
    fetcher
  )
  ...
  const isInvalid =
    verificationData &&
    !['Valid Configuration', 'Pending Verification'].includes(verificationData.status)
                  ...
                  icon={
                    <div className="flex items-center gap-1">
                      <div className="relative">
                        <Gear
                          className={cn('h-4 w-4', showDetails ? 'text-gray-800' : 'text-gray-600')}
                        />
                        {/* Error indicator */}
            ...
            {verificationData ? (
              verificationData.status === 'Valid Configuration' ? (
                <div className="mt-6 flex items-center gap-2 text-pretty rounded-lg bg-green-100/80 p-3 text-sm text-green-600">
                  ...
                  <div>
                    Good news! Your DNS records are set up correctly, but it can take some time for
                    them to propagate globally.{' '}
                    <Link
