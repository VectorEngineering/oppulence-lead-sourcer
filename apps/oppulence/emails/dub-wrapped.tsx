...
export default function DubWrapped({
  ...
  return (
    <Html>
      <Head />
      <Preview>
        In 2024, you created {nFormatter(stats['Total Links'], { full: true })} links on{' '}
        {platform.company} and got {nFormatter(stats['Total Clicks'], { full: true })} clicks.
      </Preview>
      <Tailwind>
        <Body className="mx-auto my-auto bg-white font-sans">
          <Container className="mx-auto my-10 max-w-[500px] rounded border border-solid border-gray-200 px-10 py-5">
            ...
            <Text className="text-sm leading-6 text-black">
              As we put a wrap on 2024, we wanted to say thank you for your support! Here's a look
              back at your activity in 2024:
            </Text>
            ...
            <Text className="text-sm leading-6 text-black">
              Thanks to customers like you, we had an incredible year as well, seeing record
              activity and link clicks:
            </Text>
            {dubStats.map(stat => (
              <Text key={stat.item} className="ml-1 text-sm font-medium leading-4 text-black">
                ◆ {stat.item}{' '}
                <span className="font-semibold text-green-700">({stat.increase})</span>
              </Text>
            ))}
            <Img
              src={`${platform.assetsUrl}/header.png`}
