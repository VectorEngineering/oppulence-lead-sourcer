...
export function PartnerTable() {
  ...
  const { filters, activeFilters, onSelect, onRemove, onRemoveAll, isFiltered } = usePartnerFilters(
    { sortBy, sortOrder }
  )
  ...
  const { table, ...tableProps } = useTable({
    ...
    columns: [
        ...
        cell: ({ row }) => {
          ...
          return (
            <div className="flex items-center gap-2">
              {country && (
                <img alt="" src={`https://flag.vercel.app/m/${country}.svg`} className="h-3 w-4" />
              )}
      ...
      },
      {
        ...
        header: 'Clicks',
        accessorFn: d =>
          d.status !== 'pending' ? nFormatter(d.link?.clicks, { full: true }) : '-',
      },
      {
        id: 'leads',
        header: 'Leads',
