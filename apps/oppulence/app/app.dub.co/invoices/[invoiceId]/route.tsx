  ...
  const pdf = await renderToBuffer(
    <Document>
      <Page size="A4" style={tw('p-20 bg-white')}>
        ...
        <View style={tw('mb-6 border border-neutral-200 rounded-xl')}>
          ...
          {invoice.payouts.map((payout, index) => (
            <View
              key={index}
              style={tw(
                `flex-row text-sm font-medium text-neutral-700 border-neutral-200 items-center ${
                  index + 1 === invoice.payouts.length ? '' : 'border-b'
                }`
              )}
            >
        ...
        <View
          style={tw('flex-col gap-2 mb-10 p-4 border border-neutral-100 rounded-xl bg-neutral-50')}
        >
