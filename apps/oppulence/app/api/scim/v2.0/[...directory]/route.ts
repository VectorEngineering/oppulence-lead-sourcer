...
const handler = async (req: Request, { params }: { params: Record<string, string> }) => {
  ...
  const { status, data } = await directorySyncController.requests.handle(request, handleEvents)
