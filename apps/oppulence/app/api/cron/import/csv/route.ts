    ...
    const mapper = (row: Record<string, string>): MapperResult => {
      ...
      try {
        return {
          ...
          data: {
            ...Object.fromEntries(
              Object.entries(mapping).map(([key, value]) => [key, getValueByNormalizedKey(value)])
            ),
            domain,
            key,
            createdAt: mapping.createdAt
              ? parseDateTime(getValueByNormalizedKey(mapping.createdAt)) || undefined
              : undefined,
            tags: mapping.tags
            ...
            .filter(
              (result): result is Extract<MapperResult, { success: true }> =>
                ...
                !alreadyCreatedLinks.some(
                  l => l.domain === result.data.domain && l.key === result.data.key
                ) &&
