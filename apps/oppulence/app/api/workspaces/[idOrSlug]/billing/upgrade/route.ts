...
export const POST = withWorkspace(async ({ req, workspace, session }) => {
  ...
  if (workspace.stripeId && activeSubscription) {
    ...
    const stripeSession = await stripe.checkout.sessions.create({
      ...
      success_url: `${APP_DOMAIN}/${workspace.slug}?${
        onboarding ? 'onboarded' : 'upgraded'
      }=true&plan=${plan}&period=${period}`,
      cancel_url: baseUrl,
      ...
      ...(customer?.discount?.couponId
        ? {
            discounts: [
              {
                coupon:
                  process.env.NODE_ENV !== 'production' && customer.discount.couponTestId
