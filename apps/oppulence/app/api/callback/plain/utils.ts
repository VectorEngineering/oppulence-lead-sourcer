...
export const plainTextSection = ({ label, value }: { label: string; value: string }) => ({
  componentRow: {
    rowMainContent: [
      {
        componentText: {
...
export const plainCopySection = ({ label, value }: { label: string; value: string }) => [
  {
    componentText: {
      text: label,
      textSize: 'S',
...
export const plainUsageSection = ({
  ...
  componentRow: {
    ...
    rowAsideContent: [
      {
        componentBadge: {
          badgeLabel: `${nFormatter(usage, { full: true })}${
            usageLimit ? ` of ${nFormatter(usageLimit, { full: true })}` : ''
          }`,
          badgeColor: color,
        },
      },
