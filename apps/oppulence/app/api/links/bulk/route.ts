...
export const POST = withWorkspace(
  async ({ req, headers, session, workspace }) => {
    ...
    const duplicates = links.filter(
      (link, index, self) =>
        link.key && self.slice(index + 1).some(l => l.domain === link.domain && l.key === link.key)
    )
  ...
  async ({ req, workspace, headers }) => {
    ...
    if (tagIds && tagIds.length > 0) {
      ...
      if (tags.length !== tagIds.length) {
        throw new DubApiError({
          code: 'unprocessable_entity',
          message: `Invalid tagIds detected: ${tagIds
            .filter(tagId => tags.find(({ id }) => tagId === id) === undefined)
            .join(', ')}`,
        })
      }
      ...
      if (tags.length !== tagNames.length) {
        throw new DubApiError({
          code: 'unprocessable_entity',
          message: `Invalid tagNames detected: ${tagNames
      ...
      links.map(async link =>
        processLink({
          payload: {
            ...link,
            expiresAt:
              link.expiresAt instanceof Date ? link.expiresAt.toISOString() : link.expiresAt,
