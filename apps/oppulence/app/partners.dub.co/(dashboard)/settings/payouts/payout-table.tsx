    ...
    columns: [
      {
        ...
        accessorFn: d => {
          ...
          return `${formatDate(d.periodStart, {
            month: 'short',
            year:
              new Date(d.periodStart).getFullYear() === new Date(d.periodEnd).getFullYear()
                ? undefined
                : 'numeric',
          })}-${formatDate(d.periodEnd, { month: 'short' })}`
        },
      },
      {
        header: 'Program',
        cell: ({ row }) => (
          <div className="flex items-center gap-2">
            <img
              src={
                row.original.program.logo || `${DICEBEAR_AVATAR_URL}${row.original.program.name}`
              }
