...
import { trackLeadRequestSchema, trackLeadResponseSchema } from '@/lib/zod/schemas/leads'
...
export const POST = withWorkspaceEdge(
    async ({ req, workspace }) => {
        ...
        const { success } = await ratelimit(1, '1 h').limit(`recordLead:${customerExternalId}:${eventName.toLowerCase().replace(' ', '-')}`)
