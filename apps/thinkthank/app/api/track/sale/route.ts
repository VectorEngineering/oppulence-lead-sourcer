...
import { trackSaleRequestSchema, trackSaleResponseSchema } from '@/lib/zod/schemas/sales'
...
export const POST = withWorkspaceEdge(
    async ({ req, workspace }) => {
        ...
        const clickData = clickEventSchemaTB.omit({ timestamp: true }).parse(leadEvent.data[0])
        ...
        waitUntil(
            (async () => {
                ...
                // for program links
                if (link.programId) {
                    const { program, partnerId, commissionAmount } = await prismaEdge.programEnrollment.findUniqueOrThrow({
                        where: {
                            linkId: link.id
                        },
                        select: {
