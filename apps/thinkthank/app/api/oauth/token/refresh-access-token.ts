...
import type { refreshTokenSchema } from '@/lib/zod/schemas/oauth'
import { prisma } from '@dub/prisma-thinkthank'
import { getCurrentPlan } from '@dub/utils'
import type { NextRequest } from 'next/server'

// Get new access token using refresh token
export const refreshAccessToken = async (req: NextRequest, params: z.infer<typeof refreshTokenSchema>) => {
    let { refresh_token, client_id: clientId, client_secret: clientSecret } = params
    ...
    const accessTokenExpires = new Date(Date.now() + OAUTH_CONFIG.ACCESS_TOKEN_LIFETIME * 1000)
    ...
    await prisma.$transaction([
        ...
        prisma.restrictedToken.create({
            data: {
                ...
                rateLimit: getCurrentPlan(authorizedApp.project.plan as string).limits.apis,
                ...
                refreshTokens: {
                    create: {
                        ...
                        expiresAt: new Date(Date.now() + OAUTH_CONFIG.REFRESH_TOKEN_LIFETIME * 1000)
                    }
                }
            }
        })
