...
export const PATCH = withWorkspace(
    async ({ req, params, workspace }) => {
        const { name, slug, developer, website, installUrl, description, readme, redirectUris, logo, pkce, screenshots } =
        ...
        try {
            ...
            if (logoUpdated) {
                const result = await storage.upload(`integrations/${params.appId}_${nanoid(7)}`, logo)
            ...
            waitUntil(
                (async () => {
                    if (logoUpdated && integration.logo && integration.logo.startsWith(`${R2_URL}/integrations/${params.appId}`)) {
                        await storage.delete(integration.logo.replace(`${R2_URL}/`, ''))
                    }

                    // Remove old screenshots
                    const oldScreenshots = integration.screenshots ? (integration.screenshots as string[]) : []

                    const removedScreenshots = oldScreenshots?.filter((s) => !screenshots?.includes(s))
            ...
            return NextResponse.json(oAuthAppSchema.parse({ ...oAuthApp, ...updatedIntegration }))
        } catch (error) {
            if (error.code === 'P2002') {
                throw new DubApiError({
                    code: 'conflict',
...
export const DELETE = withWorkspace(
    async ({ params, workspace }) => {
        ...
        waitUntil(
            (async () => {
                if (integration.logo && integration.logo.startsWith(`${R2_URL}/integrations`)) {
                    await storage.delete(integration.logo.replace(`${R2_URL}/`, ''))
                }
