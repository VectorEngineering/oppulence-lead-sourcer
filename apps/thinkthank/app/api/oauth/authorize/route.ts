...
export const POST = withWorkspace(async ({ session, req, workspace }) => {
    ...
    if (app.integrationId === STRIPE_INTEGRATION_ID && !workspace.conversionEnabled) {
        throw new DubApiError({
            code: 'bad_request',
            message: 'Stripe app can only be connected to a workspace with conversion enabled. Please contact us to get access.'
        })
    }
