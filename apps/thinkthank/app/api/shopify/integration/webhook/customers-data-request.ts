...
export async function customersDataRequest({ event, workspaceId }: { event: any; workspaceId: string }) {
    const {
    ...
    const rows = [
        ...
        ...(ordersRequested.length > 0 ? [{ text: 'Orders requested', value: ordersRequested.join(', ') }] : [])
    ]
