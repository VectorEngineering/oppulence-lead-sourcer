...
export async function customersRedact({ event, workspaceId }: { event: any; workspaceId: string }) {
    const { customer, shop_domain: shopDomain, orders_to_redact: ordersToRedact } = schema.parse(event)
    ...
    const rows = [
        ...
        ...(ordersToRedact.length > 0 ? [{ text: 'Orders to redact', value: ordersToRedact.join(', ') }] : [])
    ]
