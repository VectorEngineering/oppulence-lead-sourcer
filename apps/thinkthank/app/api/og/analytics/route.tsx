import { GOOGLE_FAVICON_URL, getApexDomain, linkConstructor, nFormatter } from '@dub/utils'
...
export async function GET(req: NextRequest) {
    const interMedium = await fetch(new URL('@/styles/Inter-Medium.ttf', import.meta.url)).then((res) => res.arrayBuffer())
...
const Chart = ({ data }) => {
    ...
    return (
        <svg viewBox={`0 0 ${width} ${height}`} style={{ color: '#3B82F6', marginLeft: '-4px', marginTop: '-32px' }}>
            <defs>
            ...
            <polyline fill='url(#customGradient)' stroke='currentColor' stroke-width='3' points={points} />
        </svg>
    )
}
