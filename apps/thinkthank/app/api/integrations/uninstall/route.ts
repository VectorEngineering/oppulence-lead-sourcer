...
export const DELETE = withWorkspace(
    async ({ searchParams, session, workspace }) => {
        ...
        if (installation.userId !== session.user.id) {
            throw new DubApiError({
                code: 'unauthorized',
                message: 'You are not authorized to uninstall this integration. Only the user who installed it can uninstall it.'
            })
        }
        ...
        waitUntil(
            Promise.all([
                ...(integrationId === SLACK_INTEGRATION_ID ? [uninstallSlackIntegration({ installation })] : []),

                ...(webhook && isLinkLevelWebhook(webhook) ? [webhookCache.delete(webhook.id)] : [])
            ])
        )
