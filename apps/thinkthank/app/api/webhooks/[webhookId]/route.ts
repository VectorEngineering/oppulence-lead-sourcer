...
export const GET = withWorkspace(
    ...
    },
    {
        ...
        requiredPlan: ['business', 'business plus', 'business extra', 'business max', 'enterprise']
    }
)
...
export const PATCH = withWorkspace(
    async ({ workspace, params, req }) => {
        ...
        const { name, url, triggers, linkIds } = updateWebhookSchema.parse(await parseRequestBody(req))
        ...
        if (existingWebhook.installationId && (name || url)) {
            throw new DubApiError({
                code: 'bad_request',
                message: 'This webhook is managed by an integration. Only the linkIds & triggers can be updated.'
            })
        }
        ...
        if (linkIds && linkIds.length > 0) {
            ...
            if (links.length !== linkIds.length) {
                throw new DubApiError({
                    code: 'bad_request',
                    message: 'Invalid link IDs provided. Please check the links you are adding the webhook to.'
                })
            }
        }
        ...
        waitUntil(
            (async () => {
                ...
                if (isLinkLevelWebhook(existingWebhook) && !isLinkLevelWebhook(webhook)) {
                ...
                const linksAdded = newLinkIds.filter((linkId) => !oldLinkIds.includes(linkId))

                const linksRemoved = oldLinkIds.filter((linkId) => !newLinkIds.includes(linkId))
...
export const DELETE = withWorkspace(
    async ({ workspace, params }) => {
        ...
        if (webhook.installationId) {
            throw new DubApiError({
                code: 'bad_request',
                message: 'This webhook is managed by an integration, hence cannot be deleted manually.'
            })
        }
