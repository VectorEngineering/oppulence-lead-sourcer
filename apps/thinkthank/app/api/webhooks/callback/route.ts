import { handleWebhookFailure, resetWebhookFailureCount } from '@/lib/webhook/failure'
...
export const POST = async (req: Request) => {
    ...
    const { url, status, body, sourceBody, sourceMessageId } = webhookCallbackSchema.parse(JSON.parse(rawBody))

    const { webhookId, eventId, event } = searchParamsSchema.parse(getSearchParams(req.url))
    ...
    await Promise.all([
        ...
        ...(webhook.consecutiveFailures > 0 && !isFailed ? [resetWebhookFailureCount(webhookId)] : [])
    ])
