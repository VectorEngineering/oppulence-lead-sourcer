...
import { plainCopySection, plainDivider, plainEmptyContainer, plainSpacer, plainUsageSection } from './utils'
    ...
    if (!topWorkspace) {
        return NextResponse.json({
            cards: [
                {
                    key: 'workspace',
                    components: [plainEmptyContainer('User does not belong to any workspace.')]
                }
            ]
        })
    }
