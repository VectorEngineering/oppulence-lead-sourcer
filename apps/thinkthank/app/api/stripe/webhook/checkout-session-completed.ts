...
import { User } from '@dub/prisma-thinkthank/client'
...
export async function checkoutSessionCompleted(event: Stripe.Event) {
    ...
    if (checkoutSession.client_reference_id === null || checkoutSession.customer === null) {
        await log({
...
async function completeOnboarding({ users, workspaceId }: { users: Pick<User, 'id'>[]; workspaceId: string }) {
    ...
    await Promise.allSettled([
        ...
        (async () => {
            const data = await redis.get<{ domain: string; userId: string }>(`onboarding-domain:${workspaceId}`)
            ...
            try {
                ...
                console.error('Failed to register saved domain from onboarding', { domain, userId, workspace }, e)
            }
        })()
    ])
}
