...
export async function invoicePaymentFailed(event: Stripe.Event) {
    const { customer: stripeId, attempt_count: attemptCount, amount_due: amountDue } = event.data.object as Stripe.Invoice
    ...
    await Promise.allSettled([
        ...
        ...workspace.users.map(({ user }) =>
            sendEmail({
                ...
                subject: `${attemptCount === 2 ? '2nd notice: ' : attemptCount === 3 ? '3rd notice: ' : ''}Your payment for Dub.co failed`,
                react: (
                    <FailedPayment
