...
export async function customerSubscriptionDeleted(event: Stripe.Event) {
    ...
    if (!workspace) {
        await log({
            message: 'Workspace with Stripe ID *`' + stripeId + '`* not found in Stripe webhook `customer.subscription.deleted` callback',
            type: 'errors'
        })
    ...
    await Promise.allSettled([
        ...
        log({
            message: ':cry: Workspace *`' + workspace.slug + '`* deleted their subscription',
            type: 'cron',
            mention: true
        }),
