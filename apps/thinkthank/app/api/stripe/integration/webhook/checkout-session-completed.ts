...
import { Customer } from '@dub/prisma-thinkthank/client'
...
export async function checkoutSessionCompleted(event: Stripe.Event) {
    ...
    if (invoiceId) {
        ...
        if (!ok) {
            console.info('[Stripe Webhook] Skipping already processed invoice.', invoiceId)
    ...
    // for program links
    if (link.programId) {
        const { program, partnerId, commissionAmount } = await prisma.programEnrollment.findUniqueOrThrow({
            where: {
                linkId: link.id
            },
            select: {
