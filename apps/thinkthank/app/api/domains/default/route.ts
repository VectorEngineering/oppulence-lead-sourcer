    ...
    async ({ workspace, searchParams }) => {
        ...
        if (data) {
            defaultDomains = Object.keys(data)
                .filter((key) => data[key])
                .map((domain) => DUB_DOMAINS_ARRAY.find((d) => d.replace('.', '') === domain)!)
                .filter((domain) => (search ? domain?.toLowerCase().includes(search.toLowerCase()) : true))
        }
...
export const PATCH = withWorkspace(
    async ({ req, workspace }) => {
        const { defaultDomains } = await updateDefaultDomainsSchema.parseAsync(await req.json())

        if (workspace.plan === 'free' && defaultDomains.includes('dub.link')) {
            throw new DubApiError({
                code: 'forbidden',
                message: 'You can only use dub.link on a Pro plan and above. Upgrade to Pro to use this domain.'
            })
        }
