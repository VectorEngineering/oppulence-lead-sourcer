...
import { DomainSchema, createDomainBodySchema, getDomainsQuerySchemaExtended } from '@/lib/zod/schemas/domains'
import { DEFAULT_LINK_PROPS, combineWords, nanoid } from '@dub/utils'
...
export const GET = withWorkspace(
    async ({ workspace, searchParams }) => {
        const { search, archived, page, pageSize, includeLink } = getDomainsQuerySchemaExtended.parse(searchParams)
...
export const POST = withWorkspace(
    async ({ req, workspace, session }) => {
        ...
        const { slug, logo, expiredUrl, notFoundUrl, placeholder } = createDomainBodySchema.parse(body)
        ...
        if (workspace.plan === 'free') {
            if (logo || expiredUrl || notFoundUrl) {
                const proFeaturesString = combineWords(
                    [logo && 'custom QR code logos', expiredUrl && 'default expiration URLs', notFoundUrl && 'not found URLs'].filter(
                        Boolean
                    ) as string[]
                )
        ...
        const logoUploaded = logo ? await storage.upload(`domains/${domainId}/logo_${nanoid(7)}`, logo) : null
