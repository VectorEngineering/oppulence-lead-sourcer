    ...
    const domains = await Promise.all(
        data
            ...
            .map(async ({ id, hostname }: { id: number; hostname: string }) => ({
                id,
                domain: hostname,
                links: await fetchWithTimeout(`https://api-v2.short.cm/statistics/domain/${id}?period=total`, {
                    headers: {
                        'Content-Type': 'application/json',
                        Authorization: accessToken as string
                    }
...
export const POST = withWorkspace(async ({ req, workspace, session }) => {
    ...
    const domainsNotInWorkspace = selectedDomains.filter(({ domain }) => !domains?.find((d) => d.slug === domain))
