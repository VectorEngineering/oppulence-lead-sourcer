...
export const POST = withWorkspace(
    async ({ req, workspace, session }) => {
        ...
        if (workspaceUserCount + workspaceInviteCount + teammates.length > workspace.usersLimit) {
            throw new DubApiError({
                code: 'exceeded_limit',
        ...
        if (results.some((result) => result.status === 'rejected')) {
            throw new DubApiError({
                code: 'bad_request',
                message: teammates.length > 1 ? 'Some invitations could not be sent.' : 'Invitation could not be sent.'
            })
        }
