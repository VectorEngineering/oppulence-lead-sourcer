...
export const GET = withWorkspace(
    async ({ workspace }) => {
        ...
        const { data, error } = await directorySyncController.directories.getByTenantAndProduct(workspace.id, 'Dub')
        if (error) {
...
export const POST = withWorkspace(
    async ({ req, workspace }) => {
        const { provider = 'okta-scim-v2', currentDirectoryId } = createDirectorySchema.parse(await req.json())
...
export const DELETE = withWorkspace(
    async ({ searchParams }) => {
        ...
        const { error, data } = await directorySyncController.directories.delete(directoryId)
