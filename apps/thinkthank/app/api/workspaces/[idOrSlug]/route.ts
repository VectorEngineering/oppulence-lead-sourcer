...
export const PATCH = withWorkspace(
    async ({ req, workspace }) => {
        const { name, slug, logo } = await updateWorkspaceSchema.parseAsync(await req.json())

        const logoUploaded = logo ? await storage.upload(`workspaces/ws_${workspace.id}/logo_${nanoid(7)}`, logo) : null
