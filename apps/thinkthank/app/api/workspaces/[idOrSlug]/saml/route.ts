...
const createSAMLConnectionSchema = z
    ...
    .partial()
    .refine(({ metadataUrl, encodedRawMetadata }) => metadataUrl !== undefined || encodedRawMetadata !== undefined, {
...
export const POST = withWorkspace(
    async ({ req, workspace }) => {
        const { metadataUrl, encodedRawMetadata } = createSAMLConnectionSchema.parse(await req.json())
...
export const DELETE = withWorkspace(
    async ({ searchParams }) => {
        const { clientID, clientSecret } = deleteSAMLConnectionSchema.parse(searchParams)
