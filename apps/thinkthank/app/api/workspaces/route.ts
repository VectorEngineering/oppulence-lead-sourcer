...
import { FREE_WORKSPACES_LIMIT, generateRandomString, nanoid } from '@dub/utils'
...
export const GET = withSession(async ({ session }) => {
    ...
    return NextResponse.json(workspaces.map((project) => WorkspaceSchema.parse({ ...project, id: `ws_${project.id}` })))
})

export const POST = withSession(async ({ req, session }) => {
    const { name, slug, adminRole, organizationSize, reason } = await createWorkspaceSchema.parseAsync(await req.json())
