...
export const GET = withWorkspace(async ({ workspace, params, searchParams }) => {
    ...
    const { status, country, search, ids, page, pageSize, sortBy, sortOrder } = partnersQuerySchema.parse(searchParams)
    ...
    const partners = programEnrollments.map((enrollment) => ({
        ...
        earnings:
            ((program.commissionType === 'percentage' ? enrollment.link?.saleAmount : enrollment.link?.sales) ?? 0) *
