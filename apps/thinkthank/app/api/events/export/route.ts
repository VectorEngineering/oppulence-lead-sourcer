...
const columnAccessors = {
    ...
    customer: (r: LeadEvent | SaleEvent) => r.customer.name + (r.customer.email ? ` <${r.customer.email}>` : ''),
...
export const GET = withWorkspace(
    async ({ searchParams, workspace }) => {
        ...
        const link = domain && key ? await getLinkOrThrow({ workspaceId: workspace.id, domain, key }) : null
        ...
        const data = response.map((row) =>
            Object.fromEntries(columns.map((c) => [columnNames?.[c] ?? capitalize(c), columnAccessors[c]?.(row) ?? row?.[c]]))
        )
