...
export const GET = withWorkspace(async ({ workspace, params }) => {
    ...
    const timeToLead = customer.clickedAt && customer.createdAt ? customer.createdAt.getTime() - customer.clickedAt.getTime() : null

    // Find the time to first sale of the customer
    const firstSale = activity.filter(({ event }) => event === 'sale').pop()

    const timeToSale = firstSale && customer.createdAt ? new Date(firstSale.timestamp).getTime() - customer.createdAt.getTime() : null
