import { CustomerSchema, getCustomersQuerySchema, updateCustomerBodySchema } from '@/lib/zod/schemas/customers'
...
export const PATCH = withWorkspace(
    async ({ workspace, params, req, searchParams }) => {
        ...
        const { name, email, avatar, externalId } = updateCustomerBodySchema.parse(await parseRequestBody(req))
        ...
        try {
            ...
            return NextResponse.json(CustomerSchema.parse(transformCustomer(updatedCustomer)))
        } catch (error) {
            if (error.code === 'P2002') {
