...
export async function POST() {
    try {
        ...
        const emailData = yearInReviews.flatMap(({ workspace, totalClicks, totalLinks, topCountries, topLinks }) =>
            workspace.users
                .map(({ user }) => {
                    if (!user.email) {
                        return null
        ...
        console.log(`Sent ${emailData.length} emails to ${yearInReviews.length} workspaces!`)
        ...
        return new Response(`Sent ${emailData.length} emails to ${yearInReviews.length} workspaces!`)
    } catch (error) {
