import { DEFAULT_LINK_PROPS, DUB_DOMAINS_ARRAY, getPrettyUrl, log, normalizeString, parseDateTime } from '@dub/utils'
    ...
    try {
        ...
        const mapper = (row: Record<string, string>): MapperResult => {
            const getValueByNormalizedKey = (targetKey: string): string => {
                const key = Object.keys(row).find((k) => normalizeString(k) === normalizeString(targetKey))
            ...
            try {
                return {
                    success: true,
                    data: {
                        ...Object.fromEntries(Object.entries(mapping).map(([key, value]) => [key, getValueByNormalizedKey(value)])),
                        domain,
                        key,
                        createdAt: mapping.createdAt ? parseDateTime(getValueByNormalizedKey(mapping.createdAt)) || undefined : undefined,
                        tags: mapping.tags
        ...
        let cursor = parseInt((await redis.get(`import:csv:${workspaceId}:${id}:cursor`)) ?? '0')
        ...
        await new Promise((resolve, reject) => {
            Papa.parse(csvText as any, {
                ...
                beforeFirstChunk: (chunk: string) => {
                    ...
                    // Remove the first `cursor` lines and reassemble the chunk
                ...
                chunk: async (
                        ...
                        where: {
                            ...
                            key: {
                                in: data.map((row) => {
                                    ...
                                    return 'success' in result && result.success ? result.data.key : result.link.key
                                })
                            }
                        },
                        select: {
                    ...
                    const linksToCreate = data
                        ...
                        .filter(
                            (result): result is Extract<MapperResult, { success: true }> =>
                                'success' in result &&
                                result.success === true &&
                                !alreadyCreatedLinks.some((l) => l.domain === result.data.domain && l.key === result.data.key) &&
                    ...
                    const selectedDomains = [...new Set(linksToCreate.map(({ domain }) => domain))]
                    ...
                    if (errorLinks.length > 0) {
                        await redis.rpush(`import:csv:${workspaceId}:${id}:failed`, ...errorLinks)
                    }
                    ...
                    if (selectedDomains.length > 0) {
                        await redis.sadd(`import:csv:${workspaceId}:${id}:domains`, selectedDomains)
                    }
        ...
        const errorLinks = (await redis.lrange(`import:csv:${workspaceId}:${id}:failed`, 0, -1)) as any

        const affectedDomains = (await redis.smembers(`import:csv:${workspaceId}:${id}:domains`)) as any

        await sendCsvImportEmails({
            workspaceId,
            count,
            domains: Array.isArray(affectedDomains) && affectedDomains.length > 0 ? affectedDomains : [],
            errorLinks: Array.isArray(errorLinks) && errorLinks.length > 0 ? errorLinks : []
        })
