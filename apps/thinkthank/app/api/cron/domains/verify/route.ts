import { getConfigResponse, getDomainResponse, verifyDomain } from '@/lib/api/domains'
    ...
    try {
        ...
        const domains = await prisma.domain.findMany({
            where: {
                slug: {
                    ...
                    notIn: ['dub.sh', 'chatg.pt', 'amzn.id', 'spti.fi', 'stey.me', 'steven.yt', 'steven.blue', 'owd.li', 'elegance.ai']
                }
            },
            select: {
                slug: true,
        ...
        const results = await Promise.allSettled(
            domains.map(async (domain) => {
                ...
                const [domainJson, configJson] = await Promise.all([getDomainResponse(slug), getConfigResponse(slug)])
