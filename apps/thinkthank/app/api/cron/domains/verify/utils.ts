...
export const handleDomainUpdates = async ({
    ...
    const invalidDays = Math.floor((new Date().getTime() - new Date(createdAt).getTime()) / (1000 * 3600 * 24))
    ...
    if (invalidDays >= 28) {
        const sentSecondDomainInvalidEmail = sentEmails.includes(`secondDomainInvalidEmail:${domain}`)
    ...
    if (invalidDays >= 14) {
        const sentFirstDomainInvalidEmail = sentEmails.includes(`firstDomainInvalidEmail:${domain}`)
