...
export async function POST(req: Request) {
    try {
        ...
        if (remainingLinks > 0) {
            ...
            return new Response(`Deleted ${links.length} links, ${remainingLinks} remaining. Starting next batch...`)
        }
        ...
        await Promise.all([
            ...
            domainRecord.logo && storage.delete(domainRecord.logo.replace(`${R2_URL}/`, ''))
        ])

        return new Response(`Deleted ${links.length} links, no more links remaining. Domain deleted.`)
    } catch (error) {
