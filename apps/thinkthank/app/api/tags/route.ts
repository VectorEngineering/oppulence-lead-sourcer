...
import { TagSchema, createTagBodySchema, getTagsQuerySchemaExtended } from '@/lib/zod/schemas/tags'
...
export const GET = withWorkspace(
    async ({ workspace, headers, searchParams }) => {
        const { search, ids, sortBy, sortOrder, page, pageSize, includeLinksCount } = getTagsQuerySchemaExtended.parse(searchParams)
    ...
    async ({ req, workspace, headers }) => {
        ...
        const response = await prisma.tag.create({
            data: {
                ...
                name: tag || name!,
                color: color && COLORS_LIST.map(({ color }) => color).includes(color) ? color : randomBadgeColor(),
                projectId: workspace.id
            }
        })
