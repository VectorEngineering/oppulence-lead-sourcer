...
import { UTMTags, deepEqual } from '@dub/utils'
...
export const PATCH = withWorkspace(
    async ({ req, headers, workspace, params }) => {
        ...
        const updatedLink = {
            ...
            ...Object.fromEntries(UTMTags.map((tag) => [tag, body[tag] === link[tag] ? undefined : body[tag]])),
        ...
        const skipKeyChecks = link.domain === updatedLink.domain && link.key.toLowerCase() === updatedLink.key?.toLowerCase()

        // if externalId is the same, we don't need to check if it exists
        const skipExternalIdChecks = link.externalId?.toLowerCase() === updatedLink.externalId?.toLowerCase()
