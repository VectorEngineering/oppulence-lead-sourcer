...
import { bulkCreateLinks, checkIfLinksHaveTags, checkIfLinksHaveWebhooks, processLink } from '@/lib/api/links'
import { NewLinkProps, ProcessedLinkProps } from '@/lib/types'
import { bulkCreateLinksBodySchema, bulkUpdateLinksBodySchema } from '@/lib/zod/schemas/links'
    ...
    async ({ req, headers, session, workspace }) => {
        if (!workspace) {
            throw new DubApiError({
                code: 'bad_request',
                message: 'Missing workspace. Bulk link creation is only available for custom domain workspaces.'
            })
        }
        ...
        if (workspace.linksUsage + links.length > workspace.linksLimit && (workspace.plan === 'free' || workspace.plan === 'pro')) {
            throw new DubApiError({
                code: 'exceeded_limit',
        ...
        const duplicates = links.filter(
            (link, index, self) => link.key && self.slice(index + 1).some((l) => l.domain === link.domain && l.key === link.key)
        )
        if (duplicates.length > 0) {
            throw new DubApiError({
                code: 'bad_request',
                message: `Duplicate links found: ${duplicates.map((link) => `${link.domain}/${link.key}`).join(', ')}`
            })
        }
        ...
        let validLinks = processedLinks.filter(({ error }) => error == null).map(({ link }) => link) as ProcessedLinkProps[]
        ...
        if (checkIfLinksHaveTags(validLinks)) {
            ...
            const tagIds = validLinks
                .flatMap((link) => combineTagIds({ tagId: link.tagId, tagIds: link.tagIds }))
                .filter(Boolean) as string[]
            const tagNames = validLinks.flatMap((link) => link.tagNames).filter(Boolean) as string[]
            ...
            validLinks.forEach((link, index) => {
                ...
                const invalidTagIds = combinedTagIds.filter((id) => !workspaceTagIds.includes(id))
                ...
                const invalidTagNames = link.tagNames?.filter((name) => !workspaceTagNames.includes(name))
        ...
        if (checkIfLinksHaveWebhooks(validLinks)) {
            if (workspace.plan === 'free' || workspace.plan === 'pro') {
                throw new DubApiError({
                    code: 'forbidden',
                    message: 'You can only use webhooks on a Business plan and above. Upgrade to Business to use this feature.'
                })
            }

            const webhookIds = validLinks.flatMap((link) => link.webhookIds).filter(Boolean) as string[]
            ...
            validLinks.forEach((link, index) => {
                const invalidWebhookIds = link.webhookIds?.filter((id) => !workspaceWebhookIds.includes(id))
        ...
        const validLinksResponse = validLinks.length > 0 ? await bulkCreateLinks({ links: validLinks }) : []
...
export const PATCH = withWorkspace(
    async ({ req, workspace, headers }) => {
        const { linkIds, externalIds, data } = bulkUpdateLinksBodySchema.parse(await parseRequestBody(req))
        ...
        const links = await prisma.link.findMany({
            where: {
                projectId: workspace.id,
                ...(linkIds.length > 0 ? { id: { in: linkIds } } : { externalId: { in: externalIds } })
            }
        })
        ...
        let errorLinks = linkIds
            ...
            .concat(
                externalIds
                    .filter((id) => links.find((link) => link.externalId === id) === undefined)
                    .map((id) => ({
                        error: 'Link not found',
                        code: 'not_found',
                        link: { id }
        ...
        const validLinkIds = processedLinks.filter(({ error }) => error == null).map(({ link }) => link.id) as string[]
                ...
                if (data.proxy && data.image) {
                    await Promise.allSettled(
                        links.map(async (link) => {
                            ...
                            if (link.image?.startsWith(`${R2_URL}/images/${link.id}`) && link.image !== data.image) {
...
export const DELETE = withWorkspace(
    async ({ workspace, headers, searchParams }) => {
        const searchParamsLinkIds = searchParams['linkIds'] ? searchParams['linkIds'].split(',') : []
        ...
        const links = await prisma.link.findMany({
            where: {
                ...
                OR: [
                    ...
                    ...(externalIds.size > 0 ? [{ externalId: { in: Array.from(externalIds) } }] : [])
                ]
            },
            include: {
                tags: {
