...
export const GET = withWorkspace(
    async ({ searchParams, workspace }) => {
        ...
        const { domain, tagIds, search, sort, userId, showArchived, withTags, start, end, interval, columns } =
            ...
            links.map((link) => {
                ...
                let result = new Map([
                    ...(columns.includes('link')
                        ? [['link', linkConstructor({ domain: link.domain, key: link.key })] as [string, string]]
                        : []),
                    ...Object.entries(link)
                ])
                ...
                if (columns.includes('tags')) {
                    result.set('tags', link.tags.map((tag) => (tag as any).tag.name).join(', '))
                }
