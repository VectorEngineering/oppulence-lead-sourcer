...
export default function PlaceholderContent() {
    ...
    return (
        <motion.div className='z-10 mb-20' exit={{ opacity: 0, scale: 0.95 }} transition={{ duration: 0.5, type: 'spring' }}>
            <div
                className={`${
                    loading ? 'scale-[25%] blur-md' : 'scale-100 blur-0'
                } mt-[7vh] h-[50vh] w-screen object-cover transition-all duration-1000`}
            >
                <Spline onLoad={onLoad} style={{ opacity: opacity }} scene={`${platform.assetsUrl}/scene.splinecode`} />
            </div>
            <motion.div
                variants={{
                    show: {
            ...
            >
                ...
                <motion.p className='max-w-xl text-gray-600 transition-colors sm:text-lg' variants={STAGGER_CHILD_VARIANTS}>
