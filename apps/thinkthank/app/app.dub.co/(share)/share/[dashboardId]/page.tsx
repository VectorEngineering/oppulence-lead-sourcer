...
export async function generateMetadata({ params }: { params: { dashboardId: string } }) {
...
export default async function dashboardPage({ params }: { params: { dashboardId: string } }) {
    ...
    if (data.password && cookies().get(`dub_password_${params.dashboardId}`)?.value !== data.password) {
        return (
                ...
                <div className='z-10 w-full max-w-md overflow-hidden rounded-2xl border border-gray-100 shadow-xl'>
                    <div className='flex flex-col items-center justify-center space-y-3 border-b border-gray-200 bg-white px-4 py-6 pt-8 text-center sm:px-16'>
                        ...
                        <p className='text-sm text-gray-500'>
                            This dashboard is password protected. Enter the password to view the dashboard.
                        </p>
                    </div>
