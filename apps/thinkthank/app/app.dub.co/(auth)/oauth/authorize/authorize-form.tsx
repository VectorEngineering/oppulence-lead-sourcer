...
export const AuthorizeForm = ({
    ...
    useEffect(() => {
        setSelectedWorkspace(selectOptions.find((option) => option.id === session?.user?.['defaultWorkspace']) || null)
    ...
    useEffect(() => {
        ...
        const workspace = workspaces?.find((workspace) => workspace.slug === selectedWorkspace.id)
        ...
        const scopesMissing = consolidateScopes(scope).filter((scope) => !scopesForRole.includes(scope) && scope !== 'user.read')
    ...
    const onAuthorize = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        const workspaceId = workspaces?.find((workspace) => workspace.slug === selectedWorkspace.id)?.id

        const response = await fetch(`/api/oauth/authorize?workspaceId=${workspaceId}`, {
            method: 'POST',
    ...
    return (
        <>
            ...
            <form onSubmit={onAuthorize}>
                ...
                <p className='text-sm text-gray-500'>Select a workspace to grant API access to</p>
                ...
                <div className='mt-4 flex justify-between gap-4'>
                    <Button text='Decline' type='button' onClick={onDecline} variant='secondary' disabled={submitting} />
                    <Button
                        text='Authorize'
                        type='submit'
                        loading={submitting}
