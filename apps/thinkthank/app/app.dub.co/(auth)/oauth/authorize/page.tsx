...
export default async function Authorize({ searchParams }: { searchParams?: z.infer<typeof authorizeRequestSchema> }) {
    ...
    const { error, integration, requestParams } = await vaidateAuthorizeRequest(searchParams)

    if (error || !integration) {
        return <EmptyState icon={CubeSettings} title='Invalid OAuth Request' description={error} />
    }
    ...
    return (
        <div className='relative z-10 m-auto w-full max-w-md border-y border-gray-200 sm:rounded-2xl sm:border sm:shadow-xl'>
            <div className='flex flex-col items-center justify-center gap-3 border-b border-gray-200 bg-white px-4 py-6 pt-8 text-center sm:rounded-t-2xl sm:px-16'>
                ...
                <p className='text-md'>
                    <span className='font-bold'>{integration.name}</span> is requesting API access to a workspace on Dub.
                </p>
                <span className='text-xs text-gray-500'>
                    Built by{' '}
                    <a href={integration.website} target='_blank' rel='noreferrer' className='underline'>
