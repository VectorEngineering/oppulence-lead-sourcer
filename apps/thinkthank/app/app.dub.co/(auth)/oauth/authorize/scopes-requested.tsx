...
export const ScopesRequested = ({ scopes }: ScopesProps) => {
    ...
    scopeWithDescriptions.forEach((scope) => {
        scope.description = scope.description.replace('Write', "<strong className='font-medium'>Write</strong>")

        scope.description = scope.description.replace('Read', "<strong className='font-medium'>Read</strong>")
    })
