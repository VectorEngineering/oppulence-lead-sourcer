...
export const RequestSetPassword = () => {
    ...
    const sendPasswordSetRequest = async () => {
        try {
            ...
            if (response.ok) {
                toast.success(`We've sent you an email to ${user?.email} with instructions to set your password`)
                return
            }
        ...
        <div className='rounded-lg border border-gray-200 bg-white'>
            <div className='flex flex-col gap-3 border-b border-gray-200 p-5 sm:p-10'>
                ...
                <p className='pb-2 text-sm text-gray-500'>
                    {user?.provider && (
                        <>
                            Your account is managed by <span className='uppercase'>{user?.provider}</span>.{' '}
                        </>
                    )}
