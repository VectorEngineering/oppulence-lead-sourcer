...
import { Alert, AlertDescription, AlertTitle, IconMenu, LoadingSpinner, Popover, TokenAvatar } from '@dub/ui'
...
export default function TokensPageClient() {
    const { data: tokens, isLoading } = useSWR<TokenProps[]>('/api/user/tokens', fetcher)

    return (
        <>
            <Alert>
                <Info className='mt-2 h-5 w-5 text-yellow-500' />
                <AlertTitle>User API Keys have been replaced by Workspace API Keys.</AlertTitle>
                <AlertDescription className='text-gray-500'>
                    ...
                    for more granular control over your resources such as Links, Tags, Domains, Analytics, etc.{' '}
                    <a
            ...
            <div className='rounded-lg border border-gray-200 bg-white'>
                <div className='flex flex-col space-y-3 p-5 sm:p-10'>
                    ...
                    <p className='text-sm text-gray-500'>
                        These API keys allow other apps to access your account. Use it with caution – do not share your API key with others,
                ...
                {isLoading || !tokens ? (
                    ...
                    <div className='flex flex-col items-center justify-center space-y-4 pb-20 pt-10'>
                        ...
                        <p className='text-sm text-gray-500'>No API keys found. Create one above.</p>
                    </div>
                )}
            </div>
        </>
                ...
                <Popover
                    content={
                        <div className='grid w-full gap-1 p-2 sm:w-48'>
                            ...
                            >
                                <IconMenu text='Delete API Key' icon={<Trash className='h-4 w-4' />} />
                            </button>
                        </div>
                    }
                    align='end'
