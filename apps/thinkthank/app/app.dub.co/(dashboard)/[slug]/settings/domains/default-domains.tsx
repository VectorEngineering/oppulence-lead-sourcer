...
import { Amazon, CalendarDays, ChatGPT, Figma, GitHubEnhanced, GoogleEnhanced, Spotify } from '@dub/ui/icons'
...
export function DefaultDomains() {
    ...
    return (
        <div className='my-10 grid gap-5 border-t border-gray-200 py-10'>
            <div>
                <h2 className='text-xl font-semibold tracking-tight text-black'>Default Domains</h2>
                <p className='mt-3 text-sm text-gray-500'>
                    Leverage default branded domains from {platform.company} for specific links.{' '}
                    <Link
            ...
            <div className='mt-2 grid grid-cols-1 gap-3'>
                {DUB_DOMAINS.filter((domain) => {
                    ...
                    return (
                        <div key={slug} className='flex items-center justify-between gap-4 rounded-xl border border-gray-200 bg-white p-5'>
                            <DomainCardTitleColumn domain={slug} icon={DubDomainsIcon(slug)} description={description} defaultDomain />
                            <Switch
                                disabled={submitting}
                                disabledTooltip={
                                    permissionsError ||
