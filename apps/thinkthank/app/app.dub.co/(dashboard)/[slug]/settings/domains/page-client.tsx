...
export default function WorkspaceDomainsClient() {
    const { id: workspaceId, plan, nextPlan, role, domainsLimit, exceededDomains, dotLinkClaimed } = useWorkspace()
    ...
    const { AddEditDomainModal, AddDomainButton, setShowAddEditDomainModal } = useAddEditDomainModal({
        buttonProps: {
            className: 'h-9 rounded-lg'
        }
    })

    const { RegisterDomainModal, setShowRegisterDomainModal } = useRegisterDomainModal()

    const { RegisterDomainSuccessModal, setShowRegisterDomainSuccessModal } = useRegisterDomainSuccessModal()

    useEffect(() => setShowRegisterDomainSuccessModal(searchParams.has('registered')), [searchParams])
            ...
            <div className='grid gap-5'>
                <div className='flex flex-wrap justify-between gap-6'>
                    <div className='flex items-center gap-x-2'>
                        <h1 className='text-2xl font-semibold tracking-tight text-black'>Domains</h1>
                        <InfoTooltip
                            content={
                                <TooltipContent
                    ...
                    {!loading ? (
                        allWorkspaceDomains.length > 0 ? (
                            ...
                            <div className='flex flex-col items-center gap-4 rounded-xl border border-gray-200 py-10'>
                                <EmptyState icon={Globe} title={archived ? 'No archived domains found' : 'No custom domains found'} />
                                <AddDomainButton />
                            </div>
                        ) : (
                            <AnimatedEmptyState
