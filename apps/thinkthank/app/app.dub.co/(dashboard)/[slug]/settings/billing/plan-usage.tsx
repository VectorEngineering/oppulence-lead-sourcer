...
import { CircleDollar, CursorRays, Globe, Hyperlink, Tag, Users } from '@dub/ui/icons'
import { cn, getFirstAndLastDay, INFINITY_NUMBER, nFormatter } from '@dub/utils'
    ...
    return (
        <div className='rounded-lg border border-neutral-200 bg-white'>
                ...
                <div>
                    ...
                    <p className='mt-1 text-balance text-sm leading-normal text-neutral-500'>
                        ...
                        {plan ? (
                            ...
                            <span className='rounded-full bg-neutral-200 px-2 py-0.5 text-xs text-neutral-200'>load</span>
                        )}{' '}
                        plan.
            ...
            <div className='grid grid-cols-[minmax(0,1fr)] divide-y divide-neutral-200 border-t border-neutral-200'>
                <div>
                    <div className={cn('grid gap-4 p-6 sm:grid-cols-2 md:p-8 lg:gap-6', conversionEnabled && 'sm:grid-cols-3')}>
                        <UsageTabCard id='events' icon={CursorRays} title='Events tracked' usage={usage} limit={usageLimit} />
                        <UsageTabCard id='links' icon={Hyperlink} title='Links created' usage={linksUsage} limit={linksLimit} />
                        {conversionEnabled && (
                            <UsageTabCard
                                id='revenue'
                                icon={CircleDollar}
                ...
                <div className='grid grid-cols-1 divide-y divide-neutral-200 sm:divide-x sm:divide-y-0 md:grid-cols-3'>
                    <UsageCategory title='Custom Domains' icon={Globe} usage={domains?.length} usageLimit={domainsLimit} />
                    <UsageCategory title='Tags' icon={Tag} usage={tags?.length} usageLimit={tagsLimit} />
                    <UsageCategory
                        title='Teammates'
                        icon={Users}
            ...
            {plan !== 'enterprise' && (
                <div className='flex flex-col items-center justify-between space-y-3 border-t border-neutral-200 px-6 py-4 text-center md:flex-row md:space-y-0 md:px-8 md:text-left'>
                    ...
                    <Link
                        href={`/${slug}/upgrade`}
                        className={cn(buttonVariants(), 'flex h-8 w-fit items-center justify-center rounded-md border px-3 text-sm')}
                    >
...
function UsageTabCard({
    ...
    const isActive = searchParams.get('tab') === id || (!searchParams.get('tab') && id === 'events')

    const [usage, limit] =
        unit === '$' && usageProp !== undefined && limitProp !== undefined ? [usageProp / 100, limitProp / 100] : [usageProp, limitProp]
...
function UsageCategory(data: { title: string; icon: Icon; usage?: number; usageLimit?: number }) {
        ...
        <div className='flex items-center justify-between p-6 md:p-8'>
            ...
            <div className='flex items-center gap-1.5 font-medium text-black'>
                ...
                <p className='text-neutral-400'>
                    {usageLimit && usageLimit >= INFINITY_NUMBER ? '∞' : nFormatter(usageLimit, { full: true })}
                </p>
            </div>
        </div>
    )
