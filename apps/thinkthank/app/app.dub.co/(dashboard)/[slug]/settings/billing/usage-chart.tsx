...
const resourceEmptyStates: Record<(typeof RESOURCES)[number], ComponentProps<typeof EmptyState>> = {
    links: {
        icon: Hyperlink,
        title: 'Links Created',
        description: 'No short links have been created in the current billing cycle.'
    },
    events: {
        icon: CursorRays,
...
export function UsageChart() {
    ...
    const resource = RESOURCES.find((r) => r === searchParams.get('tab')) ?? 'events'
    ...
    const allZeroes = useMemo(() => chartData?.every(({ values }) => values.usage === 0), [chartData])
                ...
                !allZeroes ? (
                    <TimeSeriesChart
                        ...
                        tooltipContent={(d) => {
                            return (
                                <>
                                    <p className='border-b border-gray-200 px-4 py-3 text-sm text-gray-900'>{formatDate(d.date)}</p>
                    ...
                    >
                        ...
                        <YAxis showGridLines tickFormat={resource === 'revenue' ? (v) => `$${nFormatter(v)}` : nFormatter} />
                        <Bars
                            seriesStyles={[
                                {
                                    id: 'usage',
