...
export default function PaymentMethods() {
    ...
    const achPaymentMethods = paymentMethods?.filter((pm) => pm.type === 'us_bank_account')
        ...
        <div className='rounded-lg border border-neutral-200 bg-white'>
            <div className='flex flex-col items-center justify-between gap-y-4 p-6 md:p-8 xl:flex-row'>
                <div>
                    ...
                    <p className='text-balance text-sm leading-normal text-neutral-500'>Manage your payment methods on Dub</p>
                </div>
                {stripeId && <ManageSubscriptionButton text='Manage' className='w-fit' />}
            </div>
                ...
                {regularPaymentMethods ? (
                    regularPaymentMethods.length > 0 ? (
                        regularPaymentMethods.map((paymentMethod) => (
                            <PaymentMethodCard key={paymentMethod.id} type={paymentMethod.type} paymentMethod={paymentMethod} />
                        ))
                    ) : (
                        <AnimatedEmptyState
...
const PaymentMethodCard = ({ type, paymentMethod }: { type: Stripe.PaymentMethod.Type; paymentMethod?: Stripe.PaymentMethod }) => {
    ...
    const managePaymentMethods = async (method: string) => {
        setIsLoading(true)
        const { url } = await fetch(`/api/workspaces/${slug}/billing/payment-methods`, {
            method: 'POST',
    ...
    return (
        <div className='flex items-center justify-between rounded-lg border border-neutral-200 p-4'>
            <div className='flex items-center gap-4'>
                <div className={cn('flex size-12 items-center justify-center rounded-lg bg-neutral-100', iconBgColor)}>
                    <Icon className='size-6 text-neutral-700' />
                </div>
                <div>
                    <div className='flex items-center gap-2'>
                        <p className='font-medium text-neutral-900'>{title}</p>
                        {type === 'us_bank_account' && <Badge variant='neutral'>Recommended for {platform.company} Partners payouts</Badge>}
                    </div>
