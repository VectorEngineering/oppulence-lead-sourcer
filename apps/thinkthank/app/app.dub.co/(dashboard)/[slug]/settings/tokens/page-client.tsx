...
import { Button, Dots, Icon, Key, PenWriting, Popover, Table, Tooltip, buttonVariants, usePagination, useTable } from '@dub/ui'
...
export default function TokensPageClient() {
    ...
    const { data: tokens, isLoading, error } = useSWR<TokenProps[]>(`/api/tokens?workspaceId=${workspaceId}`, fetcher)
    ...
    const { AddEditTokenModal, AddTokenButton, setShowAddEditTokenModal } = useAddEditTokenModal({
        ...(selectedToken && {
            token: {
                id: selectedToken.id,
                                ...
                                <img
                                    src={
                                        row.original.user.isMachine
                                            ...
                                            : row.original.user.image || `${DICEBEAR_AVATAR_URL}${row.original.user.id}`
                                    }
    ...
    return (
        <div className='grid grid-cols-1'>
            ...
            <h1 className='text-2xl font-semibold tracking-tight text-black'>Secret keys</h1>
            <p className='mb-2 mt-2 text-base text-neutral-600'>
                These API keys allow other apps to access your workspace. Use it with caution – do not share your API key with others, or
                expose it in the browser or other client-side code.{' '}
                <a
...
function RowMenuButton({ token, onEdit }: { token: TokenProps; onEdit: () => void }) {
...
function MenuItem({ icon: IconComp, label, onSelect, danger }: { icon: Icon; label: string; onSelect: () => void; danger?: boolean }) {
    return (
        <Command.Item
            className={cn(
                'flex cursor-pointer select-none items-center gap-2 whitespace-nowrap rounded-md p-2 text-sm text-neutral-600',
                danger ? buttonVariants({ variant: 'danger-outline' }) : 'text-neutral-500 data-[selected=true]:bg-gray-100'
            )}
            onSelect={onSelect}
        >
            <IconComp className={cn('size-4 shrink-0', danger ? 'hover:bg-red-600 hover:text-white' : 'text-neutral-500')} />
            {label}
        </Command.Item>
    )
}
