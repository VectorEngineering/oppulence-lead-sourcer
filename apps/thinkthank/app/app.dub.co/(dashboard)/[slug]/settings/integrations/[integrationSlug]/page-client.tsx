...
import { CircleWarning, ConnectedDots, Globe, OfficeBuilding, ShieldCheck } from '@dub/ui/icons'
import {
...
export default function IntegrationPageClient({ integration }: { integration: InstalledIntegrationInfoProps }) {
    ...
    const { UninstallIntegrationModal, setShowUninstallIntegrationModal } = useUninstallIntegrationModal({
        integration
    })
    ...
    return (
        <MaxWidthWrapper className='grid max-w-screen-lg gap-8'>
            ...
            <Link href={`/${slug}/settings/integrations`} className='flex items-center gap-x-1'>
                ...
                <div className='flex items-center gap-x-2'>
                    {slug === 'dub' && (
                        <Link
                            ...
                            className={cn(buttonVariants({ variant: 'secondary' }), 'flex h-9 items-center rounded-md border px-4 text-sm')}
                        >
                            Manage
                        </Link>
                    )}
                    {!integration.installed && integration.id !== SEGMENT_INTEGRATION_ID && (
                        <Button
                            onClick={() => {
