...
import { Avatar, Button, CardList, Popover, Tooltip, UTM_PARAMETERS, useKeyboardShortcut } from '@dub/ui'
...
export function TemplateCard({ template }: { template: UtmTemplateWithUserProps }) {
    const { id } = useWorkspace()

    const { openMenuTemplateId, setOpenMenuTemplateId } = useContext(TemplatesListContext)
    ...
    const { AddEditUtmTemplateModal, setShowAddEditUtmTemplateModal } = useAddEditUtmTemplateModal({
        props: template
    })
            ...
            >
                <div className='flex min-w-0 grow items-center gap-3'>
                    <div className='flex min-w-0 items-center gap-2'>
                        ...
                        <span className='min-w-0 truncate whitespace-nowrap font-medium text-gray-800'>{template.name}</span>
                    </div>
                        ...
                        <div className='grid max-w-[225px] grid-cols-[1fr,minmax(0,min-content)] gap-x-2 gap-y-1 whitespace-nowrap p-2 text-sm sm:min-w-[150px]'>
                            {includedParams.map(({ key, label, icon: Icon }) => (
                                <Fragment key={key}>
                                    ...
                                    <span className='truncate text-gray-500'>{template[key]}</span>
                                </Fragment>
                            ))}
                        </div>
                    }
                >
                    <div className='xs:flex hidden shrink-0 items-center gap-1 px-2 text-gray-500'>
                        {includedParams.map(({ icon: Icon, key }) => (
                            <Icon className='size-3.5' key={`${Icon}-${key}-${Math.random()}`} />
                        ))}
                    </div>
                </Tooltip>

                <div className='hidden text-sm text-gray-500 sm:block'>{formatDate(template.updatedAt, { month: 'short' })}</div>
                ...
                <div className='flex items-center gap-5 sm:gap-8 md:gap-12'>
                    ...
                    >
                        <Button
                            ...
                            icon={processing ? <LoadingSpinner className='h-5 w-5 shrink-0' /> : <ThreeDots className='h-5 w-5 shrink-0' />}
                            onClick={() => {
...
function TemplateCardKeyboardShortcuts({ enabled, onKeyDown }: { enabled: boolean; onKeyDown: (e: KeyboardEvent) => void }) {
