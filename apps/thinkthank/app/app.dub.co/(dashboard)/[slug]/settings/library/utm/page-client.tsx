...
export default function WorkspaceUtmTemplatesClient() {
    ...
    const { AddEditUtmTemplateModal, AddUtmTemplateButton } = useAddEditUtmTemplateModal()
            ...
            <div className='grid gap-4'>
                ...
                {isLoading || templates?.length ? (
                    <>
                        ...
                        <TemplatesListContext.Provider value={{ openMenuTemplateId, setOpenMenuTemplateId }}>
                            <CardList variant='compact' loading={isLoading}>
                                {templates?.length
                                    ? templates.map((template) => <TemplateCard key={template.id} template={template} />)
                                    : Array.from({ length: 6 }).map((_, idx) => <TemplateCardPlaceholder key={idx} />)}
                            </CardList>
