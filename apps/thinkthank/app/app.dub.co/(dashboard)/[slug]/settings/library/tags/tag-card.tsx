...
export function TagCard({ tag }: { tag: TagProps & { _count?: { links: number } } }) {
    ...
    const handleDelete = async () => {
        if (!confirm("Are you sure you want to delete this tag? All tagged links will be untagged, but they won't be deleted.")) return
        ...
        fetch(`/api/tags/${tag.id}?workspaceId=${id}`, {
            ...
            .then(async (res) => {
                if (res.ok) {
                    await Promise.all([mutatePrefix('/api/tags'), mutatePrefix('/api/links')])
        ...
        <>
            ...
            >
                <div className='flex min-w-0 grow items-center gap-3'>
                    ...
                    <span className='min-w-0 truncate whitespace-nowrap text-gray-800'>{tag.name}</span>
                </div>
                        ...
                        content={
                            <div className='grid w-full gap-px p-2 sm:w-48'>
                                ...
                                <Button
                                    ...
                                    variant='outline'
                                    onClick={() => copyTagId()}
                                    icon={copiedTagId ? <CircleCheck className='h-4 w-4' /> : <Copy className='h-4 w-4' />}
                                    shortcut='I'
                                    className='h-9 px-2 font-medium'
                                />
                                <Button
...
function TagCardKeyboardShortcuts({ enabled, onKeyDown }: { enabled: boolean; onKeyDown: (e: KeyboardEvent) => void }) {
