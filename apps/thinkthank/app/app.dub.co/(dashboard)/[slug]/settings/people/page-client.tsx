...
import { CheckCircleFill, Link as LinkIcon, ThreeDots } from '@/ui/shared/icons'
import { Avatar, Badge, Button, Copy, IconMenu, Popover, useCopyToClipboard } from '@dub/ui'
...
export default function WorkspacePeopleClient() {
    const { setShowInviteTeammateModal, InviteTeammateModal } = useInviteTeammateModal({ showSavedInvites: true })
            ...
            <div className='rounded-lg border border-gray-200 bg-white'>
                <div className='flex flex-col items-center justify-between space-y-3 p-5 sm:flex-row sm:space-y-0 sm:p-10'>
                    <div className='flex flex-col space-y-3'>
                        ...
                        <p className='text-sm text-gray-500'>Teammates that have access to this workspace.</p>
                    </div>
                ...
                <div className='flex space-x-3 border-b border-gray-200 px-3 sm:px-7'>
                    {tabs.map((tab) => (
                        <div key={tab} className={`${tab === currentTab ? 'border-black' : 'border-transparent'} border-b py-1`}>
                            <button
                ...
                <div className='grid divide-y divide-gray-200'>
                    {users ? (
                        users.length > 0 ? (
                            users.map((user) => <UserCard key={user.id} user={user} currentTab={currentTab} />)
                        ) : (
...
const UserCard = ({ user, currentTab }: { user: WorkspaceUserProps; currentTab: 'Members' | 'Invitations' }) => {
    ...
    const { RemoveTeammateModal, setShowRemoveTeammateModal } = useRemoveTeammateModal({ user, invite: currentTab === 'Invitations' })

    const { data: session } = useSession()

    // invites expire after 14 days of being sent
    const expiredInvite = currentTab === 'Invitations' && createdAt && Date.now() - new Date(createdAt).getTime() > 14 * 24 * 60 * 60 * 1000
            ...
            <div key={id} className='flex items-center justify-between space-x-3 px-4 py-3 sm:pl-8'>
                ...
                <div className='flex items-center gap-x-3'>
                    {currentTab === 'Members' ? (
                        ...
                        <>
                            <p className='hidden text-xs text-gray-500 sm:block' suppressHydrationWarning>
                                {capitalize(user.role)}
                            </p>
                            <p className='text-right text-xs text-gray-500 sm:min-w-28' suppressHydrationWarning>
                        ...
                        content={
                            <div className='grid w-full gap-1 p-2 sm:w-48'>
                                <Button
                                    ...
                                    variant='outline'
                                    onClick={() => copyUserId()}
                                    icon={copiedUserId ? <CheckCircleFill className='h-4 w-4' /> : <Copy className='h-4 w-4' />}
