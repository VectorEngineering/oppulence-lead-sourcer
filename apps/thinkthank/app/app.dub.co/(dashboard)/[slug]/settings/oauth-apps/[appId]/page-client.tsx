...
import { BlurImage, Button, MaxWidthWrapper, Popover, TokenAvatar } from '@dub/ui'
...
export default function OAuthAppManagePageClient({ appId }: { appId: string }) {
    ...
    const { executeAsync, result, isExecuting } = useAction(generateClientSecret, {
        onSuccess: () => {
    ...
    const { data: oAuthApp, isLoading } = useSWR<OAuthAppProps>(`/api/oauth/apps/${appId}?workspaceId=${workspaceId}`, fetcher)

    const { RemoveOAuthAppModal, setShowRemoveOAuthAppModal } = useRemoveOAuthAppModal({
        oAuthApp
    })

    const { SubmitOAuthAppModal, setShowSubmitOAuthAppModal } = useSubmitOAuthAppModal({
        oAuthApp
    })
    ...
    return (
        <>
            <MaxWidthWrapper className='grid max-w-screen-lg gap-8'>
                <RemoveOAuthAppModal />
                <SubmitOAuthAppModal />
                <Link href={`/${slug}/settings/oauth-apps`} className='flex items-center gap-x-1'>
                    <ChevronLeft className='size-4' />
                    <p className='text-sm font-medium text-gray-500'>Back to OAuth Apps</p>
                </Link>
                    ...
                    {isLoading ? (
                        ...
                        <div className='flex flex-col gap-3 sm:flex-row sm:items-center'>
                            ...
                            <div>
                                ...
                                <p className='text-pretty text-sm text-gray-500'>{oAuthApp?.description}</p>
                            </div>
                        </div>
                    )}
                ...
                {oAuthApp && (
                    <>
                        <OAuthAppCredentials
                            ...
                            clientSecret={result.data?.clientSecret || searchParams.get('client_secret') || null}
