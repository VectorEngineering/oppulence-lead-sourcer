...
import { Button, Tooltip, TooltipContent, buttonVariants } from '@dub/ui'
...
import { PayoutStatus } from '@dub/prisma-thinkthank/client'
...
export function PayoutStats() {
    ...
    const { payoutsCount: eligiblePayoutsCount, loading: eligiblePayoutsLoading } = usePayoutsCount<PayoutsCount[]>({
        groupBy: 'status',
        eligibility: 'eligible'
    })

    const allPendingPayouts = payoutsCount?.find((p) => p.status === PayoutStatus.pending)

    const eligiblePendingPayouts = eligiblePayoutsCount?.find((p) => p.status === PayoutStatus.pending)
    ...
    const totalPaid = payoutsCount?.find((p) => p.status === PayoutStatus.completed)
                ...
                <div className='flex flex-col p-4'>
                    ...
                    <div
                        className={cn(
                            ...
                            pendingIneligiblePayouts && 'underline decoration-dotted underline-offset-2'
                        )}
                    >
                        {loading || eligiblePayoutsLoading ? (
                                        ...
                                        <div className='grid gap-1 p-3'>
                                            {[
                                                ...
                                                <div className='flex justify-between' key={index}>
                                                    <div className='text-sm text-neutral-500'>{display}</div>
                            ...
                            >
                                <span className='underline decoration-dotted underline-offset-2'>
                                    {currencyFormatter(eligiblePendingPayouts?.amount ? eligiblePendingPayouts.amount / 100 : 0, {
                                        maximumFractionDigits: 2
                                    }) + ' USD'}
                                </span>
                            </Tooltip>
                ...
                <div className='flex flex-col p-4'>
                    <div className='flex justify-between gap-5'>
                        ...
                        <Link
                            ...
                            className={cn(buttonVariants({ variant: 'secondary' }), 'flex h-7 items-center rounded-md border px-2 text-sm')}
                        >
                            View invoices
                        </Link>
                    </div>
