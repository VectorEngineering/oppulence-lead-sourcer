...
import { COUNTRIES, cn, currencyFormatter, fetcher, formatDate } from '@dub/utils'
...
export function PartnerTable() {
    ...
    const { filters, activeFilters, onSelect, onRemove, onRemoveAll, isFiltered } = usePartnerFilters({ sortBy, sortOrder })
    ...
    const [detailsSheetState, setDetailsSheetState] = useState<
        { open: false; partner: EnrolledPartnerProps | null } | { open: true; partner: EnrolledPartnerProps }
        ...
        columns: [
            ...
            },
            {
                ...
                cell: ({ row }) => {
                    ...
                    return badge ? (
                        <StatusBadge icon={null} variant={badge.variant as 'pending' | 'error' | 'new' | 'success' | 'neutral' | 'warning'}>
                            {badge.label}
                        </StatusBadge>
                    ) : (
                        '-'
                ...
                cell: ({ row }) => {
                    ...
                    return (
                        <div className='flex items-center gap-2'>
                            {country && <img alt='' src={`https://flag.vercel.app/m/${country}.svg`} className='h-3 w-4' />}
            ...
            },
            {
                id: 'clicks',
                header: 'Clicks',
                accessorFn: (d) => (d.status !== 'pending' ? nFormatter(d.link?.clicks, { full: true }) : '-')
            },
            {
                id: 'leads',
                header: 'Leads',
                accessorFn: (d) => (d.status !== 'pending' ? nFormatter(d.link?.leads, { full: true }) : '-')
            },
            {
                id: 'sales',
                header: 'Sales',
                accessorFn: (d) => (d.status !== 'pending' ? nFormatter(d.link?.sales, { full: true }) : '-')
            },
            {
                id: 'earnings',
                header: 'Earnings',
        ...
        <div className='flex flex-col gap-3'>
            ...
            {partners?.length !== 0 ? (
                ...
                <AnimatedEmptyState
                    ...
                    description={
                        isFiltered ? 'No partners found for the selected filters.' : 'No partners have been added to this program yet.'
                    }
                    cardContent={() => (
                        <>
...
function RowMenuButton({ row }: { row: Row<EnrolledPartnerProps> }) {
    ...
    return (
        <Popover
                ...
                <Command tabIndex={0} loop className='focus:outline-none'>
                    <Command.List className='flex w-screen flex-col gap-1 p-1.5 text-sm sm:w-auto sm:min-w-[130px]'>
                        <MenuItem
                            ...
                            label='View sales'
                            onSelect={() => {
                                router.push(`/${slug}/programs/${programId}/sales?partnerId=${row.original.id}`)
                                setIsOpen(false)
                            }}
                        />
                    </Command.List>
            ...
            align='end'
        >
            <Button type='button' className='h-8 whitespace-nowrap px-2' variant='outline' icon={<Dots className='h-4 w-4 shrink-0' />} />
        </Popover>
    )
}

function MenuItem({ icon: IconComp, label, onSelect }: { icon: Icon; label: string; onSelect: () => void }) {
    return (
        <Command.Item
            className={cn(
