...
import { cn, DICEBEAR_AVATAR_URL, fetcher, formatDate, getPrettyUrl } from '@dub/utils'
...
export function ProgramPartnersInvitesPageClient() {
    ...
    const {
        ...
        mutate,
        error
    } = useSWR<ProgramInviteProps[]>(workspaceId && `/api/programs/${programId}/invites?workspaceId=${workspaceId}`, fetcher, {
...
function RowMenuButton({ row, mutate }: { row: Row<ProgramInviteProps>; mutate: KeyedMutator<ProgramInviteProps[]> }) {
    const { id: workspaceId } = useWorkspace()
...
function MenuItem({ icon: IconComp, label, onSelect, disabled }: { icon: Icon; label: string; onSelect: () => void; disabled?: boolean }) {
    return (
        <Command.Item
            className={cn(
