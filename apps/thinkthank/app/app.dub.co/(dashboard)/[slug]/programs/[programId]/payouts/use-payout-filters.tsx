...
export function usePayoutFilters(extraSearchParams: Record<string, string>) {
                ...
                options:
                    partners?.map(({ id, name, image }) => {
                        return {
                            value: id,
                            label: name,
                            icon: <img src={image || `${DICEBEAR_AVATAR_URL}${name}`} alt={name} className='size-4 rounded-full' />
                        }
                    }) ?? null
            },
            {
                ...
                icon: CircleDotted,
                label: 'Status',
                options: Object.entries(PayoutStatusBadges).map(([value, { label }]) => {
                    ...
                    return {
                        value,
                        label,
                        icon: <Icon className={cn(PayoutStatusBadges[value].className, 'size-4 bg-transparent')} />,
    ...
    const activeFilters = useMemo(() => {
        ...
        return [...(status ? [{ key: 'status', value: status }] : []), ...(partnerId ? [{ key: 'partnerId', value: partnerId }] : [])]
    }, [searchParamsObj])
