...
export function usePartnerFilters(extraSearchParams: Record<string, string>) {
    ...
    const filters = useMemo(
        () => [
            ...
            },
            {
                ...
                icon: CircleDotted,
                label: 'Status',
                options: Object.entries(PartnerStatusBadges).map(([value, { label }]) => {
                    const Icon = PartnerStatusBadges[value].icon
                    const count = statusCount?.find(({ status }) => status === value)?._count

                    return {
                        value,
                        label,
                        icon: <Icon className={cn(PartnerStatusBadges[value].className, 'size-4 bg-transparent')} />,
    ...
    const activeFilters = useMemo(() => {
        ...
        return [...(status ? [{ key: 'status', value: status }] : []), ...(country ? [{ key: 'country', value: country }] : [])]
    }, [searchParamsObj])
