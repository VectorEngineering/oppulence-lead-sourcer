...
import { AnimatedSizeContainer, BlurImage, Filter, StatusBadge, Table, usePagination, useRouterStuff, useTable } from '@dub/ui'
import { DICEBEAR_AVATAR_URL, currencyFormatter, fetcher, formatDate, formatDateTime } from '@dub/utils'
    ...
    ({
        ...
        const table = useTable<SaleResponse>({
                    ...
                    cell: ({ row }) => {
                        return (
                            <div className='flex items-center gap-2'>
                                <BlurImage
                                    src={row.original.customer.avatar || `${DICEBEAR_AVATAR_URL}${row.original.customer.id}`}
                                    alt={row.original.customer.email ?? row.original.customer.name}
                                    width={20}
                                    height={20}
                                    className='size-5 rounded-full'
                                />
                                <div>{row.original.customer.email ?? row.original.customer.name}</div>
                            </div>
                        )
                    },
                    meta: {
            ...
            cellRight: (cell) => {
                ...
                return !limit && meta?.filterParams && <FilterButton set={meta.filterParams(cell)} />
            },
            ...(!limit && {
                pagination,
            ...
            <div className='flex flex-col gap-3'>
                ...
                {sales?.length !== 0 ? (
                    ...
                    <AnimatedEmptyState
                        ...
                        description={
                            isFiltered ? 'No sales found for the selected filters.' : 'No sales have been made for this program yet.'
                        }
                        cardContent={() => (
                            <>
