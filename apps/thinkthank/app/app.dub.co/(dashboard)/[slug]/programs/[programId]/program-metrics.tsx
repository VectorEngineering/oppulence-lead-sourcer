...
export function ProgramMetrics() {
    ...
    return (
        <div className='grid grid-cols-1 divide-neutral-200 rounded-lg border border-neutral-200 max-md:divide-y md:grid-cols-4 md:divide-x'>
            <Stat icon={Users} label='Partners' tab='partners' value={metrics?.partnersCount} error={error} />
            <Stat icon={Check2} label='Sales' tab='sales' value={metrics?.salesCount} error={error} />
            <Stat icon={CurrencyDollar} label='Revenue' value={metrics?.revenue} tab='sales' error={error} isCurrency />
            <Stat icon={MoneyBills2} label='Payouts' tab='payouts' value={metrics?.payouts} error={error} isCurrency />
        </div>
    )
}
