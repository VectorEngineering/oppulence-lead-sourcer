...
import { Accordion, AccordionContent, AccordionItem, AccordionTrigger } from '@dub/ui'
...
export function EmbedFAQ({ program }: { program: Program }) {
    ...
    return (
        <motion.div className='rounded-lg border border-neutral-100 bg-white px-4 py-2 sm:px-8 sm:py-4' {...TAB_ITEM_ANIMATION_SETTINGS}>
            <Accordion type='multiple'>
                {items.map((item, idx) => (
                    <AccordionItem key={idx} value={idx.toString()}>
                        ...
                        <AccordionContent>
                            <BlockMarkdown className='py-2 text-left text-sm text-neutral-500 sm:text-base'>{item.content}</BlockMarkdown>
                        </AccordionContent>
                    </AccordionItem>
                ))}
            </Accordion>
