...
export function EmbedActivity({ clicks, leads, sales }: { clicks: number; leads: number; sales: number }) {
    const { data: analytics } = useSWR<AnalyticsTimeseries[]>(`/api/embed/analytics`, fetcher, {
            ...
            {[
                ...
                <div key={label} className='relative flex flex-col justify-between p-4'>
                    <div>
                        ...
                        <span className='text-base font-medium leading-none text-neutral-700'>{nFormatter(value, { full: true })}</span>
                    </div>
