...
export function EmbedPayouts({ payouts }: { payouts: { status: PayoutStatus; amount: number }[] }) {
    return (
        <div className='flex flex-col justify-between gap-4 rounded-lg border border-neutral-200 bg-white p-4'>
            ...
            <div className='grid gap-1'>
                {[
                    {
                        label: 'Upcoming',
                        value: payouts.find((payout) => payout.status === PayoutStatus.pending)?.amount ?? 0
                    },
                    {
                        label: 'Paid',
                        value: payouts.find((payout) => payout.status === PayoutStatus.completed)?.amount ?? 0
                    }
                ].map(({ label, value }) => (
