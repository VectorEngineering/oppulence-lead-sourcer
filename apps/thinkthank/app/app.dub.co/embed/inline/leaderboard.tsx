...
import { TAB_ITEM_ANIMATION_SETTINGS, currencyFormatter, fetcher } from '@dub/utils'
...
export function EmbedLeaderboard() {
    const { data: partners, isLoading } = useSWR<z.infer<typeof LeaderboardPartnerSchema>[]>('/api/embed/leaderboard', fetcher, {
    ...
    const { table, ...tableProps } = useTable({
        ...
        columns: [
            ...
            },
            {
                ...
                header: 'Sales',
                cell: ({ row }) => {
                    return currencyFormatter((row.original.link?.saleAmount ?? 0) / 100, {
    ...
    return (
        <motion.div className='relative rounded-md border border-neutral-200' {...TAB_ITEM_ANIMATION_SETTINGS}>
            <Table {...tableProps} table={table} containerClassName='border-none max-h-[26rem] overflow-auto' />
