...
import { DICEBEAR_AVATAR_URL, STAGGER_CHILD_VARIANTS, cn, smartTruncate } from '@dub/utils'
...
export default function WrappedPageClient() {
    ...
    const { totalLinks, totalClicks, topLinks, topCountries } = yearInReview || {}
    ...
    return (
        <div className='relative mx-auto my-10 max-w-lg px-4 sm:px-8'>
            ...
            <p className='animate-slide-up-fade text-balance text-center text-sm leading-6 text-black [animation-delay:300ms] [animation-duration:1s] [animation-fill-mode:both]'>
                As we put a wrap on {year}, we wanted to say thank you for your support! Here&apos;s a look back at your activity in {year}:
            </p>
            ...
            <div className='animate-slide-up-fade mb-4 mt-8 rounded-lg border border-neutral-200 bg-white p-2 shadow-md [animation-delay:450ms] [animation-duration:1s] [animation-fill-mode:both]'>
                ...
                <div className='grid gap-2 p-4'>
                    <StatTable title='Top Links' value={topLinks ? (topLinks as { item: string; count: number }[]) : placeholderArray} />
                    <StatTable
                        title='Top Countries'
                        value={topCountries ? (topCountries as { item: string; count: number }[]) : placeholderArray}
                    />
                </div>
            </div>
            ...
            >
                ...
                <div className='flex flex-col gap-2'>
                    ...
                    <p className='text-sm text-neutral-500 group-hover:underline'>
                        A full recap of some of the top features we shipped this year – and how we grew as a company.
                    </p>
                </div>
            </Link>
        </div>
...
const StatCard = ({ title, value }: { title: string; value: number | undefined }) => {
    return (
        <div className='text-center'>
            ...
            <NumberFlow value={value || 0} className={cn('text-lg font-medium text-black', value === undefined && 'text-neutral-300')} />
        </div>
    )
}

const StatTable = ({ title, value }: { title: string; value: { item: string; count: number }[] }) => {
        ...
        <div className='mb-2'>
            ...
            >
                {value.map(({ item, count }, index) => {
                    ...
                    return (
                        <motion.div key={index} variants={STAGGER_CHILD_VARIANTS} className='text-sm text-gray-500'>
                            <a
                            ...
                            >
                                    ...
                                    <div className='flex items-center gap-2'>
                                        ...
                                        <div className='flex gap-0.5'>
                                            <p className='font-medium text-black'>
                                                {title === 'Top Links' ? smartTruncate(item, 33) : COUNTRIES[item]}{' '}
                                            </p>
                                ...
                                <NumberFlow
                                    value={count}
                                    className={cn('text-neutral-600 group-hover:text-black', count === 0 && 'text-neutral-300')}
                                />
                            </a>
                        </motion.div>
                    )
