        ...
        <div>
            <div className='flex justify-center'>
                <ToggleGroup
                    ...
                    selected={periodTab}
                    selectAction={(period) => setPeriodTab(period as 'monthly' | 'yearly')}
                />
            </div>
            <div className='mt-8 grid gap-4 sm:grid-cols-2'>
                ...
                <PlanCard
                    name='Business'
                    plans={SELF_SERVE_PAID_PLANS.filter(({ name }) => name.startsWith('Business'))}
                    period={periodTab}
                />
            </div>
        </div>
            ...
            {plans.length > 1 && (
                <div className='mt-4'>
                    <ToggleGroup
                        ...
                        selectAction={(name) => setSelectedPlanIndex(plans.findIndex((p) => p.name === name))}
                    />
                </div>
            )}
            <PlanFeatures className='mt-4' plan={selectedPlan.name} />
            <div className='mt-10 flex grow flex-col justify-end'>
                <UpgradePlanButton plan={selectedPlan.name.toLowerCase()} period={period} />
            </div>
        </div>
    )
}
