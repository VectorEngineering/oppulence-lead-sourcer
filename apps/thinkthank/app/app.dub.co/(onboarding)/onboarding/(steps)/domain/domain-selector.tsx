        ...
        <>
            <div className='animate-fade-in mx-auto grid w-full max-w-[312px] gap-4 sm:max-w-2xl sm:grid-cols-2'>
                ...
                <DomainOption
                    title={
                        <>
                            Claim a free <span className='rounded border border-green-800/10 bg-lime-100 p-1 font-mono text-xs'>.link</span>{' '}
                            domain
                        </>
                    }
                    example='acme.link'
...
function DomainOption({
    ...
    return (
        ...
        >
            {isSelected && <CheckCircleFill className='absolute left-2 top-2 size-5 text-black' />}
            <div className='flex w-full flex-col gap-2 rounded-md border border-gray-300 bg-gray-100 p-2 [mask-image:linear-gradient(to_bottom,black_50%,transparent_95%)]'>
                ...
                <div className='overflow-hidden rounded border border-gray-300'>
                    <div className='aspect-[1.9/1] w-full overflow-hidden bg-gray-200'>
                        {previewImage && <img src={previewImage} alt='' className='size-full object-cover' />}
                    </div>
                </div>
            </div>
            <span className='text-center text-sm font-medium text-gray-800'>{title}</span>
            {paidPlanRequired && (
