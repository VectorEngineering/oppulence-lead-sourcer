...
export function useOnboardingProgress() {
    ...
    const { execute, executeAsync, isExecuting, hasSucceeded } = useAction(setOnboardingProgress, {
        onSuccess: () => {
    ...
    const continueTo = useCallback(
        async (step: OnboardingStep, { slug: providedSlug }: { slug?: string } = {}) => {
            ...
            const queryParams = PRE_WORKSPACE_STEPS.includes(step) ? '' : `?workspace=${providedSlug || slug}`
