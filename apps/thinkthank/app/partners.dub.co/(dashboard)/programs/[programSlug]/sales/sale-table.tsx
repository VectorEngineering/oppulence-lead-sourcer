...
import { StatusBadge, Table, usePagination, useRouterStuff, useTable } from '@dub/ui'
import { currencyFormatter, fetcher, formatDate, formatDateTime } from '@dub/utils'
        ...
        columns: [
            {
                ...
                cell: ({ row }) => (
                    <p title={formatDateTime(row.original.createdAt)}>{formatDate(row.original.createdAt, { month: 'short' })}</p>
                )
            },
            {
                id: 'customer',
    ...
    return (
        <div className='flex flex-col gap-3'>
            ...
            {isLoading || sales?.length ? (
                <Table {...tableProps} table={table} containerClassName='border-neutral-300' />
            ) : (
                <AnimatedEmptyState
                    title='No sales found'
                    description='No sales have been made for this program yet.'
