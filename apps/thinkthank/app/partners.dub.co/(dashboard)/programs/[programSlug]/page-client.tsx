...
import { Button, buttonVariants, MaxWidthWrapper, MiniAreaChart, useCopyToClipboard, useRouterStuff } from '@dub/ui'
import { Areas, ChartContext, TimeSeriesChart, XAxis, YAxis } from '@dub/ui/charts'
    ...
    return (
        <MaxWidthWrapper className='pb-10'>
            <div className='relative flex flex-col rounded-lg border border-neutral-300 bg-gradient-to-r from-neutral-50 p-4 md:p-6'>
                {program && <HeroBackground logo={program.logo} color={program.brandColor} />}
                ...
                <div className='relative mt-24 text-lg text-neutral-900 sm:max-w-[50%]'>
                    {program ? (
                        <ProgramCommissionDescription program={program} discount={programEnrollment?.discount} />
                    ) : (
                        <div className='h-7 w-5/6 animate-pulse rounded-md bg-neutral-200' />
                    )}
                </div>
                <span className='mb-1.5 mt-6 block text-sm text-neutral-800'>Referral link</span>
                    ...
                    <Button
                        icon={
                            <div className='relative size-4'>
                                <div className={cn('absolute inset-0 transition-[transform,opacity]', copied && 'translate-y-1 opacity-0')}>
                                    <Copy className='size-4' />
                                </div>
                                <div
                                    className={cn('absolute inset-0 transition-[transform,opacity]', !copied && 'translate-y-1 opacity-0')}
                                >
                        ...
                        onClick={() => programEnrollment?.link?.shortLink && copyToClipboard(programEnrollment?.link?.shortLink)}
                    />
                </div>
            </div>
                ...
                <div className='mt-6'>
                    <div className='flex items-center justify-between'>
                        ...
                        <Link
                            ...
                            className={cn(buttonVariants({ variant: 'secondary' }), 'flex h-8 items-center rounded-lg border px-2 text-sm')}
                        >
                            View all
                        </Link>
                    </div>
            ...
            <div className='relative mt-4 h-64 w-full'>
                {data ? (
                    ...
                    <div className='flex size-full items-center justify-center'>
                        {error ? <span className='text-sm text-neutral-500'>Failed to load earnings data.</span> : <LoadingSpinner />}
                    </div>
                )}
            </div>
        </div>
...
function StatCard({ title, event }: { title: string; event: 'clicks' | 'leads' | 'sales' }) {
    const { programSlug } = useParams()
        ...
        >
            ...
            <div className='mt-2 h-16 w-full'>
                {timeseries ? (
                    ...
                    <div className='flex size-full items-center justify-center'>
                        {error ? <span className='text-sm text-neutral-500'>Failed to load data.</span> : <LoadingSpinner />}
                    </div>
                )}
            </div>
        </Link>
