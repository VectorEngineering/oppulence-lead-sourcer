...
import { Button, buttonVariants, FileUpload, LoadingSpinner, MaxWidthWrapper, useEnterSubmit } from '@dub/ui'
            ...
            <div className='max-w-screen-md rounded-lg border border-neutral-200 bg-white'>
                ...
                {partner ? (
                    ...
                    <div className='flex h-32 w-full items-center justify-center'>
                        {error ? <span className='text-sm text-neutral-500'>Failed to load profile data</span> : <LoadingSpinner />}
                    </div>
                )}
            </div>
        </MaxWidthWrapper>
    ...
    return (
        ...
        >
                ...
                <div className='grid grid-cols-1 items-center sm:grid-cols-2'>
                    <FormRow>
                        <label className='contents'>
                            <span className='text-sm font-medium text-gray-800'>Display Image</span>
                            <div className='flex items-center gap-5'>
                                <Controller
                                    ...
                                    render={({ field }) => (
                                        <FileUpload
                                            ...
                                            variant='plain'
                                            imageSrc={field.value || `${DICEBEAR_AVATAR_URL}${partner?.name}`}
                                            readFile
                                ...
                                <div>
                                    ...
                                    <p className='mt-1.5 text-xs text-gray-500'>Recommended size: 160x160px</p>
                                </div>
                            </div>
                        </label>
                    </FormRow>
            ...
            <div className='flex justify-end rounded-b-lg border-t border-neutral-200 bg-neutral-100 px-5 py-3.5'>
                <Button type='submit' text='Save changes' className='h-8 w-fit px-2.5' loading={isSubmitting || isExecuting} />
            </div>
        </form>
    )
}
