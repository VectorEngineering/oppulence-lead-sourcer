...
export function PayoutStatsAndSettings() {
    ...
    const { data: bankAccount } = useSWR<Stripe.BankAccount | null>(partner && `/api/partners/${partner.id}/payouts/settings`, fetcher)
        ...
        <div className='grid grid-cols-1 divide-neutral-200 rounded-lg border border-neutral-200 bg-neutral-50 max-sm:divide-y sm:grid-cols-2 sm:divide-x'>
            <div className='flex flex-col gap-1.5 p-4'>
                <div className='flex justify-between gap-5'>
                    ...
                    <StripeConnectButton
                        text={partner?.payoutsEnabled ? 'Payout settings' : 'Connect payouts'}
                ...
                <div className='flex items-end justify-between gap-5'>
                    <NumberFlow
                        ...
                        value={(payoutsCount?.find((p) => p.status === PayoutStatus.pending)?.amount ?? 0) / 100}
                        format={{
                            style: 'currency',
                            currency: 'USD'
                        }}
            ...
            <div className='flex flex-col gap-1.5 p-4'>
                ...
                <NumberFlow
                    className='mt-2 text-2xl text-neutral-800'
                    value={(payoutsCount?.find((p) => p.status === PayoutStatus.completed)?.amount ?? 0) / 100}
                    format={{
                        style: 'currency',
                        currency: 'USD'
                    }}
