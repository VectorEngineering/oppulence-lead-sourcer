...
import type { Program } from '@dub/prisma-thinkthank/client'
...
export function Header({
    ...
    return (
        <header className={'sticky top-0 z-10 mx-px flex items-center justify-between bg-white/90 px-6 py-4 backdrop-blur-sm'}>
            ...
            <Link href={`/apply/${slug}`} className='animate-fade-in my-0.5 block'>
                {program.wordmark || program.logo ? (
                    <img className='h-7 max-w-32' src={(program.wordmark ?? program.logo) as string} alt={'wordmark'} />
                ) : (
