...
import { Program } from '@dub/prisma-thinkthank/client'
...
export function ProgramApplicationForm({ program }: { program: Pick<Program, 'id' | 'slug' | 'name'> }) {
    ...
    const { executeAsync, isExecuting } = useAction(createProgramApplicationAction, {
        async onSuccess({ data }) {
            ...
            router.push(`/apply/${program.slug}/application/success?${searchParams.toString()}`)
        },
        onError({ error }) {
    ...
    return (
        ...
        >
            ...
            <label>
                <span className='text-sm font-medium text-gray-800'>How do you plan to promote {program.name}?</span>
