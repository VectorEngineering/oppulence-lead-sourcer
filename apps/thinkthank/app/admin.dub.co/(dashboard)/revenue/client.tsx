...
export default function RevenueClient({ data }: { data: { date: string; value: number }[] }) {
    ...
    const percentageDifference = (((finalValue - previousValue) / previousValue) * 100).toFixed(0)
        ...
        <div className='mx-auto my-24 w-full max-w-screen-lg overflow-hidden border border-gray-200 bg-white sm:rounded-xl'>
            <div className='w-fit border-b-2 border-black px-8 py-6'>
                ...
                <div className='mt-1 flex items-center gap-3'>
                    <h3 className='text-3xl font-medium'>{currencyFormatter(finalValue)}</h3>
            ...
            <div className='border-t border-gray-200 p-5 sm:p-10'>
                <div className='flex h-96 w-full items-center justify-center'>
                    <TimeSeriesChart
                        ...
                        tooltipContent={(d) => (
                            <>
                                <p className='border-b border-gray-200 px-4 py-3 text-sm text-gray-900'>{dateFormatter(d.date)}</p>
                                <div className='grid grid-cols-2 gap-x-6 gap-y-2 px-4 py-3 text-sm'>
                                    <Fragment>
                                        ...
                                        <p className='text-right font-medium text-gray-900'>{currencyFormatter(d.values.value)}</p>
                                    </Fragment>
                                </div>
                            </>
                        )}
