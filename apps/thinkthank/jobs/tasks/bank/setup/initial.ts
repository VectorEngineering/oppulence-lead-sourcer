...
export const initialBankSetup = schemaTask({
    ...
    run: async (payload) => {
        const { workspaceId, connectionId } = payload
        ...
        // Add workspaceId as externalId to use it in the bankSyncScheduler task
        await schedules.create({
            task: bankSyncScheduler.id,
            cron: generateCronTag(workspaceId),
            timezone: 'UTC',
            externalId: workspaceId,
            deduplicationKey: `${workspaceId}-${bankSyncScheduler.id}`
        })
