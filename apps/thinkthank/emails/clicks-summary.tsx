                ...
                <Body className='mx-auto my-auto bg-white font-sans'>
                    <Container className='mx-auto my-10 max-w-[500px] rounded border border-solid border-gray-200 px-10 py-5'>
                        <Section className='mt-8'>
                            <Img src={platform.assets.wordmark} height='40' alt={appName} className='mx-auto my-0' />
                        </Section>
                        <Heading className='mx-0 my-7 p-0 text-center text-xl font-semibold text-black'>
                            Your 30-day {appName} summary for {workspaceName}
                        </Heading>
                        <Text className='text-sm leading-6 text-black'>
                            In the last 30 days, your {appName} workspace, <strong>{workspaceName}</strong> received{' '}
                            <strong>{nFormatter(totalClicks)} link clicks</strong>. You also created{' '}
                            <strong>{createdLinks} new links</strong> during that time.
                        </Text>
                        <Section>
                            <Row>
                                <Column align='center'>
                                    ...
                                    <p className='text-sm font-semibold text-black'>{nFormatter(totalClicks)} clicks</p>
                                </Column>
                                <Column align='center'>
                                    <div className='flex h-12 w-12 items-center justify-center rounded-full bg-green-200'>
                                        <Link2 className='h-5 w-5 text-green-600' />
                                    </div>
                                    <p className='text-sm font-semibold text-black'>{nFormatter(createdLinks)} new links</p>
                                </Column>
                            </Row>
                        </Section>
                        {topLinks.length > 0 && (
                                    ...
                                    {topLinks.map(({ link, clicks }, index) => {
                                        ...
                                        return (
                                            <div key={`${index}-${link}-${Math.random()}`}>
                                                ...
                                                {index !== topLinks.length - 1 && <Hr className='my-2 w-full border border-gray-200' />}
                                            </div>
                                        )
                                    })}
                                </Section>
                        ...
                        {createdLinks === 0 ? (
                            <>
                                <Text className='text-sm leading-6 text-black'>
                                    It looks like you haven't created any links in the last 30 days. If there's anything that we can do to
                                    help, please reply to this email to get in touch with us.
                                </Text>
