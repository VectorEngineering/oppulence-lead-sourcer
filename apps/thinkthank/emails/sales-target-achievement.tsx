...
export default function SalesTargetAchievement({
    ...
    achievement = {
        ...
        nextPeriod: {
            ...
            deals: 60,
            meetings: 240,
            focus: ['Enterprise segment expansion', 'Product upsell opportunities', 'Partner channel development', 'New market penetration']
        }
    }
}: {
    email: string
    ...
    return (
        <Html>
            <Head />
            <Preview>
                Target Achievement Report: {achievement.period} - ${achievement.achieved.revenue.toLocaleString()} revenue (
                {Math.round((achievement.achieved.revenue / achievement.target.revenue) * 100).toString()}% of target)
            </Preview>
            <Tailwind>
                <Body className='mx-auto my-auto bg-white font-sans'>
                    <Container className='mx-auto my-10 max-w-[500px] rounded border border-solid border-gray-200 px-10 py-5'>
                        ...
                        <Heading className='mx-0 mb-4 mt-8 p-0 text-xl font-semibold text-black'>Sales Target Achievement 🎯</Heading>
                        <Text className='text-sm leading-6 text-black'>
                            Congratulations on your outstanding performance for {achievement.period}! Your team has demonstrated exceptional
                            execution, surpassing key targets and setting new benchmarks for success.
                        </Text>
                        ...
                        <Heading className='mx-0 mb-4 mt-8 p-0 text-xl font-semibold text-black'>Achievement Rates 📈</Heading>
                        <Text className='mb-4 text-sm leading-6 text-black'>
                            Your team has shown remarkable performance across all key metrics. These achievement rates reflect the
                            dedication and effectiveness of your sales strategies.
                        </Text>
                        ...
                        <Heading className='mx-0 mb-4 mt-8 p-0 text-xl font-semibold text-black'>Key Highlights 🌟</Heading>
                        <Text className='mb-4 text-sm leading-6 text-black'>
                            These achievements showcase the effectiveness of your team's execution and highlight areas of exceptional
                            performance that can be leveraged for future success.
                        </Text>
                        <Section className='my-4'>
                            <div className='rounded-lg border border-solid border-gray-200 p-4'>
                        ...
                        <Heading className='mx-0 mb-4 mt-8 p-0 text-xl font-semibold text-black'>Top Contributors 🏆</Heading>
                        <Text className='mb-4 text-sm leading-6 text-black'>
                            Recognition of outstanding individual performances who have significantly contributed to exceeding our targets.
                            Their strategies and best practices can serve as valuable examples for the entire team.
                        </Text>
                        <Section className='my-4'>
                            <div className='rounded-lg border border-solid border-gray-200 p-4'>
                                {achievement.topContributors.map((contributor, index) => (
                                    <div key={`${index}-${contributor.name}-${Math.random()}`} className={index !== 0 ? 'mt-4' : ''}>
                                        <Row>
                                            ...
                                            <Column className='px-2'>
                                                ...
                                                <Text className='m-0 text-base font-medium text-green-600'>{contributor.achievement}</Text>
                                            </Column>
                                            <Column className='px-2'>
                                                <Text className='m-0 text-sm font-medium text-gray-500'>Revenue</Text>
                                                <Text className='m-0 text-base font-medium'>${contributor.revenue.toLocaleString()}</Text>
                                            </Column>
                                        </Row>
                                        {index !== achievement.topContributors.length - 1 && <Hr className='my-4 border-gray-200' />}
                                    </div>
                                ))}
                            </div>
                        </Section>

                        <Heading className='mx-0 mb-4 mt-8 p-0 text-xl font-semibold text-black'>Next Period Goals 🎯</Heading>
                        <Text className='mb-4 text-sm leading-6 text-black'>
                            Building on this success, we've set ambitious but achievable targets for the next period. These goals reflect
                            our growth trajectory and market opportunities.
                        </Text>
                        <Section className='my-4'>
                            <div className='rounded-lg border border-solid border-gray-200 p-4'>
                                <Row>
                                    ...
                                    <Column className='px-2'>
                                        ...
                                        <Text className='m-0 text-base font-medium'>{achievement.nextPeriod.deals}</Text>
                                    </Column>
                                    <Column className='px-2'>
                                        <Text className='m-0 text-sm font-medium text-gray-500'>Meetings Target</Text>
                                        <Text className='m-0 text-base font-medium'>{achievement.nextPeriod.meetings}</Text>
                                    </Column>
                                </Row>
                            </div>
                        </Section>

                        <Heading className='mx-0 mb-4 mt-8 p-0 text-xl font-semibold text-black'>Strategic Focus Areas 🎯</Heading>
                        <Text className='mb-4 text-sm leading-6 text-black'>
                            To achieve our next period goals, we've identified key focus areas that align with our growth strategy and
                        ...
                        <Text className='mb-4 mt-8 text-sm leading-6 text-black'>
                            Congratulations again on this outstanding achievement! Let's maintain this momentum and continue driving towards
                            our ambitious goals for the next period.
                        </Text>
