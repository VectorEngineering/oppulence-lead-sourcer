...
export default function DubWrapped({
    ...
    return (
        <Html>
            <Head />
            <Preview>
                In 2024, you created {nFormatter(stats['Total Links'], { full: true })} links on {platform.company} and got{' '}
                ...
                <Body className='mx-auto my-auto bg-white font-sans'>
                    <Container className='mx-auto my-10 max-w-[500px] rounded border border-solid border-gray-200 px-10 py-5'>
                        ...
                        <Text className='text-sm leading-6 text-black'>
                            As we put a wrap on 2024, we wanted to say thank you for your support! Here's a look back at your activity in
                            2024:
                        </Text>
                        ...
                        <Section className='my-8 rounded-lg border border-solid border-gray-200 p-2'>
                            ...
                            <div className='grid gap-2 px-4'>
                                <StatTable title='Top Links' value={topLinks} workspaceSlug={workspace.slug} />
                                <StatTable title='Top Countries' value={topCountries} workspaceSlug={workspace.slug} />
                            </div>
                        </Section>

                        <Heading className='mx-0 mb-4 mt-8 p-0 text-xl font-semibold text-black'>Your contribution 📈</Heading>
                        <Text className='text-sm leading-6 text-black'>
                            Thanks to customers like you, we had an incredible year as well, seeing record activity and link clicks:
                        </Text>
                        {dubStats.map((stat) => (
                            <Text key={stat.item} className='ml-1 text-sm font-medium leading-4 text-black'>
                                ◆ {stat.item} <span className='font-semibold text-green-700'>({stat.increase})</span>
                            </Text>
                        ))}
                        <Img src={`${platform.assetsUrl}/header.png`} alt='Thank you' className='max-w-[500px] rounded-lg' />

                        <Hr className='mx-0 my-6 w-full border border-gray-200' />

                        <Heading className='mx-0 mb-4 mt-8 p-0 text-xl font-semibold text-black'>What we shipped 🚢</Heading>
                        <Text className='text-sm leading-6 text-black'>Here's a rundown of what we shipped in 2024:</Text>

                        {shippedItems.map((item) => (
                            <div key={item.title} className='mb-8'>
                                <Img src={item.image} alt={item.title} className='max-w-[500px] rounded-lg' />
                                ...
                                />
                                <Link href={item.cta.href} className='rounded-md bg-black px-4 py-1.5 text-sm font-medium text-white'>
                                    {item.cta.text}
                                </Link>
                            </div>
                        ))}
                        ...
                        <Text className='text-sm leading-6 text-black'>
                            ...
                            <Link href='https://ship.dub.co/blog' className='text-black underline underline-offset-2'>
                                blog
                            </Link>{' '}
                            and{' '}
                            <Link href='https://ship.dub.co/changelog' className='text-black underline underline-offset-2'>
                                changelog
                            </Link>
                            .
                            <br />
                        ...
                        <Img src={`${platform.assetsUrl}/signature.png`} alt='Email signature' className='max-w-[200px]' />
                        <Text className='text-sm leading-6 text-black'>and the {platform.company} team 🎄</Text>
...
const StatTable = ({ title, value, workspaceSlug }: { title: string; value: { item: string; count: number }[]; workspaceSlug: string }) => {
    return (
        <Section>
            ...
            {value.map(({ item, count }, index) => {
                ...
                return (
                    <div key={`${index} - ${item} - ${Math.random()}`} className='text-sm'>
                        ...
                        {index !== value.length - 1 && <Hr className='my-0 w-full border border-gray-200' />}
                    </div>
                )
            })}
        </Section>
