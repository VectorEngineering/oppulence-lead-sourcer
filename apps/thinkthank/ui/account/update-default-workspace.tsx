    ...
    const selectOptions = useMemo(() => {
        return workspaces
            ? workspaces.map((workspace) => ({
                  ...
                  label: workspace.slug === session?.user?.['defaultWorkspace'] ? 'Current' : ''
              }))
            : []
    }, [workspaces, session])
    ...
    return (
        ...
        >
            <div className='flex flex-col space-y-3 p-5 sm:p-10'>
                ...
                <p className='text-sm text-gray-500'>Choose the workspace to show by default when you sign in.</p>
            ...
            <div className='flex items-center justify-between space-x-4 rounded-b-lg border-t border-gray-200 bg-gray-50 p-3 sm:px-10'>
                <a
                    href={`${platform.webUrl}/help/article/how-to-change-default-workspace`}
                    target='_blank'
                ...
                <div>
                    <Button
                        text='Save changes'
                        loading={saving}
                        disabled={!selectedWorkspace || selectedWorkspace.id === session?.user?.['defaultWorkspace']}
                    />
                </div>
            </div>
        </form>
