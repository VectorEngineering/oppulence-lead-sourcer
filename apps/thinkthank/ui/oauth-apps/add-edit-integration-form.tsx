...
export default function AddEditIntegrationForm({ integration }: { integration: NewOrExistingIntegration }) {
    ...
    const { executeAsync, isExecuting } = useAction(addEditIntegration, {
        ...
        onError: ({ error }) => {
            toast.error(error.validationErrors?.[0]?.message || 'Failed to update integration')
        }
    })
        ...
        <>
            <form
                onSubmit={async (e) => {
                    ...
                    await executeAsync({
                        ...data,
                        screenshots: screenshots.map((s) => s.key).filter(Boolean) as string[],
            ...
            >
                <div>
                    <FileUpload
                        ...
                        variant='plain'
                        readFile
                        imageSrc={logo || `https://api.dicebear.com/7.x/shapes/svg?seed=${integration.slug}`}
