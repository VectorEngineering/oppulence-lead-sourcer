...
import { Button, FileUpload, InfoTooltip, LoadingSpinner, Switch } from '@dub/ui'
...
export default function AddOAuthAppForm({ oAuthApp }: { oAuthApp: OAuthAppProps | null }) {
    const router = useRouter()
    const [saving, setSaving] = useState(false)
    const { slug: workspaceSlug, id: workspaceId, role } = useWorkspace()
    const [urls, setUrls] = useState<{ id: string; value: string }[]>([{ id: nanoid(), value: '' }])
    ...
    const [data, setData] = useState<NewOAuthApp | ExistingOAuthApp>(oAuthApp || defaultValues)
    ...
    const handleUpload = async (file: File) => {
        ...
        const response = await fetch(`/api/oauth/apps/upload-url?workspaceId=${workspaceId}`, {
            method: 'POST'
        })
        ...
        setScreenshots((prev) =>
            prev.map((screenshot) => (screenshot.file === file ? { ...screenshot, uploading: false, key } : screenshot))
        )
    }

    const { name, slug, description, readme, developer, website, redirectUris, logo, pkce, installUrl } = data

    const buttonDisabled = !name || !slug || !developer || !website || !redirectUris
            ...
            <form onSubmit={onSubmit} className='flex flex-col space-y-5 pb-20 text-left'>
                <div>
                    <FileUpload
                        ...
                        variant='plain'
                        readFile
                        imageSrc={logo || `https://api.dicebear.com/7.x/shapes/svg?seed=${oAuthApp?.clientId}`}
                ...
                <div>
                    <label htmlFor='name' className='flex items-center space-x-2'>
                        <h2 className='text-sm font-medium text-gray-900'>Application name</h2>
                ...
                <div>
                    <label htmlFor='slug' className='flex items-center space-x-2'>
                        <h2 className='text-sm font-medium text-gray-900'>Application slug</h2>
                ...
                <div>
                    ...
                    <Reorder.Group axis='y' values={screenshots} onReorder={setScreenshots} className='mt-2 grid w-full gap-2'>
                        {screenshots.map((screenshot) => (
                            ...
                            >
                                <div className='flex flex-1 items-center space-x-2 p-2'>
                                    ...
                                    <p className='text-center text-sm text-gray-500'>{screenshot.file?.name || screenshot.key}</p>
                                </div>
                                <button
                                    disabled={!canManageApp}
                                    className='h-full rounded-r-md border-l border-gray-200 p-2'
                                    onClick={() => {
                                        setScreenshots((prev) => prev.filter((s) => s.key !== screenshot.key))
                                    }}
                                >
                ...
                <div>
                    <label htmlFor='developer' className='flex items-center space-x-2'>
                        <h2 className='text-sm font-medium text-gray-900'>Developer name</h2>
                                            ...
                                            <div>
                                                <div className='relative'>
                                                    <input
                                                        ...
                                                        onChange={(e) => {
                                                            setUrls(
                                                                urls.map((u) => (u.id === url.id ? { ...u, value: e.target.value } : u))
                                                            )
                                                        }}
                                                        className={cn(
                                                            'block w-full rounded-md border-gray-300 text-gray-900 placeholder-gray-400 focus:border-gray-500 focus:outline-none focus:ring-gray-500 sm:text-sm',
                                                    ...
                                                    {urls.length > 1 && (
                                                        <div className='absolute inset-y-0 right-0 flex items-center space-x-1 pl-3 pr-1'>
                                                            <Button
                                                                ...
                                                                text='Remove'
                                                                onClick={() => {
                                                                    const newUrls = urls.filter((u) => u.id !== url.id)
