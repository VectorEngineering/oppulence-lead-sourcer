...
import { CircleWarning, Download, OfficeBuilding, ShieldCheck } from '@dub/ui/icons'
...
export default function IntegrationCard(integration: InstalledIntegrationProps) {
    ...
    return (
        ...
        >
            {installed && <p className='absolute right-4 top-4 text-xs text-gray-500'>INSTALLED</p>}
            ...
            <div className='items-between grid h-24 pt-4'>
                <p className='line-clamp-3 text-sm text-gray-500'>{integration.description}</p>
                <div className='flex items-center justify-end gap-1 text-gray-500'>
                    <Download className='size-4' />
                    <span className='text-sm'>
                        {integration.installations} {pluralize('install', integration.installations)}
                    </span>
                </div>
            </div>
        </Link>
