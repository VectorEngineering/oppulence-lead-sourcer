        ...
        <>
            <form onSubmit={handleSubmit((data) => executeAsync({ email: data.email }))}>
                <div className='flex flex-col space-y-4'>
                    ...
                    />
                    <Input type='password' placeholder='Password' required {...register('password')} error={errors.password?.message} />
                    <Button type='submit' text={isExecuting ? 'Submitting...' : 'Sign Up'} disabled={isExecuting} loading={isExecuting} />
                </div>
            </form>
        </>
    )
