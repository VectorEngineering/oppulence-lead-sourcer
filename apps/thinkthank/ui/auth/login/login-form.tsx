...
import { ComponentType, Dispatch, SetStateAction, createContext, useEffect, useRef, useState } from 'react'
...
export const authMethods = ['google', 'github', 'email', 'saml', 'password'] as const
...
export default function LoginForm({ methods = [...authMethods], redirectTo }: { methods?: AuthMethod[]; redirectTo?: string }) {
    ...
    const [authMethod, setAuthMethod] = useState<AuthMethod | undefined>(authMethods.find((m) => m === lastUsedAuthMethodLive) ?? 'email')

    useEffect(() => {
        const error = searchParams?.get('error')
        if (error) {
            toast.error(errorCodes[error] || 'An unexpected error occurred. Please try again later.')
        }
    }, [searchParams])
    ...
    const currentAuthProvider = authProviders.find((provider) => provider.method === authMethod)
                    ...
                    <div className='flex flex-col gap-3 p-1'>
                        {authMethod && (
                            <div className='flex flex-col gap-2'>
                                {AuthMethodComponent && <AuthMethodComponent {...currentAuthProvider?.props} />}

                                {!showEmailPasswordOnly && authMethod === lastUsedAuthMethod && (
                                    <div className='text-center text-xs'>
                                        <span className='text-gray-500'>
                                            You signed in with {lastUsedAuthMethod.charAt(0).toUpperCase() + lastUsedAuthMethod.slice(1)}{' '}
                                            last time
                                        </span>
                                    </div>
                                )}
                        ...
                        {showEmailPasswordOnly ? (
                            ...
                            authProviders
                                .filter((provider) => provider.method !== authMethod && methods.includes(provider.method))
                                .map((provider) => (
