...
export default function DomainConfiguration({ data }: { data: { status: DomainVerificationStatusProps; response: any } }) {
    ...
    if (data.status === 'Pending Verification') {
        const txtVerification = domainJson.verification.find((x: any) => x.type === 'TXT')
        return (
            <div>
                <DnsRecord
            ...
            <div className='pt-5'>
                <div className='flex justify-start space-x-4'>
                    <div className='ease border-b-2 border-black pb-1 text-sm text-black transition-all duration-150'>
                        {configJson?.conflicts.some((x) => x.type === 'A') ? 'A Record (recommended)' : 'CNAME Record (recommended)'}
                    </div>
                </div>
                <DnsRecord
                    instructions='Please remove the following conflicting DNS records from your DNS provider:'
                    records={configJson?.conflicts.map(({ name, type, value }: { name: string; type: string; value: string }) => ({
                        name,
                        type,
                        value
                    }))}
    ...
    return (
        <div className='pt-2'>
            ...
            <DnsRecord
                instructions={`To configure your ${recordType === 'A' ? 'apex domain' : 'subdomain'} <code>${
        ...
        <div className='mt-3 text-left text-gray-600'>
            ...
            <div
                className={cn(
                    ...
                    hasTtl ? 'grid-cols-[repeat(4,min-content)]' : 'grid-cols-[repeat(3,min-content)]'
                )}
            >
                ...
                {records.map((record, idx) => (
                    <Fragment key={idx}>
                        ...
                        <p key={record.value} className='flex items-end gap-1 font-mono'>
                            {record.value} <CopyButton variant='neutral' className='-mb-0.5' value={record.value} />
                        </p>
                        {hasTtl && (
            ...
            {(warning || hasTtl) && (
                <div
                    className={cn(
                        ...
                        warning ? 'bg-orange-50 text-orange-600' : 'bg-indigo-50 text-indigo-600'
                    )}
                >
