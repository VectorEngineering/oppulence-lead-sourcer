        ...
        <div className='flex min-w-0 items-center gap-4'>
            <div className='hidden rounded-full border border-gray-200 sm:block'>
                <div
                    className={cn(
                        'rounded-full',
                        (!defaultDomain || domain === 'cal.link') && 'border border-white bg-gradient-to-t from-gray-100 p-1 md:p-2'
                    )}
                >
                    <Icon className={cn('size-5', defaultDomain && domain !== 'cal.link' && 'size-8')} />
                </div>
            </div>
            <div className='overflow-hidden'>
                ...
                {(!defaultDomain || description) && (
                    <div className='mt-1 flex items-center gap-1 text-xs'>
                        {description ? (
                            <span className='whitespace-pre-wrap text-gray-500' title={description}>
                                {description}
                            </span>
                        ) : (
                            <>
                                ...
                                {url !== undefined ? (
                                    url ? (
                                        ...
                                        <span className='truncate text-gray-400'>No redirect configured</span>
                                    )
                                ) : (
