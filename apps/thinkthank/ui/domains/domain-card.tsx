import type { DomainProps, DomainVerificationStatusProps, LinkProps } from '@/lib/types'
...
import { CursorRays, Flag2, Gear, Globe, Hyperlink, PenWriting } from '@dub/ui/icons'
...
export default function DomainCard({ props }: { props: DomainProps }) {
    ...
    const { data, isValidating, mutate } = useSWRImmutable<{
        ...
        response: any
    }>(workspaceId && isVisible && `/api/domains/${domain}/verify?workspaceId=${workspaceId}`, fetcher)
    ...
    const isInvalid = verificationData && !['Valid Configuration', 'Pending Verification'].includes(verificationData.status)
                ...
                {isDubProvisioned && (
                    <div className='flex items-center justify-between gap-2 rounded-t-xl border-b border-gray-100 bg-gray-50 px-5 py-2 text-xs'>
                        <div className='flex items-center gap-1.5'>
                            ...
                            <span className='font-medium text-gray-900'>Provisioned by {platform.company}</span>
                        </div>
                        <a
                ...
                <div className='p-4 sm:p-5'>
                    <div className='grid grid-cols-[1.5fr_1fr] items-center gap-3 sm:grid-cols-[3fr_1fr_1.5fr] sm:gap-4 md:grid-cols-[2fr_1fr_0.5fr_1.5fr]'>
                        ...
                        <div className='flex justify-end gap-2 sm:gap-3'>
                                    ...
                                    icon={
                                        <div className='flex items-center gap-1'>
                                            <div className='relative'>
                                                <Gear className={cn('h-4 w-4', showDetails ? 'text-gray-800' : 'text-gray-600')} />
                                                {/* Error indicator */}
                            ...
                            <Menu props={props} linkProps={props.link} refreshProps={{ isValidating, mutate }} groupHover={groupHover} />
                        </div>
                    </div>
                        ...
                        {verificationData ? (
                            verificationData.status === 'Valid Configuration' ? (
                                <div className='mt-6 flex items-center gap-2 text-pretty rounded-lg bg-green-100/80 p-3 text-sm text-green-600'>
                                    ...
                                    <div>
                                        Good news! Your DNS records are set up correctly, but it can take some time for them to propagate
                                        globally.{' '}
                                        <Link
...
function Menu({
    ...
    const { setShowTransferDomainModal, TransferDomainModal } = useTransferDomainModal({
        props
    })

    const { setShowPrimaryDomainModal, PrimaryDomainModal } = usePrimaryDomainModal({
        props
    })

    const { setShowArchiveDomainModal, ArchiveDomainModal } = useArchiveDomainModal({
        props
    })
                    ...
                    content={
                        <div className='w-full sm:w-48'>
                            <div className='grid gap-px p-2'>
                                <p className='mb-1.5 mt-1 flex items-center gap-2 px-1 text-xs font-medium text-gray-500'>Link Settings</p>
                                <Button
                                    text='Edit Link'
                                    variant='outline'
                                    onClick={() => {
                                    ...
                                    disabledTooltip={!linkProps ? 'Retrieving link details...' : undefined}
                                ...
                                <Button
                                    ...
                                    variant='outline'
                                    onClick={() => copyLinkId()}
                                    icon={copiedLinkId ? <CheckCircleFill className='h-4 w-4' /> : <Copy className='h-4 w-4' />}
                                    disabledTooltip={!linkProps ? 'Retrieving link details...' : undefined}
                                    className='h-9 justify-start px-2 font-medium'
                                />
                            </div>
