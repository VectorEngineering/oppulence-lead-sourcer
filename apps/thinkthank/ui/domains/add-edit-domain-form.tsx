...
import { Binoculars, Crown, Milestone, QrCode, TextCursorInput } from 'lucide-react'
...
> = {
    ...
    'has site': {
        suffix: 'already has a site connected. Ensure you want to connect this domain for shortlinks.',
        icon: AlertCircleFill,
...
export function AddEditDomainForm({
    ...
    const [domainStatus, setDomainStatus] = useState<DomainStatus>(props ? 'available' : 'idle')
    ...
    const debouncedValidateDomain = useDebouncedCallback(async (value: string) => {
    ...
    const saveDisabled = useMemo(() => {
        return !['available', 'has site'].includes(domainStatus) || (props && !isDirty)
    ...
    const onSubmit = async (formData: FormData) => {
        try {
            ...
            if (res.ok) {
                await Promise.all([mutatePrefix('/api/domains'), mutatePrefix('/api/links')])
                const data = await res.json()
                ...
                if (error.message.includes('Upgrade to Pro')) {
                    toast.custom(() => <UpgradeRequiredToast title="You've discovered a Pro feature!" message={error.message} />)
                } else {
    ...
    return (
        <form onSubmit={handleSubmit(onSubmit)} className={cn('flex flex-col gap-y-6 text-left', className)}>
            <div>
                <div className='flex items-center justify-between'>
                    <label htmlFor='domain' className='flex items-center gap-x-2'>
                        <h2 className='text-sm font-medium text-neutral-700'>Your domain</h2>
                        <InfoTooltip
                            content={
                                <SimpleTooltipContent
                ...
                {props && lockDomain ? (
                    ...
                    <div className='mt-2'>
                        <div
                            className={cn('-m-1 rounded-[0.625rem] p-1', currentStatusProps.className || 'bg-neutral-200 text-neutral-500')}
                        >
                            ...
                            <AnimatedSizeContainer height transition={{ ease: 'easeInOut', duration: 0.1 }}>
                                <div className='flex items-center justify-between gap-4 p-2 text-sm'>
                                        ...
                                        {domainStatus !== 'idle' ? (
                                            <>
                                                ...
                                                {currentStatusProps.useStrong ? (
                                                    <strong className='font-semibold underline underline-offset-2'>{domain}</strong>
                                                ) : (
                                                    <span className='font-semibold underline underline-offset-2'>{domain}</span>
                                                )}{' '}
                                    ...
                                    {currentStatusProps.icon && <currentStatusProps.icon className='size-5 shrink-0' />}
                                </div>
                            </AnimatedSizeContainer>
                        </div>
                    </div>
            ...
            {showAdvancedOptions && (
                <>
                    ...
                    <div className='flex flex-col gap-y-6'>
                        {ADVANCED_OPTIONS.map(({ id, title, description, icon: Icon, proFeature }) => {
            ...
            <Button text={props ? 'Save changes' : 'Add domain'} disabled={saveDisabled} loading={isSubmitting} />
        </form>
    )
}
