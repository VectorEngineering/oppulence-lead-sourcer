import { AnimatedSizeContainer, Button, SimpleTooltipContent, TooltipContent, buttonVariants, useMediaQuery } from '@dub/ui'
...
export function RegisterDomainForm({
    ...
    const searchDomainAvailability = async () => {
        ...
        const response = await fetch(`/api/domains/search-availability?domain=${slug}.link&workspaceId=${workspace.id}`)
    ...
    const registerDomain = async (domain: string) => {
        ...
        const response = await fetch(`${baseUrl}?domain=${domain}&workspaceId=${workspace.id}`, {
            method: 'POST'
        })
        ...
        if (saveOnly) {
            ...
            await mutatePrefix([`/api/workspaces/${workspace.slug}`, '/api/domains', '/api/links'])
        }

        onSuccess(domain.toLowerCase())
    }

    const searchedDomain = searchedDomains.find((d) => d.domain === `${slug}.link`.toLowerCase())

    const availableDomains = searchedDomains.filter((d) => d.domain !== `${slug}.link`.toLowerCase() && d.available)
    ...
    return (
        ...
        >
            <div className={cn('flex flex-col gap-y-6 text-left', variant === 'modal' && 'px-4 sm:px-6')}>
                <div>
                    <div className='flex items-center gap-2'>
                        <p className='block text-sm font-medium text-gray-800'>Search domains</p>
                    ...
                    <div className='mt-2'>
                        <div
                            className={cn(
                                ...
                                searchedDomain ? (searchedDomain.available ? 'bg-[#def5c6]' : 'bg-orange-100') : 'bg-gray-100'
                            )}
                        >
                            ...
                            <AnimatedSizeContainer height transition={{ ease: 'easeInOut', duration: 0.1 }}>
                                <div className='flex justify-between gap-3 px-2 pb-2 pt-3 text-sm text-gray-700'>
                                    <p>
                                        {searchedDomain ? (
                                            searchedDomain.available ? (
                                                <>
                                                    <span className='font-semibold text-gray-800'>{searchedDomain.domain}</span> is
                                            ...
                                            <>
                                                ...
                                                <strong className='font-semibold'>{truncate(`${slug}.link`, 25)}</strong>
                                            </>
                                        ) : (
                                            <>&nbsp;</>
                                        )}
                                    ...
                                    {isSearching || (!searchedDomain && slug?.trim()) ? (
                                        ...
                                        searchedDomain?.available ? (
                                            ...
                                            <Star className='size-5 shrink-0 text-amber-500' fill='currentColor' />
                                        ) : (
                ...
                {searchedDomain && !searchedDomain.available && availableDomains.length > 0 && (
                    <div>
                        <h2 className='text-sm font-medium text-gray-800'>Available alternatives</h2>
                                ...
                                {availableDomains.map((alternative) => (
                                    ...
                                    >
                                        <div className='flex items-center gap-2'>
                                            ...
                                            <span className='text-sm font-medium'>{alternative.domain}</span>
                                        </div>
                                        <Button
                                            text='Claim domain'
                                            className='h-8 w-fit'
                                            onClick={() => registerDomain(alternative.domain)}
                                            disabled={isRegistering || (workspace.plan === 'free' && !saveOnly)}
                                            disabledTooltip={workspace.plan === 'free' && !saveOnly ? <UpgradeTooltipContent /> : undefined}
                                        />
                                    </div>
                                ))}
                            </div>
            ...
            <div className={cn('mt-8 flex justify-end gap-2', variant === 'modal' && 'border-t border-gray-200 px-4 py-4 sm:px-6')}>
                {onCancel && variant === 'modal' && (
                    <Button type='button' variant='secondary' text='Cancel' className='h-9 w-fit' onClick={onCancel} />
                )}
...
function UpgradeTooltipContent() {
    ...
    return (
        <TooltipContent
            title={
                <>
                    You can only claim a free <span className='font-semibold'>.link</span> domain on a Pro plan and above.
                </>
            }
            cta='Upgrade to Pro'
