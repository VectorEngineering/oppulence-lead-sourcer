...
import { ANALYTICS_SALE_UNIT, ANALYTICS_VIEWS, EVENT_TYPES, VALID_ANALYTICS_FILTERS } from '@/lib/analytics/constants'
import { AnalyticsResponseOptions, AnalyticsSaleUnit, AnalyticsView, EventType } from '@/lib/analytics/types'
...
export default function AnalyticsProvider({
    ...
    const showConversions = adminPage || demoPage || partnerPage || conversionEnabled || dashboardProps?.showConversions ? true : false
    ...
    const { start, end } = useMemo(() => {
        ...
        return {
            start: hasRange ? startOfDay(new Date(searchParams?.get('start') || subDays(new Date(), 1))) : undefined,
    ...
    const [persistedSaleUnit, setPersistedSaleUnit] = useLocalStorage<AnalyticsSaleUnit>(`analytics-sale-unit`, 'saleAmount')

    const saleUnit: AnalyticsSaleUnit = useMemo(() => {
        const searchParamsSaleUnit = searchParams.get('saleUnit') as AnalyticsSaleUnit
    ...
    const [persistedView, setPersistedView] = useLocalStorage<AnalyticsView>(`analytics-view`, 'timeseries')
    const view: AnalyticsView = useMemo(() => {
        ...
        return ANALYTICS_VIEWS.includes(persistedView) ? persistedView : 'timeseries'
    ...
    }, [adminPage, demoPage, slug, pathname, dashboardProps?.domain, dashboardId, partner?.id, programSlug, domainSlug, key, selectedTab])
        ...
        fetcher,
        {
            ...
            onError: (error) => {
                try {
                    ...
                    if (error.status === 403 && errorMessage.toLowerCase().includes('upgrade')) {
                        toast.custom(() => <UpgradeRequiredToast title='Upgrade for more analytics' message={errorMessage} />)
