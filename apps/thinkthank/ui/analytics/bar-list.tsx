...
export default function BarList({
    ...
    const filteredData = useMemo(() => {
        if (limit) {
            ...
            return search ? data.filter((d) => d.title.toLowerCase().includes(search.toLowerCase())) : data
        }
    }, [data, limit, search])
    ...
    const lineItem = useMemo(() => {
        return (
            <div className='z-10 flex items-center space-x-4 overflow-hidden px-3'>
                {icon}
                <div className='truncate text-sm text-gray-800'>{getPrettyUrl(title)}</div>
            </div>
        )
    }, [icon, tab, title])
        ...
        >
            <div className='group flex items-center justify-between'>
                <div className='relative z-10 flex h-8 w-full min-w-0 max-w-[calc(100%-2rem)] items-center'>
                    {tab === 'links' && linkData ? (
                        <Tooltip content={<LinkPreviewTooltip data={linkData} />}>{lineItem}</Tooltip>
                    ) : tab === 'urls' ? (
                        <Tooltip
                            content={
                ...
                <NumberFlow
                    value={unit === 'sales' && saleUnit === 'saleAmount' ? value / 100 : value}
