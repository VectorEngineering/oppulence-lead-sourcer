...
export function useAnalyticsFilterOption(
    groupByOrParams: AnalyticsGroupByOptions | ({ groupBy: AnalyticsGroupByOptions } & Record<string, any>),
    options?: { cacheOnly?: boolean }
): AnalyticsFilterResult {
    const { cache } = useSWRConfig()

    const { baseApiPath, queryString, selectedTab, requiresUpgrade } = useContext(AnalyticsContext)

    const enabled =
        !options?.cacheOnly ||
        [...cache.keys()].includes(
            `${baseApiPath}?${editQueryString(queryString, {
                ...(typeof groupByOrParams === 'string' ? { groupBy: groupByOrParams } : groupByOrParams)
            })}`
        )
