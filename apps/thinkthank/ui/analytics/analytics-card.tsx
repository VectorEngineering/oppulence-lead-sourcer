...
export function AnalyticsCard<T extends string>({
    ...
    expandLimit: number
    hasMore?: boolean
    children: (props: { limit?: number; event?: EventType; setShowModal: (show: boolean) => void }) => ReactNode
    className?: string
}) {
    ...
    return (
        <>
            ...
            <div className={cn('relative z-0 h-[400px] overflow-hidden border border-gray-200 bg-white sm:rounded-xl', className)}>
                    ...
                    {isMobile ? (
                        ...
                        >
                            <Button
                                ...
                                right={<ChevronsUpDown className='size-4 shrink-0 text-gray-400' aria-hidden='true' />}
                            />
                        </Popover>
                    ) : (
                ...
                <div className='py-4'>{children({ limit: expandLimit, event, setShowModal })}</div>
                {hasMore && (
                    <div className='absolute bottom-0 left-0 z-10 flex w-full items-end'>
                        ...
                        <button onClick={() => setShowModal(true)} className='group relative flex w-full items-center justify-center py-4'>
