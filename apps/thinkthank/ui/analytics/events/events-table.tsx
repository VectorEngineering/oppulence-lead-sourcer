...
import { CopyText, LinkLogo, Table, Tooltip, usePagination, useRouterStuff, useTable } from '@dub/ui'
import { CursorRays, Globe, Magnifier, QRCode } from '@dub/ui/icons'
import { CONTINENTS, COUNTRIES, REGIONS, capitalize, fetcher, getApexDomain, getPrettyUrl, nFormatter } from '@dub/utils'
...
type ColumnMeta = {
    filterParams?: (args: Pick<Cell<EventDatum, any>, 'getValue'>) => Record<string, any>
}

export default function EventsTable({ requiresUpgrade, upgradeOverlay }: { requiresUpgrade?: boolean; upgradeOverlay?: ReactNode }) {
    const { searchParams, queryParams } = useRouterStuff()
    const { setExportQueryString } = useContext(EventsContext)
    const { selectedTab: tab, queryString: originalQueryString, eventsApiPath, totalEvents } = useContext(AnalyticsContext)
        ...
        () =>
            [
                ...
                },
                {
                    ...
                    cell: ({ getValue }) => (
                        <div className='flex items-center gap-3'>
                            <LinkLogo apexDomain={getApexDomain(getValue().url)} className='size-4 shrink-0 sm:size-4' />
                            <CopyText value={getValue().shortLink} successMessage='Copied link to clipboard!' className='truncate'>
                ...
                },
                {
                    ...
                    cell: ({ getValue }) => (
                        <div className='flex items-center gap-3' title={COUNTRIES[getValue()] ?? getValue()}>
                            {getValue() === 'Unknown' ? (
                            ...
                            <span className='truncate'>{COUNTRIES[getValue()] ?? getValue()}</span>
                        </div>
                    )
                },
                {
                ...
                },
                {
                    ...
                    cell: ({ getValue, row }) => (
                        <div className='flex items-center gap-3' title={getValue()}>
                            ...
                            <span className='truncate'>{REGIONS[getValue()] || getValue().split('-')[1]}</span>
                        </div>
                    )
                },
                {
                    ...
                    cell: ({ getValue }) => (
                        <div className='flex items-center gap-3' title={CONTINENTS[getValue()] ?? 'Unknown'}>
                            <ContinentIcon display={getValue()} className='size-4 shrink-0' />
                            <span className='truncate'>{CONTINENTS[getValue()] ?? 'Unknown'}</span>
                        </div>
                    )
                },
                {
                    ...
                    cell: ({ getValue }) => (
                        <div className='flex items-center gap-3' title={getValue()}>
                            <DeviceIcon display={capitalize(getValue()) ?? getValue()} tab='devices' className='size-4 shrink-0' />
                            <span className='truncate'>{getValue()}</span>
                        </div>
                    )
                },
                ...
                },
                {
                    ...
                    cell: ({ getValue }) => (
                        <div className='flex items-center gap-3' title={getValue()}>
                            <DeviceIcon display={capitalize(getValue()) ?? getValue()} tab='os' className='size-4 shrink-0' />
                            <span className='truncate'>{getValue()}</span>
                        </div>
                    )
                },
                    ...
                    cell: ({ getValue }) => (
                        <div className='flex items-center gap-3' title={getValue()}>
                            {getValue() === '(direct)' ? (
                                ...
                                <LinkLogo apexDomain={getValue()} className='size-4 shrink-0 sm:size-4' />
                            )}
                            <span className='truncate'>{getValue()}</span>
                        </div>
                    )
                    ...
                    cell: ({ getValue }) => (
                        <div className='flex items-center gap-3'>
                            {getValue() === '(direct)' ? (
                                ...
                                <LinkLogo apexDomain={getApexDomain(getValue())} className='size-4 shrink-0 sm:size-4' />
                            )}
                            <CopyText value={getValue()} successMessage='Copied referrer URL to clipboard!'>
                                <span className='truncate' title={getValue()}>
                    ...
                    cell: ({ getValue }) =>
                        getValue() ? (
                            ...
                            <Tooltip content='We do not record IP addresses for EU users.'>
                                <span className='cursor-default truncate underline decoration-dotted'>Unknown</span>
                            </Tooltip>
                        )
                },
                // Sale amount
    ...
    const { table, ...tableProps } = useTable({
        ...
        cellRight: (cell) => {
            ...
            return meta?.filterParams && <FilterButton set={meta.filterParams(cell)} />
        },
    ...
    const [customerDetailsSheet, setCustomerDetailsSheet] = useState<
        { open: false; customer: Customer | null } | { open: true; customer: Customer }
    ...
    useEffect(() => {
        ...
        if (customerId) {
            const customerEvent = data.find((d) => 'customer' in d && d.customer?.id === customerId)
    ...
    return (
        <>
            ...
            <Table {...tableProps} table={table} scrollWrapperClassName={requiresUpgrade ? 'overflow-x-hidden' : undefined}>
                {requiresUpgrade && (
                    <>
