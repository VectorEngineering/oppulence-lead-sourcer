...
export default function EventsTabs({ conversionEnabled }: { conversionEnabled?: boolean }) {
    ...
    const { data: timeseriesData, isLoading: isLoadingTimeseries } = useSWRImmutable<TimeseriesData>(
        `${baseApiPath}?${editQueryString(queryString, {
            groupBy: 'timeseries',
            event: showConversions ? 'composite' : 'clicks',
    ...
    const onEventTabClick = useCallback(
        (event: string) => {
            const sortOptions = event === 'sales' ? ['timestamp', 'saleAmount'] : ['date']
            ...
            queryParams({
                ...
                del: [
                    ...
                    ...(currentSort && !sortOptions.includes(currentSort) ? ['sort'] : [])
                ]
            })
        },
    ...
    return (
        <div className='grid w-full grid-cols-3 gap-2 overflow-x-auto sm:gap-4'>
            {['clicks', ...(showConversions ? ['leads', 'sales'] : [])].map((event) => (
                <button
                    key={event}
                    className={cn(
                ...
                >
                    <div>
                        ...
                        <div className='mt-2'>
                            {totalEvents ? (
                                <NumberFlow
                                    value={event === 'sales' ? totalEvents?.saleAmount / 100 : totalEvents?.[event]}
                                    className={cn('text-2xl transition-opacity', isLoadingTotalEvents && 'opacity-40')}
                                    format={
                                        event === 'sales'
                                            ? {
                                                  style: 'currency',
                                            ...
                                            : {
                                                  notation: totalEvents?.[event] > 999999 ? 'compact' : 'standard'
                                              }
                                    }
                                />
                            ) : (
                    ...
                    {timeseriesData && !isMobile && (
                        <div className={cn('relative h-full max-w-[140px] grow transition-opacity', isLoadingTimeseries && 'opacity-40')}>
                            <MiniAreaChart
                                data={
                                    timeseriesData?.map((d) => ({
                                        date: new Date(d.start),
                                        value: ((event === 'sales' ? d?.saleAmount : d?.[event]) as number | undefined) ?? 0
                                    })) || []
                                }
                            />
                        </div>
