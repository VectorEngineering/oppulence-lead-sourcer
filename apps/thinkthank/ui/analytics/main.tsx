...
export default function Main() {
    const { totalEvents, requiresUpgrade, showConversions, selectedTab, saleUnit, view } = useContext(AnalyticsContext)
                            ...
                            <div key={id} className='relative z-0'>
                                {idx > 0 && (
                                    <div className='absolute left-0 top-1/2 z-10 -translate-x-1/2 -translate-y-1/2 rounded-full border border-gray-200 bg-white p-1.5'>
                                        <ChevronRight className='h-3 w-3 text-gray-400' strokeWidth={2.5} />
                                    </div>
                                )}
                                {id === 'sales' && (
                                    <ToggleGroup
