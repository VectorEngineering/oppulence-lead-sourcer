...
import { INTERVAL_DATA, INTERVAL_DISPLAYS, TRIGGER_DISPLAY, VALID_ANALYTICS_FILTERS } from '@/lib/analytics/constants'
...
export default function Toggle({ page = 'analytics' }: { page?: 'analytics' | 'events' }) {
    const { slug, plan } = useWorkspace()

    const { router, queryParams, searchParamsObj, getQueryString } = useRouterStuff()

    const { domain, key, url, adminPage, demoPage, partnerPage, dashboardProps, showConversions, start, end, interval } =
    ...
    const {
        ...
        ignoreParams: true,
        opts: {
            search: domainsAsync && selectedFilter === 'domain' ? debouncedSearch : ''
        }
    })

    const selectedTagIds = useMemo(() => searchParamsObj.tagIds?.split(',')?.filter(Boolean) ?? [], [searchParamsObj.tagIds])
    ...
    const activeFilters = useMemo(() => {
        const { domain, key, continent, country, region, city, device, browser, os, trigger, referer, refererUrl, url, root } =
            searchParamsObj
        return [
            ...
            ...(domain && key
                ...
                : []),
            ...(selectedTagIds.length > 0 ? [{ key: 'tagIds', value: selectedTagIds }] : []),
    ...
    const isRequested = useCallback(
        (key: string) => requestedFilters.includes(key) || activeFilters.some((af) => af.key === key),
    ...
    const filters: ComponentProps<typeof Filter.Select>['filters'] = useMemo(
        () => [
                ...
                : [
                      {
                          ...
                          options: loadingDomains
                              ...
                              : [
                                    ...
                                    ...(!searchParamsObj.domain || domains.some((d) => d.slug === searchParamsObj.domain)
                                        ? []
                                        : [
                                              {
                      ...
                      },
                      {
                          ...
                          options:
                              links?.map(({ domain, key, url, count }: LinkProps & { count?: number }) => ({
                                  value: linkConstructor({ domain, key, pretty: true }),
                      ...
                      },
                      {
                          ...
                          getOptionIcon: (value, props) => {
                              const tagColor = props.option?.data?.color ?? tags?.find(({ id }) => id === value)?.color
                              return tagColor ? <TagBadge color={tagColor} withIcon className='sm:p-1' /> : null
                          },
                          options:
            ...
            },
            {
                ...
                icon: FlagWavy,
                label: 'Country',
                getOptionIcon: (value) => <img alt={value} src={`https://flag.vercel.app/m/${value}.svg`} className='h-2.5 w-4' />,
            ...
            },
            {
                ...
                options:
                    cities?.map(({ city, country, count }) => ({
                        value: city,
                        label: city,
                        icon: <img alt={country} src={`https://flag.vercel.app/m/${country}.svg`} className='h-2.5 w-4' />,
                        right: nFormatter(count, { full: true })
                    })) ?? null
            },
            {
            ...
            },
            {
                ...
                icon: MapPosition,
                label: 'Continent',
                getOptionIcon: (value) => <ContinentIcon display={value} className='size-2.5' />,
            ...
            },
            {
                ...
                icon: MobilePhone,
                label: 'Device',
                getOptionIcon: (value) => <DeviceIcon display={capitalize(value) ?? value} tab='devices' className='h-4 w-4' />,
                options:
            ...
            },
            {
                ...
                icon: Window,
                label: 'Browser',
                getOptionIcon: (value) => <DeviceIcon display={value} tab='browsers' className='h-4 w-4' />,
                options:
            ...
            },
            {
                ...
                icon: Cube,
                label: 'OS',
                getOptionIcon: (value) => <DeviceIcon display={value} tab='os' className='h-4 w-4' />,
                options:
                    os?.map(({ os, count }) => ({
                        value: os,
                        label: os,
            ...
            },
            {
                ...
                icon: ReferredVia,
                label: 'Referer',
                getOptionIcon: (value, props) => <RefererIcon display={value} className='h-4 w-4' />,
                options:
            ...
            },
            {
                ...
                getOptionIcon: (_, props) => <LinkLogo apexDomain={getApexDomain(props.option?.value)} className='size-4 sm:size-4' />,
                options:
                    urls?.map(({ url, count }) => ({
                        value: url,
    ...
    const filterSelect = (
        <Filter.Select
            ...
            onRemove={(key, value) =>
                queryParams(
                    key === 'tagIds' && !(selectedTagIds.length === 1 && selectedTagIds[0] === value)
                        ? {
                              set: {
            ...
            onOpenFilter={(key) => setRequestedFilters((rf) => (rf.includes(key) ? rf : [...rf, key]))}
            askAI
        />
    )
    ...
    const dateRangePicker = (
        <DateRangePicker
            ...
            presets={INTERVAL_DISPLAYS.map(({ display, value, shortcut }) => {
                ...
                const requiresUpgrade = DUB_DEMO_LINKS.find((l) => l.domain === domain && l.key === key)
                    ? false
                ...
                return {
                    ...
                    requiresUpgrade,
                    tooltipContent: requiresUpgrade ? <UpgradeTooltip rangeLabel={display} plan={plan} /> : undefined,
                    shortcut
                }
            })}
        />
    ...
    return (
        <>
            ...
            >
                <div
                    className={cn('mx-auto flex w-full max-w-screen-xl flex-col gap-2 px-3 lg:px-10', {
                        'md:h-10': key
                    })}
                >
                    <div
                                                ...
                                                <Button
                                                    ...
                                                    onClick={() => {
                                                        if (dashboardProps) {
                                                            ...
                                                            router.push(`/${slug}/events${getQueryString({}, { ignore: ['view'] })}`)
                                                        }
                                                    }}
                                                />
                                                <AnalyticsOptions />
                                        ...
                                        {page === 'events' && !partnerPage && (
                                            <>
                                                <Button
                                                    ...
                                                    onClick={() => router.push(`/${slug}/analytics${getQueryString()}`)}
                                                />
                                                <EventsOptions />
                                            </>
                                        )}
                ...
                <Filter.List
                    ...
                    onRemoveAll={() =>
                        queryParams({
                            ...
                            del: VALID_ANALYTICS_FILTERS.concat(['page']).filter((f) => !['interval', 'start', 'end'].includes(f)),
                            scroll: false
                        })
                    }
                />
                <div className={cn('transition-[height] duration-[300ms]', streaming || activeFilters.length ? 'h-3' : 'h-0')} />
            </div>
        </>
    )
}

function UpgradeTooltip({ rangeLabel, plan }: { rangeLabel: string; plan?: string }) {
...
function LinkIcon({ url: urlProp, domain, linkKey }: { url?: string; domain?: string; linkKey?: string }) {
    ...
    return url ? <LinkLogo apexDomain={getApexDomain(url)} className='h-4 w-4 sm:h-4 sm:w-4' /> : <Hyperlink className='h-4 w-4' />
}
