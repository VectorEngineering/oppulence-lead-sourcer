...
import { CardList, MaxWidthWrapper, PaginationControls, usePagination } from '@dub/ui'
...
export default function LinksContainer({ CreateLinkButton }: { CreateLinkButton: () => JSX.Element }) {
...
function LinksList({
    ...
    const isFiltered = ['domain', 'tagId', 'userId', 'search', 'showArchived'].some((param) => searchParams.has(param))
                        ...
                        {links?.length
                            ...
                            : // Loading placeholder cards
                              Array.from({ length: 12 }).map((_, idx) => (
                                  <CardList.Card key={idx} outerClassName='pointer-events-none' innerClassName='flex items-center gap-4'>
                                      <LinkCardPlaceholder />
                                  </CardList.Card>
                              ))}
                    </CardList>
