...
import { ArrowTurnRight2, Avatar, CopyButton, LinkLogo, Switch, Tooltip, TooltipContent, useInViewport } from '@dub/ui'
import {
    Apple,
    ArrowRight,
    Bolt,
...
import { cn, formatDateTime, getApexDomain, getPrettyUrl, isDubDomain, linkConstructor, timeAgo } from '@dub/utils'
...
export function LinkTitleColumn({ link }: { link: ResponseLink }) {
    ...
    return (
        <div ref={ref} className='flex h-[32px] items-center gap-3 transition-[height] group-data-[variant=loose]/card-list:h-[60px]'>
            <div className={cn('relative hidden shrink-0 items-center justify-center', displayProperties.includes('icon') && 'sm:flex')}>
                    ...
                    <div className='min-w-0 shrink grow-0 text-gray-950'>
                        <div className='flex items-center gap-2'>
                            {displayProperties.includes('title') && link.title ? (
                                <span className={cn('truncate font-semibold leading-6 text-gray-800', link.archived && 'text-gray-600')}>
                                    {link.title}
                                </span>
                            ) : (
...
function UnverifiedTooltip({ domain, _key, children }: PropsWithChildren<{ domain: string; _key: string }>) {
...
const Details = memo(({ link, compact }: { link: ResponseLink; compact?: boolean }) => {
    ...
    return (
        ...
        >
            ...
            <div className={cn('hidden shrink-0', displayProperties.includes('user') && 'sm:block')}>
                <UserAvatar link={link} compact={compact} />
            </div>
            <div className={cn('hidden shrink-0', displayProperties.includes('createdAt') && 'sm:block')}>
...
function UserAvatar({ link, compact }: { link: ResponseLink; compact?: boolean }) {
    const { user } = link
    const { slug } = useWorkspace()
            ...
            content={
                <div className='w-full p-3'>
                    ...
                    <div className='mt-2 flex items-center gap-1.5'>
                        <p className='text-sm font-semibold text-gray-700'>{user?.name || user?.email || 'Anonymous User'}</p>
                        {!slug && // this is only shown in admin mode (where there's no slug)
                            user?.email && <CopyButton value={user.email} icon={Mail} className='[&>*]:h-3 [&>*]:w-3' />}
                    </div>
                    <div className='flex flex-col gap-1 text-xs text-gray-500'>{user?.name && user.email && <p>{user.email}</p>}</div>
                </div>
            }
        >
            <div>
