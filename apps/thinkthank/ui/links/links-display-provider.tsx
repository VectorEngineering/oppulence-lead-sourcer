...
export const linkDisplayPropertyIds = ['icon', 'link', 'url', 'title', 'description', 'createdAt', 'user', 'tags', 'analytics'] as const
...
export const defaultDisplayProperties: LinkDisplayProperty[] = ['icon', 'link', 'url', 'createdAt', 'user', 'tags', 'analytics']

function useLinksDisplayOption<T>(key: string, parsePersisted: (value: T) => T, defaultValue: T, overrideValue?: T) {
    const [valuePersisted, setValuePersisted] = useLocalStorage<T>(`links-display-${key}`, defaultValue)
...
const parseViewMode = (viewModeRaw: string) => linkViewModes.find((vm) => vm === viewModeRaw) ?? linkViewModes[0]

const parseDisplayProperties = (displayPropertiesRaw: string[]) =>
    linkDisplayPropertyIds.filter((p) => displayPropertiesRaw.findIndex((pr) => pr === p) !== -1)

const parseSort = (sort: string) => sortOptions.find(({ slug }) => slug === sort)?.slug ?? sortOptions[0].slug
...
export function LinksDisplayProvider({ children }: PropsWithChildren) {
    ...
    const {
        ...
        reset: resetViewMode
    } = useLinksDisplayOption<string>('view-mode', parseViewMode, linkViewModes[0])
    ...
    const {
        ...
        persist: persistSort,
        reset: resetSort
    } = useLinksDisplayOption<string>('sortBy', parseSort, sortOptions[0].slug, sortRaw ? parseSort(sortRaw) : undefined)
    ...
    } = useLinksDisplayOption<boolean>('show-archived', parseShowArchived, false, showArchivedRaw ? showArchivedRaw === 'true' : undefined)
    ...
    } = useLinksDisplayOption<LinkDisplayProperty[]>('display-properties', parseDisplayProperties, defaultDisplayProperties)
    ...
    const isDirty = useMemo(() => {
        ...
        if (displayProperties.slice().sort().join(',') !== parseDisplayProperties(displayPropertiesPersisted).slice().sort().join(','))
            return true
