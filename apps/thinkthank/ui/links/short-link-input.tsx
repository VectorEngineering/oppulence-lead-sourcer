...
import { cn, DUB_DOMAINS, getApexDomain, linkConstructor, nanoid, punycode, truncate } from '@dub/utils'
import { useCompletion } from 'ai/react'
import { TriangleAlert } from 'lucide-react'
import posthog from 'posthog-js'
import { forwardRef, HTMLProps, useCallback, useEffect, useId, useMemo, useState } from 'react'
...
export const ShortLinkInput = forwardRef<HTMLInputElement, ShortLinkInputProps>(
    (
        ...
        const { id: workspaceId, slug, mutate: mutateWorkspace, exceededAI, nextPlan, dotLinkClaimed } = useWorkspace()
        ...
        const runKeyChecks = async (value: string) => {
            const res = await fetch(`/api/links/exists?domain=${domain}&key=${value}&workspaceId=${workspaceId}`)
        ...
        const [generatedKeys, setGeneratedKeys] = useState<string[]>(existingLink && key ? [key] : [])
        ...
        const {
            ...
            onError: (error) => {
                if (error.message.includes('Upgrade to Pro')) {
                    toast.custom(() => <UpgradeRequiredToast title="You've exceeded your AI usage limit" message={error.message} />)
                } else {
            ...
            <div>
                <div className='flex items-center justify-between'>
                    ...
                    {lockKey ? (
                        ...
                        <div className='flex items-center gap-1'>
                            ...
                            >
                                {generatingRandomKey ? <LoadingCircle /> : <Random className='h-3 w-3' />}
                            </ButtonTooltip>
                            <ButtonTooltip
                                tabIndex={-1}
                                tooltipProps={{
                            ...
                            >
                                {generatingAIKey ? <LoadingCircle /> : <Magic className='h-4 w-4' />}
                            </ButtonTooltip>
                        </div>
                    )}
                </div>
                <div className='relative mt-1 flex rounded-md shadow-sm'>
                    ...
                    <input
                        ...
                        onInvalid={(e) => {
                            e.currentTarget.setCustomValidity("Only letters, numbers, '-', '_', '/', and emojis are allowed.")
                        }}
                        onBlur={(e) => {
                        ...
                        className={cn(
                            ...
                            'z-0 focus:z-[1]',
                            {
                                'border-red-300 pr-10 text-red-900 placeholder-red-300 focus:border-red-500 focus:ring-red-500': error,
                                'border-amber-300 pr-10 text-amber-900 placeholder-amber-300 focus:border-amber-500 focus:ring-amber-500':
                                    shortLink.length > 25,
                                'cursor-not-allowed border border-gray-300 bg-gray-100 text-gray-500': lockKey
                            }
                        )}
                                    ...
                                    <div className='flex max-w-xs items-start space-x-2 bg-white p-4'>
                                        ...
                                        <div>
                                            <p className='text-sm text-gray-700'>
                                                Short links longer than 25 characters will show up differently on some platforms.
                                            </p>
                ...
                {error ? (
                    error.includes('Upgrade to') ? (
                        <p className='mt-2 text-sm text-red-600' id='key-error'>
                            ...
                            <a className='cursor-pointer underline' href={`/${slug}/upgrade?exit=close`} target='_blank'>
                    ...
                    <DefaultDomainPrompt domain={domain} url={data.url} onChange={(domain) => onChange({ domain })} />
                )}
                {!onboarding && !dotLinkClaimed && (
                    <AnimatedSizeContainer height transition={{ ease: 'linear', duration: 0.1 }} className='mt-2'>
                        <FreeDotLinkBanner />
                    </AnimatedSizeContainer>
                )}
            </div>
...
function DefaultDomainPrompt({ domain, url, onChange }: { domain?: string; url?: string; onChange: (domain: string) => void }) {
    if (!url || !domain) return null

    const apexDomain = getApexDomain(url)
    const hostnameFor = DUB_DOMAINS.find((domain) => domain?.allowedHostnames?.includes(apexDomain))
    ...
    return (
        ...
        >
            ...
            <p>
                Use <strong className='font-semibold'>{domainSlug}</strong> domain instead?
            </p>
        </button>
    )
}

function DomainCombobox({ domain, setDomain, loading }: { domain?: string; setDomain: (domain: string) => void; loading: boolean }) {
    ...
    useEffect(() => {
        if (allWorkspaceDomains && !useAsync && allWorkspaceDomains.length >= DOMAINS_MAX_PAGE_SIZE) setUseAsync(true)
    ...
    return (
        <Combobox
            ...
            options={options}
            caret={true}
            placeholder={<div className='h-4 w-3/4 animate-pulse rounded bg-gray-200' />}
