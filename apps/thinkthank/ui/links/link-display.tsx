import { Button, Popover, Switch, useKeyboardShortcut, useRouterStuff } from '@dub/ui'
import { ArrowsOppositeDirectionY, BoxArchive, GridLayoutRows, Sliders, TableRows2 } from '@dub/ui/icons'
import { cn } from '@dub/utils'
import { AnimatePresence, motion } from 'framer-motion'
import { ChevronDown } from 'lucide-react'
import { useContext, useState } from 'react'
import LinkSort from './link-sort'
import { LinksDisplayContext, LinksViewMode, linkDisplayProperties } from './links-display-provider'

export default function LinkDisplay() {
    const { viewMode, setViewMode, showArchived, setShowArchived, displayProperties, setDisplayProperties, isDirty, persist, reset } =
        ...
        <Popover
            content={
                <div className='w-full divide-y divide-gray-200 text-sm md:w-80'>
                    <div className='grid grid-cols-2 gap-2 p-3'>
                        {[
                            ...
                            return (
                                ...
                                >
                                    <Icon className={cn('h-5 w-5 text-gray-600', selected && 'text-gray-800')} />
                                    {label}
                                </button>
                            )
                        })}
                    ...
                    <div className='p-4'>
                        <span className='text-xs uppercase text-gray-500'>Display Properties</span>
                                        ...
                                        onClick={() => {
                                            ...
                                            if (property.switch) {
                                                ...
                                                newDisplayProperties = [
                                                    ...newDisplayProperties.filter((p) => p !== property.switch),
                        ...
                        {isDirty && (
                            ...
                            >
                                <div className='flex items-center justify-end gap-2 p-2'>
                                    <Button className='h-8 w-auto px-2' variant='outline' text='Reset to default' onClick={reset} />
                                    <Button className='h-8 w-auto px-2' variant='primary' text='Set as default' onClick={persist} />
                                </div>
                            </motion.div>
                        )}
                    </AnimatePresence>
