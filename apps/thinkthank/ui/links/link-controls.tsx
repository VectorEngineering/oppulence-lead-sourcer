import { Button, CardList, IconMenu, PenWriting, Popover, SimpleTooltipContent, useCopyToClipboard, useKeyboardShortcut } from '@dub/ui'
...
export function LinkControls({ link }: { link: ResponseLink }) {
    ...
    const { setShowLinkBuilder: setShowDuplicateLinkModal, LinkBuilder: DuplicateLinkModal } = useLinkBuilder({
        // @ts-expect-error
        duplicateProps: {
            ...propsToDuplicate,
                    ...
                    <div className='w-full sm:w-48'>
                        <div className='grid gap-px p-2'>
                            ...
                            <Button
                                ...
                                variant='outline'
                                onClick={() => copyLinkId()}
                                icon={copiedLinkId ? <CircleCheck className='h-4 w-4' /> : <Copy className='h-4 w-4' />}
                                shortcut='I'
                                className='h-9 px-2 font-medium'
                            />
                        </div>
