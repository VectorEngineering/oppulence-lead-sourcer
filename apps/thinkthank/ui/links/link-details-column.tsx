...
import { Button, CardList, CopyButton, CursorRays, InvoiceDollar, Tooltip, useMediaQuery, UserCheck, useRouterStuff } from '@dub/ui'
import { ReferredVia } from '@dub/ui/icons'
import { APP_DOMAIN, cn, currencyFormatter, INFINITY_NUMBER, nFormatter, pluralize, timeAgo } from '@dub/utils'
...
function useOrganizedTags(tags: ResponseLink['tags']) {
    ...
    */
        const sortedTags =
            filteredTagIds.length > 0 ? [...tags].sort((a, b) => filteredTagIds.indexOf(b.id) - filteredTagIds.indexOf(a.id)) : tags
...
export function LinkDetailsColumn({ link }: { link: ResponseLink }) {
    ...
    return (
        <div ref={ref} className='flex items-center justify-end gap-2 sm:gap-5'>
            ...
            {displayProperties.includes('analytics') && <AnalyticsBadge link={link} />}
            <LinkControls link={link} />
        </div>
    )
}

function TagsTooltip({ additionalTags, children }: PropsWithChildren<{ additionalTags: TagProps[] }>) {
...
function TagButton({ tag, plus }: { tag: TagProps; plus?: number }) {
    ...
    return (
        <button
            onClick={() => {
                let newTagIds = selectedTagIds.includes(tag.id) ? selectedTagIds.filter((id) => id !== tag.id) : [...selectedTagIds, tag.id]
...
function AnalyticsBadge({ link }: { link: ResponseLink }) {
        ...
        () => [
            {
                ...
                icon: CursorRays,
                value: link.clicks,
                label: conversionEnabled ? undefined : (value) => pluralize('click', value),
    ...
    const { ShareDashboardModal, setShowShareDashboardModal } = useShareDashboardModal({ domain, _key: key })
            ...
            >
                ...
                >
                    <div className='hidden items-center gap-0.5 sm:flex'>
                        {stats.map(({ id: tab, icon: Icon, value, label, className, iconClassName }) => (
                            <div
                                key={tab}
                                className={cn(
                            ...
                            >
                                <Icon data-active={value > 0} className={cn('h-4 w-4 shrink-0', iconClassName)} />
                                <span>
                                    {tab === 'sales' ? currencyFormatter(value / 100) : nFormatter(value)}
                                    {label && <span className='hidden md:inline-block'>&nbsp;{label(value)}</span>}
                                </span>
                            </div>
                        ))}
                        {link.dashboardId && (
