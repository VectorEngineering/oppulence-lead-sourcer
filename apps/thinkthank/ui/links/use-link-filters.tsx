...
export function useLinkFilters() {
    ...
    const { tags, tagsAsync } = useTagFilterOptions(selectedFilter === 'tagIds' ? debouncedSearch : '')
        ...
        return [
            {
                ...
                label: 'Domain',
                getOptionIcon: (value) => (
                    <BlurImage src={`${GOOGLE_FAVICON_URL}${value}`} alt={value} className='h-4 w-4 rounded-full' width={16} height={16} />
                ),
    ...
    const selectedTagIds = useMemo(() => searchParamsObj['tagIds']?.split(',')?.filter(Boolean) ?? [], [searchParamsObj])
...
function useTagFilterOptions(search: string) {
    ...
    const tagIds = useMemo(() => searchParamsObj.tagIds?.split(',')?.filter(Boolean) ?? [], [searchParamsObj.tagIds])
    ...
    const tagsResult = useMemo(() => {
        return loadingTags ||
            ...
            (tagIds?.length && tagIds.some((id) => ![...(selectedTags ?? []), ...(tags ?? [])].some((t) => t.id === id)))
            ? null
            : ((
                  [
                      ...(tags ?? []),
        ...
        () =>
            users
                ? users
                      .map((user) => ({
                          ...user,
                          count: usersCount?.find(({ userId }) => userId === user.id)?._count || 0
                      }))
                      .sort((a, b) => b.count - a.count)
                : usersCount
