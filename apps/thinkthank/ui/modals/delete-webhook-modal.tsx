...
function DeleteWebhookModal({
    ...
    const deleteWebhook = async () => {
        ...
        const response = await fetch(`/api/webhooks/${webhook?.id}?workspaceId=${workspaceId}`, {
            method: 'DELETE',
    ...
    return (
        <Modal showModal={showDeleteWebhookModal} setShowModal={setShowDeleteWebhookModal}>
            <div className='flex flex-col items-center justify-center space-y-3 border-b border-gray-200 px-4 py-4 pt-8 sm:px-16'>
                ...
                <p className='text-center text-sm text-gray-500'>
                    This will stop all events from being sent to the endpoint and remove all webhook logs
                </p>
            </div>
            ...
            >
                <div>
                    <label htmlFor='verification' className='block text-sm text-gray-700'>
                        To verify, type <span className='font-semibold text-black'>{webhook.name}</span> below
                    </label>
...
export function useDeleteWebhookModal({ webhook }: { webhook: Pick<WebhookProps, 'id' | 'name' | 'url'> | undefined }) {
