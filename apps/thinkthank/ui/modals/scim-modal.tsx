import { Button, Copy, InfoTooltip, Logo, Modal, SimpleTooltipContent, Tick, useCopyToClipboard } from '@dub/ui'
...
function SCIMModal({ showSCIMModal, setShowSCIMModal }: { showSCIMModal: boolean; setShowSCIMModal: Dispatch<SetStateAction<boolean>> }) {
    const { id } = useWorkspace()
    const [submitting, setSubmitting] = useState(false)
    const { scim, provider, configured, mutate } = useSCIM()
    const [selectedProvider, setSelectedProvider] = useState<SAMLProviderProps['scim'] | undefined>(provider || undefined)
    ...
    const currentProvider = useMemo(() => SAML_PROVIDERS.find((p) => p.scim === selectedProvider), [selectedProvider])
            ...
            <div className='flex flex-col items-center justify-center space-y-3 border-b border-gray-200 px-4 py-8 sm:px-16'>
                {currentProvider ? (
                    <div className='flex items-center space-x-3 py-4'>
                        <img src={currentProvider.logo} alt={`${provider} logo`} className='h-10 w-10' />
                ...
                <h3 className='text-lg font-medium'>{currentProvider ? `${currentProvider.name} SCIM` : 'Configure Directory Sync'}</h3>
                ...
                >
                    <div>
                        <div className='flex items-center space-x-1'>
                            <h2 className='text-sm font-medium text-gray-900'>Directory Provider</h2>
                            <InfoTooltip
                                content={
                                    <SimpleTooltipContent
                                        ...
                                        href={`${platform.webUrl}/help/${
                                            currentProvider ? `article/${currentProvider.saml}-scim` : 'category/saml-sso'
                                        }`}
                                    />
                                }
                            />
                        ...
                        <select
                            ...
                            onChange={(e) => setSelectedProvider(e.target.value as SAMLProviderProps['scim'])}
                        ...
                        >
                            ...
                            {SAML_PROVIDERS.map((provider) => (
                                <option key={provider.scim} value={provider.scim} disabled={provider.wip}>
                    ...
                    {currentProvider && selectedProvider === provider && (
                        <div className='mt-4 flex flex-col space-y-4'>
                            ...
                            <div>
                                <div className='flex items-center space-x-1'>
                                    <h2 className='text-sm font-medium text-gray-900'>{currentProvider.scimModalCopy.url}</h2>
                                    <InfoTooltip
                                        content={
                                            <SimpleTooltipContent
                                                title='Your directory provider is the IDP you use to manage your users.'
                                ...
                                <div className='mt-1 flex w-full items-center justify-between rounded-md border border-gray-300 bg-white px-3 py-2 shadow-sm'>
                                    <div className='scrollbar-hide overflow-auto'>
                                        <p className='whitespace-nowrap text-gray-600 sm:text-sm'>{scim.directories[0].scim.endpoint}</p>
                                    </div>
                                    <button
                                        type='button'
                                        className='pl-2'
                            ...
                            <div>
                                <div className='flex items-center space-x-1'>
                                    <h2 className='text-sm font-medium text-gray-900'>{currentProvider.scimModalCopy.token}</h2>
                                    <InfoTooltip
                                        content={
                                            <SimpleTooltipContent
                                                title='Your directory provider is the IDP you use to manage your users.'
                                ...
                                <div className='mt-1 flex w-full items-center justify-between rounded-md border border-gray-300 bg-white px-3 py-2 shadow-sm'>
                                    ...
                                    <div className='flex space-x-2 pl-2'>
                                        ...
                                        <button type='button' onClick={() => setShowBearerToken(!showBearerToken)}>
                                            {showBearerToken ? (
                    ...
                    <Button text='Save changes' loading={submitting} disabled={!currentProvider || currentProvider.scim === provider} />
                </form>
            </div>
        </Modal>
    )
...
export function useSCIMModal() {
    ...
    const SCIMModalCallback = useCallback(() => {
        return <SCIMModal showSCIMModal={showSCIMModal} setShowSCIMModal={setShowSCIMModal} />
