...
const RegisterDomain = ({ showModal, setShowModal }: RegisterDomainProps) => {
    ...
    return (
        <Modal showModal={showModal} setShowModal={setShowModal} drawerRootProps={{ repositionInputs: false }}>
            <h3 className='border-b border-gray-200 px-4 py-4 text-lg font-medium sm:px-6'>Claim .link domain</h3>
...
export function useRegisterDomainModal() {
    ...
    const RegisterDomainModal = useCallback(() => {
        return <RegisterDomain showModal={showRegisterDomainModal} setShowModal={setShowRegisterDomainModal} />
    }, [showRegisterDomainModal, setShowRegisterDomainModal])

    return useMemo(() => ({ setShowRegisterDomainModal, RegisterDomainModal }), [setShowRegisterDomainModal, RegisterDomainModal])
}
