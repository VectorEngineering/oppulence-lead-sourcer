...
import { Button, InputSelect, InputSelectItemProps, LinkLogo, Modal } from '@dub/ui'
import { APP_NAME, DICEBEAR_AVATAR_URL, getApexDomain, isDubDomain, linkConstructor } from '@dub/utils'
...
function TransferLinkModal(props: TransferLinkModalProps) {
    return (
        <Modal showModal={props.showTransferLinkModal} setShowModal={props.setShowTransferLinkModal}>
            <TransferLinkModalInner {...props} />
        </Modal>
    )
}

function TransferLinkModalInner({ setShowTransferLinkModal, props }: TransferLinkModalProps) {
        ...
        >
            <div className='flex flex-col items-center justify-center space-y-3 border-b border-gray-200 px-4 py-4 pt-8 text-center sm:px-16'>
                ...
                <p className='text-sm text-gray-500'>
                    Transfer this link and its analytics to another {APP_NAME} workspace. Link tags will not be transferred.
                </p>
            </div>
            ...
            <div className='flex flex-col space-y-28 bg-gray-50 px-4 py-8 text-left sm:space-y-3 sm:rounded-b-2xl sm:px-16'>
                    ...
                    items={
                        workspaces
                            ? workspaces.map((workspace) => ({
                                  ...
                                  label: workspace.id.replace('ws_', '') === props.projectId ? 'Current' : ''
                              }))
                            : []
                    }
                ...
                />
                <Button disabled={!selectedWorkspace || !isDubDomain(domain)} loading={transferring} text='Confirm transfer' />
            </div>
        </form>
    )
}
