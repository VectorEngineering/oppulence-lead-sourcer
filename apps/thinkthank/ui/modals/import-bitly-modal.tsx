import { Button, LoadingSpinner, Logo, Modal, Switch, Tooltip, useRouterStuff } from '@dub/ui'
...
function ImportBitlyModal({
    ...
    const {
        ...
        isLoading,
        mutate
    } = useSWRImmutable<BitlyGroupProps[]>(workspaceId && showImportBitlyModal && `/api/workspaces/${workspaceId}/import/bitly`, fetcher, {
        onError: (err) => {
        ...
        >
            <div className='flex flex-col items-center justify-center space-y-3 border-b border-gray-200 px-4 py-8 sm:px-16'>
                <div className='flex items-center space-x-3 py-4'>
                    <img src='https://assets.dub.co/misc/icons/bitly.svg' alt='Bitly logo' className='h-10 w-10 rounded-full' />
                    <ArrowRight className='h-5 w-5 text-gray-600' />
                    <Logo />
                </div>
                <h3 className='text-lg font-medium'>Import Your Bitly Links</h3>
                <p className='text-center text-sm text-gray-500'>
                    Easily import all your existing Bitly links into {platform.company} with just a few clicks.
                </p>
            </div>
                        ...
                        <div className='divide-y divide-gray-200'>
                            {groups.length > 0 ? (
                                    ...
                                    <div key={guid} className='flex flex-col space-y-2'>
                                            ...
                                            >
                                                ...
                                                <Switch
                                                    fn={() => {
                                                        ...
                                                        if (selected) {
                                                            setSelectedDomains((prev) => prev.filter((d) => d.domain !== bsd))
                                                        } else {
                                        ...
                                        {tags?.length > 0 && (
                                            <div className='flex items-center justify-between space-x-2 rounded-md py-1 pl-2 pr-4'>
                                                <p className='text-xs text-gray-500'>{tags.length} tags found. Import all?</p>
                                                <Switch
                                                    fn={() => {
                                                        if (selectedGroupTags.includes(guid)) {
                                                            setSelectedGroupTags((prev) => prev.filter((g) => g !== guid))
                                                        } else {
                                ...
                                <div className='flex flex-col items-center justify-center gap-2 pb-2'>
                                    ...
                                    <p className='text-center text-sm text-gray-500'>
                                        It looks like you don't have any Bitly groups with custom domains (non bit.ly domains).
                                    </p>
                                </div>
                            )}
                        </div>
...
export function useImportBitlyModal() {
    ...
    const ImportBitlyModalCallback = useCallback(() => {
        return <ImportBitlyModal showImportBitlyModal={showImportBitlyModal} setShowImportBitlyModal={setShowImportBitlyModal} />
