...
function SubmitOAuthAppModal({
    ...
    const { execute, isExecuting } = useAction(submitOAuthAppForReview, {
        onSuccess: () => {
            toast.success("OAuth app submitted for review. We'll be in touch shortly.")
    ...
    return (
        <Modal showModal={showSubmitOAuthAppModal} setShowModal={setShowSubmitOAuthAppModal}>
            <div className='flex flex-col items-center justify-center space-y-3 border-b border-gray-200 px-4 py-4 pt-8 sm:px-16'>
                {oAuthApp.logo ? (
                    <BlurImage src={oAuthApp.logo} alt={oAuthApp.name} className='h-10 w-10 rounded-full' width={20} height={20} />
                ) : (
                    <Logo />
                )}

                <h3 className='text-lg font-medium'>Submit {oAuthApp.name} for review</h3>

                <p className='text-center text-sm text-gray-500'>
                    Please provide any additional information or comments for us to review your app.
                </p>
            </div>
...
export function useSubmitOAuthAppModal({ oAuthApp }: { oAuthApp: Pick<OAuthAppProps, 'id' | 'name' | 'logo' | 'slug'> | undefined }) {
