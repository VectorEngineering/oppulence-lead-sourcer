...
import { Scope, getScopesByResourceForRole, scopePresets } from '@/lib/api/tokens/scopes'
import {
...
function AddEditTokenModal({
    ...
    const buttonDisabled = (!name || token?.name === name) && token?.scopes === scopes
    ...
    return (
        <>
            ...
            >
                ...
                <form onSubmit={onSubmit} className='flex flex-col space-y-4 bg-neutral-50 px-4 py-8 text-left sm:px-10'>
                    <div>
                        <label htmlFor='name'>
                    ...
                    {!token && (
                        <div>
                            ...
                            <RadioGroup
                                ...
                                defaultValue='user'
                                required
                                onValueChange={(value) => setData({ ...data, isMachine: value === 'machine' })}
                            >
                                ...
                                >
                                    <RadioGroupItem value='machine' id='machine' className='ml-3' disabled={!isOwner} />
                                    <Label
                                        htmlFor='machine'
                                        className={cn('flex flex-1 cursor-pointer items-center justify-between space-x-1 p-3 pl-0', {
                        ...
                        {preset === 'restricted' && (
                            <div className='flex flex-col divide-y text-sm'>
                                {scopesByResources.map((resource) => (
                                    <div className='flex items-center justify-between py-4' key={resource.key}>
                                        <div className='flex items-center gap-1.5'>
                                            <span className='text-sm font-medium text-neutral-800'>{resource.name}</span>
                                            ...
                                            >
                                                ...
                                                {resource.scopes.map((scope) => (
                                                    <div className='flex items-center space-x-2' key={scope.scope}>
                                                        ...
                                                        <div className='text-sm font-normal capitalize text-neutral-800'>{scope.type}</div>
                                                    </div>
                                                ))}
                                            </RadioGroup>
                                        </div>
                    ...
                    <Button text={token ? 'Save changes' : 'Create API key'} disabled={buttonDisabled} loading={saving} />
                </form>
            </Modal>
        </>
    )
...
export function useAddEditTokenModal({
    ...
    const AddTokenButtonCallback = useCallback(() => {
        return <AddTokenButton setShowAddEditTokenModal={setShowAddEditTokenModal} />
    }, [setShowAddEditTokenModal])
    ...
    return useMemo(
        ...
        [setShowAddEditTokenModal, AddEditTokenModalCallback, AddTokenButtonCallback]
    )
}
