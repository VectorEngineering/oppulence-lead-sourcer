...
import { Dispatch, PropsWithChildren, SetStateAction, useCallback, useId, useMemo, useRef, useState } from 'react'
...
const DEFAULT_COLORS = ['#000000', '#C73E33', '#DF6547', '#F4B3D7', '#F6CF54', '#49A065', '#2146B7', '#AE49BF']
...
function LinkQRModal(
    ...
    return (
        <Modal showModal={props.showLinkQRModal} setShowModal={props.setShowLinkQRModal} className='max-w-[500px]'>
...
function LinkQRModalInner({
    ...
    const url = useMemo(() => {
        return props.key && props.domain ? linkConstructor({ key: props.key, domain: props.domain }) : undefined
    }, [props.key, props.domain])

    const [dataPersisted, setDataPersisted] = useLocalStorage<QRCodeDesign>(`qr-code-design-${workspaceId}`, {
        fgColor: '#000000',
        hideLogo: false
    })
    ...
    const logo = plan === 'free' ? DUB_QR_LOGO : domainLogo || workspaceLogo || DUB_QR_LOGO
    ...
    const onColorChange = useDebouncedCallback((color: string) => setData((d) => ({ ...d, fgColor: color })), 500)
        ...
        >
                    ...
                    <Tooltip
                        content={
                            <div className='px-2 py-1 text-xs text-gray-700'>
                                Press <strong className='font-medium text-gray-950'>Q</strong> to open this quickly
                            </div>
                        }
                        side='right'
                    >
            ...
            <div>
                ...
                <div className='relative mt-2 flex h-44 items-center justify-center overflow-hidden rounded-md border border-gray-300'>
                    {!isMobile && <ShimmerDots className='opacity-30 [mask-image:radial-gradient(40%_80%,transparent_50%,black)]' />}
                    {url && (
                        <AnimatePresence mode='wait'>
                            ...
                            >
                                <QRCode url={url} fgColor={data.fgColor} hideLogo={data.hideLogo} logo={logo} scale={1} />
                            </motion.div>
                        </AnimatePresence>
                    )}
                </div>
            ...
            <div className='flex items-center justify-between'>
                <div className='flex items-center gap-2'>
                    <label className='text-sm font-medium text-gray-700' htmlFor={`${id}-show-logo`}>
                        Logo
                    </label>
                    <InfoTooltip
                        content={
            ...
            {/* Color selector */}
            <div>
                <span className='block text-sm font-medium text-gray-700'>QR Code Color</span>
                        ...
                        <Tooltip
                            content={
                                <div className='flex max-w-xs flex-col items-center space-y-3 p-5 text-center'>
                                    <HexColorPicker color={data.fgColor} onChange={onColorChange} />
                                </div>
                            }
                        >
                            <div
                            ...
                            return (
                                <button
                                    ...
                                    className={cn(
                                        ...
                                        isSelected ? 'ring-1 ring-black ring-offset-[3px]' : 'ring-black/10 hover:ring-4'
                                    )}
            ...
            <div className='flex items-center justify-end gap-2'>
                ...
                />
                <Button type='submit' variant='primary' text='Save changes' className='h-9 w-fit' />
            </div>
        </form>
    )
}
            ...
            <Popover
                content={
                    <div className='grid p-1 sm:min-w-48'>
                        ...
                        >
                            <IconMenu text='Download SVG' icon={<Photo className='h-4 w-4' />} />
                        </button>
                        <button
                            type='button'
                            onClick={async () => {
                            ...
                            className='rounded-md p-2 text-left text-sm font-medium text-gray-500 transition-all duration-75 hover:bg-gray-100'
                        >
                            <IconMenu text='Download PNG' icon={<Photo className='h-4 w-4' />} />
                        </button>
                        <button
                            type='button'
                            onClick={async () => {
                            ...
                            className='rounded-md p-2 text-left text-sm font-medium text-gray-500 transition-all duration-75 hover:bg-gray-100'
                        >
                            <IconMenu text='Download JPEG' icon={<Photo className='h-4 w-4' />} />
                        </button>
                    </div>
                }
            ...
            content={
                <div className='grid p-1 sm:min-w-48'>
                    <button
                        ...
                        className='rounded-md p-2 text-left text-sm font-medium text-gray-500 transition-all duration-75 hover:bg-gray-100'
                    >
                        <IconMenu text='Copy Image' icon={copiedImage ? <Check className='h-4 w-4' /> : <Photo className='h-4 w-4' />} />
                    </button>
                    <button
                        type='button'
                        onClick={() => {
                        ...
                        className='rounded-md p-2 text-left text-sm font-medium text-gray-500 transition-all duration-75 hover:bg-gray-100'
                    >
                        <IconMenu text='Copy URL' icon={copiedURL ? <Check className='h-4 w-4' /> : <Hyperlink className='h-4 w-4' />} />
                    </button>
                </div>
            }
            openPopover={openPopover}
...
export function useLinkQRModal(props: LinkQRModalProps) {
    ...
    const LinkQRModalCallback = useCallback(() => {
        return <LinkQRModal showLinkQRModal={showLinkQRModal} setShowLinkQRModal={setShowLinkQRModal} {...props} />
