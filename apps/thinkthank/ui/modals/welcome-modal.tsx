...
function WelcomeModal({
    ...
    const plan = planId ? (PLANS.find((p) => p.name.toLowerCase() === planId.replace('+', ' ').toLowerCase()) ?? PRO_PLAN) : undefined
            ...
            <div className='flex flex-col'>
                ...
                <div className='px-6 py-8 sm:px-12'>
                    <div className='relative'>
                        ...
                        >
                            <h1 className={cn('text-lg font-medium text-gray-950', plan ? 'text-left' : 'text-center')}>
                                {plan ? `${platform.company} ${plan.name} looks good on you!` : `Welcome to ${platform.company}!`}
                            </h1>
                            <p className={cn('mt-2 text-sm text-gray-500', plan ? 'text-left' : 'text-center')}>
                                {upgraded
...
export function useWelcomeModal() {
    ...
    const WelcomeModalCallback = useCallback(() => {
        return <WelcomeModal showWelcomeModal={showWelcomeModal} setShowWelcomeModal={setShowWelcomeModal} />
