...
function AddEditUtmTemplateModal({
    ...
    const {
        ...
        watch
    } = useForm<Pick<UtmTemplateProps, 'name' | 'utm_campaign' | 'utm_content' | 'utm_medium' | 'utm_source' | 'utm_term' | 'ref'>>({
        values: props
    })
    ...
    return (
        <Modal showModal={showAddEditUtmTemplateModal} setShowModal={setShowAddEditUtmTemplateModal}>
            <form
                onSubmit={handleSubmit(async (data) => {
                    try {
                        ...
                        posthog.capture(props ? 'utm-template_edited' : 'utm-template_created', {
                            utmTemplateId: id,
            ...
            >
                <div className='flex items-center gap-2'>
                    <h3 className='text-lg font-medium'>{props ? 'Edit UTM Template' : 'Create UTM Template'}</h3>
                </div>
                <div className='mt-6'>
                    <label htmlFor='name'>
                ...
                <div className='mt-6'>
                    <span className='mb-2 block text-sm font-medium text-gray-700'>Parameters</span>
                    <UTMBuilder
                        values={values}
                        onChange={(key, value) => {
...
function AddUtmTemplateButton({ setShowAddEditUtmTemplateModal }: { setShowAddEditUtmTemplateModal: Dispatch<SetStateAction<boolean>> }) {
    return (
        <div>
            <Button
                variant='primary'
...
export function useAddEditUtmTemplateModal({ props }: { props?: UtmTemplateProps } = {}) {
    const [showAddEditUtmTemplateModal, setShowAddEditUtmTemplateModal] = useState(false)
    ...
    const AddUtmTemplateButtonCallback = useCallback(() => {
        return <AddUtmTemplateButton setShowAddEditUtmTemplateModal={setShowAddEditUtmTemplateModal} />
    }, [setShowAddEditUtmTemplateModal])
    ...
    return useMemo(
        ...
        [setShowAddEditUtmTemplateModal, AddEditUtmTemplateModalCallback, AddUtmTemplateButtonCallback]
    )
}
