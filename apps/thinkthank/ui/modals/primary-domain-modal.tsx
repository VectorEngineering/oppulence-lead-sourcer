...
function PrimaryDomainModal({
    ...
    const setPrimary = async () => {
        setLoading(true)
        const response = await fetch(`/api/domains/${domain}/primary?workspaceId=${workspaceId}`, {
            method: 'POST'
        })
        if (response.ok) {
    ...
    return (
        <Modal showModal={showPrimaryDomainModal} setShowModal={setShowPrimaryDomainModal}>
            <div className='flex flex-col items-center justify-center space-y-3 border-b border-gray-200 px-4 py-4 pt-8 text-center sm:px-16'>
                ...
                <p className='text-sm text-gray-500'>
                    Setting this domain as primary will make it the default domain in the link creation modal, as well as in the API.{' '}
                    <a
