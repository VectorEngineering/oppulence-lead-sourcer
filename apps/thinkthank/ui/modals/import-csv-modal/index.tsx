...
import { AnimatedSizeContainer, Button, Logo, Modal, useRouterStuff } from '@dub/ui'
...
function ImportCsvModal({
    ...
    useEffect(() => setShowImportCsvModal(searchParams?.get('import') === 'csv'), [searchParams])
        ...
        >
            <div className='flex flex-col items-center justify-center space-y-3 border-b border-gray-200 px-4 py-8 sm:px-16'>
                ...
                <p className='text-balance text-center text-sm text-gray-500'>
                    Easily import all your links into {platform.company} with just a few clicks.
                </p>
            </div>
                        ...
                        <div className='flex flex-col gap-y-6 bg-gray-50 px-4 py-8 text-left sm:px-12'>
                            <form
                                onSubmit={handleSubmit(async (data) => {
                                    const loadingId = toast.loading('Adding links to import queue...')
                                    try {
                                        // Get signed upload URL
                                        const uploadRes = await fetch(`/api/workspaces/${workspaceId}/import/csv/upload-url`, {
                                            method: 'POST'
                                        })
                                        ...
                                        const res = await fetch(`/api/workspaces/${workspaceId}/import/csv`, {
                                            method: 'POST',
                                            body: formData
                                        })
                                        ...
                                        router.push(`/${slug}`)
                                        await Promise.all([mutatePrefix('/api/links'), mutate(`/api/workspaces/${slug}`)])
                            ...
                            >
                                ...
                                {page === 'confirm-import' && (
                                    <>
                                        <FieldMapping />
                                        <Button text='Confirm import' loading={isSubmitting} disabled={!isValid} />
                                        <button
                                            type='button'
...
export function useImportCsvModal() {
    ...
    const ImportCsvModalCallback = useCallback(() => {
        return <ImportCsvModal showImportCsvModal={showImportCsvModal} setShowImportCsvModal={setShowImportCsvModal} />
