...
import { ArrowRight, Check, LoadingSpinner, TableIcon, Xmark } from '@dub/ui/icons'
import { cn, formatDate, getPrettyUrl, parseDateTime, truncate } from '@dub/utils'
                ...
                for await (const partialObject of readStreamableValue(object)) {
                    if (partialObject) {
                        Object.entries(partialObject).forEach((entry) => {
                            ...
                            if (Object.keys(mappableFields).includes(field) && fileColumns.includes(value)) {
...
function FieldRow({ field, isStreaming }: { field: keyof typeof mappableFields; isStreaming: boolean }) {
                        ...
                        <Popover
                            ...
                            content={
                                <div className='w-full p-2 md:w-48'>
                                    {[...(fileColumns || []), ...(field.value && !required ? ['None'] : [])]?.map((column) => {
                                        ...
                                        return (
                                            ...
                                            >
                                                <IconMenu text={column} icon={<Icon className='size-4 flex-none' />} />
                                                {field.value === column && <Check className='size-4 shrink-0' />}
                                            </button>
                                        )
                                    })}
                                </div>
                        ...
                        <div className='block px-4 py-3 text-sm'>
                            ...
                            <ul className='mt-0.5'>
                                {examples?.map((example, idx) => (
                                    <li key={example + idx} className='block text-xs leading-tight text-gray-500'>
                                        <span className='translate-y-1 text-base text-gray-600'>&bull;</span> {example}
                                    </li>
                                ))}
                            </ul>
                        </div>
