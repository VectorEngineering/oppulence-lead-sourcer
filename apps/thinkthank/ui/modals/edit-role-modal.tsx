        ...
        <Modal showModal={showEditRoleModal} setShowModal={setShowEditRoleModal}>
            <div className='flex flex-col items-center justify-center space-y-3 border-b border-gray-200 px-4 py-4 pt-8 sm:px-16'>
                ...
                <p className='text-center text-sm text-gray-500'>
                    This will change <b className='text-gray-800'>{name || email}</b>'s role in{' '}
                    <b className='text-gray-800'>{workspaceName}</b> to <b className='text-gray-800'>{role}</b>. Are you sure you want to
                    continue?
                </p>
            </div>
                    ...
                    onClick={() => {
                        ...
                        fetch(`/api/workspaces/${id}/users`, {
                            ...
                            if (res.status === 200) {
                                ...
                                toast.success(`Successfully changed ${name || email}'s role to ${role}.`)
                            } else {
...
export function useEditRoleModal({ user, role }: { user: UserProps; role: 'owner' | 'member' }) {
    const [showEditRoleModal, setShowEditRoleModal] = useState(false)

    const EditRoleModalCallback = useCallback(() => {
        return <EditRoleModal showEditRoleModal={showEditRoleModal} setShowEditRoleModal={setShowEditRoleModal} user={user} role={role} />
