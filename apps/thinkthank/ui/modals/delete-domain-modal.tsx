...
function DeleteDomainModal({
    ...
    return (
        <Modal showModal={showDeleteDomainModal} setShowModal={setShowDeleteDomainModal}>
            <div className='flex flex-col items-center justify-center space-y-3 border-b border-gray-200 px-4 py-4 pt-8 text-center sm:px-16'>
                <LinkLogo apexDomain={domain} />
                <h3 className='text-lg font-medium'>Delete {domain}</h3>
                <div className='space-y-2 text-sm text-gray-500'>
                    <p>Deleting this domain will delete all associated links as well as their anaytics, permanently.</p>
                    {Boolean(props.registeredDomain) && <p>The domain will also be provisioned back to Dub.</p>}
                    <p>
                        <strong className='font-semibold text-gray-700'>This action can't be undone</strong> – proceed with caution.
                    </p>
                </div>
            </div>
            ...
            >
                <div>
                    <label htmlFor='verification' className='block text-sm text-gray-700'>
                        To verify, type <span className='font-semibold'>confirm delete {domain}</span> below
                    </label>
