...
function RemoveOAuthAppModal({
    ...
    oAuthApp: Pick<OAuthAppProps, 'id' | 'name' | 'description' | 'logo' | 'installations'> | undefined
}) {
    ...
    const deleteOAuthApp = async () => {
        ...
        const response = await fetch(`/api/oauth/apps/${oAuthApp?.id}?workspaceId=${workspaceId}`, {
            method: 'DELETE',
    ...
    return (
        <Modal showModal={showRemoveOAuthAppModal} setShowModal={setShowRemoveOAuthAppModal}>
            <div className='flex flex-col items-center justify-center space-y-3 border-b border-gray-200 px-4 py-4 pt-8 sm:px-16'>
                ...
                <p className='text-center text-sm text-gray-500'>
                    Deleting this application will invalidate any access tokens authorized by users. Are you sure you want to continue?
                </p>
            </div>
            ...
            >
                <div>
                    <label htmlFor='verification' className='block text-sm text-gray-700'>
                        To verify, type <span className='font-semibold text-black'>{oAuthApp.name}</span> below
                    </label>
                ...
                <Button text='Confirm delete' variant='danger' loading={deleting} autoFocus={!isMobile} type='submit' />
            </form>
        </Modal>
    )
}
