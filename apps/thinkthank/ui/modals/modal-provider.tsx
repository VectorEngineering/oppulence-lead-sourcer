...
import { Dispatch, ReactNode, SetStateAction, Suspense, createContext, useEffect, useMemo } from 'react'
...
function ModalProviderClient({ children }: { children: ReactNode }) {
    ...
    const { setShowImportRebrandlyModal, ImportRebrandlyModal } = useImportRebrandlyModal()
    const { setShowImportCsvModal, ImportCsvModal } = useImportCsvModal()
    const { setShowWelcomeModal, WelcomeModal } = useWelcomeModal()

    useEffect(() => setShowWelcomeModal(searchParams.has('onboarded') || searchParams.has('upgraded')), [searchParams])

    const [hashes, setHashes] = useCookies<SimpleLinkProps[]>('hashes__dub', [], {
        domain: !!process.env.NEXT_PUBLIC_VERCEL_URL ? `.${platform.domain}` : undefined
    })
    ...
    useEffect(() => {
        if (workspaces && workspaces.length > 0 && session?.user && !session.user['defaultWorkspace']) {
            fetch('/api/user', {
                method: 'PATCH',
                headers: {
                    'Content-Type': 'application/json'
