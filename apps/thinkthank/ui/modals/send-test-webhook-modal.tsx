...
import { Button, InputSelect, InputSelectItemProps, Logo, Modal } from '@dub/ui'
...
function SendTestWebhookModal({
    ...
    const [selectedTrigger, setSelectedTrigger] = useState<InputSelectItemProps | null>(null)
    ...
    const triggers = Object.entries(WEBHOOK_TRIGGER_DESCRIPTIONS).map(([key, value]) => ({
        id: key,
        value: value
    }))

    return (
        <Modal showModal={showSendTestWebhookModal} setShowModal={setShowSendTestWebhookModal}>
            <div className='flex flex-col items-center justify-center space-y-3 border-b border-gray-200 px-4 py-4 pt-8 text-center sm:px-16'>
                <Logo />
                <h3 className='text-lg font-medium'>Send test webhook event</h3>
                <p className='text-center text-sm text-gray-500'>Choose a webhook event to send to your receiver endpoint</p>
            </div>
            <form
                onSubmit={async (e) => {
                    e.preventDefault()
            ...
            >
                <div className='flex flex-col space-y-28 bg-gray-50 px-4 py-8 text-left sm:space-y-3 sm:rounded-b-2xl sm:px-16'>
                    ...
                    />
                    <Button disabled={!selectedTrigger} text='Send test webhook' loading={isExecuting} />
                </div>
            </form>
        </Modal>
    )
...
export function useSendTestWebhookModal({ webhook }: { webhook: WebhookProps | undefined }) {
    const [showSendTestWebhookModal, setShowSendTestWebhookModal] = useState(false)
