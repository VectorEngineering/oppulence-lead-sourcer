...
import { Button, Checkbox, DateRangePicker, InfoTooltip, Logo, Modal, Switch, useRouterStuff } from '@dub/ui'
...
function ExportLinksModal({
    ...
    return (
        <Modal showModal={showExportLinksModal} setShowModal={setShowExportLinksModal}>
            <div className='flex flex-col items-center justify-center space-y-3 border-b border-gray-200 px-4 py-4 pt-8 sm:px-16'>
                <Logo />
                <div className='flex flex-col space-y-1 text-center'>
                    <h3 className='text-lg font-medium'>Export links</h3>
                    <p className='text-sm text-gray-500'>Export this workspace's links to a CSV file</p>
                </div>
            </div>

            <form onSubmit={onSubmit} className='flex flex-col gap-6 bg-gray-50 px-4 py-8 text-left sm:rounded-b-2xl sm:px-16'>
                <Controller
                    name='dateRange'
                    control={control}
                    render={({ field }) => (
                        <div className='flex flex-col gap-2'>
                            <label htmlFor={dateRangePickerId} className='block text-sm font-medium text-gray-700'>
                                Date Range
                            </label>
                            <DateRangePicker
...
export function useExportLinksModal() {
    ...
    const ExportLinksModalCallback = useCallback(() => {
        return <ExportLinksModal showExportLinksModal={showExportLinksModal} setShowExportLinksModal={setShowExportLinksModal} />
