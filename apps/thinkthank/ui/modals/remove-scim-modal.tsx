...
function RemoveSCIMModal({
    ...
    const currentProvider = useMemo(() => SAML_PROVIDERS.find((p) => p.scim === provider), [provider]) as SAMLProviderProps

    return (
        <Modal showModal={showRemoveSCIMModal} setShowModal={setShowRemoveSCIMModal}>
            <div className='flex flex-col items-center justify-center space-y-3 border-b border-gray-200 px-4 py-4 pt-8 sm:px-16'>
                {logo ? <BlurImage src={logo} alt='Workspace logo' className='h-10 w-10 rounded-full' width={20} height={20} /> : <Logo />}
                <h3 className='text-lg font-medium'>Remove SCIM Directory</h3>
                <p className='text-center text-sm text-gray-500'>
                    This will remove the currently configured SCIM directory from your workspace. Are you sure you want to continue?
                </p>
            </div>
            ...
            <div className='flex flex-col space-y-4 bg-gray-50 px-4 py-8 text-left sm:px-16'>
                <div className='flex items-center space-x-3 rounded-md border border-gray-300 bg-white p-3'>
                    <img src={currentProvider.logo} alt={currentProvider.name + ' logo'} className='h-8 w-8' />
                    <div className='flex flex-col'>
                        <h3 className='text-sm font-medium'>{currentProvider.name} SCIM</h3>
                        <p className='text-xs text-gray-500'>{currentProvider.name} SCIM is configured</p>
                    </div>
                </div>
                <Button
                    text='Confirm remove'
...
export function useRemoveSCIMModal() {
    ...
    const RemoveSCIMModalCallback = useCallback(() => {
        return <RemoveSCIMModal showRemoveSCIMModal={showRemoveSCIMModal} setShowRemoveSCIMModal={setShowRemoveSCIMModal} />
