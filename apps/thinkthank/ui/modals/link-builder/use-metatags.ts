...
export function useMetatags({ initial, enabled }: { initial: boolean; enabled: boolean }) {
    const { watch, setValue } = useFormContext<LinkFormData>()
    const [url, password, proxy, title, description, image] = watch(['url', 'password', 'proxy', 'title', 'description', 'image'])
    ...
    useEffect(() => {
        ...
        if (password) {
            ...
            setValue('description', 'This link is password protected. Please enter the password to view it.')
            ...
            try {
                ...
                fetch(`/api/metatags?url=${debouncedUrl}`).then(async (res) => {
                    if (res.status === 200) {
                        ...
                        if (description !== truncatedDescription) setValue('description', truncatedDescription)
