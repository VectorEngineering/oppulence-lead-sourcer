import { Button, InfoTooltip, Modal, SimpleTooltipContent, Tooltip, useKeyboardShortcut } from '@dub/ui'
...
function AdvancedModal({
    ...
    const [externalIdParent, tenantIdParent] = watchParent(['externalId', 'tenantId'])
    ...
    return (
        <Modal showModal={showAdvancedModal} setShowModal={setShowAdvancedModal} className='sm:max-w-[500px]'>
            <form
                className='px-5 py-4'
                onSubmit={(e) => {
                    e.stopPropagation()
                        ...
                        <Tooltip
                            content={
                                <div className='px-2 py-1 text-xs text-gray-700'>
                                    Press <strong className='font-medium text-gray-950'>A</strong> to open this quickly
                                </div>
                            }
                            side='right'
                        >
                ...
                <div className='mt-6 flex flex-col gap-6'>
                    ...
                    <div>
                        <div className='flex items-center gap-2'>
                            <label htmlFor={`${id}-external-id`} className='flex items-center gap-2 text-sm font-medium text-gray-700'>
                                External ID{' '}
                                <InfoTooltip
                                    content={
                    ...
                    <div>
                        <div className='flex items-center gap-2'>
                            <label htmlFor={`${id}-tenant-id`} className='flex items-center gap-2 text-sm font-medium text-gray-700'>
                                Tenant ID{' '}
...
export function useAdvancedModal() {
    ...
    const AdvancedModalCallback = useCallback(() => {
        return <AdvancedModal showAdvancedModal={showAdvancedModal} setShowAdvancedModal={setShowAdvancedModal} />
