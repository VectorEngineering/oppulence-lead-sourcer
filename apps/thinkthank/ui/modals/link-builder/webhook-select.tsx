...
export function WebhookSelect() {
    ...
    const selectedWebhooks = useMemo(
        () => webhookIds.map((id) => options?.find(({ value }) => value === id)!).filter(Boolean),
    ...
    return (
        <Combobox
            ...
            options={options}
            icon={<Webhook className={cn('size-3.5 flex-none', hasSelectedWebhooks && 'text-blue-500')} />}
            side='top'
            placeholder='Webhooks'
            searchPlaceholder='Search webhooks...'
            shortcutHint='W'
            buttonProps={{
                className: 'h-9 px-2.5 w-fit font-medium text-gray-700 max-w-48 min-w-0'
            }}
            open={isOpen}
...
const NoWebhooksFound = () => {
    ...
    return (
        <div className='flex h-full flex-col items-center justify-center gap-2 px-2 py-4 text-center text-sm'>
            ...
            <div>
                <Button className='mt-1 h-8' onClick={() => window.open(`/${slug}/settings/webhooks`, '_blank')} text='Add webhook' />
            </div>
        </div>
    )
}
