...
export function OptionsList() {
    ...
    const enabledToggles = useMemo(() => TOGGLES.filter(({ key }) => data[key]), [data])

    const enabledItems = useMemo(
        () => [
            ...enabledToggles,
            ...(isMobile
                ? MOBILE_MORE_ITEMS.filter(({ enabled }) => enabled(data)).map((item) => ({ ...item, label: item.badgeLabel(data) }))
                : [])
        ],
        ...
        <div className='flex flex-wrap gap-2'>
            <AnimatePresence>
                {enabledItems.map((item) => {
                    const Component = item.key === 'rewrite' ? LinkCloakingToggleBadge : ToggleBadge
                    return (
                        <motion.div key={item.key} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.1 }}>
                            <Component
                                toggle={item}
                                {...(item.type === 'modal' &&
                                    'enabled' in item &&
                                ...
                                onRemove={() =>
                                    'remove' in item ? item.remove(setValue) : setValue(item.key as any, false, { shouldDirty: true })
                                }
                            />
                        </motion.div>
                    )
...
function ToggleBadge({ toggle, onRemove, icon }: { toggle: (typeof TOGGLES)[number]; onRemove: () => void; icon?: ReactNode }) {
    return (
        <span className='group flex cursor-default items-center gap-1.5 rounded-md border border-gray-200 bg-gray-50 pl-1.5 text-xs text-gray-600'>
            {icon}
            {toggle.label}
            <button type='button' onClick={onRemove} className='-ml-1 p-1 text-gray-400 hover:text-gray-500'>
...
function LinkCloakingToggleBadge({ toggle, onRemove }: { toggle: (typeof TOGGLES)[number]; onRemove: () => void }) {
    const { watch } = useFormContext<LinkFormData>()
    const [url, domain] = watch(['url', 'domain'])
    const [debouncedUrl] = useDebounce(url, 500)
    const isValidUrl = useMemo(() => debouncedUrl && isValidUrlFn(debouncedUrl), [debouncedUrl])

    const { data, isLoading } = useSWR<{ iframeable: boolean }>(
        domain && isValidUrl ? `/api/links/iframeable?domain=${domain}&url=${debouncedUrl}` : null,
        fetcher
    )
