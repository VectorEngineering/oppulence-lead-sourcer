import { Button, InfoTooltip, Modal, SimpleTooltipContent, Tooltip, useKeyboardShortcut, useMediaQuery } from '@dub/ui'
import { cn, formatDateTime, getDateTimeLocal, parseDateTime } from '@dub/utils'
...
function ExpirationModal({
    ...
    return (
        <Modal showModal={showExpirationModal} setShowModal={setShowExpirationModal} className='sm:max-w-md'>
            <form
                className='px-5 py-4'
                onSubmit={(e) => {
                    e.stopPropagation()
            ...
            >
                        ...
                        <Tooltip
                            content={
                                <div className='px-2 py-1 text-xs text-gray-700'>
                                    Press <strong className='font-medium text-gray-950'>E</strong> to open this quickly
                                </div>
                            }
                            side='right'
                        >
                ...
                <div className='mt-6'>
                    <div className='flex items-center gap-2'>
                        <label htmlFor={`${id}-expiresAt`} className='block text-sm font-medium text-gray-700'>
                            Date and Time
                        </label>
                    </div>
                ...
                <div className='mt-6'>
                    <div className='flex items-center gap-2'>
                        <label htmlFor={`${id}-expiredUrl`} className='block text-sm font-medium text-gray-700'>
                            Expiration URL
                        </label>
                        <InfoTooltip
                            content={
...
export function getExpirationLabel({ expiresAt }: Pick<LinkFormData, 'expiresAt'>) {
    return expiresAt ? formatDateTime(expiresAt, { year: undefined }) : 'Expiration'
}

function ExpirationButton({ setShowExpirationModal }: { setShowExpirationModal: Dispatch<SetStateAction<boolean>> }) {
    ...
    return (
        <Button
            ...
            icon={<CircleHalfDottedClock className={cn('size-4', expiresAt && 'text-blue-500')} />}
...
export function useExpirationModal() {
    ...
    const ExpirationModalCallback = useCallback(() => {
        return <ExpirationModal showExpirationModal={showExpirationModal} setShowExpirationModal={setShowExpirationModal} />
