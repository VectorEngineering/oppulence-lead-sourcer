import { Button, Combobox, Modal, SimpleTooltipContent, Tooltip, UTM_PARAMETERS, useKeyboardShortcut } from '@dub/ui'
import { Crosshairs3, Trash } from '@dub/ui/icons'
import { COUNTRIES, cn, constructURLFromUTMParams, getParamsFromURL, isValidUrl, pluralize } from '@dub/utils'
import { Dispatch, Fragment, SetStateAction, useCallback, useId, useMemo, useState } from 'react'
...
function TargetingModal({
    ...
    const { watch: watchParent, getValues: getValuesParent, setValue: setValueParent } = useFormContext<LinkFormData>()
    ...
    const [iosParent, androidParent, geoParent] = watchParent(['ios', 'android', 'geo'])

    const parentEnabled = Boolean(iosParent || androidParent || Object.keys(geoParent || {}).length > 0)
    ...
    const getNewParams = useCallback(
        (targetURL: string) => {
            ...
            const newParams = UTM_PARAMETERS.filter(({ key }) => parentParams?.[key] && !targetParams?.[key]).map(({ key }) => [
                key,
                parentParams[key]
            ])
    ...
    return (
        <>
            <Modal showModal={showTargetingModal} setShowModal={setShowTargetingModal} className='sm:max-w-[500px]'>
                <form
                    className='px-5 py-4'
                    onSubmit={(e) => {
                        e.stopPropagation()
                        handleSubmit((data) => {
                            ...
                            const geo = Object.fromEntries(
                                Object.entries(data.geo || {}).filter(([key, value]) => key?.trim() && value?.trim())
                            )
                            ...
                            <Tooltip
                                content={
                                    <div className='px-2 py-1 text-xs text-gray-700'>
                                        Press <strong className='font-medium text-gray-950'>G</strong> to open this quickly
                                    </div>
                                }
                                side='right'
                            >
                    ...
                    <div className='scrollbar-hide -m-1 mt-6 flex max-h-[calc(100dvh-250px)] flex-col gap-6 overflow-y-auto p-1'>
                        ...
                        <div>
                            <div className='flex items-center gap-2'>
                                <span className='block text-sm font-medium text-gray-700'>Geo Targeting</span>
                                <ProBadgeTooltip
                                    content={
                            ...
                            <div className='mt-2'>
                                                ...
                                                <div className='z-[1]'>
                                                    <Combobox
                                                        ...
                                                        options={Object.entries(COUNTRIES)
                                                            .filter(([ck]) => ck === key || !Object.keys(geo).includes(ck))
                                                            .map(([key, value]) => ({
                                                                icon: (
                                                                    <img
                                                                        alt={value}
                                ...
                                <Button
                                    ...
                                    className='h-9'
                                    onClick={() => {
                                        setValue('geo', { ...((geo as object) || {}), '': '' }, { shouldDirty: true })
                                    }}
                        ...
                        <div>
                            <div className='flex items-center gap-2'>
                                <label htmlFor={`${id}-ios-url`} className='block text-sm font-medium text-gray-700'>
                                    iOS Targeting
                                </label>
                                <ProBadgeTooltip
                                    content={
                                ...
                                <input
                                    ...
                                    {...register('ios', {
                                        onBlur: (e) => {
                                            ...
                                            if (newParams)
                                                setValue('ios', constructURLFromUTMParams(e.target.value, newParams), { shouldDirty: true })
                                        }
                                    })}
                                />
                            </div>
                        ...
                        <div>
                            <div className='flex items-center gap-2'>
                                <label htmlFor={`${id}-android-url`} className='block text-sm font-medium text-gray-700'>
                                    Android Targeting
                                </label>
                                <ProBadgeTooltip
                                    content={
                                ...
                                <input
                                    ...
                                    {...register('android', {
                                        onBlur: (e) => {
                                            ...
                                            if (newParams)
                                                setValue('android', constructURLFromUTMParams(e.target.value, newParams), {
                                                    shouldDirty: true
                                                })
                                        }
                                    })}
...
export function getTargetingLabel({ ios, android, geo }: Pick<LinkFormData, 'ios' | 'android' | 'geo'>) {
...
function TargetingButton({ setShowTargetingModal }: { setShowTargetingModal: Dispatch<SetStateAction<boolean>> }) {
    ...
    const label = useMemo(() => getTargetingLabel({ ios, android, geo }), [ios, android, geo])

    return (
        <Button
            variant='secondary'
            text={label}
            icon={<Crosshairs3 className={cn('size-4', enabled && 'text-blue-500')} />}
...
export function useTargetingModal() {
    ...
    const TargetingModalCallback = useCallback(() => {
        return <TargetingModal showTargetingModal={showTargetingModal} setShowTargetingModal={setShowTargetingModal} />
