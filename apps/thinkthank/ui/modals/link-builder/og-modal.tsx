import { Button, ButtonTooltip, FileUpload, Modal, Popover, SimpleTooltipContent, Tooltip } from '@dub/ui'
...
function OGModal({ showOGModal, setShowOGModal }: { showOGModal: boolean; setShowOGModal: Dispatch<SetStateAction<boolean>> }) {
    // Rerender the modal when the showOGModal state changes to reset the form's values
    return showOGModal ? <OGModalInner showOGModal={showOGModal} setShowOGModal={setShowOGModal} /> : null
}

function OGModalInner({ showOGModal, setShowOGModal }: { showOGModal: boolean; setShowOGModal: Dispatch<SetStateAction<boolean>> }) {
    const { id: workspaceId, mutate, exceededAI } = useWorkspace()

    const { generatingMetatags } = useContext(LinkModalContext)
    const { getValues: getValuesParent, watch: watchParent, setValue: setValueParent } = useFormContext<LinkFormData>()
    ...
    return (
        <>
            <PromptModal />
            <Modal showModal={showOGModal} setShowModal={setShowOGModal} className='sm:max-w-[500px]'>
                <form
                    className='px-5 py-4'
                    onSubmit={(e) => {
                        e.stopPropagation()
                        handleSubmit(async (data) => {
                            ...
                            ;(['image', 'title', 'description', 'proxy'] as const).forEach((key) =>
                                setValueParent(key, data[key], { shouldDirty: true })
                            )
                        })(e)
                    }}
                >
                            ...
                            <Tooltip
                                content={
                                    <div className='px-2 py-1 text-xs text-gray-700'>
                                        Press <strong className='font-medium text-gray-950'>L</strong> to open this quickly
                                    </div>
                                }
                                side='right'
                            >
                        ...
                        <div>
                            <div className='flex items-center justify-between'>
                                ...
                                <div className='flex items-center gap-2'>
                                    ...
                                    >
                                        {generatingTitle ? <LoadingCircle /> : <Magic className='size-4' />}
                                    </ButtonTooltip>
                                </div>
                            </div>
                        ...
                        <div>
                            <div className='flex items-center justify-between'>
                                ...
                                <div className='flex items-center gap-2'>
                                    ...
                                    >
                                        {generatingDescription ? <LoadingCircle /> : <Magic className='size-4' />}
                                    </ButtonTooltip>
                                </div>
                            </div>
                            <div className='relative mt-1 flex rounded-md shadow-sm'>
                    ...
                    <div className='mt-6 flex items-center justify-between'>
                            ...
                            {parentProxy && (
                                <button
                                    ...
                                    onClick={() => {
                                        ...
                                        ;['title', 'description', 'image'].forEach((key: 'title' | 'description' | 'image') =>
                        ...
                        <div className='flex items-center gap-2'>
                            ...
                            />
                            <Button type='submit' variant='primary' text='Save changes' className='h-9 w-fit' disabled={!isDirty} />
                        </div>
                    </div>
                </form>
            </Modal>
...
export function useOGModal() {
    ...
    const OGModalCallback = useCallback(() => {
        return <OGModal showOGModal={showOGModal} setShowOGModal={setShowOGModal} />
