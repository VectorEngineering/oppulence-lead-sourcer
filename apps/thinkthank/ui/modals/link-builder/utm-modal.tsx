...
import { cn, constructURLFromUTMParams, getParamsFromURL, isValidUrl } from '@dub/utils'
...
function UTMModal(props: UTMModalProps) {
    return (
        <Modal showModal={props.showUTMModal} setShowModal={props.setShowUTMModal} className='px-5 py-4 sm:max-w-md'>
            <UTMModalInner {...props} />
        </Modal>
    )
}
...
function UTMModalInner({ setShowUTMModal }: UTMModalProps) {
    const { getValues: getValuesParent, setValue: setValueParent } = useFormContext<LinkFormData>()

    const form = useForm<Pick<LinkFormData, 'url' | 'utm_source' | 'utm_medium' | 'utm_campaign' | 'utm_term' | 'utm_content'>>({
        values: {
    ...
    const updateTargeting = useCallback(
        (data: Pick<LinkFormData, 'url' | 'utm_source' | 'utm_medium' | 'utm_campaign' | 'utm_term' | 'utm_content'>) => {
            const [parentUrl, ios, android, geo] = getValuesParent(['url', 'ios', 'android', 'geo'])
            ...
            Object.entries({ ios, android }).forEach(([target, targetUrl]) => {
                ...
                if (newParams)
                    setValueParent(target as 'ios' | 'android', constructURLFromUTMParams(targetUrl, newParams), {
                        shouldDirty: true
                    })
            })
            ...
            if (geo && Object.keys(geo).length > 0) {
                ...
                if (Object.keys(newGeo).length > 0)
                    setValueParent('geo', { ...(geo as Record<string, string>), ...newGeo }, { shouldDirty: true })
            }
        },
        []
    )
        ...
        >
                    ...
                    <Tooltip
                        content={
                            <div className='px-2 py-1 text-xs text-gray-700'>
                                Press <strong className='font-medium text-gray-950'>U</strong> to open this quickly
                            </div>
                        }
                        side='right'
                    >
            ...
            <div className='py-4'>
                <UTMBuilder
                    ...
                    disabledTooltip={isValidUrl(url) ? undefined : 'Enter a destination URL to add UTM parameters'}
                    autoFocus
                />
            </div>
            ...
            {isValidUrl(url) && (
                <div className='mt-4 grid gap-y-1'>
                    <span className='block text-sm font-medium text-gray-700'>URL Preview</span>
                ...
                <div>
                    <FormProvider {...form}>
                        <UTMTemplatesCombo
                            ...
                            disabledTooltip={isValidUrl(url) ? undefined : 'Enter a destination URL to use UTM templates'}
                        />
                    </FormProvider>
                </div>
                <div className='flex items-center gap-2'>
                    ...
                    />
                    <Button type='submit' variant='primary' text='Save' className='h-9 w-fit' disabled={!isDirty} />
                </div>
            </div>
        </form>
    )
...
function UTMButton({ setShowUTMModal }: { setShowUTMModal: Dispatch<SetStateAction<boolean>> }) {
    const { watch } = useFormContext<LinkFormData>()
    const url = watch('url')
    const enabled = useMemo(
        () => Object.keys(getParamsFromURL(url)).some((k) => UTM_PARAMETERS.findIndex((p) => p.key === k) !== -1),
        [url]
    )
    ...
    return (
        <Button
            variant='secondary'
            text='UTM'
            icon={<DiamondTurnRight className={cn('size-4', enabled && 'text-blue-500')} />}
...
export function useUTMModal() {
    ...
    const UTMModalCallback = useCallback(() => {
        return <UTMModal showUTMModal={showUTMModal} setShowUTMModal={setShowUTMModal} />
