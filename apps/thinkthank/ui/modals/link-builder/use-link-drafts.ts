...
export function useLinkDrafts({ linkId, workspaceId }: { linkId?: string; workspaceId: string }) {
    const [drafts, setDrafts] = useLocalStorage<LinkDraft[]>(`link-drafts:${workspaceId}`, [])
    ...
    const saveDraft = (id: string, link: Partial<LinkFormData>) => {
        setDrafts([...drafts.filter((d) => d.id !== id), { id, link, timestamp: new Date().getTime() }])
    }
    ...
    const filteredDrafts = useMemo(
        () => (linkId ? drafts.filter((draft) => draft.link.id === linkId) : drafts.filter((draft) => !draft.link.id)),
        [drafts, linkId]
    )
