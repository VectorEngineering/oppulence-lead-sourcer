...
import { Button, Popover, SimpleTooltipContent, useKeyboardShortcut, useMediaQuery } from '@dub/ui'
...
export function MoreDropdown() {
    ...
    const options = useMemo(() => {
        const toggles = TOGGLES.filter((toggle) => (toggle.conversionEnabled ? conversionEnabled || data[toggle.key] : true))
    ...
    return (
        <>
            ...
            <Popover
                ...
                content={
                    <div className='grid p-1 max-sm:w-full md:min-w-72'>
                        {options.map((option) => {
                            const enabled = 'enabled' in option ? option.enabled(data) : data[option.key]
                                    ...
                                    text={
                                        <div className='flex items-center justify-between gap-2'>
                                            <div className='flex items-center gap-1'>
                                                ...
                                                {option.type === 'modal' ? (enabled ? '' : 'Add ') : enabled ? 'Remove ' : 'Add '}
                                                {option.label}
                                                <ProBadgeTooltip
                                                    content={
                        ...
                        <Button
                            ...
                            text={
                                <div className='flex items-center justify-between gap-2'>
                                    <div className='flex items-center gap-1'>
                                        <Settings className={cn('mr-1 size-4 text-gray-950', data.externalId && 'text-blue-500')} />
                                        Advanced Settings
                                    </div>
                                    <kbd className='hidden size-6 cursor-default items-center justify-center rounded-md border border-gray-200 font-sans text-xs text-gray-800 sm:flex'>
                                        A
            ...
            >
                <Button variant='secondary' icon={<Dots className='size-4' />} className='h-9 w-fit px-2.5' />
            </Popover>
        </>
    )
}
