...
function LinkBuilderInner({ showLinkBuilder, setShowLinkBuilder, props, duplicateProps, homepageDemo }: LinkBuilderProps) {
    const params = useParams() as { slug?: string }
    const { slug } = params
    const searchParams = useSearchParams()
    const { queryParams } = useRouterStuff()
    const { id: workspaceId, plan, nextPlan, logo, flags, conversionEnabled } = useWorkspace()
    ...
    const [url, domain, key, title, description] = watch(['url', 'domain', 'key', 'title', 'description'])
    ...
    */
        return Boolean(!showLinkBuilder || isSubmitting || isSubmitSuccessful || (props && !isDirty))
                            ...
                            try {
                                ...
                                if (res.status === 200) {
                                    ...
                                    if (error) {
                                        ...
                                        if (message.includes('key')) setError('key', { message: error.message })
                                        else if (message.includes('url')) setError('url', { message: error.message })
                    ...
                    >
                        <div className='flex items-center justify-between px-6 py-4'>
                            <div className='flex items-center gap-3'>
                                ...
                                />
                                <h3 className='!mt-0 max-w-sm truncate text-lg font-medium'>{props ? `Edit ${shortLink}` : 'New link'}</h3>
                            </div>
                            {!homepageDemo && (
                                <div className='flex items-center gap-4'>
                                    {!homepageDemo && workspaceId && (
                                        <DraftControls ref={draftControlsRef} props={props} workspaceId={workspaceId} />
                                    )}
                                    <button
                                        type='button'
                                        onClick={() => {
                                ...
                                <div className='flex min-h-full flex-col gap-8 py-4'>
                                    ...
                                    {key !== '_root' && (
                                        <ShortLinkInput
                                            ...
                                            onChange={(d) => {
                                                clearErrors('key')
                                                if (d.domain !== undefined) setValue('domain', d.domain, { shouldDirty: true })
                                                if (d.key !== undefined) setValue('key', d.key, { shouldDirty: true })
                                            }}
...
export function CreateLinkButton({ setShowLinkBuilder }: { setShowLinkBuilder: Dispatch<SetStateAction<boolean>> }) {
