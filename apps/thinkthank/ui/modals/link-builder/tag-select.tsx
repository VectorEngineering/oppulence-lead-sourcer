import { AnimatedSizeContainer, Combobox, InfoTooltip, Magic, SimpleTooltipContent, Tag, Tooltip, useKeyboardShortcut } from '@dub/ui'
...
export function TagSelect() {
    ...
    const [tags, linkId, url, title, description] = watch(['tags', 'id', 'url', 'title', 'description'])
    ...
    return (
        <div>
                ...
                {selectedTags.length > 0 ? (
                    <div className='flex flex-wrap gap-2'>
                        {selectedTags.slice(0, 10).map((tag) => (
                            <TagBadge key={tag.value} name={tag.label} color={tag.meta.color} className='animate-fade-in' />
                        ))}
                    </div>
            ...
            <AnimatedSizeContainer height transition={{ ease: 'linear', duration: 0.1 }}>
                {suggestedTags.length > 0 && (
