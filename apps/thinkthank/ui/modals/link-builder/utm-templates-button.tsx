...
import { AnimatedSizeContainer, ButtonTooltip, Popover, useMediaQuery } from '@dub/ui'
...
export function UTMTemplatesButton({ onLoad }: { onLoad: (params: Record<string, string>) => void }) {
    const { isMobile } = useMediaQuery()
    const { id: workspaceId } = useWorkspace()

    const { data, isLoading } = useSWR<UtmTemplateProps[]>(workspaceId && `/api/utm?workspaceId=${workspaceId}`, fetcher, {
...
function UTMTemplateList({ data, onLoad }: { data: UtmTemplateProps[]; onLoad: (params: Record<string, string>) => void }) {
    const { setValue } = useFormContext()

    return data.length ? (
        <div className='scrollbar-hide grid max-h-64 overflow-y-auto p-1 md:min-w-48'>
            <span className='block pb-2 pl-2.5 pt-2 text-xs font-medium text-gray-500'>UTM Templates</span>
            {data.map((template) => (
                <UTMTemplateOption
                    key={template.id}
                    template={template}
                    onClick={() => {
                        ...
                        paramEntries.forEach(([key, value]) => setValue(key, value, { shouldDirty: true }))
        ...
        <div className='flex items-center justify-center p-3 text-center text-xs text-gray-500'>No templates found</div>
    )
}

function UTMTemplateOption({ template, onClick }: { template: UtmTemplateProps; onClick: () => void }) {
