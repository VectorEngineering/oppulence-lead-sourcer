import { Button, FileUpload, Icon, ShimmerDots, SimpleTooltipContent, useKeyboardShortcut, useMediaQuery } from '@dub/ui'
import { Facebook, GlobePointer, LinkedIn, LoadingCircle, NucleoPhoto, Pen2, Twitter } from '@dub/ui/icons'
import { cn, getDomainWithoutWWW, resizeImage } from '@dub/utils'
import { ChangeEvent, ComponentType, PropsWithChildren, useCallback, useContext, useMemo, useRef, useState } from 'react'
        ...
        <div>
            ...
            <div className='flex items-center justify-between'>
                ...
                <Button
                    type='button'
                    variant='outline'
                    icon={<Pen2 className={cn('mx-px size-4', proxy && 'text-blue-500')} />}
                ...
                {tabs.map((tab) => {
                    ...
                    return (
                        <Button
                            ...
                            icon={<Icon className='size-4 text-current' fill='currentColor' />}
                            className={cn(
                                'h-7 text-gray-800',
                                tab === selectedTab
            ...
            <div className='mt-2'>
                <OGPreview title={title} description={description} hostname={hostname} password={password}>
...
export const ImagePreview = ({ image, onImageChange }: { image: string | null; onImageChange: (image: string) => void }) => {
    ...
    const onInputFileChange = useCallback(async (e: ChangeEvent<HTMLInputElement>) => {
            ...
            return (
                <div className='relative aspect-[var(--aspect,1200/630)] w-full bg-white'>
                    ...
                    <div className='pointer-events-none relative flex size-full flex-col items-center justify-center gap-2'>
                        ...
                        <p className='max-w-32 text-center text-xs text-gray-700'>Enter a link to generate a preview</p>
                    </div>
                </div>
            )
        }
...
function DefaultOGPreview({ title, description, children }: OGPreviewProps) {
    ...
    return (
        <div>
            <div className='group relative overflow-hidden rounded-md border border-gray-300'>{children}</div>
...
function FacebookOGPreview({ title, description, hostname, children }: OGPreviewProps) {
    const { setValue } = useFormContext<LinkFormData>()
            ...
            <div className='relative border border-gray-300'>
                ...
                {(hostname || title || description) && (
                    <div className='grid gap-1 border-t border-gray-300 bg-[#f2f3f5] p-2'>
                        {hostname && <p className='text-xs uppercase text-[#606770]'>{hostname}</p>}
                        <input
...
function LinkedInOGPreview({ title, hostname, children }: OGPreviewProps) {
    ...
    return (
        <div className='flex items-center gap-3 rounded-lg border border-[#8c8c8c33] px-4 py-3'>
            <div className='relative w-32 shrink-0 overflow-hidden rounded-lg' style={{ '--aspect': '128/72' } as any}>
                {children}
            </div>
            ...
            <div className='group relative overflow-hidden rounded-2xl border border-gray-300'>
                ...
                <div className='absolute bottom-2 left-0 w-full px-2'>
                    <div className='w-fit max-w-full rounded bg-black/[0.77] px-1.5 py-px'>
                        <span className='block max-w-sm truncate text-xs text-white'>{title || 'Add a title...'}</span>
                    </div>
                </div>
            </div>
            {hostname && <p className='mt-1 text-xs text-[#606770]'>From {hostname}</p>}
        </div>
    )
}
