...
import { Button, ShimmerDots, useInViewport, useKeyboardShortcut, useLocalStorage, useMediaQuery } from '@dub/ui'
...
export function QRCodePreview() {
    ...
    const { id: workspaceId, logo: workspaceLogo, plan: workspacePlan } = useWorkspace()
    ...
    const [data, setData] = useLocalStorage<QRCodeDesign>(`qr-code-design-${workspaceId}`, {
        fgColor: '#000000',
        hideLogo: true
    })
    ...
    const logo = workspacePlan === 'free' ? DUB_QR_LOGO : domainLogo || workspaceLogo || DUB_QR_LOGO
        ...
        <div ref={ref}>
            ...
            <div className='flex items-center justify-between'>
                ...
                <Button
                    ...
                    disabledTooltip={key && domain ? undefined : 'Enter a short link to customize'}
                />
            </div>
            <div className='relative mt-2 h-24 overflow-hidden rounded-md border border-gray-300'>
                ...
                {shortLinkUrl ? (
                    <AnimatePresence mode='wait'>
                        ...
                        >
                            <QRCode url={shortLinkUrl} fgColor={data.fgColor} hideLogo={hideLogo} logo={logo} scale={0.5} />
                        </motion.div>
                    </AnimatePresence>
                ) : (
                    <div className='flex size-full flex-col items-center justify-center gap-2'>
                        <QRCodeIcon className='size-5 text-gray-700' />
                        <p className='max-w-32 text-center text-xs text-gray-700'>Enter a short link to generate a QR code</p>
                    </div>
                )}
            </div>
        </div>
