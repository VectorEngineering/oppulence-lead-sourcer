...
import { AnimatedSizeContainer, Button, Copy, Input, LinkLogo, Modal, Switch, Tick, useCopyToClipboard } from '@dub/ui'
import { ArrowTurnRight2, Globe } from '@dub/ui/icons'
import { APP_DOMAIN, fetcher, getApexDomain, getPrettyUrl, linkConstructor } from '@dub/utils'
...
function ShareDashboardModalInner({ domain, _key: key }: ShareDashboardModalProps) {
    const { id: workspaceId } = useWorkspace()
    const [isRemoving, setIsRemoving] = useState(false)

    const { data: link, error: linkError } = useSWR<LinkProps>(
        workspaceId && domain && key ? `/api/links/info?${new URLSearchParams({ workspaceId, domain, key }).toString()}` : undefined,
        fetcher,
        {
    ...
    const { data: dashboard, mutate } = useSWR<DashboardProps>(
        link?.id ? `/api/links/${link.id}/dashboard?workspaceId=${workspaceId}` : undefined,
        fetcher,
        {
            dedupingInterval: 60000
        }
    ...
    const handleCreate = async () => {
        ...
        const res = await fetch(`/api/dashboards?${new URLSearchParams({ workspaceId, domain, key }).toString()}`, {
            method: 'POST',
            headers: {
        ...
        if (res.ok) {
            ...
            toast.promise(copyToClipboard(`${APP_DOMAIN}/share/${data.id}`), {
                success: 'Successfully created shared dashboard! Copied link to clipboard.'
            })
        } else {
    ...
    const handleUpdate = async (formData: z.infer<typeof updateDashboardBodySchema>) => {
        if (!dashboard) {
            return
        }

        const res = await fetch(`/api/dashboards/${dashboard.id}?workspaceId=${workspaceId}`, {
            method: 'PATCH',
            headers: {
                'Content-Type': 'application/json'
            },
    ...
    return (
        <>
            <h3 className='border-b border-gray-200 px-4 py-4 text-lg font-medium sm:px-6'>Share dashboard</h3>
                            ...
                            {checked &&
                                (dashboard ? (
                                    <div className='pt-4 text-sm'>
                                        ...
                                        <form className='grid w-full gap-3 px-px pt-4' onSubmit={handleSubmit(handleUpdate)}>
                                            <p className='text-base font-medium'>Settings</p>
                                            <div className='flex items-center justify-between gap-2'>
                                                <p className='text-sm text-gray-600'>Search engine indexing</p>
                                                <Switch
                                            ...
                                            {watch('password') !== null && (
                                                <Input data-1p-ignore type='password' {...register('password')} required />
                                            )}
                                            <Button
                                                type='submit'
...
function LinkCard({ link, isError }: { link: LinkProps | undefined; isError: boolean }) {
    return (
...
export function useShareDashboardModal(props: Pick<ShareDashboardModalProps, 'domain' | '_key'>) {
    const [showShareDashboardModal, setShowShareDashboardModal] = useState(false)

    const ShareDashboardModalCallback = useCallback(() => {
        return <ShareDashboardModal showModal={showShareDashboardModal} setShowModal={setShowShareDashboardModal} {...props} />
