...
function TransferDomainModal({
    ...
    const transferDomain = async (domain: string, newWorkspaceId: string) => {
        return await fetch(`/api/domains/${domain}/transfer?workspaceId=${currentWorkspace.id}`, {
            method: 'POST',
            headers: {
    ...
    return (
        <Modal showModal={showTransferDomainModal} setShowModal={setShowTransferDomainModal}>
            <form
                onSubmit={async (e) => {
                    e.preventDefault()
                    if (selectedWorkspace) {
                        ...
                        toast.promise(transferDomain(domain, selectedWorkspace.id), {
                            ...
                            success: "Domain transfer initiated. We'll send you an email once it's complete.",
            ...
            >
                <div className='flex flex-col items-center justify-center space-y-3 border-b border-gray-200 px-4 py-4 pt-8 text-center sm:px-16'>
                    ...
                    <p className='text-sm text-gray-500'>
                        Transfer this domain and its links to another {APP_NAME} workspace. Link tags will not be transferred.
                    </p>
                </div>
                <div className='flex flex-col space-y-28 bg-gray-50 px-4 py-8 text-left sm:space-y-3 sm:rounded-b-2xl sm:px-16'>
                    ...
                    />
                    <Button disabled={!selectedWorkspace} loading={transferring} text='Confirm transfer' />
                </div>
            </form>
        </Modal>
    )
