...
function RemoveSAMLModal({
    ...
    return (
        <Modal showModal={showRemoveSAMLModal} setShowModal={setShowRemoveSAMLModal}>
            <div className='flex flex-col items-center justify-center space-y-3 border-b border-gray-200 px-4 py-4 pt-8 sm:px-16'>
                ...
                <p className='text-center text-sm text-gray-500'>
                    This will remove SAML from your workspace. Are you sure you want to continue?
                </p>
            </div>
            ...
            <div className='flex flex-col space-y-4 bg-gray-50 px-4 py-8 text-left sm:px-16'>
                <div className='flex items-center space-x-3 rounded-md border border-gray-300 bg-white p-3'>
                    <img src={SAML_PROVIDERS.find((p) => p.name === provider)!.logo} alt={provider + ' logo'} className='h-8 w-8' />
                    <div className='flex flex-col'>
                        <h3 className='text-sm font-medium'>{provider} SAML</h3>
                        <p className='text-xs text-gray-500'>{provider} SAML is configured</p>
                    </div>
                </div>
                <Button
                    text='Confirm remove'
...
export function useRemoveSAMLModal() {
    ...
    const RemoveSAMLModalCallback = useCallback(() => {
        return <RemoveSAMLModal showRemoveSAMLModal={showRemoveSAMLModal} setShowRemoveSAMLModal={setShowRemoveSAMLModal} />
