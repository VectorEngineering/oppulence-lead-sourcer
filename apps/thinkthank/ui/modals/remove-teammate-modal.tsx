...
function RemoveTeammateModal({
    ...
    return (
        <Modal showModal={showRemoveTeammateModal} setShowModal={setShowRemoveTeammateModal}>
            <div className='flex flex-col items-center justify-center space-y-3 border-b border-gray-200 px-4 py-4 pt-8 sm:px-16'>
                ...
                <h3 className='text-lg font-medium'>
                    {invite ? 'Revoke Invitation' : session?.user?.email === email ? 'Leave Workspace' : 'Remove Teammate'}
                </h3>
                <p className='text-center text-sm text-gray-500'>
                    {invite ? 'This will revoke ' : session?.user?.email === email ? "You're about to leave " : 'This will remove '}
                    <span className='font-semibold text-black'>{session?.user?.email === email ? workspaceName : name || email}</span>
                    {invite
                ...
                <Button
                    ...
                    onClick={() => {
                        ...
                        fetch(
                            ...
                            if (res.status === 200) {
                                await mutate(`/api/workspaces/${workspaceId}/${invite ? 'invites' : 'users'}`)
...
export function useRemoveTeammateModal({ user, invite }: { user: UserProps; invite?: boolean }) {
