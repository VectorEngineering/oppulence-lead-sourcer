import { Button, InfoTooltip, LoadingSpinner, Logo, Modal, SimpleTooltipContent, Switch, useMediaQuery, useRouterStuff } from '@dub/ui'
...
function ImportRebrandlyModal({
    ...
    }>(workspaceId && showImportRebrandlyModal && `/api/workspaces/${workspaceId}/import/rebrandly`, fetcher, {
        onError: (err) => {
    ...
    const [selectedDomains, setSelectedDomains] = useState<ImportedDomainCountProps[]>([])
        ...
        >
            <div className='flex flex-col items-center justify-center space-y-3 border-b border-gray-200 px-4 py-8 sm:px-16'>
                <div className='flex items-center space-x-3 py-4'>
                    <img src='https://assets.dub.co/misc/icons/rebrandly.svg' alt='Rebrandly logo' className='h-12 w-12' />
                    <ArrowRight className='h-5 w-5 text-gray-600' />
                    <Logo />
                </div>
                <h3 className='text-lg font-medium'>Import Your Rebrandly Links</h3>
                <p className='text-center text-sm text-gray-500'>
                    Easily import all your existing Rebrandly links into {platform.company} with just a few clicks.
                </p>
            </div>
                        ...
                        <div className='flex flex-col space-y-2'>
                            ...
                            {domains.map(({ id, domain, links }) => (
                                <div className='flex items-center justify-between space-x-2 rounded-md border border-gray-200 bg-white px-4 py-2'>
                                    <div>
                                        ...
                                        {links > 0 && <p className='text-xs text-gray-500'>{nFormatter(links)} links found</p>}
                                    </div>
                                    <Switch
                                        fn={() => {
                            ...
                            {tagsCount && (
                                <div className='flex items-center justify-between space-x-2 rounded-md py-1 pl-2 pr-4'>
                                    <p className='text-xs text-gray-500'>{tagsCount} tags found. Import all?</p>
                                    <Switch fn={() => setImportTags(!importTags)} checked={importTags} />
                                </div>
                            )}
                        </div>
                        <Button text='Confirm import' loading={importing} disabled={selectedDomains.length === 0} />
                    </form>
                ) : (
                    ...
                    >
                        <div>
                            <div className='flex items-center space-x-2'>
                                <h2 className='text-sm font-medium text-gray-900'>Rebrandly API Key</h2>
                                <InfoTooltip
                                    content={
...
export function useImportRebrandlyModal() {
    const [showImportRebrandlyModal, setShowImportRebrandlyModal] = useState(false)
