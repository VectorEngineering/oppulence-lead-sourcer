...
function InviteTeammateModal({
    ...
    const { data: invites, isLoading } = useSWR<Invite[]>(
        showInviteTeammateModal && workspaceId && plan === 'free' && showSavedInvites && `/api/workspaces/${workspaceId}/invites/saved`,
        fetcher
    )

    return (
        <Modal showModal={showInviteTeammateModal} setShowModal={setShowInviteTeammateModal} className='max-h-[95dvh]'>
            <div className='flex flex-col items-center justify-center space-y-3 border-b border-gray-200 px-4 py-4 pt-8 sm:px-16'>
                {logo ? (
                    <BlurImage src={logo} alt='Invite Teammates' className='h-10 w-10 rounded-full' width={20} height={20} />
                ) : (
                    <Logo />
                )}
