...
import { Button, InfoTooltip, Label, Logo, Modal, RadioGroup, RadioGroupItem, TooltipContent, useMediaQuery } from '@dub/ui'
...
function AddEditTagModal({
    ...
    const saveDisabled = useMemo(
        () => saving || !name || !color || (props && Object.entries(props).every(([key, value]) => data[key] === value)),
        [props, data]
    )
        ...
        <Modal showModal={showAddEditTagModal} setShowModal={setShowAddEditTagModal}>
            <div className='flex flex-col items-center justify-center space-y-3 border-b border-gray-200 px-4 py-4 pt-8 sm:px-16'>
                ...
                <div className='flex flex-col space-y-1 text-center'>
                    <h3 className='text-lg font-medium'>{props ? 'Edit' : 'Create'} tag</h3>
            ...
            >
                ...
                <Button disabled={saveDisabled} loading={saving} text={props ? 'Save changes' : 'Create tag'} />
            </form>
        </Modal>
    )
}

function AddTagButton({ setShowAddEditTagModal }: { setShowAddEditTagModal: Dispatch<SetStateAction<boolean>> }) {
...
export function useAddEditTagModal({ props }: { props?: TagProps } = {}) {
    ...
    const AddEditTagModalCallback = useCallback(() => {
        return <AddEditTagModal showAddEditTagModal={showAddEditTagModal} setShowAddEditTagModal={setShowAddEditTagModal} props={props} />
