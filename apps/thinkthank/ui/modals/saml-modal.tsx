import { Button, InfoTooltip, Modal, SimpleTooltipContent, useMediaQuery } from '@dub/ui'
...
function SAMLModal({ showSAMLModal, setShowSAMLModal }: { showSAMLModal: boolean; setShowSAMLModal: Dispatch<SetStateAction<boolean>> }) {
    const { id } = useWorkspace()
    const [selectedProvider, setSelectedProvider] = useState<SAMLProviderProps['saml'] | undefined>()
    const [submitting, setSubmitting] = useState(false)
    const { mutate } = useSAML()

    const currentProvider = useMemo(() => SAML_PROVIDERS.find((p) => p.saml === selectedProvider), [selectedProvider])
        ...
        <Modal showModal={showSAMLModal} setShowModal={setShowSAMLModal}>
            <div className='flex flex-col items-center justify-center space-y-3 border-b border-gray-200 px-4 py-8 sm:px-16'>
                ...
                <p className='text-center text-sm text-gray-500'>
                    Select a provider to configure SAML for your {platform.company} workspace.
                </p>
            </div>
                    ...
                    onSubmit={async (e) => {
                        ...
                        fetch(`/api/workspaces/${id}/saml`, {
                            ...
                            body: JSON.stringify({
                                ...
                                encodedRawMetadata: fileContent ? Buffer.from(fileContent).toString('base64') : undefined
                            })
                        }).then(async (res) => {
                            if (res.ok) {
                                await mutate()
                ...
                >
                    <div>
                        <div className='flex items-center space-x-1'>
                            <h2 className='text-sm font-medium text-gray-900'>SAML Provider</h2>
                        ...
                        <select
                            ...
                            onChange={(e) => setSelectedProvider(e.target.value as SAMLProviderProps['saml'])}
                        ...
                        >
                            ...
                            {SAML_PROVIDERS.map((provider) => (
                                <option key={provider.saml} value={provider.saml} disabled={provider.wip}>
                                    {provider.name}
                    ...
                    {currentProvider &&
                        (selectedProvider === 'google' ? (
                            <div className='border-t border-gray-200 pt-4'>
                                <div className='flex items-center space-x-1'>
                                    <h2 className='text-sm font-medium text-gray-900'>{currentProvider.samlModalCopy}</h2>
                                    <InfoTooltip
                                        content={
                                ...
                                >
                                    {file ? (
                                        ...
                                        <>
                                            ...
                                            <p className='mt-2 text-sm text-gray-500'>Choose an .xml file to upload</p>
                                        </>
                                    )}
                                </label>
                                <input
                    ...
                    <Button text='Save changes' disabled={!selectedProvider} loading={submitting} />
                </form>
            </div>
        </Modal>
    )
...
export function useSAMLModal() {
    ...
    const SAMLModalCallback = useCallback(() => {
        return <SAMLModal showSAMLModal={showSAMLModal} setShowSAMLModal={setShowSAMLModal} />
