...
function SurveyModal({
    ...
    return (
        <Modal showModal={showSurveyModal} setShowModal={setShowSurveyModal}>
            ...
            <motion.div className='bg-gray-50 px-4 py-8 sm:px-16' initial='hidden' animate='show' exit='exit' variants={FADE_IN_VARIANTS}>
                <form
                    onSubmit={async (e) => {
                        e.preventDefault()
                        if (!source) return
...
export function useSurveyModal() {
    ...
    const SurveyModalCallback = useCallback(
        ({ onSubmit }: { onSubmit: (source: string) => void }) => {
            return <SurveyModal showSurveyModal={showSurveyModal} setShowSurveyModal={setShowSurveyModal} onSubmit={onSubmit} />
        },
