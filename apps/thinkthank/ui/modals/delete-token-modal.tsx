...
function DeleteTokenModal({
    ...
    return (
        <Modal showModal={showDeleteTokenModal} setShowModal={setShowDeleteTokenModal} className='max-w-md'>
            <div className='space-y-2 border-b border-neutral-200 px-4 py-4 sm:px-6'>
                <h3 className='text-lg font-medium'>Delete API Key</h3>
                <p className='text-sm text-gray-500'>
                    This will permanently delete the API key for and revoke all access to your account. Are you sure you want to continue?
                </p>
            </div>
            ...
            <div className='flex flex-col space-y-4 bg-neutral-50 px-4 py-4 sm:px-6'>
                <div className='relative flex items-center gap-2 space-x-3 rounded-md border border-gray-300 bg-white px-4 py-2'>
                    ...
                    <div className='flex flex-1 flex-col gap-0.5'>
                        <h3 className='line-clamp-1 text-sm font-medium text-neutral-600'>{token.name}</h3>
                        <p className='text-xs font-medium text-neutral-500' suppressHydrationWarning>
                            {token.partialKey}
                        </p>
                    </div>
...
export function useDeleteTokenModal({ token }: { token: TokenProps }) {
    ...
    const DeleteTokenModalCallback = useCallback(() => {
        return (
            <DeleteTokenModal showDeleteTokenModal={showDeleteTokenModal} setShowDeleteTokenModal={setShowDeleteTokenModal} token={token} />
        )
