...
import { Button, FileUpload, LoadingSpinner, useEnterSubmit, useMediaQuery } from '@dub/ui'
...
export function ContactForm({ setScreen }: { setScreen: Dispatch<SetStateAction<'main' | 'contact'>> }) {
    ...
    const handleUpload = async (file: File) => {
        ...
        setUploads((prev) => prev.map((upload) => (upload.file === file ? { ...upload, uploading: false, attachmentId } : upload)))
        setData((prev) => ({
            ...prev,
    ...
    return (
        <div className='relative w-full px-3 pb-16 pt-5 sm:px-6'>
            <button type='button' className='-ml-2 flex items-center space-x-1 px-2 py-1' onClick={() => setScreen('main')}>
                ...
                {formStatus === 'success' ? (
                    ...
                    >
                                ...
                                >
                                    ...
                                    <button
                                        ...
                                        onClick={() => {
                                            ...
                                            setData((prev) => ({
                                                ...prev,
                                                attachmentIds: prev.attachmentIds.filter((id) => id !== upload.attachmentId)
                                            }))
                                        }}
                                    >
                        ...
                        <div className='fixed bottom-0 left-0 z-10 flex h-16 w-full items-center justify-center rounded-b-lg bg-white px-3 sm:px-6'>
                            <Button className='h-9' disabled={!data.message} loading={formStatus === 'loading'} text='Send message' />
                        </div>
                    </motion.form>
                )}
            </AnimatePresence>
