...
import { forwardRef, type HTMLAttributes, type Ref, useMemo, useState } from 'react'

import { BusinessConfig as platform } from '@dub/platform-config'

export function OnboardingButton() {
    const { isMobile } = useMediaQuery()
    const [hideForever, setHideForever] = useLocalStorage('onboarding-hide-forever', false)

    return !isMobile && !hideForever ? <OnboardingButtonInner onHideForever={() => setHideForever(true)} /> : null
}

function OnboardingButtonInner({ onHideForever }: { onHideForever: () => void }) {
    ...
    const loading = domainsLoading || linksLoading || usersLoading || invitesLoading
    ...
    return loading || completedTasks === tasks.length ? null : (
                        ...
                        <div className='flex items-start justify-between gap-2'>
                            <div>
                                ...
                                <p className='mt-1 text-sm text-gray-300'>
                                    Get familiar with {platform.company} by completing the <br className='hidden sm:block' />
                                    following tasks
                                </p>
                            </div>
        ...
        >
            ...
            >
                ...
                <span className='text-gray-400'>{Math.round((completedTasks / tasks.length) * 100)}% complete</span>
            </button>
        </Popover>
    )
}

const MiniButton = forwardRef((props: HTMLAttributes<HTMLButtonElement>, ref: Ref<HTMLButtonElement>) => {
    return (
        <button
            ref={ref}
            type='button'
...
function OnboardingMenu({ onHideForever }: { onHideForever: () => void }) {
    ...
    return (
        <Popover
            ...
            content={
                <div className='p-1'>
                    <Button onClick={onHideForever} variant='outline' text='Dismiss forever' className='h-9' />
                </div>
            }
            openPopover={isOpen}
            setOpenPopover={setIsOpen}
