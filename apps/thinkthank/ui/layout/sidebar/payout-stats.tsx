    ...
    return (
        <AnimatedSizeContainer height>
            <div className='border-t border-neutral-300/80 p-3'>
                ...
                <div className='mt-4 flex flex-col gap-2'>
                        ...
                        <div className='flex items-center gap-2'>
                            ...
                            {payoutsCount ? (
                                <p className='text-black'>
                                    {currencyFormatter(
                                        (payoutsCount.find((payout) => payout.status === PayoutStatus.pending)?.amount || 0) / 100,
                                        {
                    ...
                    <div className='grid gap-1 text-sm'>
                        ...
                        {payoutsCount ? (
                            <p className='text-black'>
                                {currencyFormatter(
                                    (payoutsCount?.find((payout) => payout.status === PayoutStatus.completed)?.amount || 0) / 100,
                                    {
                                        maximumFractionDigits: 2
                                    }
                                )}
                ...
                {partner && !partner.payoutsEnabled && CONNECT_SUPPORTED_COUNTRIES.includes(partner.country) && (
                    <StripeConnectButton className='mt-4 h-9 w-full' text='Connect payouts' />
                )}
            </div>
