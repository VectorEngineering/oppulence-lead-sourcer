...
import { INFINITY_NUMBER, cn, getFirstAndLastDay, getNextPlan, nFormatter } from '@dub/utils'
...
function UsageInner() {
    const { usage, usageLimit, linksUsage, linksLimit, salesUsage, salesLimit, billingCycleStart, plan, slug, paymentFailedAt, loading } =
    ...
    const warnings = useMemo(
        () =>
            ...
            ].map(([usage, limit]) => usage !== undefined && limit !== undefined && usage / Math.max(0, usage, limit) >= 0.9),
        ...
        <AnimatedSizeContainer height>
            <div className='border-t border-neutral-300/80 p-3'>
                ...
                {paymentFailedAt ? (
                    ...
                    <Link
                        href={`/${slug}/upgrade`}
                        className={cn(buttonVariants(), 'mt-4 flex h-9 items-center justify-center rounded-md border px-4 text-sm')}
                        onMouseEnter={() => {
                            setHovered(true)
                        }}
                        onMouseLeave={() => {
