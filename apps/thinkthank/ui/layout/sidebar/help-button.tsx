...
export function HelpButton({
    ...
    return (
        <HelpContext.Provider value={{ popularHelpArticles, allHelpArticles }}>
            <Popover content={<HelpSection />} openPopover={isOpen} setOpenPopover={setIsOpen} align='start'>
                <button
                    type='button'
                    onClick={() => {
                        if (!isOpen) {
                ...
                >
                    <AnimatePresence>
                        ...
                        >
                            {isOpen ? <XIcon className='size-4' strokeWidth={2} /> : <CircleHelp className='size-4' />}
                        </motion.div>
                    </AnimatePresence>
                </button>
            </Popover>
