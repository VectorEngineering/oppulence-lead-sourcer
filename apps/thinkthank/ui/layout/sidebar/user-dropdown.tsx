...
export default function UserDropdown() {
    ...
    return (
        <Popover
            content={
                <div className='flex w-full flex-col space-y-px rounded-md bg-white p-2 sm:w-56'>
                    ...
                    <UserOption as={Link} label='Account' icon={User} href='/account/settings' onClick={() => setOpenPopover(false)} />
        ...
        >
            ...
            >
                {session?.user ? (
                    <Avatar user={session.user} className='size-6 border-none duration-75 sm:size-6' />
                ) : (
...
function UserOption<T extends ElementType = 'button'>({
    ...
    children,
    ...rest
}: UserOptionProps<T> & Omit<ComponentPropsWithoutRef<T>, keyof UserOptionProps<T>>) {
