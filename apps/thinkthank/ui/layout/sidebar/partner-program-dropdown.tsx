...
export function PartnerProgramDropdown() {
    ...
    const selectedProgram = useMemo(() => {
        const program = programEnrollments?.find((programEnrollment) => programEnrollment.program.slug === programSlug)

        return programSlug && program
            ? {
                  ...program.program,
                  logo: program.program.logo || `${DICEBEAR_AVATAR_URL}${program.program.name}`
              }
            : undefined
                            ...
                            <div className='border-b border-neutral-200 p-2'>
                                <ProgramList
                                    ...
                                    programs={programEnrollments
                                        .filter((programEnrollment) => programEnrollment.status === 'approved')
                        ...
                        <div className='p-2'>
                            ...
                            >
                                ...
                                <div>
                                    ...
                                    <div className={cn('truncate text-xs capitalize leading-tight text-neutral-600')}>Partner</div>
                                </div>
                            </Link>
            ...
            >
                ...
                >
                    <div className='flex min-w-0 items-center gap-x-2.5 pr-2'>
                        <BlurImage
                            src={selectedProgram?.logo || partner.image || `${DICEBEAR_AVATAR_URL}${partner.id}`}
                        ...
                        <div className='min-w-0'>
                            ...
                            <div className={cn('truncate text-xs capitalize leading-tight text-neutral-600')}>
...
function ProgramList({
    ...
    const href = useCallback(
        (slug: string) => (selectedProgram ? pathname?.replace(selectedProgram.slug, slug).split('?')[0] || '/' : `/programs/${slug}`),
                ...
                {programs.map(({ slug, name, logo }) => {
                    ...
                    return (
                        ...
                        >
                            ...
                            />
                            <div>
                                <span className='block truncate text-sm leading-5 text-neutral-900 sm:max-w-[140px]'>{name}</span>
                                <div className={cn('truncate text-xs capitalize leading-tight text-neutral-600')}>Program</div>
                            </div>
