...
import { PropsWithChildren, ReactNode, Suspense, useMemo, useState } from 'react'
        ...
        <ClientOnly className='scrollbar-hide relative flex h-full w-full flex-col overflow-y-auto overflow-x-hidden'>
            <nav className='relative flex grow flex-col p-3 text-gray-500'>
                        ...
                        return (
                            <Link
                                ...
                                className={cn(
                                    ...
                                    area === currentArea ? 'relative opacity-100' : 'pointer-events-none absolute opacity-0',
                                    (!title || !backHref) && 'mb-1'
                                )}
                ...
                <div className='relative w-full grow'>
                    {Object.entries(areas).map(([area, areaConfig]) => {
                        const { content, showSwitcher, showNews, direction } = areaConfig(data)
                        return (
                            <Area key={area} visible={area === currentArea} direction={direction ?? 'right'}>
        ...
        <div>
            <Link
                ...
                className={cn(
                    ...
                    isActive && !items && 'bg-blue-100/50 font-medium text-blue-600 hover:bg-blue-100/80 active:bg-blue-100'
                )}
            >
                {Icon && (
                    <Icon
            ...
            {items && (
                <AnimatedSizeContainer height transition={{ duration: 0.2, ease: 'easeInOut' }}>
                    <div className={cn('transition-opacity duration-200', isActive ? 'h-auto' : 'h-0 opacity-0')} aria-hidden={!isActive}>
...
export function Area({ visible, direction, children }: PropsWithChildren<{ visible: boolean; direction: 'left' | 'right' }>) {
    return (
        <div
            className={cn(
                ...
                visible
                    ? 'opacity-1 relative'
                    : cn('pointer-events-none absolute opacity-0', direction === 'left' ? '-translate-x-full' : 'translate-x-full')
            )}
