import { Button, Github, Google, Label, LinkedIn, ProductHunt, RadioGroup, RadioGroupItem, Twitter, Wordmark, useMediaQuery } from '@dub/ui'
...
export default function SurveyForm({ onSubmit }: { onSubmit: (source: string) => void }) {
    ...
    return (
        <div className='grid gap-4'>
            ...
            <p className='text-sm font-medium text-gray-800'>Where did you hear about Vector?</p>
            <form
                onSubmit={(e) => {
                    e.preventDefault()
            ...
            >
                ...
                >
                    {options.map((option) => (
                        ...
                        >
                            ...
                            >
                                <option.icon
                                    className={cn('h-5 w-5 transition-all group-hover:grayscale-0', {
                                ...
                                {option.value === 'other' && (
                                    <div className='flex grow justify-end'>
                                        <ChevronRight
                                            className={cn('h-4 w-4 transition-transform', source === option.value && 'rotate-90')}
                                        />
                                    </div>
                                )}
                            </Label>
                ...
                {source !== undefined && (
                    <Button
                        ...
                        disabled={status === 'success' || !source.length || (source === 'other' && !otherSource)}
                    />
                )}
            </form>
        </div>
