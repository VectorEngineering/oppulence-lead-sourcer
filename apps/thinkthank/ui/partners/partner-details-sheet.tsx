import { Button, Sheet, StatusBadge, Table, buttonVariants, useRouterStuff, useTable } from '@dub/ui'
import { CursorRays, GreekTemple, LinesY, Link4 } from '@dub/ui/icons'
import { COUNTRIES, DICEBEAR_AVATAR_URL, cn, currencyFormatter, formatDate, getPrettyUrl, nFormatter } from '@dub/utils'
...
function PartnerDetailsSheetContent({ partner, setIsOpen }: PartnerDetailsSheetProps) {
    ...
    const { createPayoutSheet, setIsOpen: setCreatePayoutSheetOpen } = useCreatePayoutSheet({ nested: true, partnerId: partner.id })

    return (
        <>
            <div className='flex grow flex-col'>
                <div className='flex items-start justify-between p-6'>
                    <Sheet.Title className='text-xl font-semibold'>Partner details</Sheet.Title>
                    <Sheet.Close asChild>
                ...
                <div className='border-y border-neutral-200 bg-neutral-50 p-6'>
                    ...
                    <div className='flex items-start justify-between gap-6'>
                        <div className='flex flex-col'>
                            ...
                            <div className='mt-4 flex items-start gap-2'>
                                <span className='text-lg font-semibold leading-tight text-neutral-900'>{partner.name}</span>
                        ...
                        <div className='flex min-w-[40%] shrink grow basis-1/2 flex-col items-end justify-end gap-2'>
                            {partner.link && (
                                <div className='group flex min-w-0 items-center gap-1 overflow-hidden rounded-full border border-neutral-200 bg-white px-1.5 py-0.5 text-xs text-neutral-700'>
                                    ...
                                    <span className='truncate'>{getPrettyUrl(partner.link.shortLink)}</span>
                                </div>
                            )}
                            {partner.country && (
                                <div className='flex min-w-20 items-center gap-2 rounded-full border border-neutral-200 bg-white px-1.5 py-0.5 text-xs text-neutral-700'>
                                    <img alt='' src={`https://flag.vercel.app/m/${partner.country}.svg`} className='h-3 w-4 rounded-sm' />
                    ...
                    {partner.status === 'approved' && (
                        <div className='xs:grid-cols-4 mt-6 grid grid-cols-2 gap-px overflow-hidden rounded-lg border border-neutral-200 bg-neutral-200'>
                            {[
                                ['Clicks', !partner.link ? '-' : nFormatter(partner.link?.clicks, { full: true })],
                                ['Leads', !partner.link ? '-' : nFormatter(partner.link?.leads, { full: true })],
                                ['Sales', !partner.link ? '-' : nFormatter(partner.link?.sales, { full: true })],
                                [
                                    'Revenue',
                                    !partner.link
                                        ? '-'
                ...
                <div className='grow p-6'>
                    {partner.status === 'approved' ? (
                        ...
                        <div className='flex flex-col gap-6 text-sm text-neutral-500'>
                            <h3 className='text-base font-semibold text-neutral-900'>About this partner</h3>

                            <div>
                                <h4 className='font-semibold text-neutral-900'>Description</h4>
                                <p className='mt-1.5'>
                                    {partner.bio || <span className='italic text-neutral-400'>No description provided</span>}
                                </p>
                            </div>
                        </div>
                    )}
            ...
            {partner.status === 'approved' && (
                <>
                    ...
                    <div className='flex grow flex-col justify-end'>
                        <div className='border-t border-neutral-200 p-5'>
                            <Button variant='primary' text='Create payout' onClick={() => setCreatePayoutSheetOpen(true)} />
                        </div>
                    </div>
                </>
            )}
...
function PartnerApproval({ partner, setIsOpen }: { partner: EnrolledPartnerProps; setIsOpen: Dispatch<SetStateAction<boolean>> }) {
    ...
    const createLink = async (search: string) => {
        ...
        const shortKey = search.startsWith(`${program?.domain}/`) ? search.substring(`${program?.domain}/`.length) : search
    ...
    return (
        <div className='flex'>
            <div className={cn('transition-[width] duration-300', isApproving ? 'w-[52px]' : 'w-[83px]')}>
                {isApproving ? (
                    <Button
                        type='button'
                        variant='secondary'
            ...
            <div className='flex grow pl-2'>
                <div className={cn('w-0 transition-[width] duration-300', isApproving && 'w-full')}>
                ...
                <div className='grow'>
                    <Button type='button' variant='primary' text='Approve' loading={isExecuting} onClick={handleApprovePartner} />
                </div>
            </div>
        </div>
    )
...
function PartnerRejectButton({ partner, setIsOpen }: { partner: EnrolledPartnerProps; setIsOpen: Dispatch<SetStateAction<boolean>> }) {
    const { id: workspaceId } = useWorkspace()
    ...
    return (
        <Button type='button' variant='secondary' text={isExecuting ? '' : 'Decline'} loading={isExecuting} onClick={handleRejectPartner} />
    )
}
...
function PartnerPayouts({ partner }: { partner: EnrolledPartnerProps }) {
    ...
    interface PayoutTableConfig {
        ...
        columns: Array<{
            id?: string
            header: string
            accessorFn?: (d: any) => string
            cell?: (props: { row: { original: any } }) => React.ReactNode
        }>
        onRowClick: (row: { original: any }) => void
        resourceName: (p: boolean) => string
        thClassName: (id: string) => string
        tdClassName: (id: string) => string
        className: string
    ...
    const table = useTable({
            ...
            },
            {
                ...
                cell: ({ row }) => {
                    ...
                    return badge ? (
                        <StatusBadge
                            icon={badge.icon}
                            variant={badge.variant as 'pending' | 'error' | 'new' | 'success' | 'neutral' | 'warning'}
                        >
                            {badge.label}
                        </StatusBadge>
                    ) : (
        ...
        onRowClick: (row) => {
            window.open(`/${slug}/programs/${partner.programId}/payouts?payoutId=${row.original.id}`, '_blank')
        },
        resourceName: (p) => `payout${p ? 's' : ''}`,
        thClassName: (id) => cn(id === 'total' && '[&>div]:justify-end', 'border-l-0'),
        tdClassName: (id) => cn(id === 'total' && 'text-right', 'border-l-0'),
        className: '[&_tr:last-child>td]:border-b-transparent',
        scrollWrapperClassName: 'min-h-[40px]'
    } satisfies PayoutTableConfig)
            ...
            {payouts && payouts.length === SHEET_MAX_ITEMS && (
                <div className='mt-2 flex justify-end'>
                    <Link
                        ...
                        className={cn(buttonVariants({ variant: 'secondary' }), 'flex h-7 items-center rounded-lg border px-2 text-sm')}
                    >
                        View all
                    </Link>
                </div>
...
export function PartnerDetailsSheet({
    ...
    return (
        <Sheet open={isOpen} onOpenChange={rest.setIsOpen} onClose={() => queryParams({ del: 'partnerId', scroll: false })}>
