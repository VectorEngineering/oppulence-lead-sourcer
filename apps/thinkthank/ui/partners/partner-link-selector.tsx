    ...
    return (
        <>
            <Combobox
                ...
                shouldFilter={false}
                onCreate={onCreate}
                createLabel={(search) => `Create "${search.startsWith(domain + '/') ? search : domain + '/' + search}"`}
            />
            {selectedLink?.url && showDestinationUrl && (
                <div className='ml-2 mt-2 flex items-center gap-1 text-xs text-gray-500'>
                    ...
                    <span className='min-w-0 truncate'>
                        Destination URL:{' '}
                        <a href={selectedLink.url} target='_blank' className='underline-offset-2 hover:underline'>
                            {selectedLink.url}
                        </a>
                    </span>
                </div>
