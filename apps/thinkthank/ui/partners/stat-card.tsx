...
export function StatCard({ title, event, total, timeseries, error, color, href }: StatCardProps) {
    return (
        <Wrapper
            href={href}
        ...
        >
            ...
            {total !== undefined ? (
                <div className='flex items-center gap-1 text-2xl text-neutral-800'>
                    ...
                    {event === 'sales' && <span className='text-base text-neutral-500'>({currencyFormatter(total.saleAmount / 100)})</span>}
                </div>
            ) : (
...
const Wrapper = ({ href, children, className }: { href?: string; children: React.ReactNode; className?: string }) => {
    return href ? (
