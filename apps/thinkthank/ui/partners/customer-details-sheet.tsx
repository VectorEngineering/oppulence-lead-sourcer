...
import { Customer, CustomerActivity, CustomerActivityResponse } from '@/lib/types'
import { X } from '@/ui/shared/icons'
import { Button, CopyButton, Sheet, TabSelect, Tooltip, useRouterStuff } from '@dub/ui'
import { ArrowRight, CursorRays, Link4, MoneyBill2, UserCheck } from '@dub/ui/icons'
import { cn, COUNTRIES, currencyFormatter, DICEBEAR_AVATAR_URL, fetcher, getPrettyUrl } from '@dub/utils'
        ...
        <>
            <div className='flex grow flex-col'>
                <div className='flex items-start justify-between p-6'>
                    <Sheet.Title className='text-xl font-semibold'>Customer details</Sheet.Title>
                    <Sheet.Close asChild>
                ...
                <div className='border-y border-neutral-200 bg-neutral-50 p-6 pb-0'>
                        ...
                        <div className='flex min-w-[40%] shrink grow basis-1/2 flex-col items-end justify-end gap-2'>
                            {link ? (
                                ...
                                >
                                    ...
                                    <span className='truncate'>{getPrettyUrl(link.shortLink)}</span>
                                </Link>
                            ) : (
                            ...
                            {country && (
                                ...
                                >
                                    <img alt='' src={`https://flag.vercel.app/m/${country}.svg`} className='h-3 w-4 rounded-sm' />
                    ...
                    <div className='mt-4 flex flex-col gap-1.5'>
                        <div className='flex items-center gap-2'>
                            <span className='text-lg font-semibold leading-tight text-neutral-900'>{customer.name}</span>
                            ...
                            {[
                                {
                                    ...
                                    value: customerActivity?.timeToLead // in milliseconds
                                        ...
                                        : '-',
                                    description: 'The time it took for this customer to convert from last click to a signup.'
                                },
                                {
                                    label: 'Sale',
                                    value: customerActivity?.timeToSale ? formatDistanceStrict(0, customerActivity.timeToSale) : '-',
                                    description: 'The time it took for this customer to convert from signup to a purchase.',
                                    className: 'pl-6'
                                }
                            ].map(({ label, value, description, className }, index) => (
                                <div key={label} className={cn('relative flex flex-col bg-neutral-50 p-3', className)}>
                            ...
                            {[
                                {
                                    ...
                                    value:
                                        customerActivity?.ltv !== undefined
                                            ...
                                            : '-',
                                    description: 'The total amount of money this customer has spent on your products.'
                                }
                    ...
                    <TabSelect className='mt-2' options={[{ id: 'activity', label: 'Activity' }]} selected={'activity'} />
                </div>
                    ...
                    {isLoading && !events ? (
                        ...
                        <ul className='flex flex-col gap-5'>
                            {(events || []).map((activity, index) => (
                                <Activity activity={activity} key={index} isLast={index === (events || []).length - 1} />
                            ))}
                        </ul>
                    )}
                </div>
...
function Activity({ activity, isLast }: { activity: CustomerActivity; isLast: boolean }) {
    ...
    return (
        <li className='flex items-center'>
            <div className='relative mr-3 flex-shrink-0'>
                ...
                {!isLast && <div className='absolute left-1/2 mt-1 h-4 border-l border-neutral-300' />}
            </div>
            <span className='flex-grow text-sm text-neutral-700'>
                {activity.eventName}

                {activity.eventDetails && <span className='ml-1 font-medium text-neutral-700'>({activity.eventDetails})</span>}
            </span>
...
export function CustomerDetailsSheet({
    ...
    return (
        <Sheet open={isOpen} onOpenChange={rest.setIsOpen} onClose={() => queryParams({ del: 'customerId', scroll: false })}>
