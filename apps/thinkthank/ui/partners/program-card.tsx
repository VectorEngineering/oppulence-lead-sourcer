...
import { DICEBEAR_AVATAR_URL, cn, currencyFormatter, formatDate } from '@dub/utils'
...
export function ProgramCard({ programEnrollment }: { programEnrollment: ProgramEnrollmentProps }) {
    ...
    const card = (
        <div
            className={cn(
                ...
                status === 'approved' ? 'hover:drop-shadow-card-hover transition-[filter]' : ''
            )}
        >
            <div className='flex items-center gap-4'>
                ...
                <div className='flex flex-col gap-0.5'>
                    <span className='text-base font-medium text-neutral-900'>{program.name}</span>
                    <StatusBadge
                        variant={
                            ProgramEnrollmentStatusBadges[status].variant as 'pending' | 'error' | 'new' | 'success' | 'neutral' | 'warning'
                        }
                        icon={null}
            ...
            {status === 'approved' ? (
                ...
                <div className='mt-4 flex h-24 items-center justify-center text-balance rounded-md border border-neutral-100 bg-neutral-50 p-5 text-center text-sm text-neutral-500'>
                    {status === 'pending'
                        ...
                        : `You will be able to apply again after ${formatDate(addDays(createdAt, 30))}`}
                </div>
            )}
        </div>
    )

    return status === 'approved' ? <Link href={`/programs/${program.slug}`}>{card}</Link> : card
}
...
function ProgramCardEarnings({ program }: { program: ProgramProps }) {
    ...
    return (
        <div className='mt-4 grid h-24 grid-cols-[min-content,minmax(0,1fr)] gap-4 rounded-md border border-neutral-100 bg-neutral-50 p-5'>
            <div>
                <div className='whitespace-nowrap text-sm text-neutral-500'>Earnings</div>
                {analytics ? (
