...
import { CircleHalfDottedClock, Dots, Duplicate, ShieldAlert } from '@dub/ui/icons'
...
export function SaleRowMenu({ row }: { row: Row<SaleResponse> }) {
    ...
    const { executeAsync } = useAction(updateSaleStatusAction, {
        onSuccess: async () => {
            await mutatePrefix([`/api/programs/${programId}/payouts`, `/api/programs/${programId}/sales`])
        }
    })
    ...
    return (
        <Popover
            ...
            content={
                <Command tabIndex={0} loop className='pointer-events-auto focus:outline-none'>
