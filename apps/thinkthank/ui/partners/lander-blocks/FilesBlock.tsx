...
export function FilesBlock({ block, logo }: { block: z.infer<typeof programLanderFilesBlockSchema>; logo: string | null }) {
    return (
        <div>
                        ...
                        <div className='flex min-w-0 items-center gap-4'>
                            <div className='rounded-full border border-gray-200'>
                                <div className='rounded-full border border-white bg-gradient-to-t from-gray-100 p-1 md:p-2'>
                                    {logo ? <img src={logo} alt='' className='size-4 rounded-full' /> : <Download className='size-4' />}
                                </div>
                            </div>
