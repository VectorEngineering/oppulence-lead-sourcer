...
export function ProgramCommissionDescription({
    ...
    program: Pick<ProgramProps, 'commissionAmount' | 'commissionType' | 'commissionDuration' | 'commissionInterval'>
    ...
    return (
        <>
            ...
            {program.commissionDuration === INFINITY_NUMBER ? (
                <strong className={cn('font-semibold', periodClassName)}> for the customer's lifetime.</strong>
            ) : program.commissionDuration && program.commissionDuration > 1 ? (
                <>
                    , and again{' '}
                    <strong className={cn('font-semibold', periodClassName)}>
                        every {program.commissionInterval || 'cycle'} for {program.commissionDuration}{' '}
                        {pluralize(program.commissionInterval || 'cycle', program.commissionDuration)}
                    </strong>
                    .
                </>
            ) : (
