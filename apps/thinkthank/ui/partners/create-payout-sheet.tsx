...
import { Button, DateRangePicker, Sheet, useEnterSubmit, useRouterStuff } from '@dub/ui'
import { DICEBEAR_AVATAR_URL, capitalize, cn, currencyFormatter, fetcher, formatDate } from '@dub/utils'
import { endOfMonth, endOfQuarter, endOfYear, startOfMonth, startOfQuarter, startOfYear, subMonths } from 'date-fns'
import { useParams, useRouter } from 'next/navigation'
import { Dispatch, Fragment, ReactNode, SetStateAction, useId, useMemo, useRef, useState } from 'react'
...
import { PayoutType } from '@dub/prisma-thinkthank/client'
...
function CreatePayoutSheetContent({ setIsOpen, partnerId }: CreatePayoutSheetProps) {
    ...
    const isPercentageBased = payoutType === 'sales' && program?.commissionType === 'percentage'
    ...
    const { executeAsync, isExecuting } = useAction(createManualPayoutAction, {
        onSuccess: async (res) => {
            ...
            if (payoutId) {
                router.push(`/${slug}/programs/${programId}/payouts?payoutId=${payoutId}`)
            }
        },
        onError({ error }) {
    ...
    const onSubmit = async (data: FormData) => {
        if (workspaceId && program) {
            const startDate = data.start ? data.start.getTime() - data.start.getTimezoneOffset() * 60000 : undefined
            const endDate = data.end ? data.end.getTime() - data.end.getTimezoneOffset() * 60000 : undefined
    ...
    const { data: salesAmount, isValidating: isValidatingSalesAmount } = useSWR<{
        amount: number
    }>(shouldRequestSales && `/api/programs/${program?.id}/sales/amount?${salesSearchParams.toString()}`, fetcher)

    const { data: salesCount, isValidating: isValidatingSalesCount } = useSWR<{
        pending: number
        duplicate: number
        fraud: number
    }>(shouldRequestSales && `/api/programs/${program?.id}/sales/count?${salesSearchParams.toString()}`, fetcher)

    const payoutAmount = useMemo(() => {
        const quantity = payoutType === 'sales' ? salesCount?.pending : totalEvents?.[payoutType]
    ...
    }, [payoutType, salesCount, totalEvents, amount, isPercentageBased, salesAmount, program])
        ...
        return {
            ...(selectedPartner && {
                Partner: (
                    <div className='flex items-center gap-2'>
                        <img
                            src={selectedPartner.image || `${DICEBEAR_AVATAR_URL}${selectedPartner.id}`}
            ...
            ...(start &&
                    ...
                    Period: `${formatDate(start, {
                        month: 'short',
                        year: new Date(start).getFullYear() === new Date(end).getFullYear() ? undefined : 'numeric',
                        timeZone: Intl.DateTimeFormat().resolvedOptions().timeZone
    ...
    const buttonDisabled = isExecuting || isValidating || isValidatingSalesAmount || isValidatingSalesCount || !partnerId || !payoutAmount

    return (
        <form ref={formRef} onSubmit={handleSubmit(onSubmit)} className='flex h-full flex-col'>
            <div>
                <div className='flex items-start justify-between border-b border-neutral-200 p-6'>
                    <Sheet.Title className='text-xl font-semibold'>Create manual payout</Sheet.Title>
                    <Sheet.Close asChild>
                        <Button variant='outline' icon={<X className='size-5' />} className='h-auto w-fit p-1' />
                    </Sheet.Close>
                </div>
                <div className='flex flex-col gap-4 p-6'>
                    <div className='flex flex-col gap-2'>
                        <label htmlFor={dateRangePickerId} className='block text-sm font-medium text-gray-900'>
                        ...
                        />
                        {(errors.start || errors.end) && <p className='text-xs text-red-600'>{errors.start?.message}</p>}
                    </div>

                    <div className='flex flex-col gap-2'>
                        <label htmlFor='type' className='flex items-center space-x-2 text-sm font-medium text-gray-900'>
                            Reward type
                        </label>
                        <select
                            ...
                            onChange={(e) => {
                                ...
                                if (type === 'sales' && program?.commissionAmount) {
                                    setValue(
                                        'amount',
                                        program.commissionType === 'percentage' ? program.commissionAmount : program.commissionAmount / 100
                                    )
                                } else {
                                    // @ts-ignore
                    ...
                    <div>
                        <label htmlFor='amount' className='flex justify-between text-sm font-medium text-neutral-800'>
                            Reward amount
                            {payoutType === 'sales' && (
                                <a
                        ...
                        <div className='relative mt-2 rounded-md shadow-sm'>
                            {!isPercentageBased && (
                                <span className='absolute inset-y-0 left-0 flex items-center pl-3 text-sm text-neutral-400'>$</span>
                            )}
                            <input
                                className={cn(
                                    'block w-full rounded-md border-neutral-300 text-neutral-900 placeholder-neutral-400 focus:border-neutral-500 focus:outline-none focus:ring-neutral-500 sm:text-sm',
                                    'pr-[6.5rem]',
                                    payoutType === 'sales' && 'cursor-not-allowed bg-neutral-100 text-neutral-500',
                                    payoutType === 'custom' && 'pr-12',
                                    !isPercentageBased && 'pl-6',
                                    errors.amount && 'border-red-600 focus:border-red-500 focus:ring-red-600'
                                )}
                                {...register('amount', {
                                    required: true,
                                    valueAsNumber: true,
                            ...
                            <span className='absolute inset-y-0 right-0 flex items-center pr-3 text-sm text-neutral-400'>
                                ...
                                {payoutType !== 'custom' && ` per ${payoutType.replace(/s$/, '')}`}
                            </span>
                        </div>
                    </div>

                    <div className='flex flex-col gap-2'>
                        <label htmlFor='description' className='flex items-center space-x-2 text-sm font-medium text-gray-900'>
                            ...
                            <div className='grid grid-cols-2 gap-3 rounded-md border border-neutral-200 bg-neutral-50 p-4 text-sm text-neutral-600'>
                                {Object.entries(invoiceData).map(([key, value]) => (
                                    <Fragment key={key}>
                                        ...
                                        <div className='text-neutral-800'>{value as ReactNode | string}</div>
                                    </Fragment>
                                ))}
                            </div>
                        </div>
...
export function useCreatePayoutSheet(props: { nested?: boolean } & Omit<CreatePayoutSheetProps, 'setIsOpen'>) {
    const [isOpen, setIsOpen] = useState(false)

    return {
        createPayoutSheet: <CreatePayoutSheet setIsOpen={setIsOpen} isOpen={isOpen} {...props} />,
        setIsOpen
    }
}
