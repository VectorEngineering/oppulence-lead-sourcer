...
export const UpgradeRequiredToast = ({ title, message }: { title: string; message: string }) => {
    const { slug, nextPlan } = useWorkspace()

    return (
        <div className='flex flex-col space-y-3 rounded-lg bg-white p-6 shadow-lg'>
            <div className='flex items-center space-x-1.5'>
                <Crown className='h-5 w-5 text-black' /> <p className='font-semibold'>{title}</p>
            </div>
            <p className='text-sm text-gray-600'>{message}</p>
            <Link
                href={slug ? `/${slug}/upgrade?exit=close` : `${platform.webUrl}/pricing`}
                target='_blank'
