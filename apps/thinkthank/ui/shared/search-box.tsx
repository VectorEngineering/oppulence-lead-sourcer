...
export const SearchBox = forwardRef(
    (
        { value, loading, showClearButton = true, onChange, onChangeDebounced, debounceTimeoutMs = 500, inputClassName }: SearchBoxProps,
        forwardedRef
    ) => {
        const inputRef = useRef<HTMLInputElement>(null)
        useImperativeHandle(forwardedRef, () => inputRef.current)

        const debounced = useDebouncedCallback((value) => onChangeDebounced?.(value), debounceTimeoutMs)
        ...
        return (
            <div className='relative'>
                <div className='pointer-events-none absolute inset-y-0 left-0 flex items-center pl-4'>
                    {loading && value.length > 0 ? <LoadingSpinner className='h-4 w-4' /> : <Magnifier className='h-4 w-4 text-gray-400' />}
                </div>
                <input
                    ref={inputRef}
                    type='text'
...
export function SearchBoxPersisted({ urlParam = 'search', ...props }: { urlParam?: string } & Partial<SearchBoxProps>) {
    ...
    useEffect(() => {
        if (searchParams.get(urlParam) ?? '' !== debouncedValue)
            queryParams(debouncedValue === '' ? { del: [urlParam, 'page'] } : { set: { search: debouncedValue }, del: 'page' })
    }, [debouncedValue])
    ...
    useEffect(() => {
        ...
        if ((search ?? '' !== value) && value === debouncedValue) setValue(search ?? '')
    }, [searchParams.get(urlParam)])

    return <SearchBox value={value} onChange={setValue} onChangeDebounced={setDebouncedValue} {...props} />
}
