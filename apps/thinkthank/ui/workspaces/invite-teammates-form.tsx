    ...
    return (
        <form
            onSubmit={handleSubmit(async (data) => {
                ...
                const res = await fetch(`/api/workspaces/${id}/invites${saveOnly ? '/saved' : ''}`, {
                    method: 'POST',
                ...
                if (res.ok) {
                    ...
                    if (saveOnly) {
                        toast.custom(() => <InviteSavedToast teammates={teammates.length} />, { duration: 7000 })
                    } else {
        ...
        >
            <div className='flex flex-col gap-2'>
                {fields.map((field, index) => (
                    <div key={field.id} className='relative'>
                        <label>
                            {index === 0 && (
                                <span className='mb-2 block text-sm font-medium text-gray-700'>{pluralize('Email', fields.length)}</span>
                            )}
            ...
            <Button
                ...
                text={saveOnly ? 'Continue' : `Send ${pluralize('invite', fields.length)}`}
            />
        </form>
    )
}
    ...
    return (
        <div className='flex items-center gap-1.5 rounded-lg bg-white p-4 text-sm shadow-[0_4px_12px_#0000001a]'>
            ...
            <p className='text-[13px] font-medium text-gray-900'>
                {pluralize('Invitation', teammates)} saved. You'll need a pro plan to invite teammates.{' '}
                <a
