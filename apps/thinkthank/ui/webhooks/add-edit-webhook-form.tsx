...
import { LINK_LEVEL_WEBHOOK_TRIGGERS, WEBHOOK_TRIGGER_DESCRIPTIONS, WORKSPACE_LEVEL_WEBHOOK_TRIGGERS } from '@/lib/webhook/constants'
...
export default function AddEditWebhookForm({ webhook, newSecret }: { webhook: WebhookProps | null; newSecret?: string }) {
    ...
    const updateDisabled = (webhook && webhook?.installationId !== null) || permissionsError !== false

    const disabledTooltip =
        webhook && webhook?.installationId ? `This webhook is managed by an integration.` : permissionsError ? permissionsError : undefined

    const enableLinkSelection = LINK_LEVEL_WEBHOOK_TRIGGERS.some((trigger) => triggers.includes(trigger))

    return (
        <>
            <form onSubmit={onSubmit} className='flex flex-col space-y-5 pb-20 text-left'>
                <div>
                ...
                <div className='space-y-2'>
                    <label className='flex items-center space-x-2'>
                        <h2 className='text-sm font-medium text-gray-900'>Signing secret</h2>
                    ...
                    <div className='flex items-center justify-between rounded-md border border-gray-300 bg-white px-3 py-1'>
                        <p className='text-nowrap font-mono text-sm text-gray-500'>{secret}</p>
                ...
                <div className='rounded-md border border-gray-200 p-4'>
                    <label htmlFor='triggers' className='flex flex-col gap-1'>
                        <h2 className='text-sm font-medium text-gray-900'>Workspace level events</h2>
                        <span className='text-xs text-gray-500'>These events are triggered at the workspace level.</span>
                    </label>
                            ...
                            <div key={trigger} className='group flex gap-2'>
                                <Checkbox
                                    ...
                                    onCheckedChange={(checked) => {
                                        setData({
                                            ...data,
                                            triggers: checked ? [...triggers, trigger] : triggers.filter((t) => t !== trigger)
                                        })
                                    }}
                                />
                                <label htmlFor={trigger} className='select-none text-sm text-gray-600 group-hover:text-gray-800'>
                ...
                <div className='rounded-md border border-gray-200 p-4'>
                    <label htmlFor='triggers' className='flex flex-col gap-1'>
                        <h2 className='text-sm font-medium text-gray-900'>
                            Link level events{' '}
                            <span className='rounded bg-yellow-100 px-1 py-0.5 text-xs font-medium text-yellow-800'>High traffic</span>
                        </h2>
                        <span className='text-xs text-gray-500'>These events are triggered at the link level.</span>
                    </label>
                    <div className='mt-3 flex flex-col gap-2'>
                    ...
                    {enableLinkSelection || linkIds.length ? (
                        <div className='mt-4'>
                            <h2 className='text-sm font-medium text-gray-900'>Choose links we should send events for</h2>
                            <div className='mt-3'>
                                <LinksSelector
