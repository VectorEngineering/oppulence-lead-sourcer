...
import { Button, CircleCheck, Copy, MaxWidthWrapper, Popover, TabSelect, TokenAvatar, useCopyToClipboard } from '@dub/ui'
...
export default function WebhookHeader({ webhookId }: { webhookId: string }) {
    ...
    const { SendTestWebhookModal, setShowSendTestWebhookModal } = useSendTestWebhookModal({
        webhook
    })
    ...
    const { execute, isExecuting } = useAction(enableOrDisableWebhook, {
        onSuccess: async ({ data }) => {
            await mutate()
            toast.success(data?.disabledAt ? 'Webhook disabled.' : 'Webhook enabled.')
        },
        onError: ({ error }) => {
    ...
    const disabled = webhook?.installationId !== null || permissionsError !== false
    ...
    return (
        <>
            <MaxWidthWrapper className='grid max-w-screen-lg gap-8'>
                ...
                <DeleteWebhookModal />
                <Link href={`/${slug}/settings/webhooks`} className='flex items-center gap-x-1'>
                            ...
                            <div className='w-screen sm:w-48'>
                                <div className='grid gap-px p-2'>
                                    <Button
                                        ...
                                        variant='outline'
                                        icon={copiedWebhookId ? <CircleCheck className='h-4 w-4' /> : <Copy className='h-4 w-4' />}
                                ...
                                <div className='grid gap-px p-2'>
                                    <Button
                                        text={webhook?.disabledAt ? 'Enable webhook' : 'Disable webhook'}
                                        variant='outline'
                                        icon={webhook?.disabledAt ? <CircleCheck className='size-4' /> : <CircleX className='size-4' />}
                                        className='h-9 justify-start px-2'
                                        onClick={async () => {
                                            if (
                                                !confirm(
                                                    `Are you sure you want to ${webhook?.disabledAt ? 'enable' : 'disable'} this webhook?`
                                                )
                                            ) {
                                                return
                                            }
