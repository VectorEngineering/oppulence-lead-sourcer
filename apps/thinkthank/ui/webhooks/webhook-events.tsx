...
import { Button, ButtonTooltip, Sheet, Tooltip, useCopyToClipboard, useMediaQuery } from '@dub/ui'
...
const WebhookEvent = ({ event }: { event: WebhookEventProps }) => {
    ...
    useEffect(() => {
        if (highlighter) {
            ...
            const responseBodyHtml = highlighter.codeToHtml(JSON.stringify(responseBodyDecoded, null, 2), {
                theme: 'min-light',
                lang: 'json'
            })
            setResponseBody(responseBodyHtml)
            const requestBodyHtml = highlighter.codeToHtml(JSON.stringify(event.request_body, null, 2), {
                theme: 'min-light',
                lang: 'json'
            })
            ...
            >
                ...
                <div className='text-xs text-gray-400'>
                    {(() => {
                        ...
                        return isMobile ? localDate.toLocaleTimeString() : localDate.toLocaleString()
                    })()}
                </div>
            </button>
            <Sheet open={isOpen} onOpenChange={setIsOpen} contentProps={{ className: 'md:w-[650px]' }}>
                <div className='p-6'>
                ...
                <div className='grid gap-4 border-t border-gray-200 bg-white p-6'>
                    ...
                    <div className='shiki-wrapper overflow-y-scroll text-sm' dangerouslySetInnerHTML={{ __html: responseBody }} />
                </div>
                <div className='grid gap-4 border-t border-gray-200 bg-white p-6'>
                    <h4 className='font-semibold'>Request</h4>
                    <div className='shiki-wrapper overflow-y-scroll text-sm' dangerouslySetInnerHTML={{ __html: requestBody }} />
                </div>
            </Sheet>
        </>
    )
