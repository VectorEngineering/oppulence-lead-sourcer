...
const getLinkOption = (link: LinkProps) => ({
    ...
    icon: <LinkLogo apexDomain={getApexDomain(link.url)} className='h-4 w-4 sm:h-4 sm:w-4' />,
    meta: {
        url: link.url
    }
})
...
export function LinksSelector({
    ...
    const options = useMemo(() => links?.map((link) => getLinkOption(link)), [links])

    const selectedOptions = useMemo(
        () =>
            selectedLinkIds
                .map((id) => [...(links || []), ...(selectedLinks || [])].find((l) => l.id === id))
                .filter(Boolean)
    ...
    return (
        <Combobox
            ...
            buttonProps={{
                disabled,
                className: cn('h-auto py-1.5 px-2.5 w-full max-w-full text-gray-700 border-gray-300 items-start')
            }}
        >
            {selectedLinkIds.length === 0 ? (
                ...
                <div className='flex flex-wrap gap-2'>
                    {selectedOptions.slice(0, 10).map((option) => (
                        ...
                        >
                            <LinkLogo apexDomain={getApexDomain(option.meta.url)} className='size-3 shrink-0 sm:size-3' />
                            <span className='min-w-0 truncate'>{truncate(option.label, 32)}</span>
                        </span>
                    ))}
                </div>
            ) : (
