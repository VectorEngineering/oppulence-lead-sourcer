...
export async function generateCsvMapping(fieldColumns: string[], firstRows: Record<string, string>[]) {
    ...
    ;(async () => {
        const { partialObjectStream } = streamObject({
            ...
            schema: z.object({
                link: z.string().optional().describe('The shortlink (link), including the domain and path.'),
                ...
                tags: z
                    .string()
                    .optional()
                    .describe('A comma-separated list of tags for shortlink organization (NOT to be mapped to a description).'),
                createdAt: z.string().optional().describe('The date and time the shortlink was created (createdAt)')
            }),
            prompt:
