...
export async function completeProgramApplications(userId: string) {
    try {
        ...
        const programApplicationIds = cookieStore.get('programApplicationIds')?.value?.split(',')
        ...
        const { success } = await ratelimit(3, '1 m').limit(`complete-program-applications:${userId}`)
        ...
        let programApplications = await prisma.programApplication.findMany({
            where: {
                ...
                programId: {
                    notIn: user.partners.map((p) => p.partner.programs.map((pp) => pp.programId)).flat()
                }
            },
            orderBy: {
                createdAt: 'desc'
        ...
        console.error('Failed to complete program applications from cookies', error)
    }
}
