...
import type { Payout } from '@dub/prisma-thinkthank/client'
...
export const createSalesPayout = async ({
    ...
    return await prisma.$transaction(async (tx) => {
        ...
        if (!periodStart) {
            ...
            periodStart = sales.reduce((min, sale) => (sale.createdAt < min ? sale.createdAt : min), sales[0].createdAt)
        }

        if (!periodEnd) {
            // get the end of the month of the latest sale
            // e.g. if the latest sale is 2024-12-16, the periodEnd should be 2024-12-31
            const latestSale = sales.reduce((max, sale) => (sale.createdAt > max ? sale.createdAt : max), sales[0].createdAt)
            periodEnd = new Date(latestSale.getFullYear(), latestSale.getMonth() + 1, 0)
        }
            ...
            if (existingPayout) {
                payout = await tx.payout.update({
                    ...
                    data: {
                        ...
                        periodEnd,
                        description: existingPayout.description ?? `${platform.company} Partners payout`
                    }
                })
