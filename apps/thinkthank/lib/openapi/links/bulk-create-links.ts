...
import { LinkErrorSchema, LinkSchema, createLinkBodySchema } from '@/lib/zod/schemas/links'
            ...
            content: {
                'application/json': {
                    schema: z.array(z.union([LinkSchema, LinkErrorSchema])).openapi({
                        type: 'array',
                        items: {
                            oneOf: [{ $ref: '#/components/schemas/LinkSchema' }, { $ref: '#/components/schemas/LinkErrorSchema' }]
                        }
                    })
                }
            }
