    ...
    providers: [
        ...
        CredentialsProvider({
            ...
            async authorize(credentials, req) {
                ...
                if (!passwordMatch) {
                    const exceededLoginAttempts = exceededLoginAttemptsThreshold(await incrementLoginAttempts(user))
    ...
    events: {
        async signIn(message) {
            if (message.isNewUser) {
                ...
                if (user.createdAt && new Date(user.createdAt).getTime() > Date.now() - 10000 && process.env.NEXT_PUBLIC_IS_DUB) {
                    waitUntil(
                        Promise.allSettled([
