...
export const onboardPartnerAction = authUserActionClient.schema(onboardPartnerSchema).action(async ({ ctx, parsedInput }) => {
    const { user } = ctx
    const { name, email, image, country, description } = parsedInput

    const partnersPortalEnabled = await userIsInBeta(user.email, 'partnersPortal')
    ...
    if (emailInUse) {
        throw new Error(`"${email}" is already in use by another partner. Please use a different email.`)
    }
    ...
    const imageUrl = await storage.upload(`partners/${partnerId}/image_${nanoid(7)}`, image).then(({ url }) => url)
