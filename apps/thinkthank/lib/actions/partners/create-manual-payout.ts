...
import { Payout } from '@dub/prisma-thinkthank/client'
...
export const createManualPayoutAction = authActionClient.schema(schema).action(async ({ parsedInput, ctx }) => {
    const { workspace } = ctx
    const { programId, partnerId, start, end, type, amount, description } = parsedInput
    ...
    if (type === 'sales') {
        ...
        if (!payout) {
            throw new Error('No valid sales found for this period. Payout was not created.')
        }
    }
