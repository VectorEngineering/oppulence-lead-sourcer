...
import { Partner, Program, ProgramEnrollment } from '@dub/prisma-thinkthank/client'
...
export const createProgramApplicationAction = actionClient.schema(createProgramApplicationSchema).action(
    async ({
        parsedInput
    }): Promise<{
        ...
        const { success } = await ratelimit(3, '1 m').limit(`create-program-application:${programId}:${getIP()}`)
...
async function createApplication({ program, data }: { program: Program; data: z.infer<typeof createProgramApplicationSchema> }) {
    ...
    const existingApplicationIds = cookieStore.get('programApplicationIds')?.value?.split(',') || []

    cookieStore.set('programApplicationIds', [...existingApplicationIds, application.id].join(','), {
        httpOnly: true,
