...
export const acceptProgramInviteAction = authPartnerActionClient.schema(acceptProgramInviteSchema).action(async ({ parsedInput, ctx }) => {
    const { partner } = ctx
    ...
    const [programEnrollment, _] = await Promise.all([
        prisma.programEnrollment.create({
            data: {
                ...
                status: 'approved',
                discountId: programInvite.program.discounts.length > 0 ? programInvite.program.discounts[0].id : null
            }
        }),
