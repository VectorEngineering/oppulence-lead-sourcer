...
import { SaleStatus } from '@dub/prisma-thinkthank/client'
import { z } from 'zod'
...
export const updateSaleStatusAction = authActionClient.schema(updateSaleStatusSchema).action(async ({ parsedInput, ctx }) => {
    const { workspace } = ctx
