...
export const sendInviteReferralEmail = authActionClient
    ...
    .action(async ({ ctx, parsedInput }) => {
        ...
        const { success: successWorkspace } = await ratelimit(5, '1 m').limit(`invite-referral-email:${workspace.id}`)

        // Allow 2 emails to a specific address / 2 hours (not one / hour to allow one quick retry)
        const { success: successEmail } = await ratelimit(2, '2 h').limit(`invite-referral-email:${email}`)

        if (!successWorkspace || !successEmail) throw new Error('Failed to send: rate limit exceeded')
