...
export const addEditIntegration = authActionClient
    ...
    .action(async ({ parsedInput }) => {
        ...
        if (integration.logo && !isStored(integration.logo)) {
            const result = await storage.upload(`integrations/${id}_${nanoid(7)}`, integration.logo)
        ...
        if (id) {
            ...
            waitUntil(
                (async () => {
                    ...
                    if (logoUpdated && integration.logo && integration.logo.startsWith(`${R2_URL}/integrations/${id}`)) {
                        await storage.delete(integration.logo.replace(`${R2_URL}/`, ''))
                    }

                    const removedScreenshots =
                        oldIntegration.screenshots && Array.isArray(oldIntegration.screenshots)
                            ? oldIntegration.screenshots.filter((s) => typeof s === 'string' && !integration.screenshots.includes(s))
                            : []
