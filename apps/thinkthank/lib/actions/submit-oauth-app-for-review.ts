...
export const submitOAuthAppForReview = authActionClient.schema(schema).action(async ({ ctx, parsedInput }) => {
    ...
    const { success } = await ratelimit(1, '1 m').limit(`submit-oauth-app-for-review:${integrationId}`)
