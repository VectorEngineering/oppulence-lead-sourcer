import { LOCALHOST_GEO_DATA, LOCALHOST_IP, capitalize, getDomainWithoutWWW } from '@dub/utils'
...
import { detectBot, detectQr, getFinalUrlForRecordClick, getIdentityHash } from '../middleware/utils'
...
export async function recordClick({
    ...
    const [, , , , workspaceRows] = await Promise.all([
        fetch(`${process.env.TINYBIRD_API_URL}/v0/events?name=dub_click_events&wait=true`, {
            method: 'POST',
            headers: {
        ...
        conn.execute('UPDATE Link SET clicks = clicks + 1, lastClicked = NOW() WHERE id = ?', [linkId]),
        ...
        url && conn.execute('UPDATE Project p JOIN Link l ON p.id = l.projectId SET p.usage = p.usage + 1 WHERE l.id = ?', [linkId]),

        // fetch the workspace usage for the workspace
        workspaceId && hasWebhooks ? conn.execute('SELECT usage, usageLimit FROM Project WHERE id = ? LIMIT 1', [workspaceId]) : null
    ])

    const workspace =
        workspaceRows && workspaceRows.rows.length > 0 ? (workspaceRows.rows[0] as Pick<WorkspaceProps, 'usage' | 'usageLimit'>) : null

    const hasExceededUsageLimit = workspace && workspace.usage >= workspace.usageLimit
...
async function sendLinkClickWebhooks({ webhookIds, linkId, clickData }: { webhookIds: string[]; linkId: string; clickData: any }) {
    ...
    const activeLinkWebhooks = webhooks.filter((webhook) => {
        return !webhook.disabledAt && webhook.triggers && Array.isArray(webhook.triggers) && webhook.triggers.includes('link.clicked')
    })
    ...
    const link = await conn.execute('SELECT * FROM Link WHERE id = ?', [linkId]).then((res) => res.rows[0])
