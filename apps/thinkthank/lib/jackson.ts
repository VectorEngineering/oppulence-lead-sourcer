import type { IConnectionAPIController, IDirectorySyncController, IOAuthController, JacksonOption } from '@boxyhq/saml-jackson'
...
const opts: JacksonOption = {
    externalUrl: process.env.NODE_ENV === 'production' ? `https://api.${platform.domain}` : `${process.env.NEXTAUTH_URL}`,
    samlPath: process.env.NODE_ENV === 'production' ? '/auth/saml/callback' : '/api/auth/saml/callback',
    samlAudience,
    db: {
        engine: 'planetscale',
        type: 'mysql',
    ...
    scimPath: process.env.NODE_ENV === 'production' ? '/scim/v2.0' : '/api/scim/v2.0', // custom SCIM endpoint
...
export default async function init() {
    if (!globalThis.apiController || !globalThis.oauthController || !globalThis.directorySyncController) {
