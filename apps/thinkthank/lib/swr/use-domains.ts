import { DUB_DOMAINS, DUB_WORKSPACE_ID, SHORT_DOMAIN, fetcher } from '@dub/utils'
...
export default function useDomains({
    ...
    const { defaultDomains: workspaceDefaultDomains, loading: loadingDefaultDomains } = useDefaultDomains(opts)

    const allWorkspaceDomains = useMemo(() => data || [], [data])
    const activeWorkspaceDomains = useMemo(() => data?.filter((domain) => !domain.archived), [data])

    const activeDefaultDomains = useMemo(
        () => (workspaceDefaultDomains && DUB_DOMAINS.filter((d) => workspaceDefaultDomains?.includes(d.slug))) || DUB_DOMAINS,
        [workspaceDefaultDomains]
    )

    const allDomains = useMemo(
        () => [...allWorkspaceDomains, ...(workspaceId === `ws_${DUB_WORKSPACE_ID}` ? [] : DUB_DOMAINS)],
        [allWorkspaceDomains, workspaceId]
    )
    const allActiveDomains = useMemo(
        () => [...(activeWorkspaceDomains || []), ...(workspaceId === `ws_${DUB_WORKSPACE_ID}` ? [] : activeDefaultDomains)],
        [activeWorkspaceDomains, activeDefaultDomains, workspaceId]
    )

    const primaryDomain = useMemo(() => {
        if (activeWorkspaceDomains && activeWorkspaceDomains.length > 0) {
            return activeWorkspaceDomains.find(({ primary }) => primary)?.slug || activeWorkspaceDomains[0].slug
