...
export const installIntegration = async ({ userId, workspaceId, integrationId, credentials }: InstallIntegration) => {
    ...
    waitUntil(
        (async () => {
            ...
            const email = workspace.users.length > 0 ? workspace.users[0].user.email : null
            const integration = workspace.installedIntegrations.length > 0 ? workspace.installedIntegrations[0].integration : null
