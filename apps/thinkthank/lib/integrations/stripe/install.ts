...
export const getStripeInstallationUrl = async (workspaceId: string) => {
    ...
    if (!env.success) {
        throw new Error('Stripe App environment variables are not configured properly.')
    }

    const { STRIPE_APP_INSTALL_URL } = env.data

    const url = new URL(STRIPE_APP_INSTALL_URL)
    url.searchParams.set('redirect_uri', `${APP_DOMAIN_WITH_NGROK}/api/stripe/integration/callback`)
