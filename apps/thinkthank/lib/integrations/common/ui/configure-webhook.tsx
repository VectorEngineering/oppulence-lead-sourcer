...
export function ConfigureWebhook({
    ...
    const { data: webhook, isLoading } = useSWR<WebhookProps>(`/api/webhooks/${webhookId}?workspaceId=${workspaceId}`, fetcher)
    ...
    const onSubmit = async (e: FormEvent) => {
        ...
        const response = await fetch(`/api/webhooks/${webhookId}?workspaceId=${workspaceId}`, {
            method: 'PATCH',
            headers: {
    ...
    const canManageWebhook = !permissionsError || plan === 'free' || plan === 'pro'
                    ...
                    <div>
                        ...
                        <div className='mt-3 flex flex-col gap-2'>
                            {WORKSPACE_LEVEL_WEBHOOK_TRIGGERS.filter(
                                (trigger) => canManageWebhook && supportedEvents.includes(trigger)
                            ).map((trigger) => (
                                <div key={trigger} className='group flex gap-2'>
                                    <Checkbox
                                        ...
                                        disabled={!canManageWebhook || !supportedEvents.includes(trigger)}
                                        ...
                                        title={!supportedEvents.includes(trigger) ? 'Not supported' : undefined}
                                    />
                                    <label
                                        htmlFor={trigger}
                                        className={cn(
                                            'select-none text-sm text-gray-600',
                                            supportedEvents.includes(trigger) ? 'group-hover:text-gray-800' : 'opacity-50'
                                        )}
                                    >
