import { clickWebhookEventSchema, webhookPayloadSchema } from '@/lib/webhook/schemas'
...
import { Webhook } from '@dub/prisma-thinkthank/client'
import { WebhookTrigger } from '../types'
import z from '../zod'
import { WebhookSchema } from '../zod/schemas/webhooks'
import { WEBHOOK_EVENT_ID_PREFIX } from './constants'

interface TransformWebhookProps extends Pick<Webhook, 'id' | 'name' | 'url' | 'secret' | 'triggers' | 'disabledAt'> {
    links: { linkId: string }[]
}
...
export const transformClickEventData = (
    ...
    const click = Object.fromEntries(Object.entries(data).map(([key, value]) => [toCamelCase(key), value]))
...
export const transformLeadEventData = (data: any) => {
    const lead = Object.fromEntries(Object.entries(data).map(([key, value]) => [toCamelCase(key), value]))
    ...
    return leadWebhookEventSchema.parse({
        ...
        customer: {
            ...
            avatar: lead.customerAvatar || `https://api.dicebear.com/9.x/micah/svg?seed=${lead.customerId}`,
...
export const transformSaleEventData = (data: any) => {
    const sale = Object.fromEntries(Object.entries(data).map(([key, value]) => [toCamelCase(key), value]))
    ...
    return saleWebhookEventSchema.parse({
        ...
        customer: {
            ...
            avatar: sale.customerAvatar || `https://api.dicebear.com/9.x/micah/svg?seed=${sale.customerId}`,
