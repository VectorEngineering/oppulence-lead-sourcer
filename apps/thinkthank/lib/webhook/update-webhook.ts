...
export const toggleWebhooksForWorkspace = async ({ workspaceId }: { workspaceId: string }) => {
...
export const propagateWebhookTriggerChanges = async ({ webhookIds }: { webhookIds: string[] | undefined }) => {
    if (!webhookIds) {
        return
    }
    ...
    const linkLevelWebhooks = webhooks.filter(
        (webhook) => webhook._count.links > 0 && !(webhook.triggers as string[])?.includes('link.clicked')
    )
    ...
    if (workspaceLevelWebhooks.length > 0) {
        await Promise.all([
            ...
            webhookCache.deleteMany(workspaceLevelWebhooks.map((webhook) => webhook.id))
        ])
    }
}
