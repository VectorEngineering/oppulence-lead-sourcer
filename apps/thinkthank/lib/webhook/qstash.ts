import { Webhook, WebhookReceiver } from '@dub/prisma-thinkthank/client'
...
export const sendWebhooks = async ({
    ...
    return await Promise.all(webhooks.map((webhook) => publishWebhookEventToQStash({ webhook, payload })))
}
...
const transformPayload = ({ payload, receiver }: { payload: z.infer<typeof webhookPayloadSchema>; receiver: WebhookReceiver }) => {
    switch (receiver) {
        case 'slack':
