...
export default async function AppMiddleware(req: NextRequest) {
    ...
    const isWorkspaceInvite = req.nextUrl.searchParams.get('invite') || path.startsWith('/invites/')
    ...
    if (
      ...
      */
            ...
            if (defaultWorkspace) {
                ...
                return NextResponse.redirect(new URL(`/onboarding/${step}?workspace=${defaultWorkspace}`, req.url))
            } else {
                return NextResponse.redirect(new URL('/onboarding', req.url))
            }
            ...
            ['/', '/login', '/register', '/workspaces', '/analytics', '/events', '/programs', '/settings', '/upgrade', '/wrapped'].includes(
                path
            ) ||
