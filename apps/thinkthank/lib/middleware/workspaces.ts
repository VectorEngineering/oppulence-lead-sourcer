...
export default async function WorkspacesMiddleware(req: NextRequest, user: UserProps) {
    ...
    if (defaultWorkspace) {
        ...
        return NextResponse.redirect(new URL(`/${defaultWorkspace}${redirectPath}${searchParamsString}`, req.url))
    } else {
