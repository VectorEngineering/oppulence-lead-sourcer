import { createResponseWithCookie, detectBot, getFinalUrl, isSupportedDeeplinkProtocol, parse } from '@/lib/middleware/utils'
import { DUB_HEADERS, LEGAL_WORKSPACE_ID, LOCALHOST_GEO_DATA, isDubDomain, isUnsupportedKey, nanoid, punyEncode } from '@dub/utils'
...
export default async function LinkMiddleware(req: NextRequest, ev: NextFetchEvent) {
    ...
    if (inspectMode && !password) {
        return NextResponse.rewrite(new URL(`/inspect/${domain}/${encodeURIComponent(key)}+`, req.url), {
            headers: {
                ...DUB_HEADERS,
    ...
    if (password) {
        const pw = req.nextUrl.searchParams.get('pw') || req.cookies.get(`dub_password_${linkId}`)?.value
    ...
    const { country } = process.env.VERCEL === '1' && req.geo ? req.geo : LOCALHOST_GEO_DATA

    // rewrite to proxy page (/proxy/[domain]/[key]) if it's a bot and proxy is enabled
    if (isBot && proxy) {
        return createResponseWithCookie(
            NextResponse.rewrite(new URL(`/proxy/${domain}/${encodeURIComponent(key)}`, req.url), {
                headers: {
                    ...DUB_HEADERS,
                    ...(!shouldIndex && { 'X-Robots-Tag': 'googlebot: noindex' })
                }
