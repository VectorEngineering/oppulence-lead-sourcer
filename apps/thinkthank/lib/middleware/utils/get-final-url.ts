...
export const getFinalUrl = (url: string, { req, clickId }: { req: NextRequest; clickId?: string }) => {
    ...
    const redirectionUrl = getUrlFromStringIfValid(searchParams.get(REDIRECTION_QUERY_PARAM) ?? '')
...
const allowedQueryParams = ['utm_source', 'utm_medium', 'utm_campaign', 'utm_term', 'utm_content', 'ref']

// Get final cleaned url for storing in TB
export const getFinalUrlForRecordClick = ({ req, url }: { req: Request; url: string }) => {
