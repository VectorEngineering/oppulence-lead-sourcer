...
const UNAUTHENTICATED_PATHS = ['/login', '/register', '/forgot-password', '/auth/reset-password', '/apply']
...
export default async function PartnersMiddleware(req: NextRequest) {
    ...
    const isUnauthenticatedPath = UNAUTHENTICATED_PATHS.some((p) => path.startsWith(p))
