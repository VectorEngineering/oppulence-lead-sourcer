...
export const getFeatureFlags = async ({ workspaceId, workspaceSlug }: { workspaceId?: string; workspaceSlug?: string }) => {
    if (workspaceId) {
    ...
    if (!process.env.NEXT_PUBLIC_IS_DUB || !process.env.EDGE_CONFIG) {
        ...
        return Object.fromEntries(Object.entries(workspaceFeatures).map(([key, _v]) => [key, true]))
    ...
    if (betaFeatures) {
        for (const [featureFlag, workspaceIdsOrSlugs] of Object.entries(betaFeatures)) {
            if (
