...
export const getTagsQuerySchema = z
    .object({
        sortBy: z.enum(['name', 'createdAt']).optional().default('name').describe('The field to sort the tags by.'),
        sortOrder: z.enum(['asc', 'desc']).optional().default('asc').describe('The order to sort the tags by.'),
...
export const createTagBodySchema = z
    .object({
        name: z.string().trim().min(1).max(50).describe('The name of the tag to create.'),
        ...
        tag: z.string().trim().min(1).describe('The name of the tag to create.').openapi({ deprecated: true })
    })
    .partial()
