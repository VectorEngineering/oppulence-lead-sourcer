...
export const workspaceIdSchema = z.object({
    workspaceId: z.string().min(1, 'Workspace ID is required.').describe('The ID of the workspace the link belongs to.')
})
...
export const WorkspaceSchema = z
    .object({
        ...
        billingCycleStart: z.number().describe('The date and time when the billing cycle starts for the workspace.'),
        paymentFailedAt: z.date().nullable().describe('The date and time when the payment failed for the workspace.'),
        stripeConnectId: z.string().nullable().describe('The Stripe Connect ID of the workspace.'),
        payoutMethodId: z
            .string()
            .nullable()
            .describe(`[BETA – ${platform.company} Partners]: The ID of the payment method for partner payouts.`),
        ...
        salesUsage: z.number().describe('The dollar amount of tracked revenue in the current billing cycle (in cents).'),
        salesLimit: z.number().describe('The limit of tracked revenue in the current billing cycle (in cents).'),
        ...
        adminRole: z.nativeEnum(AdminRole).optional().describe('The admin role of the workspace.'),
        organizationSize: z.nativeEnum(OrganizationSize).optional().describe('The organization size of the workspace.'),
        reason: z.string().optional().describe('The reason for creating the workspace.'),

        conversionEnabled: z.boolean().describe('Whether the workspace has conversion tracking enabled (d.to/conversions).'),
        dotLinkClaimed: z.boolean().describe('Whether the workspace has claimed a free .link domain. (dub.link/free)'),
        partnersEnabled: z.boolean().describe(`Whether the workspace has ${platform.company} Partners enabled.`),
        ...
        flags: z.record(z.boolean()).optional().describe('The feature flags of the workspace, indicating which features are enabled.')
    })
    .openapi({
        title: 'Workspace'
    })
