import { PayoutStatus, PayoutType } from '@dub/prisma-thinkthank/client'
import { PAYOUTS_MAX_PAGE_SIZE, PartnerSchema } from './partners'
...
export const createManualPayoutSchema = z.object({
    ...
    amount: z.preprocess((val) => parseFloat(val as string), z.number().default(0)),
    type: z.nativeEnum(PayoutType),
    description: z.string().max(190, 'Description must be less than 190 characters').nullable()
})
