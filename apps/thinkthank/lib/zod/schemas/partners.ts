import { PartnerStatus, ProgramEnrollmentStatus, SaleStatus } from '@dub/prisma-thinkthank/client'
...
export const partnersQuerySchema = z
    .object({
        ...
        sortBy: z.enum(['createdAt', 'clicks', 'leads', 'sales', 'earnings']).default('createdAt'),
...
export const PartnerSaleResponseSchema = SaleResponseSchema.omit({
    ...
    z.object({
        customer: z.object({
            email: z.string().transform((email) => email.replace(/(?<=^.).+(?=.@)/, '********')),
