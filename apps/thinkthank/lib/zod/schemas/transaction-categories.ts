...
export const getTransactionCategoriesQuerySchemaExtended = getTransactionCategoriesQuerySchema.merge(
    z.object({
...
export const createTransactionCategoryBodySchema = z.object({
    ...
    color: z.custom<TransactionCategoryColorProps>().optional().describe('The color of the category badge.'),
    icon: z.string().nullable().optional().describe('Icon identifier for the category.'),
    parentId: z.string().nullable().optional().describe('Optional parent category ID for hierarchical categories.'),
    isSystem: z.boolean().optional().default(false).describe('Whether this is a system-defined category.'),
    ...
    trackingEnabled: z.boolean().optional().default(true).describe('Whether to track budget for this category.'),
    // Tax Information
    ...
    taxDeductible: z.boolean().optional().default(false).describe('Whether expenses in this category are tax deductible.'),
    // Additional Metadata
    metadata: z.record(z.any()).nullable().optional().describe('Additional metadata for the category.')
})
