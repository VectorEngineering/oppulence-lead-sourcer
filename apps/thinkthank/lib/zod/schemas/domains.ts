...
export const DomainSchema = z.object({
    ...
    slug: z.string().describe('The domain name.').openapi({ example: 'acme.com' }),
    verified: z.boolean().default(false).describe('Whether the domain is verified.'),
    primary: z.boolean().default(false).describe('Whether the domain is the primary domain for the workspace.'),
    archived: z.boolean().describe('Whether the domain is archived.').default(false),
    placeholder: z
        .string()
        .nullable()
        .describe('Provide context to your teammates in the link creation modal by showing them an example of a link to be shortened.')
        .openapi({ example: `${platform.webUrl}/help/article/what-is-dub` }),
    expiredUrl: z
        .string()
        .nullable()
        .describe('The URL to redirect to when a link under this domain has expired.')
        .openapi({ example: 'https://acme.com/expired' }),
    notFoundUrl: z
        .string()
        .nullable()
        .describe("The URL to redirect to when a link under this domain doesn't exist.")
...
export const getDomainsQuerySchema = z
    .object({
        archived: booleanQuerySchema
            .optional()
            .default('false')
            .describe('Whether to include archived domains in the response. Defaults to `false` if not provided.'),
        search: z.string().optional().describe('The search term to filter the domains by.')
    })
...
export const createDomainBodySchema = z.object({
    ...
    expiredUrl: parseUrlSchemaAllowEmpty({ trim: true })
        .nullish()
        .transform((v) => v || null)
        .describe('Redirect users to a specific URL when any link under this domain has expired.')
        .openapi({ example: 'https://acme.com/expired' }),
    notFoundUrl: parseUrlSchemaAllowEmpty({ trim: true })
        .nullish()
        .transform((v) => v || null)
        .describe("Redirect users to a specific URL when a link under this domain doesn't exist.")
        .openapi({ example: 'https://acme.com/not-found' }),
    archived: z
        .boolean()
        .optional()
        .default(false)
        .describe('Whether to archive this domain. `false` will unarchive a previously archived domain.')
