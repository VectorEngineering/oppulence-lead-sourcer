...
export const trackSaleRequestSchema = z.object({
    ...
    paymentProcessor: z.enum(['stripe', 'shopify', 'paddle']).describe('The payment processor via which the sale was made.'),
    eventName: z
        .string()
        .max(50)
        .optional()
    ...
    invoiceId: z.string().nullish().default(null).describe('The invoice ID of the sale.'),
    currency: z.string().default('usd').describe('The currency of the sale. Accepts ISO 4217 currency codes.'),
    metadata: z.record(z.unknown()).nullish().default(null).describe('Additional metadata to be stored with the sale event.')
})
...
export const saleEventSchemaTB = clickEventSchemaTB.omit({ timestamp: true }).and(
    z.object({
...
export const saleEventResponseSchema = z
    .object({
        ...
        saleAmount: z.number().describe('Deprecated. Use `sale.amount` instead.').openapi({ deprecated: true }),
        invoice_id: z.string().describe('Deprecated. Use `sale.invoiceId` instead.').openapi({ deprecated: true }),
        payment_processor: z.string().describe('Deprecated. Use `sale.paymentProcessor` instead.')
    })
