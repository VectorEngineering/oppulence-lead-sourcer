...
export const validDateRangeForPlan = ({
    ...
    if (
        ...
        (interval === 'all' ||
            ...
            interval === '1y' ||
            interval === 'ytd' ||
            (start && (getDaysDifference(start, new Date(Date.now())) > 31 || getDaysDifference(start, end || new Date(Date.now())) > 31)))
    ) {
        if (throwError) {
    ...
    if (
        ...
        (interval === 'all' ||
            (start &&
                (getDaysDifference(start, new Date(Date.now())) > 366 || getDaysDifference(start, end || new Date(Date.now())) > 366)))
    ) {
        if (throwError) {
            throw new DubApiError({
                code: 'forbidden',
