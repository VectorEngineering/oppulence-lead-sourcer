import { clickEventResponseSchema, clickEventSchema, clickEventSchemaTBEndpoint } from '../zod/schemas/clicks'
import { leadEventResponseSchema, leadEventSchemaTBEndpoint } from '../zod/schemas/leads'
import { saleEventResponseSchema, saleEventSchemaTBEndpoint } from '../zod/schemas/sales'

import { tb } from '@/lib/tinybird'
import type { Tinybird } from '@chronark/zod-bird'
import { prisma } from '@dub/prisma-thinkthank'
import { Link } from '@dub/prisma-thinkthank/client'
...
export const getEvents = async (params: EventsFilters) => {
    let { event: eventType, workspaceId, interval, start, end, qr, trigger, region, country, isDemo, order, sortOrder } = params
    ...
    const response = await (pipe as ReturnType<Tinybird['buildPipe']>)({
        ...params,
        eventType,
        workspaceId,
        qr,
