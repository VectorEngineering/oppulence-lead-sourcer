import { ACME_WORKSPACE_ID, APP_DOMAIN_WITH_NGROK, DEFAULT_LINK_PROPS } from '@dub/utils'
...
export async function claimDotLinkDomain({ domain, workspace, userId }: { domain: string; workspace: WorkspaceWithUsers; userId: string }) {
...
const sendDomainClaimedEmails = async ({ workspace, domain }: { workspace: Pick<WorkspaceWithUsers, 'id' | 'slug'>; domain: string }) => {
