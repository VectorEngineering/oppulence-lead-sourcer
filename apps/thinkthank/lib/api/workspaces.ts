import { APP_DOMAIN_WITH_NGROK, DUB_DOMAINS_ARRAY, LEGAL_USER_ID, LEGAL_WORKSPACE_ID, R2_URL } from '@dub/utils'
...
export async function deleteWorkspace(workspace: Pick<WorkspaceProps, 'id' | 'slug' | 'logo' | 'stripeId'>) {
    await Promise.all([
        // Remove the users
...
export async function deleteWorkspaceAdmin(workspace: Pick<WorkspaceProps, 'id' | 'slug' | 'logo' | 'stripeId'>) {
...
export async function queueWorkspaceDeletion({ workspaceId, delay }: { workspaceId: string; delay?: number }) {
