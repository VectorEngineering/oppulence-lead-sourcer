...
export class DubApiError extends Error {
    ...
    constructor({ code, message, docUrl }: { code: z.infer<typeof ErrorCode>; message: string; docUrl?: string }) {
        super(message)
        this.code = code
...
export const errorSchemaFactory = (code: z.infer<typeof ErrorCode>, description: string): ZodOpenApiResponseObject => {
    return {
        description,
        content: {
            'application/json': {
...
export const exceededLimitError = ({
    ...
    return `You've reached your ${type === 'links' || type === 'AI' ? 'monthly' : ''} limit of ${limit} ${
