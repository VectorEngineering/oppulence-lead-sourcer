...
export async function bulkUpdateLinks(
    ...
    const { url, title, description, image, proxy, expiresAt, geo, tagId, tagIds, tagNames, webhookIds, ...rest } = data
        ...
        linkIds.map((linkId) =>
            prisma.link.update({
                ...
                data: {
                    ...
                    image: proxy && image && !isStored(image) ? `${R2_URL}/images/${linkIds[0]}_${imageUrlNonce}` : image,
