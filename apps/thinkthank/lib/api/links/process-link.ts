...
export async function processLink<T extends Record<string, any>>({
    ...
    workspace?: Pick<WorkspaceProps, 'id' | 'plan' | 'conversionEnabled' | 'flags'>
    userId?: string
    bulk?: boolean
...
> {
    ...
    if (!workspace || workspace.plan === 'free') {
        if (key === '_root' && url) {
            return {
                link: payload,
                error: 'You can only set a redirect for a root domain link on a Pro plan and above. Upgrade to Pro to use this feature.',
                code: 'forbidden'
            }
        }

        if (proxy || password || rewrite || expiresAt || ios || android || geo || doIndex) {
    ...
    if (domain === 'dub.sh' || domain === 'dub.link') {
        ...
        if (domain === 'dub.link' && (!workspace || workspace.plan === 'free')) {
            return {
                link: payload,
                error: 'You can only use dub.link on a Pro plan and above. Upgrade to Pro to use this domain.',
                code: 'forbidden'
            }
        }
        ...
        if (key !== '_root' && allowedHostnames && !allowedHostnames.includes(urlDomain)) {
            return {
                link: payload,
        ...
        if (registeredDomain) {
            return {
                link: payload,
                error: 'You can only use your free .link domain on a Pro plan and above. Upgrade to Pro to use this domain.',
                code: 'forbidden'
            }
        }
    }
    ...
    if (bulk) {
            ...
            if (tags.length !== tagNames.length) {
                return {
                    ...
                    error:
                        ...
                        tagNames.filter((tagName) => tags.find(({ name }) => tagName === name) === undefined).join(', '),
                    code: 'unprocessable_entity'
                }
            }
        }
        ...
        if (webhookIds && webhookIds.length > 0) {
            if (!workspace || workspace.plan === 'free' || workspace.plan === 'pro') {
                return {
                    link: payload,
                    error: 'You can only use webhooks on a Business plan and above. Upgrade to Business to use this feature.',
                    code: 'forbidden'
                }
            }
            ...
            if (webhooks.length !== webhookIds.length) {
                const invalidWebhookIds = webhookIds.filter((webhookId) => webhooks.find(({ id }) => webhookId === id) === undefined)
