...
export async function updateLinksUsage({ workspaceId, increment }: { workspaceId: string; increment: number }) {
    ...
    const sentNotification = sentEmails.some(
        (email) => email.type === (percentage >= 80 && percentage < 100 ? 'firstLinksLimitEmail' : 'secondLinksLimitEmail')
    )
    ...
    return await Promise.allSettled([
        sendLimitEmail({
            ...
            type: percentage >= 80 && percentage < 100 ? 'firstLinksLimitEmail' : 'secondLinksLimitEmail'
        }),
