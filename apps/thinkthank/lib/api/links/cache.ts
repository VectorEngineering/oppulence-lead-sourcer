...
class LinkCache {
    async mset(links: ExpandedLink[]) {
        ...
        redisLinks.map(({ domain, key, ...redisLink }) => {
            ...
            pipeline.set(this._createKey({ domain, key }), redisLink, hasWebhooks ? undefined : { ex: CACHE_EXPIRATION })
        })
    ...
    async rename({ links, oldDomain }: { links: Pick<LinkProps, 'domain' | 'key'>[]; oldDomain: string }) {
        if (links.length === 0) {
            return
        }
