import { getParamsFromURL, linkConstructor, linkConstructorSimple, truncate } from '@dub/utils'
import { ExpandedLink, checkIfLinksHaveTags, checkIfLinksHaveWebhooks, transformLink } from './utils'
    ...
    if (hasTags || hasWebhooks) {
        ...
        createdLinks = await Promise.all(
            links.map(({ tagId, tagIds, tagNames, webhookIds, ...link }) => {
                ...
                const { utm_source, utm_medium, utm_campaign, utm_term, utm_content } = getParamsFromURL(link.url)
