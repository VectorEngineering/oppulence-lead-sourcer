...
import { R2_URL, getParamsFromURL, linkConstructorSimple, nanoid, truncate } from '@dub/utils'

import { recordLink } from '@/lib/tinybird'
import { propagateWebhookTriggerChanges } from '@/lib/webhook/update-webhook'
import { prisma } from '@dub/prisma-thinkthank'
import { Prisma } from '@dub/prisma-thinkthank/client'
...
export async function updateLink({
    ...
    updatedLink: ProcessedLinkProps & Pick<LinkProps, 'id' | 'clicks' | 'lastClicked' | 'updatedAt'>
}) {
    let { id, domain, key, url, expiresAt, title, description, image, proxy, geo, publicStats } = updatedLink
    ...
    const { id: _, clicks, lastClicked, updatedAt, tagId, tagIds, tagNames, webhookIds, ...rest } = updatedLink
    ...
    const response = await prisma.link.update({
        ...
        data: {
            ...
            image: proxy && image && !isStored(image) ? `${R2_URL}/images/${id}_${imageUrlNonce}` : image,
