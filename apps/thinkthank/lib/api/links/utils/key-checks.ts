...
import { DEFAULT_REDIRECTS, isDubDomain, isReservedKeyGlobal } from '@dub/utils'
...
export async function keyChecks({
    ...
    if ((key.length === 0 || key === '_root') && workspace?.plan === 'free') {
        return {
            error: 'You can only set a redirect for your root domain on a Pro plan and above. Upgrade to Pro to unlock this feature.',
            code: 'forbidden'
        }
    }
    ...
    if (isDubDomain(domain) && process.env.NEXT_PUBLIC_IS_DUB) {
        ...
        if (domain === 'dub.link' && key.length <= 5 && (!workspace || workspace.plan === 'free' || workspace.plan === 'pro')) {
            return {
        ...
        if ((await isReservedUsername(key)) && (!workspace || workspace.plan === 'free')) {
            return {
                error: 'This is a premium key. You can only use this key on a Pro plan and above. Upgrade to Pro to register this key.',
                code: 'forbidden'
            }
        }
    }
