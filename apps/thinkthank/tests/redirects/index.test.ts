...
describe.runIf(env.CI)('Link Redirects', async () => {
    ...
    test('with passthrough query', async () => {
        const response = await fetch(`${h.baseUrl}/checkly-check-passthrough?utm_source=checkly`, fetchOptions)

        expect(response.headers.get('location')).toBe('https://www.checklyhq.com/?utm_source=checkly&utm_medium=social&utm_campaign=checks')
        expect(response.headers.get('x-powered-by')).toBe(poweredBy)
        expect(response.status).toBe(302)
    })

    test('with complex query', async () => {
        const response = await fetch(`${h.baseUrl}/checkly-check-query`, fetchOptions)
    ...
    test('with password', async () => {
        const response = await fetch(`${h.baseUrl}/password/check?pw=dub`, fetchOptions)
    ...
    test('redirection url', async () => {
        const response = await fetch(`${h.baseUrl}/redir-url-test?${REDIRECTION_QUERY_PARAM}=https://dub.co/blog`, {
            ...fetchOptions,
            headers: {}
        })
