...
async function main() {
    ...
    const leadEvents = await getLeadEvents({ customerIds }).then((res) => res.data)

    // Find clicks
    // We're fetching clicks, because leads doesn't have information about when click happened
    const clickIds = leadEvents.map((event) => event.click_id).filter(Boolean) as string[]

    const clickEvents = await getClickEvents({ clickIds }).then((res) => res.data)

    // Update customers
    const result = await Promise.all(
        customers.map(async (customer) => {
            const leadEvent = leadEvents.find((lead) => lead?.customer_id === customer.id)

            const clickEvent = clickEvents.find((click) => click?.click_id === leadEvent?.click_id)
