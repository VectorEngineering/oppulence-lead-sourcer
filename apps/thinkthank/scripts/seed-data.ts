...
async function seedLinkMetadata() {
    for (let i = 0; i < 5; i++) {
        ...
        fetch(`${process.env.TINYBIRD_API_URL}/v0/events?name=dub_links_metadata&wait=true`, {
            method: 'POST',
            headers: {
...
async function seedClicks(count = 10000) {
    const data = Array.from({ length: count }).map(() => {
        ...
        const { country, continent } = countries[Math.floor(Math.random() * countries.length)]
    ...
    const response = await fetch(`${process.env.TINYBIRD_API_URL}/v0/events?name=dub_click_events&wait=true`, {
        method: 'POST',
        headers: {
            Authorization: `Bearer ${process.env.TINYBIRD_DEMO_API_KEY}`
        },
...
async function seedLeads(count = 10) {
    ...
    const response = await fetch(`${process.env.TINYBIRD_API_URL}/v0/events?name=dub_lead_events&wait=true`, {
        method: 'POST',
        headers: {
            Authorization: `Bearer ${process.env.TINYBIRD_DEMO_API_KEY}`
        },
...
async function seedSales(count = 5) {
    ...
    const response = await fetch(`${process.env.TINYBIRD_API_URL}/v0/events?name=dub_sale_events&wait=true`, {
        method: 'POST',
        headers: {
            Authorization: `Bearer ${process.env.TINYBIRD_DEMO_API_KEY}`
        },
