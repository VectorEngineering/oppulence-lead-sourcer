...
async function main() {
    ...
    const sortedApexDomains = Object.keys(apexDomainsWithCount).sort((a, b) => apexDomainsWithCount[b] - apexDomainsWithCount[a])
    ...
    const popularApexDomains = sortedApexDomains.filter((domain) => apexDomainsWithCount[domain] >= 5)
