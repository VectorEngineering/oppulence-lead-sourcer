...
const RoadmapViewToggleContext = createContext<RoadmapViewToggleContextType | null>(null);

function useRoadmapViewToggle() {
    const context = useContext(RoadmapViewToggleContext);
    if (!context) {
        throw new Error("useRoadmapViewToggle must be used within a RoadmapViewToggleProvider");
    }
    return context;
}
...
function RoadmapViewToggleContent({ kanbanView, timelineView }: RoadmapViewToggleContentProps) {
    const searchParams = useSearchParams();
    const viewParam = searchParams?.get("view") as ViewMode | null;
    const currentView = viewParam === "timeline" ? "timeline" : "kanban";
    ...
    return (
        <div>
            {currentView === "kanban" ? kanbanView : timelineView}
        </div>
    );
}
