...
export default function TableOfContents({ content, className }: TableOfContentsProps) {
    ...
    return (
        <div className={cn("w-full", className)}>
            <div className="sticky top-24 rounded-xl bg-white p-6 dark:bg-gray-900">
                <div className="mb-6 flex items-center gap-2">
                    <BookOpen className="h-5 w-5 text-black dark:text-white" />
                    <h3 className="text-xl font-semibold text-black dark:text-white">Table of Contents</h3>
                </div>

                <div className="mb-4 h-1 w-20 bg-gradient-to-r from-black to-gray-500 dark:from-white dark:to-gray-500"></div>

                <nav className="max-h-[calc(100vh-200px)] overflow-y-auto pr-2 scrollbar-thin scrollbar-thumb-gray-300 dark:scrollbar-thumb-gray-700">
                    <div className="space-y-2">
                        {headings.map((heading) => (
                            <button
                                ...
                                className={cn(
                                    "block w-full text-left text-sm transition-all duration-200 hover:text-black dark:hover:text-white",
                                    ...
                                    activeId === heading.id
                                        ? "text-black dark:text-white font-medium border-l-2 border-black dark:border-white pl-[calc(0.5rem+var(--pl,0px))]"
                                        : "text-gray-500 dark:text-gray-400 border-l-2 border-transparent pl-[calc(0.5rem+var(--pl,0px))]",
                                    ...
                                    "py-1.5 hover:bg-gray-50 dark:hover:bg-gray-800 rounded-r-md"
                                )}
                            >
                                {heading.text}
                            </button>
