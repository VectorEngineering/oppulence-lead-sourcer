...
const normalizeParams = (
  params?: LogParams | Record<string, unknown> | Error,
): LogParams => {
  if (!params) return {};
...
const formatLog = (
  ...
  const traceId = normalizedParams?.traceId
    ? `(trace: ${normalizedParams.traceId})`
    : "";

  return {
    message:
      `${timestamp} ${level.toUpperCase()} ${source} ${message} ${traceId}`.trim(),
    data: {
...
export const logger = {
  ...
  debug: (
    message: string,
    params?: LogParams | Record<string, unknown> | Error,
  ) => logWithLevel("debug", message, params),
  ...
  info: (
    message: string,
    params?: LogParams | Record<string, unknown> | Error,
  ) => logWithLevel("info", message, params),
  ...
  warn: (
    message: string,
    params?: LogParams | Record<string, unknown> | Error,
  ) => logWithLevel("warn", message, params),
  ...
  error: (
    message: string,
    params?: LogParams | Record<string, unknown> | Error,
  ) => logWithLevel("error", message, params),
};
