...
export const getLeadAndErrorCounts = authenticatedAction
    ...
    `
        ...
        const leadCountMap = new Map(leadCounts.map((lc) => [lc.createdAt.toISOString().split('T')[0], lc._count.id]))

        const errorCountMap = new Map(errorCounts.map((ec) => [ec.createdAt.toISOString().split('T')[0], ec._count.id]))
