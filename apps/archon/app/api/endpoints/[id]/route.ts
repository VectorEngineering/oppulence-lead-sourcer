...
export async function POST(
  ...
  try {
    ...
    if (!planResult?.data) {
      return NextResponse.json(
        { message: "Could not determine user plan." },
        { status: 500 },
      );
    }
    ...
    if (!leadCountResult?.data) {
      return NextResponse.json(
        { message: "Could not determine lead count." },
        { status: 500 },
      );
    }
    ...
    if (endpoint.data?.webhookEnabled && endpoint.data?.webhook) {
      ...
      const webhookFetchPromise: Promise<Response> = fetch(
        endpoint.data?.webhook,
        {
          method: "POST",
          headers: {
  ...
  try {
    ...
    if (!parsedData.success) {
      ...
      return NextResponse.redirect(
        new URL(endpoint?.data?.failUrl || referer || "/fail"),
      );
    }
    ...
    return NextResponse.redirect(
      new URL(endpoint?.data?.successUrl || referer || "/success"),
    );
  } catch (error: unknown) {
    await createLog({
      postType: "http",
