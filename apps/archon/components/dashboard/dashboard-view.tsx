...
const Links = () => {
  return (
    <>
      {navLinks.map((link) => (
        <Link
          className="bg-background hover:bg-accent/75 rounded-lg border p-4 transition-all"
          key={link.href}
          href={link.href}
        >
          {link.name}
...
const DashboardView = async () => {
  ...
  const [charts, leads, endpoints, usage] = await Promise.all([
    getLeadAndErrorCounts(),
    getLeads(),
    getEndpoints(),
    getUsageForUser(),
  ]);
    ...
    <div className="grid grid-cols-3 gap-4">
      ...
      {usage?.data?.usage.plan !== "enterprise" && (
        <Usage
          ...
          used={usage?.data?.usage.leadCount || 0}
          plan={usage?.data?.usage.plan || ""}
        />
      )}
      <Links />
      <div className="mt-8">
