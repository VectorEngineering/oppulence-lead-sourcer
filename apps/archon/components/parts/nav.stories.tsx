...
const MockImage = ({
    ...
    height: number
    alt: string
}) => <div className={className} style={{ width, height, backgroundColor: '#ccc' }} aria-label={alt} />
 ...
 */
const AccountWidget = ({ plan }: { plan: string }) => <div className='rounded border p-4'>Account Widget (Plan: {plan || 'Free'})</div>
...
const otherLinks = [{ href: '/portal/support', text: 'Support', icon: LifeBuoy }]
...
const NavLink = ({ href, children, icon: Icon, className }: NavLinkProps) => {
    return (
        <MockLink className={`group -ml-2 flex items-center gap-2 rounded-md p-2 transition-all ${className}`} href={href}>
            <Icon className='text-muted-foreground group-hover:text-foreground transition-all' size={20} />
            {children}
        </MockLink>
    )
}
...
const Nav = ({ plan = 'free' }) => {
    return (
        <nav className='flex h-screen flex-col justify-between gap-4 p-4'>
            <MockLink href='/' className='bg-muted/50 flex items-center gap-2 rounded-lg border p-6'>
                <MockImage
