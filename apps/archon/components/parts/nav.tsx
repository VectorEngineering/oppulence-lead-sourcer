...
export default async function Nav() {
    ...
    return (
        <nav className='flex h-screen flex-col justify-between gap-4 p-4'>
            <Link href='/' className='bg-muted/50 flex items-center gap-2 rounded-lg border p-6'>
                <Image className='-mt-px mb-px dark:invert' src={Logo} width={100} height={18.53} alt={`${BusinessConfig.name} Wordmark`} />
            </Link>
...
const NavLink = ({ href, children, icon: Icon, className }: NavLinkProps) => {
    return (
        <Link className={`group -ml-2 flex items-center gap-2 rounded-md p-2 transition-all ${className}`} href={href}>
