...
import { Activity, Bell, Calendar, CircuitBoardIcon, Contact, HelpCircle, PhoneForwardedIcon, Search } from 'lucide-react'
        ...
        <TooltipProvider>
            <Tooltip>
                <TooltipTrigger asChild>
                    <button type='button' onClick={handleClick} onKeyUp={(e) => e.key === 'Enter' && handleClick()}>
                ...
                <TooltipContent side='top' className='flex items-center gap-2'>
                    ...
                    {item.route && <span className='text-muted-foreground text-xs'>→ {item.route}</span>}
                </TooltipContent>
            </Tooltip>
        </TooltipProvider>
    )
...
function HelpDockButton({ sections }: HelpDockButtonProps) {
    return (
        <Popover>
            ...
            <PopoverContent className='w-[320px] p-0 shadow-xl' align='end' sideOffset={16}>
                <div className='relative overflow-hidden rounded-md'>
                    ...
                    <div className='border-t border-neutral-200 bg-neutral-50 p-4 dark:border-neutral-800 dark:bg-neutral-950/50'>
                        <h4 className='text-foreground mb-2 text-sm font-medium'>Navigation Tips</h4>
 ...
 */
function useKeyboardShortcuts(onSectionChange: (section: PlatformSection) => void, sections: PlatformSection[]) {
    const router = useRouter()
...
export function AppleStyleDock() {
    ...
    const [activeSection, setActiveSection] = useState(PLATFORM_SECTIONS[0].title)
            ...
            <div className='absolute bottom-[8%] left-1/2 max-w-full -translate-x-1/2'>
                <Dock className='flex justify-center space-x-1 rounded-2xl border border-black/10 px-4 py-2 backdrop-blur-xl'>
                    {PLATFORM_SECTIONS.map((item, index) => (
                        <DockButton key={item.title} item={item} shortcut={`⌘${index + 1}`} onClick={() => handleSectionChange(item)} />
                    ))}
