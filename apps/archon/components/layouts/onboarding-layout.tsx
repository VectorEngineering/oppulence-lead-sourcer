                ...
                <div className='relative w-[300px] border-r border-gray-200 p-6'>
                    ...
                    <div className='space-y-6'>
                        {steps.map((step, index) => (
                            <div key={step.title} className={`flex gap-3 ${currentStep === index ? 'opacity-100' : 'opacity-60'}`}>
                                <div className='mt-1'>
                                    <div className='flex h-8 w-8 items-center justify-center rounded-full bg-gray-100'>{step.icon}</div>
                                </div>
                                <div>
                            ...
                            {steps.map((_, index) => (
                                <div
                                    ...
                                    className={`h-1 w-16 rounded-full ${
                                        index === currentStep ? 'bg-primary text-secondary' : 'bg-gray-200 text-gray-500'
                                    }`}
                                />
                            ))}
                        </div>
