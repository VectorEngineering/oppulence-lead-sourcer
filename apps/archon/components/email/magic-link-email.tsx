...
export default function MagicLinkEmail({ url, host }: { url: string; host: string }) {
    return (
        <>
            ...
            <p className='my-4 opacity-50'>If you didn&apos;t try to login, you can safely ignore this email.</p>
            <a href={BusinessConfig.platformUrl} target='_blank' className='underline' rel='noreferrer'>
                {BusinessConfig.name}
            </a>
        </>
    )
