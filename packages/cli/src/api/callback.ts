...
export function oauthCallbackServer({ oauthClient, redirectUri, codeVerifier, spinner }: OAuthCallbackServerProps) {
    const server = http.createServer(async (req, res) => {
        ...
        try {
            ...
            const { accessToken, refreshToken, expiresAt } = await oauthClient.authorizationCode.getToken({
                code,
                redirectUri,
                codeVerifier
            })
