    ...
    run: async () => {
        ...
        let workspaces = await db.query.workspaces.findMany({
            where: (table, { isNotNull, isNull, not, eq, and }) =>
                and(isNotNull(table.stripeCustomerId), isNotNull(table.subscriptions), not(eq(table.plan, 'free')), isNull(table.deletedAt))
        })
        // hack to filter out workspaces with `{}` as subscriptions
        workspaces = workspaces.filter((ws: { subscriptions: {} }) => ws.subscriptions && Object.keys(ws.subscriptions).length > 0)
        ...
        return {
            workspaceIds: workspaces.map((w: { id: any }) => w.id)
        }
    }
})
