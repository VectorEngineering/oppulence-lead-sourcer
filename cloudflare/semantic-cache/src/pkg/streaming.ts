...
import type { CacheError } from '@playbookmedia/cache'
import { BaseError, Err, Ok, wrap, type Result } from '@playbookmedia/error'

import type { Context } from './hono/app'
import { createCompletionChunk, OpenAIResponse, parseMessagesToString } from './util'
...
export async function handleNonStreamingRequest(
    ...
    const chatCompletion = await wrap(openai.chat.completions.create(request), (err) => new OpenAiError({ message: err.message }))
    if (chatCompletion.err) {
        return c.text(chatCompletion.err.message, { status: 400 })
    }
...
async function createEmbeddings(c: Context, text: string): Promise<Result<AiTextEmbeddingsOutput, CloudflareAiError>> {
...
async function updateCache(
    ...
    const vectorizeRes = await wrap(
        c.env.VECTORIZE_INDEX.insert([{ id, values: vector as number[], metadata: { response, tokens } }]),
        (err) => new CloudflareVectorizeError({ message: err.message })
    )
    if (vectorizeRes.err) {
