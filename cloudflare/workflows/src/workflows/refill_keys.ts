...
export class RefillRemaining extends WorkflowEntrypoint<Env, Params> {
    async run(event: WorkflowEvent<Params>, step: WorkflowStep) {
        ...
        const lastDayOfMonth = new Date(event.timestamp.getFullYear(), event.timestamp.getMonth() + 1, 0).getDate()
        ...
        for (const key of keys) {
            const bucketId = await step.do(`fetch bucketId for ${key.workspaceId}::${BUCKET_NAME}`, async () => {
                const found = await db.query.auditLogBucket.findFirst({
                    where: (table, { eq, and }) => and(eq(table.workspaceId, key.workspaceId), eq(table.name, BUCKET_NAME)),
                    columns: {
                        id: true
                    }
                })
