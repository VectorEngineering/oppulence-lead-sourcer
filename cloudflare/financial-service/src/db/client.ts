...
export class DatabaseClient {
     ...
     * });
     */
    public async executeQuery<T>(queryFn: (db: DrizzleDB) => Promise<T>): Promise<T> {
        try {
    ...
    public async executeTransaction<T>(
        transactionFn: (
            tx: SQLiteTransaction<'async', D1Result<unknown>, typeof schema, ExtractTablesWithRelations<typeof schema>>
        ) => Promise<T>
    ): Promise<T> {
        try {
