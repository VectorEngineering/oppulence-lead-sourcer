...
export class PlaidApi {
    ...
    constructor(params: Omit<ProviderParams, 'provider'>) {
        ...
        const configuration = new Configuration({
            basePath: PlaidEnvironments[params.envs.PLAID_ENVIRONMENT || 'production'],
            baseOptions: {
                headers: {
     ...
     */
    async getAccountBalance({ accessToken, accountId }: GetAccountBalanceRequest): Promise<GetAccountBalanceResponse | undefined> {
        try {
     ...
     * @throws {ProviderError} If an error occurs during the API call.
     */
    async getAccounts({ accessToken, institutionId }: GetAccountsRequest): Promise<GetAccountsResponse | undefined> {
        try {
            const accounts = await this.#client.accountsGet({
                access_token: accessToken
            })
     ...
     * @throws {ProviderError} If an error occurs during the API call.
     */
    async getTransactions({ accessToken, accountId, syncCursor, maxCalls = 15 }: GetTransactionsRequest): Promise<GetTransactionsResponse> {
    ...
    async linkTokenCreate({
        ...
        try {
            ...
            return response as AxiosResponse<LinkTokenCreateResponse>
        } catch (error: unknown) {
    ...
    async itemPublicTokenExchange({
        publicToken
    }: ItemPublicTokenExchangeRequest): Promise<import('axios').AxiosResponse<ItemPublicTokenExchangeResponse>> {
        const response = await this.#client.itemPublicTokenExchange({
            public_token: publicToken
        })
        return response as AxiosResponse<ItemPublicTokenExchangeResponse>
    }
    ...
    async getInstitutions(params?: GetInstitutionsRequest) {
        const countryCode = params?.countryCode ? [params.countryCode as CountryCode] : this.#countryCodes
     ...
     * @throws {ProviderError} If an error occurs during the API call.
     */
    async getStatements({ accessToken }: GetStatementsRequest): Promise<GetStatementsResponse> {
        try {
            const response = await this.#client.statementsList({
                access_token: accessToken
            })

            const { accounts, institution_id, item_id, institution_name } = response.data
    ...
    async getRecurringTransactions({
        accessToken,
        accountId
    }: GetRecurringTransactionsRequest): Promise<GetRecurringTransactionsResponse | undefined> {
        try {
