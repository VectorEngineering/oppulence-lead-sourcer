 ...
 */
export const mapTransactionCategory = ({ transaction, amount }: MapTransactionCategory): string | null => {
    ...
    if (
        ...
        transaction.personal_finance_category?.detailed === 'RENT_AND_UTILITIES_SEWAGE_AND_WASTE_MANAGEMENT' ||
 ...
 */
export const transformTransaction = ({ transaction }: TransformTransactionPayload): BaseTransaction => {
 ...
 */
export const transformAccount = ({ account_id, name, balances, institution, type }: TransformAccount): BaseAccount => {
    return {
        id: account_id,
        name,
 ...
 */
export const transformAccountBalance = (balances?: TransformAccountBalance): BaseBalance => ({
    currency: balances?.iso_currency_code || balances?.unofficial_currency_code || 'USD',
 ...
 */
const transformRecurringTransactionFrequency = (frequency: RecurringTransactionFrequency) => {
    switch (frequency) {
        case 'WEEKLY':
            return 'weekly'
        case 'MONTHLY':
 ...
 */
export const transformRecurringTransactionAmount = (amount: TransactionStreamAmount): RecurringTransactionAmount => {
    return {
 ...
 */
export const transformRecurringTransactionStatus = (status: TransactionStreamStatus): RecurringTransactionStatus => {
    switch (status) {
        case 'MATURE':
            return 'mature'
 ...
 */
export const transformRecurringTransaction = (transaction: TransformRecurringTransaction): RecurringTransaction => {
    // transform the recurrent transaction frequency
    const frequency = transformRecurringTransactionFrequency(transaction.frequency)

    // convert average_amount to amount
    const average_amount = transformRecurringTransactionAmount(transaction.average_amount)

    // convert last_amount to amount
    const last_amount = transformRecurringTransactionAmount(transaction.last_amount)
    ...
    const personal_finance_category = transformRecurringTransactionCategory(transaction.personal_finance_category)
