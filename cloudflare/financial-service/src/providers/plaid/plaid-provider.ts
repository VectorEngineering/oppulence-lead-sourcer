...
export class PlaidProvider implements Provider {
     ...
     * @throws {Error} If accessToken or accountId is missing.
     */
    async getRecurringTransactions({ accessToken, accountId }: GetRecurringTransactionsRequest): Promise<GetRecurringTransactionsResponse> {
        if (!accessToken || !accountId) {
            throw Error('accessToken or accountId is missing')
        }
        ...
        const incomingTransactions = recurringInflow?.map(transformRecurringTransaction) ?? []
        const outgoingTransactions = recurringOutflow?.map(transformRecurringTransaction) ?? []
