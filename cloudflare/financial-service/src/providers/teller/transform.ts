...
import type { Account as BaseAccount, Balance as BaseAccountBalance, Transaction as BaseTransaction } from '../types'
import type {
    FormatAmount,
    Transaction,
    TransformAccount,
...
export const mapTransactionCategory = ({ transaction, amount }: MapTransactionCategory) => {
    // Priority checks
    ...
    const hasKeyword = (keywords: string[]) => keywords.some((keyword) => description?.includes(keyword))
...
export const transformTransaction = ({ transaction, accountType }: TransformTransaction): BaseTransaction => {
...
export const transformAccount = ({ id, name, currency, enrollment_id, type, institution, balance }: TransformAccount): BaseAccount => {
    return {
        id,
        name,
        currency,
...
export const transformAccountBalance = (account: TransformAccountBalance): BaseAccountBalance => ({
