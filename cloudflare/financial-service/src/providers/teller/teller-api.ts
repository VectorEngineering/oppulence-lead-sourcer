...
export class TellerApi {
    ...
    async getTransactions({ accountId, accessToken, latest, count }: GetTransactionsRequest): Promise<GetTransactionsResponse> {
        const result = await this.#get<GetTransactionsResponse>(`/accounts/${accountId}/transactions`, accessToken, {
    ...
    async getAccountBalance({ accountId, accessToken }: GetAccountBalanceRequest): Promise<GetAccountBalanceResponse> {
        ...
        const amount = transactions.find((transaction) => transaction.running_balance !== null)?.running_balance
    ...
    async #get<TResponse>(path: string, token?: string, params?: Record<string, string | number | undefined>): Promise<TResponse> {
