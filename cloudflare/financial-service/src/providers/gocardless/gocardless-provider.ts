...
import { transformAccount, transformAccountBalance, transformInstitution, transformTransaction } from './transform'
...
export class GoCardLessProvider implements Provider {
    ...
    async getTransactions({ accountId, latest }: GetTransactionsRequest): Promise<GetTransactionsResponse> {
        ...
        const data = (response ?? []).map((transaction) => transformTransaction(transaction, accountId))
