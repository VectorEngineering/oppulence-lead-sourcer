...
export class GoCardLessApi {
    ...
    async #getRefreshToken(refresh: string): Promise<string> {
        const response = await this.#post<GetRefreshTokenResponse>('/api/v2/token/refresh/', undefined, {
            refresh
        })
    ...
    async getAccountBalance(accountId: string): Promise<GetAccountBalanceResponse['balances'][0]['balanceAmount'] | undefined> {
        const token = await this.#getAccessToken()

        try {
            const { balances } = await this.#get<GetAccountBalanceResponse>(`/api/v2/accounts/${accountId}/balances/`, token)

            const foundAccount = balances?.find((account) => account.balanceType === 'interimAvailable')
    ...
    async getInstitutions(params?: GetInstitutionsRequest): Promise<GetInstitutionsResponse> {
        ...
        const response = await this.#get<GetInstitutionsResponse>('/api/v2/institutions/', token, undefined, {
            params: {
                country: countryCode
            }
        })
    ...
    async buildLink({ institutionId, agreement, redirect }: PostRequisitionsRequest): Promise<PostRequisitionsResponse> {
        const token = await this.#getAccessToken()
    ...
    async createEndUserAgreement({
        ...
        return this.#post<PostCreateAgreementResponse>('/api/v2/agreements/enduser/', token, {
            institution_id: institutionId,
    ...
    async getTransactions({
        accountId,
        latest
    }: GetTransactionsRequest): Promise<GetTransactionsResponse['transactions']['booked'] | undefined> {
        const token = await this.#getAccessToken()
    ...
    async getRequestion(id: string): Promise<GetRequisitionResponse | undefined> {
        try {
            ...
            return this.#get<GetRequisitionResponse>(`/api/v2/requisitions/${id}/`, token)
        } catch (error) {
            const parsedError = isError(error)
    ...
    async #get<TResponse>(path: string, token?: string, params?: Record<string, string>, config?: XiorRequestConfig): Promise<TResponse> {
        const api = await this.#getApi(token)

        return api.get<TResponse>(path, { params, ...config }).then(({ data }) => data)
    }

    async #post<TResponse>(path: string, token?: string, body?: unknown, config?: XiorRequestConfig): Promise<TResponse> {
        const api = await this.#getApi(token)
