...
export class Provider {
    ...
    async getHealthCheck(params: Omit<ProviderParams, 'provider'>): Promise<GetHealthCheckResponse> {
    ...
    async getAccountBalance(params: GetAccountBalanceRequest) {
        logger('getAccountBalance:', `provider: ${this.#name} id: ${params.accountId}`)

        const data = await withRetry(() => this.#provider?.getAccountBalance(params))
    ...
    async getRecurringTransactions(params: GetRecurringTransactionsRequest) {
        ...
        const data = await withRetry(() => this.#provider?.getRecurringTransactions(params))
