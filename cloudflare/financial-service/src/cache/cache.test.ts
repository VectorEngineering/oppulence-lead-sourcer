...
describe('ServiceCache', () => {
    ...
    describe('getOrSet', () => {
        ...
        it('should handle concurrent calls correctly', async () => {
            ...
            const promises = [cache.getOrSet('test-key', fetchFn), cache.getOrSet('test-key', fetchFn), cache.getOrSet('test-key', fetchFn)]
    ...
    it('should clear all cache entries', async () => {
        ...
        // Verify data was set
        const beforeClear = await Promise.all(entries.map(([key]) => cache.get(key)))
        ...
        const afterClear = await Promise.all(entries.map(([key]) => cache.get(key)))
