...
export type V1GetInstitutionApiResponse = z.infer<(typeof route.responses)[200]['content']['application/json']['schema']>
...
export const registerV1GetInstitutionApi = (app: App) => {
    app.openapi(route, async (c) => {
        ...
        try {
            ...
            const resultString: string = typeof result === 'string' ? result : JSON.stringify(result)
            ...
            return c.json(
                {
                    data: data.hits?.map(({ document }) => ({
                        ...
                        available_history: document.available_history ? +document.available_history : null,
