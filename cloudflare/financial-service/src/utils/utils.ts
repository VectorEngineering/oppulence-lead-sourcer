...
class TestDataGenerator {
    ...
    private generateSeededRandom(): number {
        const hash = Array.from(this.seed + this.counter.toString()).reduce((hash, char) => {
    ...
    generateEmail(
        ...
        const { prefix = 'test', domain = 'example.com', length = 8 } = options
    ...
    generateUsername(
        ...
        const { prefix = 'user', length = 8 } = options
    ...
    generateName(
        ...
        const { firstName = true, lastName = true } = options

        const firstNames = ['John', 'Jane', 'Alice', 'Bob', 'Charlie', 'David', 'Eva', 'Frank', 'Grace', 'Henry']
...
export { TestDataGenerator, generator as testDataGenerator }
