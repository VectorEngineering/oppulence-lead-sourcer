...
export async function getPlaidInstitutions() {
    ...
    return data.map((institution) => {
        return {
            ...
            name: institution.name,
            logo: institution.logo ? getLogoURL(institution.institution_id) : matchLogoURL(institution.institution_id),
...
export async function getInstitutions() {
    const data = await Promise.all([getGoCardLessInstitutions(), getTellerInstitutions(), getPlaidInstitutions()])
