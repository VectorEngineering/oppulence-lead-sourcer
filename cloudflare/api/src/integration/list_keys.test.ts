...
test(
    ...
    async (t) => {
        ...
        expect(createApiResponse.status, `status mismatch - ${JSON.stringify(createApiResponse)}`).toEqual(200)
        expect(createApiResponse.body.apiId).toBeDefined()
        expect(createApiResponse.headers).toHaveProperty('unkey-request-id')
        ...
        for (let i = 0; i < 10; i++) {
            ...
            if (i % 2 === 0) {
                ...
                expect(deleteKeyResponse.status, `status mismatch - ${JSON.stringify(deleteKeyResponse)}`).toEqual(200)
