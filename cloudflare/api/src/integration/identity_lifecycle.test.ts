...
test('create new identity, update it, add ratelimits and verify associated keys', async (t) => {
     ...
     */
    const getIdentityResponse = await fetch(`${h.baseUrl}/v1/identities.getIdentity?identityId=${identityId}`, {
        method: 'GET',
        headers: {
            Authorization: `Bearer ${root.key}`
        }
    ...
    expect(verifiedWithRatelimitsResponse.status, `expected 200, got: ${verifiedWithRatelimitsResponse.status} - ${body}`).toBe(200)
