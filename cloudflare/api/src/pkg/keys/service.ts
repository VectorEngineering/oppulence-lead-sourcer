...
import { BaseError, Err, FetchError, Ok, type Result, SchemaError, wrap } from '@playbookmedia/error'
import { sha256 } from '@playbookmedia/hash'
import type { PermissionQuery, RBAC } from '@playbookmedia/rbac'
import type { Logger } from '@playbookmedia/worker-logging'
...
type InvalidResponse = {
    ...
    code: 'FORBIDDEN' | 'RATE_LIMITED' | 'EXPIRED' | 'USAGE_EXCEEDED' | 'DISABLED' | 'INSUFFICIENT_PERMISSIONS'
    key: Key
    identity: {
        id: string
        externalId: string
...
export class KeyService {
    ...
    ): Promise<Result<VerifyKeyResult, SchemaError | FetchError | DisabledWorkspaceError | MissingRatelimitError>> {
        try {
    ...
    private async getData(hash: string) {
        ...
        if (dbRes.ratelimitAsync !== null && dbRes.ratelimitDuration !== null && dbRes.ratelimitLimit !== null) {
            ratelimits.default = {
                name: 'default',
    ...
    ): Promise<Result<VerifyKeyResult, FetchError | SchemaError | DisabledWorkspaceError | MissingRatelimitError>> {
