...
import { auditLogSchemaV1, unkeyAuditLogEvents } from '@playbookmedia/auditlog/src/auditlog'
...
export async function insertGenericAuditLogs(
    ...
    for (const log of arr) {
        ...
        let { val: bucket, err } = await cache.auditLogBucketByWorkspaceIdAndName.swr(cacheKey, async () => {
            ...
            const bucket = await db.readonly.query.auditLogBucket.findFirst({
                where: (table, { eq, and }) => and(eq(table.workspaceId, log.workspaceId), eq(table.name, log.bucket))
            })
