...
export function metrics(): MiddlewareHandler<HonoEnv> {
    return async (c, next) => {
        ...
        requestBody = requestBody.replaceAll(/"plaintext":\s*"[a-zA-Z0-9_]+"/g, '"plaintext": "<REDACTED>"')
        ...
        try {
            ...
            responseBody = responseBody.replaceAll(/"plaintext":\s*"[a-zA-Z0-9_]+"/g, '"plaintext": "<REDACTED>"')
