...
import { ConsoleLogger } from '@playbookmedia/worker-logging'
...
export async function migrateKey(message: MessageBody, env: Env): Promise<Result<{ keyId: string }, MigrationError>> {
    ...
    if (message.roles && message.roles.length > 0) {
        const found = await db.query.roles.findMany({
            where: (table, { inArray, and, eq }) => and(eq(table.workspaceId, message.workspaceId), inArray(table.name, message.roles!))
        })
    ...
    if (message.permissions && message.permissions.length > 0) {
        ...
        const missingRoles = message.permissions.filter((name) => !found.some((permission) => permission.name === name))
        if (missingRoles.length > 0) {
            return Err(
                new MigrationError({
                    message: `Roles ${JSON.stringify(missingRoles)} are missing, please create them first`
