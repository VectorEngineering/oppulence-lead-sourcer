...
import { type Api, type Database, type KeyAuth, type Permission, type Role, type Workspace, drizzle, eq, schema } from '../db'
...
export abstract class Harness {
    ...
    private async teardown(): Promise<void> {
        const deleteWorkspaces = async () => {
            for (const workspaceId of [this.resources.userWorkspace.id, this.resources.unkeyWorkspace.id]) {
                await this.db.primary.delete(schema.workspaces).where(eq(schema.workspaces.id, workspaceId))
            }
        }
        for (let i = 1; i <= 5; i++) {
            try {
    ...
    public async createKey(opts?: {
        ...
        for (const role of opts?.roles ?? []) {
            const { id: roleId } = await this.optimisticUpsertRole(this.resources.userWorkspace.id, role.name)
            ...
            for (const permissionName of role.permissions ?? []) {
                const permission = await this.optimisticUpsertPermission(this.resources.userWorkspace.id, permissionName)
