...
export type V1MigrationsEnqueueKeysRequest = z.infer<(typeof route.request.body.content)['application/json']['schema']>
export type V1MigrationsEnqueueKeysResponse = z.infer<(typeof route.responses)[202]['content']['application/json']['schema']>
    ...
    app.openapi(route, async (c) => {
        ...
        const api = await cache.apiById.swr(req.apiId, async () =>
            db.readonly.query.apis.findFirst({
                where: (table, { and, eq }) => and(eq(table.workspaceId, authorizedWorkspaceId), eq(table.id, req.apiId)),
                with: {
                    keyAuth: true
                }
            })
