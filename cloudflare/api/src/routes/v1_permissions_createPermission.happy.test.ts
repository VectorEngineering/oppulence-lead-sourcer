...
import type { V1PermissionsCreatePermissionRequest, V1PermissionsCreatePermissionResponse } from './v1_permissions_createPermission'

test('creates new permission', async (t) => {
    const h = await IntegrationHarness.init(t)
    const root = await h.createRootKey(['rbac.*.create_permission'])
    const res = await h.post<V1PermissionsCreatePermissionRequest, V1PermissionsCreatePermissionResponse>({
