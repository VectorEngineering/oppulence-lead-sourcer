...
import { eq, inArray, schema } from '@playbookmedia/db'
...
export type V1IdentitiesDeleteIdentityRequest = z.infer<(typeof route.request.body.content)['application/json']['schema']>
export type V1IdentitiesDeleteIdentityResponse = z.infer<(typeof route.responses)[200]['content']['application/json']['schema']>
...
export const registerV1IdentitiesDeleteIdentity = (app: App) =>
    app.openapi(route, async (c) => {
        ...
        const auth = await rootKeyAuth(
            c,
            buildUnkeyQuery(({ or }) => or('identity.*.delete_identity', `identity.${identityId}.delete_identity`))
        )
