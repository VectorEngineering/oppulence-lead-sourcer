...
export type V1PermissionsGetRoleResponse = z.infer<(typeof route.responses)[200]['content']['application/json']['schema']>
export const registerV1PermissionsGetRole = (app: App) =>
    app.openapi(route, async (c) => {
        ...
        const role = await db.readonly.query.roles.findFirst({
            where: (table, { eq, and }) => and(eq(table.workspaceId, auth.authorizedWorkspaceId), eq(table.id, roleId))
        })
        if (!role) {
