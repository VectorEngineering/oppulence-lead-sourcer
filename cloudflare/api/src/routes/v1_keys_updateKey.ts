...
import { type Key, schema } from '@playbookmedia/db'
import { eq } from '@playbookmedia/db'
...
export type V1KeysUpdateKeyRequest = z.infer<(typeof route.request.body.content)['application/json']['schema']>
export type V1KeysUpdateKeyResponse = z.infer<(typeof route.responses)[200]['content']['application/json']['schema']>
...
export const registerV1KeysUpdate = (app: App) =>
    app.openapi(route, async (c) => {
        ...
        const rbacRes = rbac.evaluatePermissions(
            buildUnkeyQuery(({ or }) => or('*', 'api.*.update_key', `api.${key.keyAuth.api.id}.update_key`)),
            auth.permissions
        )
        if (rbacRes.err) {
            throw new UnkeyApiError({
        ...
        await Promise.all([
            typeof req.roles !== 'undefined' ? setRoles(c, auth, req.keyId, req.roles) : Promise.resolve(),
            typeof req.permissions !== 'undefined' ? setPermissions(c, auth, req.keyId, req.permissions) : Promise.resolve()
        ])
