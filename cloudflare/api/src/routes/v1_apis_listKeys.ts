...
import type { Identity } from '@playbookmedia/db'
...
export type V1ApisListKeysResponse = z.infer<(typeof route.responses)[200]['content']['application/json']['schema']>

export const registerV1ApisListKeys = (app: App) =>
    app.openapi(route, async (c) => {
        const { apiId, revalidateKeysCache, limit, cursor, externalId, ownerId, decrypt } = c.req.valid('query')
        const { cache, db, rbac, vault } = c.get('services')

        const auth = await rootKeyAuth(
            c,
            buildUnkeyQuery(({ or, and }) =>
                or('*', and(or('api.*.read_key', `api.${apiId}.read_key`), or('api.*.read_api', `api.${apiId}.read_api`)))
            )
        )
        ...
        if (externalId) {
            const { val, err } = await cache.identityByExternalId.swr(externalId, async () => {
                return db.readonly.query.identities.findFirst({
                    where: (table, { and, eq }) => and(eq(table.externalId, externalId), eq(table.workspaceId, auth.authorizedWorkspaceId))
                })
            })
            if (err) {
                throw new UnkeyApiError({
        ...
        async function loadKeys() {
            const keySpace = await db.readonly.query.keyAuth.findFirst({
                where: (table, { and, eq, isNull }) => and(eq(table.id, api!.keyAuthId!), isNull(schema.keys.deletedAt)),
                        ...
                        where: (table, { and, isNull, gt, eq }) =>
                            and(
                                ...[
                                    ...
                                    identity ? eq(table.identityId, identity.id) : ownerId ? eq(table.ownerId, ownerId) : undefined
                                ].filter(Boolean)
                            ),
                        with: {
                            identity: true,
