...
import { type PermissionQuery, buildQuery } from '@playbookmedia/rbac'
...
describe.each<TestCase>([
    ...
    test(
        ...
        async (t) => {
            ...
            expect(res.status, `exptected ${expected.status}, received: ${JSON.stringify(res, null, 2)}`).toEqual(expected.status)
            expect(res.body.valid, `key is ${res.body.valid ? 'valid' : 'not valid'}, received body: ${JSON.stringify(res.body)}`).toBe(
                expected.valid
            )
        },
        { timeout: 60_000 }
