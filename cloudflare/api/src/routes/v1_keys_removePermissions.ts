...
export type V1KeysRemovePermissionsRequest = z.infer<(typeof route.request.body.content)['application/json']['schema']>
export type V1KeysRemovePermissionsResponse = z.infer<(typeof route.responses)[200]['content']['application/json']['schema']>
    ...
    app.openapi(route, async (c) => {
        ...
        const [key, connectedPermissions] = await Promise.all([
            db.primary.query.keys.findFirst({
                where: (table, { and, eq }) => and(eq(table.workspaceId, auth.authorizedWorkspaceId), eq(table.id, req.keyId))
            }),
