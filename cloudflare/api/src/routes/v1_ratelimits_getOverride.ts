...
export type V1RatelimitGetOverrideResponse = z.infer<(typeof route.responses)[200]['content']['application/json']['schema']>
...
export const registerV1RatelimitGetOverride = (app: App) =>
    app.openapi(route, async (c) => {
        ...
        const namespace = await db.primary.query.ratelimitNamespaces.findFirst({
            where: (table, { eq, and }) =>
                and(eq(table.workspaceId, authorizedWorkspaceId), namespaceId ? eq(table.id, namespaceId) : eq(table.name, namespaceName!)),
            with: {
                overrides: {
