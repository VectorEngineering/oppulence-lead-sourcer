...
import { type EncryptedKey, type Key, type KeyPermission, type KeyRole, schema } from '@playbookmedia/db'
...
export type V1MigrationsCreateKeysRequest = z.infer<(typeof route.request.body.content)['application/json']['schema']>
export type V1MigrationsCreateKeysResponse = z.infer<(typeof route.responses)[200]['content']['application/json']['schema']>
    ...
    app.openapi(route, async (c) => {
        ...
        if (roleNames.length > 0) {
            const found = await db.primary.query.roles.findMany({
                where: (table, { inArray, and, eq }) => and(eq(table.workspaceId, authorizedWorkspaceId), inArray(table.name, roleNames))
            })
        ...
        const permissionNames = req.filter((k) => k.permissions).flatMap((k) => k.permissions) as string[]
        // name -> id
        ...
        if (permissionNames.length > 0) {
            ...
            const missingPermissions = permissionNames.filter((name) => !found.some((permission) => permission.name === name))
                ...
                const { val: api, err } = await cache.apiById.swr(key.apiId, async () => {
                    return (
                        (await db.readonly.query.apis.findFirst({
                            where: (table, { eq, and, isNull }) => and(eq(table.id, key.apiId), isNull(table.deletedAt)),
                            with: {
                                keyAuth: true
                            }
                        })) ?? null
