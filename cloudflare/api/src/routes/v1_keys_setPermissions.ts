...
import { and, eq, inArray, schema } from '@playbookmedia/db'
...
export type V1KeysSetPermissionsRequest = z.infer<(typeof route.request.body.content)['application/json']['schema']>
export type V1KeysSetPermissionsResponse = z.infer<(typeof route.responses)[200]['content']['application/json']['schema']>
...
export async function setPermissions(
    ...
    const requestedNames = requested.filter(({ name }) => !!name).map(({ name, create }) => ({ name: name!, create: create! }))

    const [key, existingPermissions, connectedPermissions] = await Promise.all([
        db.primary.query.keys.findFirst({
            where: (table, { eq, and }) => and(eq(table.workspaceId, auth.authorizedWorkspaceId), eq(table.id, keyId))
        }),
        ...
        await db.primary.query.keysPermissions.findMany({
            where: (table, { eq, and }) => and(eq(table.workspaceId, auth.authorizedWorkspaceId), eq(table.keyId, keyId)),
            with: {
                permission: {
                    columns: {
                        name: true
    ...
    const addPermissions = allPermissions.filter((ar) => !connectedPermissions.some((cp) => cp.permissionId === ar.id))
    ...
    c.executionCtx.waitUntil(Promise.all([cache.keyById.remove(key.id), cache.keyByHash.remove(key.hash)]))
    const auditLogs = [
