...
import { buildUnkeyQuery } from '@playbookmedia/rbac'
...
export type V1KeysCreateKeyRequest = z.infer<(typeof route.request.body.content)['application/json']['schema']>
export type V1KeysCreateKeyResponse = z.infer<(typeof route.responses)[200]['content']['application/json']['schema']>
...
export const registerV1KeysCreateKey = (app: App) =>
    app.openapi(route, async (c) => {
        ...
        const [permissionIds, roleIds, identity] = await Promise.all([
            ...
            externalId ? upsertIdentity(db.primary, authorizedWorkspaceId, externalId) : Promise.resolve(null)
        ])
...
async function getPermissionIds(db: Database, workspaceId: string, permissionNames: Array<string>): Promise<Array<string>> {
    if (permissionNames.length === 0) {
        return []
    }
    const permissions = await db.query.permissions.findMany({
        where: (table, { inArray, and, eq }) => and(eq(table.workspaceId, workspaceId), inArray(table.name, permissionNames)),
        columns: {
            id: true,
            name: true
        }
    ...
    if (permissions.length < permissionNames.length) {
        const missingPermissions = permissionNames.filter((name) => !permissions.some((permission) => permission.name === name))
        throw new UnkeyApiError({
            code: 'PRECONDITION_FAILED',
            message: `Permissions ${JSON.stringify(missingPermissions)} are missing, please create them first`
        })
    }
    return permissions.map((r) => r.id)
}

async function getRoleIds(db: Database, workspaceId: string, roleNames: Array<string>): Promise<Array<string>> {
    if (roleNames.length === 0) {
        return []
    }
    const roles = await db.query.roles.findMany({
        where: (table, { inArray, and, eq }) => and(eq(table.workspaceId, workspaceId), inArray(table.name, roleNames)),
        columns: {
            id: true,
            name: true
        }
...
export async function upsertIdentity(db: Database, workspaceId: string, externalId: string): Promise<Identity> {
    let identity = await db.query.identities.findFirst({
        where: (table, { and, eq }) => and(eq(table.workspaceId, workspaceId), eq(table.externalId, externalId))
    })
    if (identity) {
        return identity
    }
