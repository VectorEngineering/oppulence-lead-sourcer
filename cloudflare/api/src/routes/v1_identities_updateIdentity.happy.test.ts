...
import type { V1IdentitiesUpdateIdentityRequest, V1IdentitiesUpdateIdentityResponse } from './v1_identities_updateIdentity'
...
test('can be identified via externalId', async (t) => {
    ...
    for (const rl of ratelimits) {
        expect(found.some((f) => f.name === rl.name && f.limit === rl.limit && f.duration === rl.duration))
    }
})
...
test('works with thousands of keys', { timeout: 300_000 }, async (t) => {
    ...
    for (const keyId of keyIds) {
        ...
        for (const rl of ratelimits) {
            expect(key!.identity!.ratelimits.some((r) => r.name === rl.name && r.limit === rl.limit && r.duration === rl.duration))
        }
    }
})
