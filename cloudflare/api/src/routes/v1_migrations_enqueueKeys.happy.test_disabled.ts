...
describe('with metadata', () => {
    test('creates 100 keys', async (t) => {
        ...
        const root = await h.createRootKey([`api.${h.resources.userApi.id}.create_key`, `api.${h.resources.userApi.id}.encrypt_key`])
...
test('creates and encrypts a key', async (t) => {
    ...
    const root = await h.createRootKey([`api.${h.resources.userApi.id}.create_key`, 'api.*.encrypt_key'])
