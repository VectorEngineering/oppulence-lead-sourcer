...
import { buildUnkeyQuery, type unkeyPermissionValidation } from '@playbookmedia/rbac'
    ...
    request: {
        query: z.object({
            ...
            start: z.coerce.number().int().optional().openapi({
                description: 'The start of the period to fetch usage for as unix milliseconds timestamp, defaults to 24h ago.',
                example: 1620000000000
            }),
            end: z.coerce.number().int().optional().openapi({
                description: 'The end of the period to fetch usage for as unix milliseconds timestamp, defaults to now.',
                example: 1620000000000
            }),
...
export type V1KeysGetVerificationsResponse = z.infer<(typeof route.responses)[200]['content']['application/json']['schema']>
export const registerV1KeysGetVerifications = (app: App) =>
    app.openapi(route, async (c) => {
        ...
        if (keyId) {
            ...
            const keys = await cache.keysByOwnerId.swr(ownerId, async () => {
                const dbRes = await db.readonly.query.keys.findMany({
                    where: (table, { eq, and, isNull }) => and(eq(table.ownerId, ownerId), isNull(table.deletedAt)),
                    with: {
                        encrypted: true,
                        keyAuth: {
                            with: {
        ...
        const auth = await rootKeyAuth(
            ...
            buildUnkeyQuery(({ or, and }) =>
                or(
                    '*',
                    'api.*.read_key',
                    and(...apiIds.map((apiId) => `api.${apiId}.read_key` satisfies z.infer<typeof unkeyPermissionValidation>))
                )
            )
        )
        ...
        return c.json({
            verifications: Object.entries(verifications).map(([time, { success, rateLimited, usageExceeded }]) => ({
