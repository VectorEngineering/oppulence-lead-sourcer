...
import type { V1KeysSetPermissionsRequest, V1KeysSetPermissionsResponse } from './v1_keys_setPermissions'
...
test('not desired permissions are removed', async (t) => {
    const h = await IntegrationHarness.init(t)
    const root = await h.createRootKey(['rbac.*.create_permission', 'rbac.*.remove_permission_from_key', 'rbac.*.add_permission_to_key'])
