...
export type V1PermissionsGetPermissionResponse = z.infer<(typeof route.responses)[200]['content']['application/json']['schema']>
export const registerV1PermissionsGetPermission = (app: App) =>
    app.openapi(route, async (c) => {
        ...
        const permission = await db.readonly.query.permissions.findFirst({
            where: (table, { eq, and }) => and(eq(table.workspaceId, auth.authorizedWorkspaceId), eq(table.id, permissionId))
        })
        if (!permission) {
