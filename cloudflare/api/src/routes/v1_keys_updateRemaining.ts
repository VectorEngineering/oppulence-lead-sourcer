...
export type V1KeysUpdateRemainingRequest = z.infer<(typeof route.request.body.content)['application/json']['schema']>
export type V1KeysUpdateRemainingResponse = z.infer<(typeof route.responses)[200]['content']['application/json']['schema']>
...
export const registerV1KeysUpdateRemaining = (app: App) =>
    app.openapi(route, async (c) => {
        ...
        await Promise.all([usageLimiter.revalidate({ keyId: key.id }), cache.keyByHash.remove(key.hash), cache.keyById.remove(key.id)])
