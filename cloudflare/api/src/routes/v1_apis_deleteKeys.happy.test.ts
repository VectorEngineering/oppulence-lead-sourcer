...
test('deletes the keys', async (t) => {
    ...
    expect(softDeleteRes.status, `expected 200, received: ${JSON.stringify(softDeleteRes)}`).toBe(200)
    ...
    expect(hardDeleteRes.status, `expected 200, received: ${JSON.stringify(hardDeleteRes)}`).toBe(200)
