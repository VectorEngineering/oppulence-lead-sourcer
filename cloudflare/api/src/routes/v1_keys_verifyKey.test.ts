...
describe('disabled workspace', () => {
    test('should reject the request', async (t) => {
        const h = await IntegrationHarness.init(t)
        await h.db.primary.update(schema.workspaces).set({ enabled: false }).where(eq(schema.workspaces.id, h.resources.userWorkspace.id))
...
describe('key is soft deleted', () => {
    test('returns NOT_FOUND', async (t) => {
        ...
        await h.db.primary.update(schema.keys).set({ deletedAt: new Date() }).where(eq(schema.keys.id, key.keyId))
...
describe('key exists but keyspace is soft deleted', () => {
    test('returns NOT_FOUND', async (t) => {
        ...
        await h.db.primary.update(schema.keyAuth).set({ deletedAt: new Date() }).where(eq(schema.keyAuth.id, h.resources.userKeyAuth.id))
...
describe('key exists but api is soft deleted', () => {
    test('returns NOT_FOUND', async (t) => {
        ...
        await h.db.primary.update(schema.apis).set({ deletedAt: new Date() }).where(eq(schema.apis.id, h.resources.userApi.id))
