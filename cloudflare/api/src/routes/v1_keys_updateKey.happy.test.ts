...
describe('update roles', () => {
    test('creates all missing roles', async (t) => {
        const h = await IntegrationHarness.init(t)
        const root = await h.createRootKey(['rbac.*.create_permission', 'api.*.update_key', 'rbac.*.add_role_to_key', 'rbac.*.create_role'])
        ...
        const found = await h.db.primary.query.roles.findFirst({
            where: (table, { and, eq }) => and(eq(table.workspaceId, h.resources.userWorkspace.id), eq(table.name, name))
        })
        expect(found).toBeDefined()
    })
    ...
    test('additional roles does not remove existing roles', async (t) => {
        const h = await IntegrationHarness.init(t)
        const root = await h.createRootKey(['rbac.*.create_permission', 'rbac.*.add_role_to_key', 'api.*.update_key'])
...
describe('update permissions', () => {
    test('creates all missing permissions', async (t) => {
        const h = await IntegrationHarness.init(t)
        const root = await h.createRootKey(['rbac.*.create_permission', 'api.*.update_key', 'rbac.*.add_permission_to_key'])
    ...
    test('additional permissions does not remove existing permissions', async (t) => {
        const h = await IntegrationHarness.init(t)
        const root = await h.createRootKey(['rbac.*.create_permission', 'rbac.*.add_permission_to_key', 'api.*.update_key'])
...
test('update ratelimit should not disable it', async (t) => {
    ...
    const root = await h.createRootKey([`api.${h.resources.userApi.id}.create_key`, `api.${h.resources.userApi.id}.update_key`])
