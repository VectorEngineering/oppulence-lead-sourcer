...
export type V1KeysAddPermissionsRequest = z.infer<(typeof route.request.body.content)['application/json']['schema']>
export type V1KeysAddPermissionsResponse = z.infer<(typeof route.responses)[200]['content']['application/json']['schema']>
    ...
    app.openapi(route, async (c) => {
        ...
        const [key, existingPermissions, connectedPermissions] = await Promise.all([
            db.primary.query.keys.findFirst({
                where: (table, { eq, and }) => and(eq(table.workspaceId, auth.authorizedWorkspaceId), eq(table.id, req.keyId))
            }),
            ...
            await db.primary.query.keysPermissions.findMany({
                where: (table, { eq, and }) => and(eq(table.workspaceId, auth.authorizedWorkspaceId), eq(table.keyId, req.keyId))
            })
        ])
        ...
        const addPermissions = allPermissions.filter((ap) => !connectedPermissions.some((cp) => cp.permissionId === ap.id))
