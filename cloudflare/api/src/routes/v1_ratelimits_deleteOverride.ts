...
import { eq, schema } from '@playbookmedia/db'
...
export type V1RatelimitDeleteOverrideRequest = z.infer<(typeof route.request.body.content)['application/json']['schema']>
export type V1RatelimitDeleteOverrideResponse = z.infer<(typeof route.responses)[200]['content']['application/json']['schema']>
    ...
    app.openapi(route, async (c) => {
        ...
        await db.primary.transaction(async (tx) => {
            const namespace = await db.primary.query.ratelimitNamespaces.findFirst({
                ...
                with: {
                    overrides: {
                        where: (table, { eq, and, isNull }) => and(isNull(table.deletedAt), eq(table.identifier, identifier))
                    }
                }
            })
            ...
            await tx.update(schema.ratelimitOverrides).set({ deletedAt: new Date() }).where(eq(schema.ratelimitOverrides.id, override.id))
