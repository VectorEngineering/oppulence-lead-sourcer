...
import { and, eq } from '@playbookmedia/db'
...
export type V1ApisDeleteApiRequest = z.infer<(typeof route.request.body.content)['application/json']['schema']>
export type V1ApisDeleteApiResponse = z.infer<(typeof route.responses)[200]['content']['application/json']['schema']>
    ...
    app.openapi(route, async (c) => {
        ...
        await db.primary.transaction(async (tx) => {
            ...
            const keyIds = await tx.query.keys.findMany({
                where: (table, { eq, and, isNull }) => and(eq(table.keyAuthId, api.keyAuthId!), isNull(table.deletedAt)),
                columns: {
                    id: true
                }
            })
