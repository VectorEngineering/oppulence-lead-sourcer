...
import type { V1MigrationsEnqueueKeysRequest, V1MigrationsEnqueueKeysResponse } from './v1_migrations_enqueueKeys'
...
describe('correct roles', () => {
    describe.each([
        ...
        test('returns 202', async (t) => {
            ...
            const root = await h.createRootKey(roles.map((role) => (typeof role === 'string' ? role : role(apiId))))
...
describe('encrypting requires permissions', () => {
    describe.each([
        ...
        },
        {
            ...
            status: 202,
            roles: [(apiId: string) => `api.${apiId}.encrypt_key`, (apiId: string) => `api.${apiId}.create_key`]
        },
        {
        ...
        test('encrypting requires permissions', async (t) => {
            ...
            const root = await h.createRootKey(roles.map((role) => (typeof role === 'string' ? role : role(h.resources.userApi.id))))
            ...
            expect(res.status, `expected ${status}, received: ${JSON.stringify(res, null, 2)}`).toBe(status)
        })
    })
})
