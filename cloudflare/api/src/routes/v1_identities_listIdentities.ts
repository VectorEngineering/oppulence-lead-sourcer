...
import { and, eq, gt, sql } from '@playbookmedia/db'
...
export type V1IdentitiesListIdentitiesResponse = z.infer<(typeof route.responses)[200]['content']['application/json']['schema']>
...
export const registerV1IdentitiesListIdentities = (app: App) =>
    app.openapi(route, async (c) => {
        ...
        const auth = await rootKeyAuth(
            c,
            buildUnkeyQuery(({ or }) => or('*', 'identity.*.read_identity', `identity.${req.environment}.read_identity`))
        )
        ...
        const [identities, total] = await Promise.all([
            ...
            db.readonly
                ...
                .where(
                    and(eq(schema.identities.workspaceId, auth.authorizedWorkspaceId), eq(schema.identities.environment, req.environment))
                )
