...
import { type RatelimitNamespace, schema } from '@playbookmedia/db'
...
export type V1RatelimitLimitRequest = z.infer<(typeof route.request.body.content)['application/json']['schema']>
export type V1RatelimitLimitResponse = z.infer<(typeof route.responses)[200]['content']['application/json']['schema']>
...
export const registerV1RatelimitLimit = (app: App) =>
    app.openapi(route, async (c) => {
        ...
        const { val, err } = await cache.ratelimitByIdentifier.swr(
            ...
            async () => {
                const dbRes = await db.readonly.query.ratelimitNamespaces.findFirst({
                    where: (table, { eq, and }) => and(eq(table.workspaceId, rootKey.authorizedWorkspaceId), eq(table.name, req.namespace)),
                    columns: {
                        id: true,
                        workspaceId: true
                    },
                    ...
                    try {
                        ...
                        if (e instanceof DatabaseError && e.body.message.includes('desc = Duplicate entry')) {
                            ...
                            namespace = (await db.readonly.query.ratelimitNamespaces.findFirst({
                                where: (table, { eq, and }) =>
                                    and(eq(table.name, req.namespace), eq(table.workspaceId, rootKey.authorizedWorkspaceId))
                            }))!
                        } else {
                            throw e
                        }
        ...
        const authResult = rbac.evaluatePermissions(
            buildUnkeyQuery(({ or }) => or('*', 'ratelimit.*.limit', `ratelimit.${val.namespace.id}.limit`)),
            rootKey.permissions ?? []
        )
        if (authResult.err) {
            throw new UnkeyApiError({
        ...
        const shard =
            sharding === 'edge'
                ? // @ts-ignore - this is a bug in the types
                  (c.req.raw?.cf?.colo as string | undefined)
                : 'global'
