...
import type { V1RatelimitDeleteOverrideRequest, V1RatelimitDeleteOverrideResponse } from './v1_ratelimits_deleteOverride'
...
describe('correct roles', () => {
    describe.each([{ name: 'delete override', roles: ['ratelimit.*.delete_override'] }])('$name', ({ roles }) => {
        test('returns 200', async (t) => {
            ...
            const found = await h.db.primary.query.ratelimitOverrides.findFirst({
                where: (table, { eq, and, isNull }) => and(isNull(table.deletedAt), eq(table.id, overrideId))
            })
...
describe('incorrect roles', () => {
    describe.each([{ name: 'delete override', roles: ['ratelimit.*.create_override'] }])('$name', ({ roles }) => {
        test('returns 403', async (t) => {
            ...
            expect(res.status, `expected status 403, received: ${JSON.stringify(res, null, 2)}`).toEqual(403)
