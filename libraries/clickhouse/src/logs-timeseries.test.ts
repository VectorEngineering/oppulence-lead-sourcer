...
function generateTimeBasedData(n: number, workspaceId: string) {
    ...
    return Array.from({ length: n }).map(() => {
        const timeRange = Math.random() < 0.6 ? intervals.hour : Math.random() < 0.8 ? intervals.day : intervals.week
        ...
        return {
            ...
            ip_address: `${Math.floor(Math.random() * 256)}.${Math.floor(Math.random() * 256)}.${Math.floor(
                Math.random() * 256
