...
async extractData(data: AnalyzeResultOperationOutput) {
    ...
    const customerName =
      fields?.CustomerName?.valueString
        ? capitalCase(fields.CustomerName.valueString)
        : null;
    ...
    const previousUnpaidBalance =
      fields?.PreviousUnpaidBalance?.valueNumber ?? null;
    ...
    const hasAllEssentials = essentialFields.every(
      (field) => result[field as keyof typeof result] !== null
    );
