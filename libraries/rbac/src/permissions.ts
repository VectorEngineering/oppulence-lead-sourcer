...
export function buildIdSchema(prefix: string) {
    return z.string().refine((s) => {
        ...
        const regex = new RegExp(`^${prefix}_[123456789ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz]{8,32}$`)
        return regex.test(s)
    })
}
...
export const identityActions = z.enum(['create_identity', 'read_identity', 'update_identity', 'delete_identity'])
