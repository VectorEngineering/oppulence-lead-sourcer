import { type PermissionQuery, buildQuery } from './queries.js'
...
describe('evaluating a query', () => {
    const testCases: {
        ...
        },
        {
            ...
            query: buildQuery(({ and, or }) => or(and('admin', or('user', and('guest', 'moderator'))), 'editor')),
            permissions: ['admin', 'user', 'guest', 'moderator', 'editor', 'viewer'],
            valid: true
        },
        {
            name: 'Multiple levels of nesting (Fail)',
            query: buildQuery(({ and, or }) => or(and('admin', or('developer', and('guest', 'moderator'))), 'viewer')),
            permissions: ['user', 'guest', 'moderator', 'editor'],
            valid: false
        },
        {
            name: "Complex combination of 'and' and 'or' at different levels (Pass)",
            query: buildQuery(({ and, or }) => or(and('admin', or('user', and('guest', 'moderator'))), and('editor', 'viewer'))),
            permissions: ['admin', 'user', 'guest', 'moderator', 'editor', 'viewer'],
            valid: true
        },
        {
            name: "Complex combination of 'and' and 'or' at different levels (Fail)",
            query: buildQuery(({ and, or }) => or(and('admin', or('developer', and('guest', 'moderator'))), and('editor', 'developer'))),
            permissions: ['user', 'guest', 'moderator', 'editor', 'viewer'],
            valid: false
        },
        {
            name: "Deep nesting of 'and' and 'or'(Pass)",
            query: buildQuery(({ and, or }) => and('admin', or('user', and('guest', or('moderator', 'editor'))))),
            permissions: ['admin', 'user', 'guest', 'moderator', 'editor', 'viewer'],
            valid: true
        },
        {
            name: "Deep nesting of 'and' and 'or' (Fail)",
            query: buildQuery(({ and, or }) => and('admin', or('developer', and('guest', or('moderator', 'editor'))))),
            permissions: ['user', 'guest', 'moderator', 'editor', 'viewer'],
            valid: false
        }
    ]
...
describe('bad queries', () => {
    test('catch empty {}', () => {
        ...
        const errorObj = JSON.parse(res.err!.message)
