...
function getRedirectURL() {
    return `https://dashboard.stripe.com/${STRIPE_MODE === 'test' ? 'test/' : ''}apps-oauth/dub.co`
}

// Returns the authorization URL
export function getOAuthUrl({ state, challenge }: { state: string; challenge: string }) {
...
export async function getToken({ code, verifier }: { code: string; verifier: string }) {
    ...
    try {
        ...
        console.error(`Unable to retrieve ${platform.company} access token:`, (e as Error).message)
    }
}
