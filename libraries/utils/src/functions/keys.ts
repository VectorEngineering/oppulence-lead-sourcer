...
export const isUnsupportedKey = (key: string) => {
    ...
    return excludedPrefix.some((prefix) => key.startsWith(prefix)) || excludedSuffix.some((suffix) => key.endsWith(suffix))
}

export const isReservedKeyGlobal = (key: string) => {
    const reservedKeys = ['favicon.ico', 'sitemap.xml', 'robots.txt', 'manifest.webmanifest', 'manifest.json', 'apple-app-site-association']
