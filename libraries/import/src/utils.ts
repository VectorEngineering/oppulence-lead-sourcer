...
function getAdjustedDate(date: Date | string, dateAdjustment?: number) {
    ...
    if (dateAdjustment) {
        return dateAdjustment > 0 ? addDays(dateObj, Math.abs(dateAdjustment)) : subDays(dateObj, Math.abs(dateAdjustment))
    }
 ...
 * ```
 */
export function formatDate(date: string, timezone = 'America/New_York', dateAdjustment?: number) {
    const formats = [
        'dd/MM/yyyy',
        'yyyy-MM-dd',
        'MM/dd/yyyy',
