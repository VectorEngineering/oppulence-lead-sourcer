...
const firstNames = ['Charles', 'Cooper', 'Johhny', 'Elvis', 'John', 'Jane', 'Joe', 'Jack', 'Jill', 'Jenny']
            ...
            <div className='flex w-[80vw] items-center justify-center'>
                <Table>
                    ...
                    <Table.Body>
                        {fakeData.map((order) => {
                            return (
                                <Table.Row key={order.id} className='[&_td:last-child]:w-[1%] [&_td:last-child]:whitespace-nowrap'>
                                    <Table.Cell>{order.displayId}</Table.Cell>
                                    <Table.Cell>{order.customer}</Table.Cell>
                                    <Table.Cell>{order.email}</Table.Cell>
                                    <Table.Cell className='text-right'>{formatCurrency(order.amount, order.currency)}</Table.Cell>
                                    <Table.Cell className='text-ui-fg-muted'>{order.currency}</Table.Cell>
                                </Table.Row>
                            )
                        })}
                    </Table.Body>
