...
import { CheckCircleSolid, CircleDottedLine, CircleHalfSolid } from '@playbookmedia/icons'
...
interface IndicatorProps extends Omit<React.ComponentPropsWithoutRef<'span'>, 'children'> {
    /**
     * The current status.
     */
...
const ProgressIndicator = ({ status, className, ...props }: IndicatorProps) => {
    ...
    return (
        <span className={clx('text-ui-fg-muted group-data-[state=active]/trigger:text-ui-fg-interactive', className)} {...props}>
            <Icon />
        </span>
    )
}
...
const ProgressTabsTrigger = React.forwardRef<React.ElementRef<typeof ProgressTabsPrimitives.Trigger>, ProgressTabsTriggerProps>(
...
>(({ className, ...props }, ref) => <ProgressTabsPrimitives.List ref={ref} className={clx('flex items-center', className)} {...props} />)
...
>(({ className, ...props }, ref) => {
    return <ProgressTabsPrimitives.Content ref={ref} className={clx('outline-none', className)} {...props} />
})
