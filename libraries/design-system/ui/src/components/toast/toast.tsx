...
import { CheckCircleSolid, ExclamationCircleSolid, InformationCircleSolid, Spinner, XCircleSolid, XMark } from '@playbookmedia/icons'
...
export const Toast = ({
    ...
    return (
        <div className='shadow-elevation-flyout bg-ui-bg-component flex w-fit min-w-[360px] max-w-[440px] gap-x-3 overflow-hidden rounded-lg p-3'>
            ...
            >
                ...
                <div className='flex flex-col gap-y-3'>
                    <div className='flex flex-col'>
                        <span className='txt-compact-small-plus text-ui-fg-base'>{title}</span>
                        {description && <span className='txt-small text-ui-fg-subtle text-pretty'>{description}</span>}
                    </div>
                    {!!action && (
                        <button
                            type='button'
            ...
            {!!dismissable && (
                <IconButton size='2xsmall' variant='transparent' type='button' onClick={() => toast.dismiss(id)}>
                    <XMark />
                </IconButton>
            )}
        </div>
