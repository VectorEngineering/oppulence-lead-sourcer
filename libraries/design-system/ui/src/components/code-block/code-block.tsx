...
const useCodeBlockContext = () => {
    ...
    if (context === null) throw new Error('useCodeBlockContext can only be used within a CodeBlockContext')
...
const Root = ({
    ...
    return (
        <CodeBlockContext.Provider value={{ snippets, active, setActive }}>
            <div
                className={clx('bg-ui-contrast-bg-base shadow-elevation-code-block flex flex-col overflow-hidden rounded-xl', className)}
                {...props}
            >
                {children}
            </div>
...
const HeaderComponent = ({
    ...
    React.useEffect(() => {
        const activeTabRef = tabRefs.current.find((ref) => ref?.dataset.label === active.label)
        ...
        if (activeTabRef && tabIndicatorRef.current) {
            ...
            tabIndicatorRef.current.style.left = prevTabRef
                ? `${tabRefs.current.slice(0, activeTabIndex).reduce((total, tab) => total + (tab?.offsetWidth || 0) + 12, 0) + 15}px`
                : '15px'
        }
    }, [active])
    ...
    return (
        <div>
            <div className={clx('flex items-start px-4 pt-2.5', className)} {...props}>
                {!hideLabels &&
            ...
            <div className='w-full px-0.5'>
                <div className='bg-ui-contrast-border-top relative h-px w-full'>
                    <div
                        ref={tabIndicatorRef}
                        className={clx('absolute bottom-0 transition-all motion-reduce:transition-none', 'duration-150 ease-linear')}
                    >
...
const Meta = ({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) => {
    return <div className={clx('txt-compact-small text-ui-contrast-fg-secondary ml-auto', className)} {...props} />
}
        ...
        <div>
            {showToolbar && (
                <div className='border-ui-contrast-border-bot flex min-h-10 items-center gap-x-3 border-t px-4 py-2'>
                    <div className='code-body text-ui-contrast-fg-secondary flex-1'>{children}</div>
                    {!active.hideCopy && <Copy content={active.code} className='text-ui-contrast-fg-secondary' />}
                </div>
            )}
                                ...
                                >
                                    {!active.hideLineNumbers && (
                                        <div role='presentation' className='flex flex-col text-right'>
                                            {tokens.map((_, i) => (
                                                <span key={i} className='text-ui-contrast-fg-secondary tabular-nums'>
                                                    {i + 1}
                                                </span>
                                            ))}
                                        </div>
