...
import { Spinner } from '@playbookmedia/icons'
...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
    (
            ...
            if (isLoading) {
                return (
                    <span className='pointer-events-none'>
                        <div className={clx('bg-ui-bg-disabled absolute inset-0 flex items-center justify-center rounded-md')}>
        ...
        return (
            <Component ref={ref} {...props} className={clx(buttonVariants({ variant, size }), className)} disabled={disabled || isLoading}>
                {renderInner()}
            </Component>
        )
    }
