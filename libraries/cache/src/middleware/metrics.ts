...
export function withMetrics(metrics: Metrics<Metric>): StoreMiddleware<any, any> {
    function wrap<TNamespace extends string, TValue>(store: Store<TNamespace, TValue>): Store<TNamespace, TValue> {
...
class StoreWithMetrics<TNamespace extends string, TValue> implements Store<TNamespace, TValue> {
    ...
    public async get(namespace: TNamespace, key: string): Promise<Result<Entry<TValue> | undefined, CacheError>> {
        ...
        this.metrics.emit({
            ...
            status: res.val ? (now <= res.val.freshUntil ? 'fresh' : now <= res.val.staleUntil ? 'stale' : undefined) : undefined,
