...
export class CloudflareStore<TNamespace extends string, TValue = any> implements Store<TNamespace, TValue> {
    ...
    private createCacheKey(namespace: TNamespace, key: string): URL {
        return new URL(`https://${this.config.domain}/cache/${this.config.cacheBuster ?? 'v1'}/${String(namespace)}/${key}`)
    }

    public async get(namespace: TNamespace, key: string): Promise<Result<Entry<TValue> | undefined, CacheError>> {
        let res: Response
        try {
    ...
    public async set(namespace: TNamespace, key: string, entry: Entry<TValue>): Promise<Result<void, CacheError>> {
    ...
    public async remove(namespace: TNamespace, keys: string | string[]): Promise<Result<void, CacheError>> {
        const cacheKeys = (Array.isArray(keys) ? keys : [keys]).map((key) => this.createCacheKey(namespace, key))

        // @ts-expect-error I don't know why this is not working
        await Promise.all(cacheKeys.map((key) => caches.default.delete(key)))

        return await fetch(`https://api.cloudflare.com/client/v4zones/${this.config.zoneId}/purge_cache`, {
            method: 'POST',
            headers: {
