...
export class SwrCache<TNamespace extends string, TValue> {
    ...
    constructor(ctx: Context, store: Store<TNamespace, TValue | undefined>, fresh: number, stale: number) {
        this.ctx = ctx
        this.store = store
        this.fresh = fresh
        this.stale = stale
     ...
     */
    public async get(namespace: TNamespace, key: string): Promise<Result<TValue | undefined, CacheError>> {
    ...
    public async swr(
        ...
        if (typeof value !== 'undefined') {
            if (revalidate) {
                this.ctx.waitUntil(
                    this.deduplicateLoadFromOrigin(namespace, key, loadFromOrigin).then((res) => this.set(namespace, key, res))
                )
            }
