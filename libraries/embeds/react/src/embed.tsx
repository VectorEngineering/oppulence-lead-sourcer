...
export const DubEmbed = memo(({ token, options, ...rest }: DubEmbedProps) => <DubInline options={{ ...options, token }} {...rest} />)

function DubInline({ options, ...rest }: { options: DubEmbedOptions } & HTMLProps<HTMLDivElement>) {
    const id = useId()
