...
class SmallBusinessAccountingFactors {
     ...
     */
    private calculateFactors(startDate?: Date, endDate?: Date): AccountingFactors {
     ...
     */
    private sumTransactions(transactions: PlaidAccountTransaction[], categories: string[]): number {
        return transactions
    ...
    private getCurrentLiabilities(transactions: PlaidAccountTransaction[]): number {
    ...
    private getCash(transactions: PlaidAccountTransaction[]): number {
        return this.sumTransactions(transactions, ['TRANSFER_IN_DEPOSIT', 'TRANSFER_IN_SAVINGS'])
    }
    ...
    private getDebtToEquityRatio(transactions: PlaidAccountTransaction[]): number {
    ...
    private getInventoryTurnover(transactions: PlaidAccountTransaction[]): number {
    ...
    private getDaysInventoryOutstanding(transactions: PlaidAccountTransaction[]): number {
        const inventoryTurnover = this.getInventoryTurnover(transactions)
        return inventoryTurnover !== 0 ? 365 / inventoryTurnover : 0
    }

    private getReceivablesTurnover(transactions: PlaidAccountTransaction[]): number {
        const netCreditSales = this.getRevenue(transactions) // Simplified
        const averageAccountsReceivable = this.sumTransactions(transactions, ['TRANSFER_IN_OTHER_TRANSFER_IN']) // Simplified
        return averageAccountsReceivable !== 0 ? netCreditSales / averageAccountsReceivable : 0
    }

    private getDaysSalesOutstanding(transactions: PlaidAccountTransaction[]): number {
    ...
    private getPayablesTurnover(transactions: PlaidAccountTransaction[]): number {
        ...
        return averageAccountsPayable !== 0 ? totalSupplierPurchases / averageAccountsPayable : 0
    }

    private getDaysPayablesOutstanding(transactions: PlaidAccountTransaction[]): number {
        const payablesTurnover = this.getPayablesTurnover(transactions)
        return payablesTurnover !== 0 ? 365 / payablesTurnover : 0
    }

    private getCashConversionCycle(transactions: PlaidAccountTransaction[]): number {
        return (
    ...
    private getGrossProfitMargin(transactions: PlaidAccountTransaction[]): number {
        const revenue = this.getRevenue(transactions)
        const costOfGoodsSold = this.getExpenses(transactions) // Simplified
        return revenue !== 0 ? (revenue - costOfGoodsSold) / revenue : 0
    }

    private getOperatingProfitMargin(transactions: PlaidAccountTransaction[]): number {
        const revenue = this.getRevenue(transactions)
    ...
    private getReturnOnEquity(transactions: PlaidAccountTransaction[]): number {
        const netIncome = this.getRevenue(transactions) - this.getExpenses(transactions)
        const equity = this.getCurrentAssets(transactions) - this.getCurrentLiabilities(transactions)
    ...
    private getCashFlowToDebtRatio(transactions: PlaidAccountTransaction[]): number {
    ...
    public compareFactors(
        ...
        period2Start: Date,
        period2End: Date
    ): Record<keyof AccountingFactors, { period1: number; period2: number; change: number }> {
        ...
        for (const key in factors1) {
            comparison[key] = {
                ...
                change: factors2[key as keyof AccountingFactors] - factors1[key as keyof AccountingFactors]
            }
        }
