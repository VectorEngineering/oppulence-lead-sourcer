...
export class FinancialAnalysis {
    ...
    getARRBreakdownBySegment(): Record<string, number> {
        ...
        return Object.fromEntries(Object.entries(segmentARR).map(([segment, value]) => [segment, value * 12]))
    }
    ...
    getARRBreakdownByPaymentChannel(): Record<string, number> {
        ...
        this.transactions.forEach((transaction) => {
            ...
            const amount = transaction.amount || 0
            paymentChannelARR[paymentChannel] = (paymentChannelARR[paymentChannel] || 0) + amount
        })

        return Object.fromEntries(Object.entries(paymentChannelARR).map(([channel, value]) => [channel, value * 12]))
    }
    ...
    getARRBreakdownByTransactionType(): Record<string, number> {
        ...
        this.transactions.forEach((transaction) => {
            ...
            const amount = transaction.amount || 0
            transactionTypeARR[transactionType] = (transactionTypeARR[transactionType] || 0) + amount
        })

        return Object.fromEntries(Object.entries(transactionTypeARR).map(([type, value]) => [type, value * 12]))
    }
    ...
    private getMonthlyIncomeForPeriod(startDate: Date, endDate: Date): number {
        ...
        this.transactions.forEach((transaction) => {
            if (transaction.currentDate && transaction.currentDate >= startDate && transaction.currentDate <= endDate) {
                const amount = transaction.amount || 0
                totalIncome += amount
            }
        })
