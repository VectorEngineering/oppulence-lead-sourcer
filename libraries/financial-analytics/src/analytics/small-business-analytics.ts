...
class SmallBusinessAnalytics {
  ...
  private getTransactionsPerDay(
    ...
    const datesWithTransactions = transactions
      .filter(t => t.currentDate)
      .map((t) => t.currentDate?.toDateString());
  ...
  private getAverageTransactionsByDayOfWeek(
    ...
    const weekCount: Record<string, number> = dayNames.reduce(
      (acc, day) => ({ ...acc, [day]: 0 }),
      {} as Record<string, number>,
    );
    ...
    return Object.fromEntries(
      dayNames.map(day => [
        day,
        weekCount[day] ? Math.round(dayCounts[day] / weekCount[day]) : 0
      ])
    );
  }
  ...
  private getCategoriesDistribution(
    ...
    return Object.fromEntries(
      Object.entries(categoryCounts).map(([category, count]) => [
        category,
        Number((total ? (count / total) * 100 : 0).toFixed(2)),
      ]),
    );
  }
